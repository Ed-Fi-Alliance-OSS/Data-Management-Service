...
2. **PostgresqlDocumentStoreRepository**
   ...
   - On any failure path (e.g., `UpsertFailureWriteConflict`, foreign-key violations, optimistic-lock failures), the repository is responsible for calling `await dbSession.RollbackAsync()` before returning the failure result or rethrowing. That keeps the session clean for Polly retries while also clearing `HasActiveTransaction`. Remove only the success-path commits—`CommitAsync` should never appear inside a repository once the middleware owns the final commit—and allow the middleware to commit the unit of work when `HasActiveTransaction` is still true after the method returns a success result.
   - With that contract in place, handlers do not need to sprinkle extra rollback calls around resilience pipelines—the repository guarantees the transaction is reset before the result propagates to Polly.
