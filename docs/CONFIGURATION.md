...
| CorrelationIdHeader              | Name of the request header a client-supplied correlation ID is read from. When this setting is non-empty and the named header is present with a non-empty value, that value identifies the request in logs and in the `correlationId` of error responses; otherwise the server-generated trace identifier is used. Client-supplied values are normalized — a logging-safe character allowlist and `CorrelationIdMaxLength` are applied — before use; see [Logging Policy](./LOGGING.md#correlation-id-normalization). Leave empty to disable client-supplied correlation IDs entirely. Environment override: `AppSettings__CorrelationIdHeader`. Default: empty |
| CorrelationIdMaxLength           | Maximum number of characters retained from a correlation ID, client-supplied or server-generated, before it is used in a log event or an error response body; longer values are truncated rather than rejected. Must be greater than `0`; the service refuses to start otherwise. See [Logging Policy](./LOGGING.md#correlation-id-normalization). Environment override: `AppSettings__CorrelationIdMaxLength`. Default: `255` |
