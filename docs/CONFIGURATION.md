...
| UseLegacyDocumentIdOrderingForChangeQueries | When `true`, restores unconditional `DocumentId` ordering for change-version-filtered GET-many page selection, disabling the conditional `ContentVersion` ordering used for bounded and max-only change-version windows. Deployment-wide rollback switch for incident response; not per-client. Default: `false` |
