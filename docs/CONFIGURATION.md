...
| UseApiSchemaPath                 | When set to `true`, the application loads core data standard and extension artifacts from `ApiSchemaPath`. When `false`, it loads bundled ApiSchema package content from the application output. |
| ApiSchemaPath                    | Specifies the directory where core and extension ApiSchema package content files are located. The ApiSchemaDownloader CLI can be used to download and extract the published ApiSchema packages. |
