...
| CorrelationIdMaxLength           | Maximum number of characters retained from a correlation ID, client-supplied or server-generated, before it is used in a log event or an error response body; longer values are truncated rather than rejected. Truncation happens before disallowed characters are removed, so a value that is both over-length and contains control characters ends up shorter than this limit. Must be greater than `0`; the service refuses to start otherwise. See [Logging Policy](./LOGGING.md#correlation-id-normalization). Environment override: `AppSettings__CorrelationIdMaxLength`. Default: `255` |
