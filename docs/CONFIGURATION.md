...
| CorrelationIdHeader              | Name of the request header a client-supplied correlation ID is read from. When this setting is non-empty and the named header is present with a non-empty value, that value identifies the request in logs and in the `correlationId` (or `traceId` on the generic 500 body) of error responses, including the catch-all 404 for unmatched routes; otherwise the server-generated trace identifier is used. Client-supplied values are normalized before use — first truncated to `CorrelationIdMaxLength`, then stripped of control characters while preserving every other printable character. Leave empty to disable client-supplied correlation IDs entirely everywhere, including the catch-all 404. See [Logging Policy](./LOGGING.md#correlation-id-normalization). Environment override: `AppSettings__CorrelationIdHeader`. Default: empty |
| CorrelationIdMaxLength           | Maximum number of characters retained from a correlation ID, client-supplied or server-generated, before it is used in a log event or an error response body. Longer values are truncated rather than rejected, and control-character removal runs after truncation, so a normalized value can end up shorter than this limit. Must be greater than `0`; the service refuses to start otherwise. See [Logging Policy](./LOGGING.md#correlation-id-normalization). Environment override: `AppSettings__CorrelationIdMaxLength`. Default: `255` |
...
selection the API's other error responses, including the unmatched-route
404, use.
