...
if ($d) {
    ...
    # (a shell export wins over the env file) for the readiness poll and the setup-openiddict.ps1 credential,
    # supplies the engine-aware OpenIddict parameters (the key store lives in the Configuration Service
    # database DMS_CONFIG_DATABASE_NAME, which the topology resolver materialized to a concrete name), and
    # validates engine agreement + the connection so a shell override cannot split the container from
