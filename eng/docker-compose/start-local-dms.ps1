...
if($LoadExtensionResourceClaims)
{
    ...
    dotnet run --configuration Release --project "CmsHierarchy.csproj" --no-launch-profile -- --command Transform --input "E2E-NameSpaceBasedClaimSet.json;E2E-NoFurtherAuthRequiredClaimSet.json;E2E-RelationshipsWithEdOrgsOnlyClaimSet.json;SampleExtensionResourceClaims.json"  --outputFormat ToFile --output ../docker-compose/SecurityMetadata.json --skipAuths "RelationshipsWithEdOrgsAndPeopleInverted;RelationshipsWithStudentsOnlyThroughResponsibility;RelationshipsWithStudentsOnlyThroughResponsibilityIncludingDeletes"
    Pop-Location
