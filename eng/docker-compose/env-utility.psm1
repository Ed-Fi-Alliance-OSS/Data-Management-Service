...
function Write-DmsComposeDiagnosticValue {
    param(
        [string]$Name,
        [AllowNull()]
        [object]$Value
    ...
    $valueLines = $valueText -split '\r?\n'

    if ($valueLines.Count -eq 1) {
        Write-Output "  $Name = $valueText"
        return
    }
...
function Get-DmsComposeEnvironmentValues {
    param(
        [string[]]$Output,
        [string[]]$Keys
    )
    ...
    foreach ($line in $Output) {
        ...
        $match = [Regex]::Match($lineText, '^(?<key>[A-Za-z_][A-Za-z0-9_]*)=(?<value>.*)$')

        if ($match.Success) {
            $matchedKey = $match.Groups["key"].Value

            if ($Keys -contains $matchedKey) {
                $currentKey = $matchedKey
                $currentValueLines = [System.Collections.Generic.List[string]]::new()
                $currentValueLines.Add($match.Groups["value"].Value)
                $values[$currentKey] = $currentValueLines
            }
            else {
                $currentKey = $null
                $currentValueLines = $null
        ...
        if ($null -ne $currentKey) {
            $currentValueLines.Add($lineText)
        }
    }

    $resolvedValues = @{}
    foreach ($key in $values.Keys) {
        $resolvedValues[$key] = [string]::Join("`n", $values[$key])
    }

    return $resolvedValues
}

function Write-DmsComposeSchemaDiagnostics {
    param(
        [string]$Stage,
        [string[]]$ComposeFiles,
        [string]$EnvironmentFile,
    ...
    Write-Output "DMS compose schema diagnostics ($Stage):"
    Write-Output "  Environment file: $EnvironmentFile"
    Write-Output "  Compose files: $([string]::Join(' ', $ComposeFiles))"
    Write-Output "  Project name: $ProjectName"

    Write-Output "Process environment values:"
    foreach ($key in $interpolationKeys) {
        Write-DmsComposeDiagnosticValue `
            -Name $key `
            -Value ([System.Environment]::GetEnvironmentVariable($key))
    }

    $composeArguments = @("compose") + $ComposeFiles + @("--env-file", $EnvironmentFile, "-p", $ProjectName)

    try {
        $composeEnvironmentOutput = @(& docker @composeArguments "config" "--environment" 2>&1)

        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Unable to resolve Docker Compose interpolation environment for diagnostics. $([string]::Join(' ', $composeEnvironmentOutput))"
        }
        else {
            $composeEnvironmentValues = Get-DmsComposeEnvironmentValues `
                -Output $composeEnvironmentOutput `
                -Keys $interpolationKeys

            Write-Output "Docker Compose interpolation values:"
            foreach ($key in $interpolationKeys) {
                $value =
                    if ($composeEnvironmentValues.ContainsKey($key)) {
                        $composeEnvironmentValues[$key]
                    }
                    else {
                        $null
                    }

                Write-DmsComposeDiagnosticValue -Name $key -Value $value
            }
        }
    }
    catch {
        Write-Warning "Unable to resolve Docker Compose interpolation environment for diagnostics. $($_.Exception.Message)"
    }

    try {
        $composeConfigOutput = @(& docker @composeArguments "config" "--format" "json" 2>&1)

        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Unable to resolve Docker Compose service environment for diagnostics. $([string]::Join(' ', $composeConfigOutput))"
            return
        }

        $composeConfig = $composeConfigOutput -join "`n" | ConvertFrom-Json -Depth 100
        $dmsService = $composeConfig.services.dms

        if ($null -eq $dmsService) {
            Write-Warning "Unable to resolve Docker Compose service environment for diagnostics. The dms service was not found."
            return
        }

        $dmsEnvironment = $dmsService.environment

        Write-Output "Docker Compose dms service environment values:"
        foreach ($key in $serviceEnvironmentKeys) {
            $property = $dmsEnvironment.PSObject.Properties[$key]
            $value =
                if ($null -ne $property) {
                    $property.Value
                }
        ...
        Write-Warning "Unable to resolve Docker Compose service environment for diagnostics. $($_.Exception.Message)"
    }
}
