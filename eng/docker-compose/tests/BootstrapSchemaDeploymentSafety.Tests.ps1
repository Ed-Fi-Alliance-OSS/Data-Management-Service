    ...
    Context "public script contracts" {
        ...
        It "start scripts keep Kafka compose files behind explicit opt-in" {
            foreach ($name in @("start-local-dms.ps1", "start-published-dms.ps1")) {
                ...
                # Kafka selection is engine-neutral in start-local-dms.ps1: the deployment-owned CDC
                # workflow captures from SQL Server as well as PostgreSQL. The optional engine clause is
                # retained here only so a start script that still carries one would also match.
