...
.DESCRIPTION
    ...
    Each deployment is torn down before the next begins, and the teardown is asserted rather than
    hoped for: they share a compose project name, so a survivor from the previous run would answer
...
# Resolved once, and every deletion below is checked against it. Nothing here should be able to
# remove a path a caller supplied by accident, and "it is under a directory this script owns" is the
# only check that holds when the caller controls the parameter.
...
function New-DeploymentEnvironmentFile {
    ...
    $source = Join-Path $composeRoot ".env.plugins.e2e"
    $destination = Join-Path $WorkspaceRoot "$Name.env"

    $lines = [System.Collections.Generic.List[string]]::new()
    $lines.Add("# Generated by Invoke-PluginDeploymentCheck.ps1. Do not edit; regenerate.")
...
function Invoke-ImageVersionProof {
    ...
    $extractRoot = Join-Path $WorkspaceRoot "image-assemblies"
...
# The failure has to be the checksum and nothing incidental: an unreachable feed, or a script that
# fell over before it downloaded anything, would also exit non-zero and would prove nothing at all
# about digest verification.
Assert-True `
    ($fetchLog -match "did NOT match" -or $fetchLog -match "FAILED") `
    "the fetch step failed on the checksum comparison specifically"

# StartedAt rather than the absence of a log line. A container that started and then died would carry
# a start time; this one was never started at all, which is what service_completed_successfully buys.
