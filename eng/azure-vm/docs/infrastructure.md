...
> that deployment's **private** credentials doc / vault — not here. Mint secrets `+`/`%`-free
> so HTTP Basic works without URL-encoding (see DMS-1231).
...
5. **Seed data:**
   ...
   - **multi-tenant** (`edfi_mt` = tenant1, `edfi_mt_t2` = tenant2): clone the seeded
     `edfi_st` with `seed/clone-data.sh` (the BulkLoadClient cannot seed multi-tenant —
     issue 1).
...
1. **Multi-tenant API seeding is blocked by an XSD bug — `DMS-1230`.** The DMS XSD metadata
   file-content endpoint 404s under a `/{tenant}` prefix (unanchored regex), so the
   BulkLoadClient can't download schemas against `/mt-dms`. **Workaround used here:** seed
   `edfi_st` via the API, then `seed/clone-data.sh` copies it into the per-tenant DBs. Drop
   the clone step once fixed upstream.
2. **CMS-minted secrets containing `+`/`%` break Basic-auth tokens — `DMS-1231`.** Clients
   that don't URL-encode the Basic secret (e.g. the BulkLoadClient) get `invalid_client`.
   **Workaround:** re-mint the application until the secret has no `+`/`%`/space, or
   URL-encode it in the `Authorization` header.
