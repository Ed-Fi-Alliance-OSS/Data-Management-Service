...
## Follow-up: current-lag and Kafka policy evidence

Full run `35018130919` on `a57ab747f` completed with 256 passes, three failures, and zero skips or environment failures. All required PR checks passed. PostgreSQL Lifecycle failed after worker restart and guarded start because the required current-lag sample was missing or invalid (`CdcConnectorTelemetryMetrics.Parse:32`). The SQL Server `offset` unavailable-evidence case and intact-running restart case failed during initial admission, before their scenarios, at topic-policy validation (`CdcKafkaProvisioning.PrepareAsync:348`).

The instrumented PostgreSQL replay passed with valid current-lag values on every consumed scrape. Debezium 3.6.0.Final's `StreamingMeter` returns `-1` before its first measurement, but the hosted evidence does not yet prove that this caused the failure. The SQL Server artifacts similarly identify policy rejection without identifying the observed setting that failed validation.

Add bounded fixture evidence for the exact consumed metrics response (sample/type counts and numeric classification only) and observed Kafka topic policy (hashed topic identity, partition/replica counts, allow-listed numeric settings and override flags). Verify response preservation, redaction, and retention limits. Preserve production validation, startup deadlines, and assertions. Run focused local scenarios and both hosted Lifecycle suites with these diagnostics, then fix only the demonstrated cause. Full qualification still requires all 13 live jobs on the final code commit; targeted runs do not replace it.

Local validation of this evidence extension passed all 108 selected cases (100 offline and eight live lifecycle cases across PostgreSQL and SQL Server), with zero failures or skips. All eight live attachments were checked for the intended allow-listed fields, and fixture containers were removed while the seven pre-existing containers remained. After adding explicit absent/unavailable topic outcomes, the final Release build passed with zero warnings/errors and all 100 offline tests passed again. The hosted causes remain unconfirmed; the next runs collect evidence rather than claim a fix for those failures.
