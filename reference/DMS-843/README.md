...
This package keeps snapshot history tables out of scope, but snapshot lifecycle handling is in scope. Synchronization reads run in one of two built-in flows selected by `Use-Snapshot`: absent or `false` uses the live non-snapshot flow, and `true` uses the snapshot-backed flow against the configured snapshot source with DMS-managed validation of the currently bound derivative for that request. Snapshot-backed pass stability is therefore operationally bounded: DMS resolves the active instance-scoped binding on each request, while stable multi-request passes depend on operators keeping that binding unchanged because DMS-843 does not introduce a client-visible binding token or server-side cross-request pinning.
...
- when the selected synchronization surface has not yet allocated any change-version values, `availableChangeVersions` returns bootstrap `0/0`; this is verified ODS-output-compatible behavior because current ODS and DMS both use the sequence ceiling rather than `MAX(ChangeVersion)` over empty retained tables
- invalid `Use-Snapshot` handling is an explicit DMS contract choice: the header is validated as a strict boolean and malformed values return `400 Bad Request`
...
The alignment target is behavioral and contract parity where appropriate, with explicit package-level decisions wherever DMS makes a product-specific choice. Those explicit choices include `Use-Snapshot` as the selector between the live and snapshot-backed synchronization flows over the same tracking artifacts, adoption of legacy ODS parity for `/keyChanges` window-collapsing semantics plus distinct tracked key-change token sequencing, the accepted DMS-specific tracked-change ownership exception justified by redesign auth, bootstrap `0/0` as ODS-output-compatible behavior for empty synchronization surfaces (verified), ODS-compatible sequence-ceiling `availableChangeVersions.newestChangeVersion` semantics, strict boolean validation of `Use-Snapshot`, and retention of current-backend `_etag/_lastModifiedDate` storage ownership.
