...
**`If-Match` comparison (decision needed).** Because the etag embeds `variantKey`, strict RFC strong comparison is over the full opaque tag. The recommended strict-adherence behavior: the precondition check reconstructs the expected etag as `"{currentContentVersion}-{requestVariantKey}"` from the inbound request's representation context and compares the full string. A client should therefore present the etag it obtained for the representation it is acting on — normally the same profile/format it reads and writes through. The only visible effect is that a cross-variant `If-Match` (e.g. an etag read under profile A used to guard a write under profile B) returns `412` even if the underlying state is unchanged; the common case (same variant for read and write, no profile, default link mode) is unaffected. A more lenient alternative — comparing only the `ContentVersion` component — avoids cross-variant `412`s but deviates from strict RFC strong comparison; the team should choose explicitly.
...
- The server recomputes the full tag deterministically from request context (negotiated format, profile in effect, link mode) plus the loaded schema at three points — read response, write response, and `If-Match` precondition — with **no database dependency** and **no document hashing**.
...
`_etag` values become compact, opaque, quoted strong entity-tags of the form `"{ContentVersion}-{variantKey}"` rather than hashes; the `ContentVersion` portion is treated as an opaque string, never a number. Acceptable because the contract has not shipped. `If-Match` (strong comparison) and `If-None-Match` (weak comparison) semantics are preserved. Unlike the redesign's original profile/link-insensitive etag, the served etag now varies by profile, format, and link mode — a deliberate change for RFC 7232 compliance (see "ETag format and HTTP validator semantics (RFC 7232)").
