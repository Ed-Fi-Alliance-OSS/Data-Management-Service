...
The [final reconciliation](#final-reconciliation-t20) records executed procedure cases
and Contract checks. Historical task sections below retain their original dates,
rehearsal failures and then-pending scope; the procedure table and T20 reconciliation
provide current status. Fixture presence alone is never evidence of execution.
...
`Both` means PostgreSQL and SQL Server; each provider result is recorded separately. Recorded image/profile values
apply only to their cited run; the supported local profile is
described in the [entry point](README.md#supported-deployment).

| Procedure | Design link / invariant contribution | Provider | Stable test identifiers | Snippet ID / checked example | Qualification profile/image | Sanitized artifact reference | Actual result |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| [Interrupted initial-enable retry](operations-runbook.md#initial-enable-retry) | [CDC-INV-14; CDC-INV-15](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#enablement-and-initial-readiness-sequence) | PostgreSQL | [Given_Cdc_command_enable_retry](../../src/dms/clis/EdFi.DataManagementService.SchemaTools.Tests.Unit/CdcCommandEnableRetryTests.cs) — `It_resumes_the_same_command_after_a_stage_interruption` (provider/broker-start/worker-start/preflight/post-after/barrier/metrics); `It_rejects_ineligible_original_evidence_before_provider_or_Kafka_effects`; `It_preserves_combined_initial_containment_failures_in_command_json`; both-provider Contract cases; T20 shared CLI fixture | `cdc-enable-retry` | LocalSingleBroker / AuthorizationDisabledLocal; [T20 images/revision](evidence/t20-kafka-consumer/run-details.json) | [Marked retry and rejected repeat](evidence/t20-kafka-consumer/marked-commands.json) | Passed T20 after retained connector registration; original workflow ID and writer authorization, then rejected repeat. T16 wrapper evidence retained |
| [Interrupted initial-enable retry](operations-runbook.md#initial-enable-retry) | [CDC-INV-14; CDC-INV-15](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#enablement-and-initial-readiness-sequence) | SQL Server | [Given_Cdc_command_enable_retry](../../src/dms/clis/EdFi.DataManagementService.SchemaTools.Tests.Unit/CdcCommandEnableRetryTests.cs) — `It_resumes_the_same_command_after_a_stage_interruption` (provider/broker-start/worker-start/preflight/post-after/barrier/metrics); `It_rejects_ineligible_original_evidence_before_provider_or_Kafka_effects`; `It_preserves_combined_initial_containment_failures_in_command_json`; both-provider Contract cases; T20 shared CLI fixture | `cdc-enable-retry` | T17 SQL Server local profile; both-provider Contract | [T17 wrapper retry](#sql-server-setup-qualification-t17), [Contract cases](evidence/t20-kafka-consumer/contract-cases.json) | SQL Server wrapper retry passed T17; shared exact CLI exercised on PostgreSQL T20. No separate SQL Server execution of this CLI alternative claimed |
...
| [Managed shutdown and startup](operations-runbook.md#managed-lifecycle) | [CDC-INV-14; CDC-INV-15](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#controller-managed-lifecycle-and-native-recovery-boundary) | SQL Server | [Given_Cdc_Controller_Managed_Lifecycle](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/CdcManagedLifecycleTests.cs) — `It_keeps_committed_offsets_and_no_tasks_across_worker_restart_until_guarded_start`; `It_waits_for_retained_projection_work_in_the_single_start_invocation` (persistent false/true). [CdcLifecycleOrdering.Tests.ps1](../../eng/docker-compose/tests/CdcLifecycleOrdering.Tests.ps1) — `CDC-DOC cdc-managed-stop` (both connectors, retained custom roots); `does not shutdown the worker after delayed/unverified final task status`; `blocks DMS and a later unmanaged startup retry after one resume fails`. Live wiring qualified in T19 | `cdc-managed-stop`, `cdc-managed-start` | LocalSingleBroker / AuthorizationDisabledLocal; [immutable images](evidence/t19-sqlserver-lifecycle/run-details.json) | [Live assertions](evidence/t19-sqlserver-lifecycle/managed-lifecycle-runbook.json), [required cases](evidence/t19-sqlserver-lifecycle/qualification.json) | Passed T19; [exact outcomes and limits](#sql-server-lifecycle-qualification-t19) |
...
| [Intact connector restart and resume](operations-runbook.md#intact-restart) | [CDC-INV-14; CDC-INV-15](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#controller-managed-lifecycle-and-native-recovery-boundary) | SQL Server | [Given_Cdc_Controller_Managed_Lifecycle](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/CdcManagedLifecycleTests.cs) — `It_restarts_an_intact_running_connector_with_fresh_ready_evidence`; `It_rejects_unavailable_live_evidence_while_stopped` (offset/provider). [Given_CdcManagedLifecycle](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Unit/CdcManagedLifecycleTests.cs) — `It_does_not_infer_a_restart_from_unchanged_running_status_after_a_lost_reply` (unit). Live wiring qualified in T19 | `cdc-intact-restart`, `cdc-intact-resume` | LocalSingleBroker / AuthorizationDisabledLocal; [immutable images](evidence/t19-sqlserver-lifecycle/run-details.json) | [Live assertions](evidence/t19-sqlserver-lifecycle/managed-lifecycle-runbook.json), [required cases](evidence/t19-sqlserver-lifecycle/qualification.json) | Passed T19; [exact outcomes and limits](#sql-server-lifecycle-qualification-t19) |
...
| [Local policy and shared offsets](operations-runbook.md#security-consumer-evidence) | [CDC-INV-14/15; offset store](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#kafka-connect-offset-store) | PostgreSQL-hosted shared CLI; provider-neutral Kafka policy | [Given_Cdc_runbook_local_topic_policy](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/CdcKafkaPolicyStatusTests.cs) — `It_executes_marked_topic_policy_status_with_shared_offset_drift` | `cdc-topic-policy-inspect`, `cdc-enable-retry` | LocalSingleBroker / AuthorizationDisabledLocal; [T20 images/revision](evidence/t20-kafka-consumer/run-details.json) | [Marked commands](evidence/t20-kafka-consumer/marked-commands.json), [results](evidence/t20-kafka-consumer/kafka-results.json) | Passed T20; fresh policy then real shared-offset drift, no repair; ACL isolation false throughout; standalone status exits 1 |
| [Broker policy and cleaner inspection](operations-runbook.md#security-consumer-evidence) | [CDC-INV-15; topic contract](../design/backend-redesign/design-docs/cdc/0002-kafka-topic-and-message-contract.md#topic) | Provider-neutral; SQL Server history topic included | [Given_authorized_three_broker_cdc_policy](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/CdcKafkaPolicyTests.cs) — `It_executes_marked_broker_retention_inspections_and_rejects_denied_authority`; exact policy/drift IDs in T20 | `cdc-kafka-retention-inspect` | AuthorizationEnabled / ProductionDurability; pinned Kafka/Connect in [T20](evidence/t20-kafka-consumer/run-details.json) | [Inspections](evidence/t20-kafka-consumer/inspection-observations.json), [required cases](evidence/t20-kafka-consumer/kafka-qualification.json) | Passed T20; two samples on all three brokers, denied consumer inspection; no workload capacity or purge certification |
| [Effective-access qualification](operations-runbook.md#security-consumer-evidence) | [CDC-INV-12/15; security](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#security-telemetry-and-operations) | Provider-neutral Kafka fixture, including SQL Server history artifact | [Given_authorized_three_broker_cdc_policy](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/CdcKafkaPolicyTests.cs) — `It_prepares_live_worker_only_offsets_before_the_qualified_worker_starts`; `It_allows_only_the_configured_public_topic_and_consumer_group` (a/b); `It_denies_cross_binding_and_internal_reads` (all topic/group cases); `It_denies_consumer_writes_and_connector_access_to_worker_offsets`; `It_preserves_unavailable_acl_authority_when_description_is_denied`; `It_fails_closed_on_unsafe_effective_grants_even_with_a_missing_required_grant` (all six cases). Same file: `Given_explicit_authorization_disabled_local_kafka_policy.It_labels_local_policy_without_claiming_acl_or_production_durability_proof` | `cdc-access-inspect` | AuthorizationEnabled / ProductionDurability; explicitly separate local profile, [T20 images/revision](evidence/t20-kafka-consumer/run-details.json) | [Kafka reports](evidence/t20-kafka-consumer/kafka-qualification.json), [case outcomes](evidence/t20-kafka-consumer/kafka-results.json) | Passed T20; authenticated allow/deny and effective wildcard/prefix/deny authority cases; local results prove no isolation |
| [Consumer-owner reference evidence](operations-runbook.md#consumer-owner-proof-and-invalidation-handoff) | [CDC-INV-13/15; public consumer bootstrap](../design/backend-redesign/design-docs/cdc/cdc-streaming.md#public-consumer-bootstrap) | Provider-neutral consumer assertions hosted by PostgreSQL fixture | [Given_MessageContractConsumerBroker](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Integration/MessageContractConsumerBrokerTests.cs) — `MC-CONSUMER-BROKER-BOOTSTRAP-DURABILITY`, `MC-CONSUMER-BROKER-ORDERING`, `MC-CONSUMER-BROKER-IDLE-RENEWAL`, `MC-CONSUMER-BROKER-CONTINUATION`, `MC-CONSUMER-BROKER-CHECKPOINT-MISSING`, `MC-CONSUMER-BROKER-CHECKPOINT-CORRUPT`; [bootstrap](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Unit/MessageContractConsumerBootstrapTests.cs) and [continuity](../../src/dms/backend/EdFi.DataManagementService.Backend.Cdc.Tests.Unit/MessageContractConsumerContinuityTests.cs) unit deadline/fault cases | `cdc-consumer-evidence` | Real Kafka transport / synthetic public records / simulated durable store and clock; [images/revision](evidence/t20-kafka-consumer/run-details.json) | [Broker cases](evidence/t20-kafka-consumer/consumer-results.json), [observations](evidence/t20-kafka-consumer/consumer-observations.json), [Contract consumer cases](evidence/t20-kafka-consumer/contract-cases.json) | Passed T20: 502 MessageContract cases, six required consumer broker scenarios; independent consumer capacity/correctness remain owner obligations |
...
<a id="kafka-and-consumer-qualification-t20"></a>

## Kafka inspections and consumer conformance (T20)

The exact `cdc-access-inspect` and `cdc-consumer-evidence` runner snippets were extracted
with the shared marked-example helper, supplied fresh results paths and the declared
immutable images, and executed from the repository root. The Kafka lane retains separate
secured and local reports; neither is inferred from the other. See [run details](evidence/t20-kafka-consumer/run-details.json)
for revisions, snippet/source hashes, image references, corrected attempts and substitutions.
All published observations passed through the existing safe evidence exporter; raw logs,
credentials, broker client properties, topic names and source offsets stay private.

| Layer | Exact cases and outcome | Evidence and limits |
| --- | --- | --- |
| Secured Kafka | 49 passed, zero failures/skips; required-case guard verifies the broker snippet, two allowed public/group cases, 16 denied cross-binding/internal reads, denied writes/offset access, six unsafe-effective-grant cases, denied inspection authority and policy drift/retention/replica cases | [Reports](evidence/t20-kafka-consumer/kafka-qualification.json), [stable IDs](evidence/t20-kafka-consumer/kafka-results.json). Three brokers, authenticated ephemeral identities, RF3/ISR2; no production network/TLS or operator-deployment certification |
| Exact broker inspection | `It_executes_marked_broker_retention_inspections_and_rejects_denied_authority`: six successful samples (two on every broker), one denied-consumer sample | [Selected fields](evidence/t20-kafka-consumer/inspection-observations.json): policy sources/replicas for public, progress, SQL history and shared offsets; retained public bytes and ordered partition bounds; cleaner gauges/dead threads, disk headroom. JMX is loopback within each owned broker. Small active segments need not compact: no cleaner-progress rate, capacity threshold, error injection or physical purge claim |
| Local Kafka and exact status | Two passed, zero skips: honest authorization-disabled policy plus `It_executes_marked_topic_policy_status_with_shared_offset_drift` | [Commands](evidence/t20-kafka-consumer/marked-commands.json). Real managed PostgreSQL provenance/CMS/schema, retained worker/broker, shared CLI initial retry returns original workflow authorization; repeat rejects. Status observes satisfied policy then actual shared-offset drift without repair; `aclIsolationProven: false` throughout. Standalone projection health remains unavailable, so status exits 1 |
| DMS-1324 broker consumer reference | `MC-CONSUMER-BROKER-BOOTSTRAP-DURABILITY`, `ORDERING`, `IDLE-RENEWAL`, `CONTINUATION`, `CHECKPOINT-MISSING`, `CHECKPOINT-CORRUPT` (each with the full `MC-CONSUMER-BROKER-` prefix); all six required scenarios passed within 502 passing MessageContract cases | [Cases](evidence/t20-kafka-consumer/consumer-results.json), [observations](evidence/t20-kafka-consumer/consumer-observations.json), [report](evidence/t20-kafka-consumer/consumer-qualification.json). Real Kafka, synthetic E18-derived public values, simulated durable persistence and clock; PostgreSQL hosts the fixture, consumer assertions are provider-neutral |
| DMS-1324 consumer units | Exact method/case outcomes from the final Contract run: complete partition/checkpoint proof; deadline boundary and one-tick-late invalidation; renewed idle barriers; missing/corrupt checkpoint; stale callbacks and complete store discard | [Contract cases](evidence/t20-kafka-consumer/contract-cases.json). These are reference-model fault/deadline assertions, not proof of an independent store's persistence or workload capacity |

The consumer checklist maps barrier/durability to
`It_requires_durable_application_and_every_checkpoint_including_the_empty_partition`;
renewal to `It_renews_an_idle_proof_from_real_unchanged_ends_only_after_all_checkpoints_complete`;
continuation to `It_continues_from_durable_next_offsets_without_replaying_bootstrap`;
checkpoint invalidation to both `It_reconstructs_from_earliest_after_checkpoint_*` methods.
Contract adds `It_permits_durable_completion_up_to_and_including_the_deadline`,
`It_restarts_when_state_or_checkpoint_persistence_misses_the_deadline`,
`It_discards_all_state_one_tick_after_an_unrenewed_deadline`,
`It_fences_delayed_persistence_from_the_discarded_attempt` and
`It_invalidates_incomplete_mismatched_or_regressing_barrier_observations`, including
all recorded parameter cases. Consumer correctness and capacity remain owner obligations.

<a id="final-reconciliation-t20"></a>

## Final CDC-INV-14/15 reconciliation (T20)

[Procedure reconciliation](evidence/t20-kafka-consumer/procedure-reconciliation.json)
retains the successful Admission, Lifecycle, Recovery, RecordSize, History and Telemetry
reports for both providers, with exact required-case results, artifact hashes and the
commits that published them. It also includes T29's SQL Server initial-user mapping
qualification and owning contract amendment. No unaffected provider lane was rerun for
aggregation. Shared-helper changes in T20 only add the Kafka inspection ID and command
artifact names; existing provider snippet bodies and production behavior are unchanged.
Earlier failed rehearsals remain separately identified, never summed into passing results.

[Snippet reconciliation](evidence/t20-kafka-consumer/snippet-reconciliation.json) maps
every marked example to its exact executed test/inspection artifact, equal-body example,
or explicitly limited Contract/prerequisite alternative. JSON excerpts are production
serialization checks, not live deployment claims. The interactive PostgreSQL role prompt,
PostgreSQL published-wrapper alternative, build-based E2E alternatives and E2E teardown
alternative have no additional live execution claim; selected local/published/direct-E2E
paths and governed stack cleanup retain their required live evidence. SQL Server CLI
initial retry retains T17 wrapper behavior and shared command/Contract coverage as noted
above. All required live procedure cases and final Contract reports are nonzero and pass;
no missing or skipped required case is accepted.

[Final Contract report](evidence/t20-kafka-consumer/contract-qualification.json) and
[case outcomes](evidence/t20-kafka-consumer/contract-cases.json) cover commands/settings,
acknowledgements, serialized output, packaged stream/exit behavior, required wrapper
cases, consumer conformance and touched relative links/anchors. Qualification guards
reject missing, skipped and duplicate required cases. Exact images/revisions remain
attached to each prior procedure; later test/helper revisions do not imply a rerun.

This story contributes operational documentation and exercised handoffs to CDC-INV-14
and CDC-INV-15. Sibling evidence remains linked for all other invariants. Adoption,
physical-source replacement and new-generation cutover remain unsupported; native
recovery cannot certify its unobserved interval. Controller-only results supply no
