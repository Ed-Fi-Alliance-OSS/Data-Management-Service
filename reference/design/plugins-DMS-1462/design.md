...
**A plugin that registers a declared contract under `KeyedService.AnyKey` is fatal.**
The plugin startup guard refuses this registration before activation, naming the plugin and the contract.
...
An unkeyed declared-contract registration or one under a concrete key remains supported by this audit, subject to the contract's own rules. A wildcard registration of a service that is not a declared contract is unaffected.
This refusal applies to plugin-attributed additions; it does not introduce a refusal of host-supplied wildcard registrations.
...
| A plugin registers a declared plugin contract under `KeyedService.AnyKey` | **Fatal**, naming the plugin and the contract, before activation. Enumerable resolution cannot reach the wildcard registration, so the required startup probe cannot cover it. Unkeyed and concrete-key registrations remain supported by this audit, subject to the contract's own rules; wildcard registrations of services that are not declared contracts are unaffected |
