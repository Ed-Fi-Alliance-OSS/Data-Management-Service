...
The host-integration story owns **this** Dockerfile change, dropping the publish command line's version stamping, because it is the story that also changes `build-dms.ps1`'s `DockerBuild` to stamp through `Directory.Build.props` instead.
...
That set is written as service type names rather than as an assembly or namespace rule, because it has to be exactly the types whose descriptors carry the process's logging and nothing else in `Microsoft.Extensions.*`.
...
`IServiceCollection` itself belongs on that list and is the sharpest example, because something in DMS already depends on it.
DMS-1434's guard reads the final descriptor set through a collection Core captures as `services.AddSingleton<IServiceCollection>(services)` (design.md, "Core's own contribution is the guard, not the registration"), and `IServiceCollection` is declared in `Microsoft.Extensions.DependencyInjection.Abstractions`, so a plugin registering its own `IServiceCollection` is an ordinary permitted registration that displaces the collection that audit reads.
The disclaimer below applies to it as it does to the rest: a plugin has full process trust, so this is not a hole an isolation claim has to plug.
It is worth naming because the fix on the DMS-1434 side is free - a guard that closes over the collection instead of resolving it from DI cannot be displaced at all - and the custom validation composition story carries that as a recommendation for its own owners to take or decline.
...
It is a build-stage change like the other one and it changes no image layout, and it is in the host-integration story because that is the story that also changes `build-dms.ps1`'s `DockerBuild`, which has to run `SetDMSAssemblyInfo` in the same breath.
...
DMS's own `Program.cs` fixes that ordering already - the host is built at `Program.cs:123-129` and `app.Logger` is first used at `:131`, while the startup-task executor's `RunByOrderRangeAsync` calls are at `:315`, `:329`, and `:341` - so the plugin-to-service-type mapping is in the log above any audit failure that needs it.
...
The harness therefore lays the packed nupkg out as `<id>/<version>/<id>.<version>.nupkg` in lower case, serves that directory over HTTP from a pinned static-file container in the test's own Compose overlay, and points `ACME_PLUGIN_PACKAGE_URL` at it.
The published overlay is unchanged and is still run exactly as `docs/OPERATIONS.md` prints it; what the test adds is a feed, which is the deployment's side of the recipe in every real deployment too.
...
| DMS host integration: one `LoadPlugins` constant, both call sites, the inventory event before the startup tasks run, and moving the Docker lane's version stamping off the command line | Small | `RunBootstrapPhase` and the `DmsStartupPhases` constants (`Program.cs:443-462` and `Infrastructure/StartupPhaseExecutor.cs:23-30`) already exist, and `app.Logger` is available at `Program.cs:130` where the inventory event goes. Dropping `/p:AssemblyVersion` and the `ASSEMBLY_VERSION` argument is three lines and is what keeps one contract assembly from carrying two versions across two images. The build-stage change that lets the image see `src/plugins/` is the row above, not this one |
