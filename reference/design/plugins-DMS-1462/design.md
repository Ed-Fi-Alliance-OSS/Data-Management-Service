...
**Revision 12 (2026-09-10, follow-up review on PR #1243).**
A second pass over the audit implementation found two diagnostics that did not match what the mechanism can deliver, and the document was one half of each.
Both are decisions rather than wording, because each changes what an operator or an implementer is told to do.

- **The replace-cardinality fatal gives a remedy per claimant shape.** Revision 11's text and the implementation both sent every reader to `Plugins:Allowed`, and that remedy resolves nothing when one plugin registered the contract twice: the allowlist holds one entry for it, and removing that entry withdraws the plugin along with everything else it contributes rather than choosing between its two registrations. The single-claimant fatal now names the plugin and its registration count and addresses the plugin's author; the multi-claimant fatal keeps the operator remedy unchanged. The counting is untouched, historical claims and descriptor multiplicity included. See [Contract Cardinality](#contract-cardinality).
- **The wildcard-key refusal is restricted to surviving registrations.** Revision 11 removed the host exemption and had the check read the per-plugin records and the composition snapshot alike, which left a wildcard registration a later plugin permissibly removed being refused on the strength of a record alone. The rule's whole rationale is that the required startup activation cannot cover the shape, and nothing is activated for a descriptor that is no longer on the collection, so record-sourced candidates are now intersected against the composition snapshot by reference identity before attribution and de-duplication. Attribution, de-duplication, and the refusal of a surviving host or unattributed wildcard registration are unchanged, the records stay historical, and the replace-cardinality accounting stays deliberately different. See [Contract Cardinality](#contract-cardinality) and [Startup Failure Semantics](#startup-failure-semantics).

A regression case covers the second decision through the real loader: an earlier plugin registers a removable declared contract under the wildcard key beside a surviving unkeyed contribution, a later plugin removes the wildcard registration, and the audit passes with the removal still recorded and the surviving implementation activated.

**Revision 11 (2026-09-10, team review on PR #1243).**
A reviewer read the recording-wrapper implementation against this document and found four places where the two disagreed and the document was the half that was wrong.
All four are decisions rather than wording, because each one changes what the mechanism refuses.

- **The logging carve-out protects addition as well as removal.** The document protected the logging pipeline only against removal and overwrite, and a plugin does not need either: `ILoggerFactory` and `ILogger<T>` are resolved singly, the last registration wins, so a plain `Add` inside a hook takes the host's logging with no call the removal rules can see. The host now reserves its **unkeyed** `ILoggerFactory`, non-generic `ILogger`, and `ILogger<>` open or closed, refused on `Add`, `Insert`, and the indexer setter before the descriptor lands. It is written as a reservation rather than as duplicate detection, because it holds where the host has registered nothing of that type. Adding an `ILoggerProvider` and any keyed logging registration stay permitted, which is what keeps a plugin's own sink ordinary work, and the two `AddSerilog` overloads are separated where the document previously ran them together: `ILoggingBuilder.AddSerilog` adds a provider and is permitted, `IServiceCollection.AddSerilog` can register an `ILoggerFactory` and is refused. It fires at the wrapper because the post-container audit needs a logger before it can report anything. See [Contract Cardinality](#contract-cardinality).
- **The no-contract-registered rule measures survival, not registration.** Removing a pre-existing descriptor that is neither host-owned nor part of the logging pipeline is permitted, so a later plugin may legitimately remove an earlier one's declared-contract registration and leave a plugin the host will never call while the operator believes otherwise. The rule now compares each plugin's declared-contract additions against the composition snapshot by reference identity. The removal stays permitted and stays recorded, the per-plugin record stays historical so the inventory event still reports what the plugin did, and the message distinguishes a plugin that registered nothing declared from one whose declared registrations did not survive. The replace-conflict count is deliberately **not** changed to match: two claims on one replace contract are an operator decision that a later removal does not resolve. See [Contract Cardinality](#contract-cardinality).
- **The wildcard-key refusal loses its host exemption.** Revision 10's text refused a declared contract under `KeyedService.AnyKey` only where the records attributed it to a plugin, which left the audit asserting a startup activation it never performed for the identical host registration. The requirement is a property of the registration rather than of who made it, so the check now reads the per-plugin records and the composition snapshot alike, reports a registration present in both once, and names no plugin where the records attribute none rather than blaming whichever plugin happened to be loaded. See [Contract Cardinality](#contract-cardinality) and [Startup Failure Semantics](#startup-failure-semantics).
- **The activation promise is stated as a composition contract rather than as detection.** The document described the probe in terms that implied it saw the container's final registrations. Declared-contract registration must be finished by the composition snapshot; infrastructure the host registers afterwards is supported and unexamined; a declared-contract registration or removal made after it is outside what this audit undertakes to cover. When the static checks pass, the probe resolves the **surviving** supported registrations through the unkeyed group and each discovered concrete-key group. No removed descriptor is described as activatable, the snapshot is not described as the final container, and the probe is not described as detecting every possible later registration. The scoped-and-transient versus root-cached-singleton distinction is unchanged. See [Startup Failure Semantics](#startup-failure-semantics).

Two placement notes are recorded with them, both belonging to draft 03 rather than to this document.
`Microsoft.Extensions.Azure` 1.11.0 moves out of `src/Directory.Packages.props` and into the fixture tree's own `Directory.Packages.props`, which imports the parent, so the fixture keeps every version the repository manages and a test-only dependency stops appearing in the file every production project reads.
And draft 03's absolute "Core is untouched" is replaced by a prohibition with one named exception, `src/dms/core/EdFi.DataManagementService.Core.Tests.Unit/packages.lock.json`, which is regenerated because that test project references the frontend and therefore inherits the new project reference's closure.

**Revision 13 (2026-09-15, external review on PR #1256).**
A reviewer outside the pair read the host-integration branch and found five defects and four simplification opportunities.
Two of the defects were this document's and its drafts' rather than the implementation's, and one of the simplifications withdraws a Revision 10 decision.
All three are taken here, because each changes what a story is required to deliver.

- **Recipe 2's deployment lifecycle is part of the recipe: stop, fetch, start.** The sketch presented `service_completed_successfully` as though ordering the fetcher ahead of DMS were the whole story, and it orders container startup only; a repeat deployment over a live stack runs the completed one-shot service again while an unchanged DMS keeps running on the same volume, and the fetcher clears the plugin directory before it extracts. Lazy assembly loading turns that into a request meeting a missing file or a dependency from a version the startup inventory did not record. No overlay can prevent it, so the requirement is carried by the overlay header, by `eng/docker-compose/README.md`, and by draft 04 as an acceptance criterion. See [Acquisition](#acquisition).
- **The inventory event's attribution criterion names the implementation type, and its test loads two plugins.** [Observability](#observability) and draft 04 specified `RegisteredServiceTypes` as a bare property and had the attribution test boot one plugin and assert the service type. That satisfies the words and proves nothing the story promises: every plugin registering the fan-in contract produces the same `ICustomResourceValidator` entry, while DMS-1434's audit names the offending **implementation** class, and no rule makes an implementation's namespace resemble its plugin's name. With one plugin loaded the attribution is true for want of an alternative. The event's registration and removal entries are now structures carrying the implementation type, the lifetime and a keyed flag, and the removal entry carries the replacement flag that finally consumes draft 03's `ReplacedServiceTypes`; the test boots two plugins that both register the contract, one refused, and asserts the named class appears against the guilty plugin and not the blameless one. Nothing is activated to produce any of it, and a service key is reported as a flag rather than rendered. See [Observability](#observability).
- **The plugin end-to-end lane has no committed environment file of its own.** Revision 10 gave Recipe 2's three variables a third documentation home, "the `.env.*` files the plugin end-to-end test uses", and what that produced was a copy of `.env.e2e` whose own header told the next person to keep the two in sync by hand. Every value the lane actually needs is computed per run, a scratch mount path, a scratch feed address and the digest of a nupkg packed moments earlier, so the committed half carried nothing but a commented-out block duplicating the README and `.env.example`. The harness reads `.env.e2e` and appends its per-run values, and the documentation homes are the two tracked files an operator reads. See [Acquisition](#acquisition).

Three of the reviewer's five were implementation defects with no half in this document, and are recorded so they are not read back into it as design gaps: the loader's warnings reached `Console.Error` and never the configured application logger, which [Observability](#observability) already promised and draft 04 now states as a criterion; `eng/docker-compose/README.md` illustrated the recipes with bare `docker compose` lines that skip the bootstrap the backend redesign assigns schema provisioning to (`reference/design/backend-redesign/design-docs/bootstrap/bootstrap-design.md:2522`, "DMS never provisions schema at startup"), so on a fresh environment neither example could produce the working deployment it illustrated; and the end-to-end harness tore its deployments down only on success, leaving the ordinary local stack's containers behind on any failure.

**Revision 14 (2026-09-15, second external review round on PR #1256).**
The same reviewer read Revision 13's result and found four defects, two of which change what draft 04 is required to deliver.

- **Stop, fetch, start is `-d`, not `-d -v`.** Revision 13 made the re-fetch lifecycle an acceptance criterion and the README illustrated it with `bootstrap-local-dms.ps1 -d -v`, which forwards Compose's volume deletion to the whole stack and takes the PostgreSQL data volume with it. A routine plugin update would therefore destroy the deployment's data. What the lifecycle needs is DMS stopped, which `down` alone does, and keeping the `plugins` volume is safe because the fetcher clears the plugin directory before it extracts. The README and draft 04 both name `-d`; `-v` belongs to an explicitly requested environment reset. See [Acquisition](#acquisition).
- **Draft 04's task list still named the withdrawn third documentation home.** Revision 13 removed "the `.env.*` files the plugin end-to-end test uses" from the acceptance criteria and left it standing in the task that implements them, so the two halves of the same story disagreed. The task now names the two tracked files and nothing else.

Two were implementation defects with no half in this document: the four host-integration plugin fixtures carried no test category, so both CI integration lanes, which filter on `ApiIntegration` plus a database category, built them and never ran them; and the attribution test searched every captured event for the refused implementation's name, which the inventory event already carries, so the assertion would have held even had the validator audit stopped naming the offending implementation.
...
**Fetching again is stop, fetch, start, and that is a property of the recipe rather than an operational footnote.**
`depends_on: condition: service_completed_successfully` orders container *startup*: it holds DMS back until the one-shot service has exited zero, and it says nothing about a completed one-shot service running again beside a container that is already up.
Re-running the deployment over a live stack does exactly that, because Compose recreates a service whose definition changed and leaves an unchanged one running, and the fetcher's third command clears the plugin directory before the fourth extracts into it.
The volume is mounted read-only into DMS, which stops DMS writing it and does nothing about a writer outside the container.
Assemblies load lazily, so the process does not hold every declared file open: a request arriving during or after the swap can meet a missing file, or load a dependency from a version other than the one the inventory event recorded, which is the one record tying a running process to the bytes it is serving.
Nothing inside an overlay can prevent this, so the recipe carries the requirement instead: take the stack down, fetch, bring it up.
Recipe 1 has no equivalent, because nothing in that recipe writes the mounted directory; replacing its contents is the operator's own step and the same rule applies to it for the same reason.
The overlay's own header and `eng/docker-compose/README.md` both state it, and the story that creates the overlay carries it as an acceptance criterion.
The end-to-end tiers tear down before every deployment, so they never exercise this and are not the evidence for it.
...
The remedy the message gives differs by who claimed, because only one of the two is an operator's to apply: removing all but one entry from `Plugins:Allowed` resolves one claim per plugin, and it cannot choose between two claims made inside a single plugin, where the allowlist holds one entry and removing it withdraws that plugin entirely.
So a single-claimant fatal names the plugin and how many registrations it made and addresses its author, and a multi-claimant fatal keeps the allowlist remedy.

**The claims counted are historical, and that is deliberate rather than an oversight beside the surviving-contribution rule below.**
Every descriptor the per-hook diffs attributed is counted, whether or not it survived a later hook.
Two plugins that each claimed one replace contract have made a decision only an operator can take, and a later removal of one of the two claims does not resolve which of them the operator meant; counting survivors here would let the pair pass whenever the second plugin happened to tidy up after the first.
The no-contract-registered rule reads survival because its question is different: whether anything the plugin contributed will ever be called.
The two accountings are not to be quietly aligned.
...
The set is deliberately inclusive rather than minimal, and non-generic `ILogger` is the reason to say so: measured on `net10.0`, neither `AddLogging` nor `ILoggingBuilder.AddSerilog` registers a descriptor for it, so no removal can match it in DMS as shipped.
...
**Protecting the removal alone leaves the pipeline open, so the host also reserves its unkeyed logging services against *addition*.**
A plugin does not have to remove anything to take the host's logging.
`ILoggerFactory` and `ILogger<T>` are resolved singly rather than enumerated, and a single-service resolve takes the **last** registration for a service type, so `services.AddSingleton<ILoggerFactory, Mine>()` inside a hook displaces the host's without a removal, without a `Replace`, and without any call the removal rules above can see.
So the wrapper refuses an **unkeyed** plugin registration of `Microsoft.Extensions.Logging.ILoggerFactory`, the non-generic `Microsoft.Extensions.Logging.ILogger`, and `Microsoft.Extensions.Logging.ILogger<>` open or closed, on every incoming write path: `Add`, `Insert`, and the indexer setter, before the descriptor lands.
The removal protection above is unchanged and still applies to whatever an indexer assignment overwrites.

**This is a reservation, and the document says so rather than presenting it as duplicate detection.**
The rule does not read the collection and does not ask whether a descriptor of that service type is already there.
It holds even where the host has registered none, because what it protects is the resolve a host component makes later rather than a descriptor that exists now; a rule conditioned on an existing descriptor would pass for a plugin that got there first.
The host's unkeyed logging services are the host's, on the same footing as the host-owned service types below and for a sharper reason: everything the host would use to report a plugin problem runs through them.

Two things stay permitted, and they are what keeps the reservation from being a ban on a plugin logging at all.
Adding an `ILoggerProvider` is untouched, because providers are **enumerated** into the factory rather than resolved instead of it, so a plugin's sink composes with the host's; that is the supported way for a plugin to write to the log.
A **keyed** logging registration is also permitted, because an unkeyed resolve reaches no keyed descriptor, so it replaces nothing the host resolves.
The two `AddSerilog` overloads are worth separating precisely, because an implementer reaching for the name will find both: `ILoggingBuilder.AddSerilog` adds a provider and is permitted, and it is the overload DMS itself calls (`Infrastructure/WebApplicationBuilderExtensions.cs:125`), while the `IServiceCollection.AddSerilog` overload, which `Serilog.Extensions.Hosting` supplies and a plugin may bring even though this repository carries only `Serilog.Extensions.Logging`, can register an `ILoggerFactory` and is consequently forbidden inside a plugin hook by this policy.
The real `AddLogging` helper stays permitted for a different reason again: every call it makes against the collection is in the `TryAdd` family, so against a host that has already wired logging - which both Ed-Fi hosts do before any hook runs - each one declines and nothing arrives for the reservation to refuse.
Against a collection holding no logging at all it would be refused, which is the reservation working as specified rather than a gap in it.

**The reservation is enforced at the wrapper rather than in the post-container audit, and that placement is a requirement rather than a convenience.**
The audit needs a logger before it can report anything, and the registration under test is precisely what decides which logger it gets.
A rule that only ran after the container was built would be enforced by machinery the offending registration had already taken over.
It reuses the wrapper's diagnostic channel and its latched-refusal mechanism for the same reason the removal rules do: a hook that wraps its setup in a catch-all must not be able to turn a host decision into a swallowed exception.
...
**A declared contract registered under `KeyedService.AnyKey` is fatal.**
The plugin startup guard refuses such a registration before activation, naming the contract.
On .NET 10, `GetKeyedServices(T, KeyedService.AnyKey)` enumerates concrete-key registrations and excludes the wildcard registration; enumerating a concrete key does not reach the wildcard fallback either.
A single-service lookup with a concrete key can reach that fallback, but the host has no concrete key supplied for it, so the required startup probe cannot cover this registration shape.
See [Microsoft's keyed-service documentation](https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection#keyedserviceanykey-property).

**There is no exemption for a wildcard registration the records do not attribute to a plugin.**
The requirement is a property of the registration and not of who made it: a host wildcard registration of a declared contract is exactly as unactivatable as a plugin's, and exempting it would leave the audit asserting a startup activation it never performed.
So the check reads both sources it has, the per-plugin records and the composition snapshot, and a registration present in both is reported once rather than twice.
What attribution changes is only what the finding can honestly say: one the records attribute names the contributing plugin, and one they do not names no plugin at all rather than inventing one from whichever plugin happened to be loaded.
An unkeyed declared-contract registration or one under a concrete key remains supported by this audit, subject to the contract's own rules, and a wildcard registration of a service that is not a declared contract is ordinary permitted work that nothing here inspects.

**What is refused is a surviving registration.**
The rule exists because the required startup activation cannot cover this registration shape, so it has nothing to say about a wildcard registration a later plugin removed: that descriptor is absent from the container, no activation is asserted for it, and refusing it would fail a composition whose final contents are entirely supported.
The records are read for attribution and are historical, so a record-sourced candidate is intersected against the composition snapshot by reference identity before it is reported.
Removing a permitted non-host descriptor remains permitted and remains recorded; whether the plugin that registered it is left with anything live is the separate question the surviving-contribution rule answers.
This is not the accounting the replace-cardinality rule above uses, which counts historical claims on purpose because a later removal does not resolve which claimant the operator meant.

**A plugin that leaves no surviving declared plugin contract registration is fatal.**
...
**Survival is what the check measures, not registration.**
A plugin's declared-contract additions are compared by reference identity against the descriptors on the collection when composition ended, and a plugin with none left there fails this rule.
The case that forces it is two plugins: removing a pre-existing descriptor that is neither host-owned nor part of the logging pipeline stays **permitted**, so a later plugin may legitimately take away an earlier one's declared-contract registration, and the composition that results has a plugin the host will never call while the operator believes otherwise.
The removal is nobody's fatal and remains recorded in the inventory event with what it displaced; the composition is what fails, and it fails against the plugin left with nothing live rather than against the plugin that removed it, which may well have contributed a contract of its own.
The two shapes read differently in the message, because the first question an operator asks differs: whether the plugin registered nothing this host declares, or whether no declared registration it made survived composition.
The per-plugin record keeps every addition and every removal either way, so the inventory event still reports what the plugin actually did rather than only what outlived it.
...
| Two plugins register the same replace-cardinality contract | **Fatal**, naming both plugins and the contract, with the allowlist remedy only an operator can apply. One plugin registering it twice is the same fatal, counted the same way, and names that plugin and its registration count with the remedy addressed to its author, because no `Plugins:Allowed` change can choose between two registrations made inside one plugin |
| A declared plugin contract is registered under `KeyedService.AnyKey` | **Fatal**, naming the contract, before activation. Enumerable resolution cannot reach the wildcard registration, so the required startup probe cannot cover it. Refused whoever registered it, because the requirement is a property of the registration rather than of who made it: the check reads the per-plugin records and the composition snapshot, reports a registration present in both once, names the contributing plugin where the records attribute one, and names no plugin at all where they do not. What is refused is a registration that survived composition: the rule exists because the required activation cannot cover this shape, and a wildcard registration a later plugin permissibly removed is activated by nothing, so record-sourced candidates are intersected against the composition snapshot by reference identity before they are reported. Unkeyed and concrete-key registrations remain supported by this audit, subject to the contract's own rules; wildcard registrations of services that are not declared contracts are unaffected |
...
| A plugin **adds** an unkeyed registration of `Microsoft.Extensions.Logging.ILoggerFactory`, the non-generic `ILogger`, or `ILogger<>` open or closed | **Fatal**, naming the plugin and the service type, before the descriptor reaches the real collection, on `Add`, `Insert`, and the indexer setter alike. These are resolved singly, so the last registration wins and a plugin displaces the host's logging by adding rather than removing, which the removal row above cannot see. An explicit reservation of the host's unkeyed logging services and not duplicate detection: it holds whether or not the host has registered one, because what it protects is the later resolve. Adding an `ILoggerProvider` and any keyed logging registration stay permitted, since neither is reached by the unkeyed single-service resolve a host component makes. `ILoggingBuilder.AddSerilog` adds a provider and is permitted; `IServiceCollection.AddSerilog` can register an `ILoggerFactory` and is refused. `AddLogging` reaches the collection only through the `TryAdd` family, so it declines against a host that has already wired logging and nothing arrives to refuse. Enforced at the wrapper rather than in the post-container audit because that audit needs a logger before it can report anything |
| A plugin leaves no surviving declared plugin contract registration and contributes no configuration source | **Fatal**, naming the plugin and listing what it did register. It ran and contributed nothing the host will call; the likeliest causes are a plugin allowlisted on the wrong host and a replace-contract claim made with `TryAdd`, which declines invisibly and adds nothing. This row reaches that second cause only when the decline was the plugin's one contribution; [Contract Cardinality](#contract-cardinality) records the residual for a plugin that also contributed something else. Survival is measured against the descriptors on the collection when composition ended, so a plugin whose only declared registration a later plugin permissibly removed lands here too, with a message that says so rather than saying it registered nothing |
...
| A registration of a declared plugin contract cannot be constructed | **Fatal**, via the plugin startup guard, which resolves the surviving supported declared-contract registrations once from a throwaway scope, through the unkeyed group and each discovered concrete-key group, after every static check has passed. Scoped and transient instances are discarded with the scope; a singleton resolved inside it is cached in the root container and remains the instance production uses, which is measured rather than assumed and is why the probe is an activation check and not a disposal boundary. Declared-contract registration must be finished by the composition snapshot: infrastructure the host registers after it is supported and unexamined, while a declared-contract registration or removal made after it is outside this audit's supported composition contract rather than something the probe undertakes to detect. This is the plugin guard's own activation probe, not the per-contract one: DMS-1434's guard activates `ICustomResourceValidator` because that contract's own story asked it to, and a contract added later would otherwise arrive with no activation at all |
...
It transfers unchanged, and the plugin mechanism adds five checks beside it that it did not need when the registrations were first-party: the replace-conflict check, the host-service displacement check, the wildcard-key refusal, the surviving-contribution check, and the declared-contract activation probe, all fed by the per-plugin records and the composition snapshot, while the recording wrapper rejects host-owned removals and overwrites, and registrations of the host's reserved unkeyed logging services, before they reach the collection.
Those five live in a startup task of their own; [Where the Code Lives](#where-the-code-lives) says why they cannot live in Core's.
...
**Each registration is reported as the service type together with the implementation type the descriptor names, its lifetime, and whether it is keyed**, because the service type on its own attributes nothing.
Every plugin registering the fan-in contract produces an entry naming `ICustomResourceValidator`, while the host guard that refuses a badly shaped registration names the offending **implementation** class and knows nothing about plugins, and no rule obliges an implementation's namespace to resemble its plugin's directory name.
So an event carrying only the service type tells an operator which contract was offended and leaves them to guess which of the loaded plugins supplied the offender, which is the gap the ordering requirement was meant to close and does not close by itself.
None of this activates anything: the implementation type is read off the descriptor, an instance registration is asked for its runtime type and never for its value, and a keyed registration's key is reported as a flag rather than rendered, since a key is a plugin-supplied object whose `ToString` the host does not control.
...
Second, every pre-existing non-host descriptor the plugin removed or overwrote, as the service type, the implementation it displaced, and whether the same plugin also registered that service type, since [Contract Cardinality](#contract-cardinality) permits those and this event is the only trace they leave.
That last flag is what distinguishes a replacement from a bare removal, and it is the consumer the per-plugin record's `ReplacedServiceTypes` member is kept for; deriving it again at the log site would be the second source that record exists to avoid.
...
| **Attaching a contract's SBOM and provenance to the GitHub release** | **Rejected at PR review.** Three shapes were decided here in turn: attaching on every prerelease, attaching from a combined job gated on the publishers' push decisions, and attaching per contract only after the publisher confirmed that the feed served that run's exact bytes, with the generator's envelope retained thirty days and a manual backfill script for the run that had published | A contract's version does not move with the release. The run that first pushes a version is an alpha prerelease, which attaches nothing to any release and whose GitHub release is deleted at the next final release; every later run, the non-alpha prerelease that populates the `vX.Y.Z` release included, finds the version on the feed, packs a different archive (a nupkg is not byte-reproducible) and pushes nothing, so its SBOM and provenance describe bytes the feed does not serve. Attaching on every prerelease shipped that wrong evidence. Gating on the push decision, or on byte confirmation, attached nothing in steady state. The backfill depended on run artifacts retained thirty days, while final releases here are 36 to 214 days apart, so the retained evidence expired before the release it belonged on existed, and nothing invoked the script. The contracts' SBOM and provenance are produced as run artifacts on every run and uploaded to no release; `attach-dms-artifacts-to-release` keeps its original five dependencies. Two read-only checks gate `pack-dms` and `pack-schema-tools` so no DMS artifact is built from a refused contract, which by design also skips the DMS release upload for that run |
