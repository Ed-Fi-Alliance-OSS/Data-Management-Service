...
| **Attaching a contract's SBOM and provenance from a combined job gated on the publisher's push decision** | **Rejected at PR review.** It was the previous decision here, after the earlier one of attaching on every prerelease | A contract's version does not move with the release, so most prereleases find it already on the feed and push nothing, and a nupkg is not byte-reproducible, so a run's evidence describes the feed's bytes only when the feed serves that run's exact archive. A combined job that needed both publishers let a refused sibling suppress unrelated evidence, and a push decision is an intent, not a publication. Each contract now has its own evidence job under `!cancelled()`, depending only on its own producers and publisher; the publisher rechecks before an optional push, then confirms by reading the feed again and comparing the served archive's SHA-256 with the packed file's, and only that confirmed result gates attachment, so a rerun that pushes nothing still attaches when the feed holds its bytes. The evidence is bound to the run, package file and digest before upload, uploads are idempotent by content, the signed envelope is retained thirty days beside the generator's five-day artifact, and `Invoke-ContractEvidenceBackfill.ps1` attaches an original run's evidence when the release's own run did not publish the bytes. Two read-only checks gate `pack-dms` and `pack-schema-tools` so no DMS artifact is built from a refused contract |
