...
| **Attaching a contract's SBOM and provenance to the release on every prerelease**, pushed or not | **Rejected at PR review.** It was the previous decision here | A contract's version does not move with the release, so most prereleases find it already on the feed, unchanged, and push nothing. The nupkg that run packed is not the nupkg on the feed, because a nupkg is not byte-reproducible, so its provenance names a subject hash no restore can reproduce, and a publish job that refused a changed package would still have had its evidence attached. `attach-dms-artifacts-to-release` now needs both contract publish jobs and gates each contract's download and upload steps on that job's `should-push` output; the run's own artifacts are still produced for every contract. The semantic comparison is unchanged: it decides whether a version may be reused, and provenance identifies the bytes published |
