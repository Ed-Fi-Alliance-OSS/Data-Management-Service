...
`SetDMSAssemblyInfo` regenerates the **tracked** `src/dms/Directory.Build.props` into a different file (`build-dms.ps1:210-244`): it drops `AssemblyVersion`, `FileVersion`, and `InformationalVersion`, adds `Copyright` and an empty `VersionSuffix`, declares `VersionPrefix` in their place, and writes unconditionally.
Its only caller today is `BuildAndPublish` (`build-dms.ps1:1868`, `:2006`), while `DockerBuild` is also reached from `E2ETests` (`:870-871`) and `InstanceE2ETests` (`:976-977`) unless `-SkipDockerBuild` is passed, so wiring it in would rewrite a tracked file and dirty `git status` on every local end-to-end run.
...
And it would not stamp what the lane stamps today: `Convert-ToAssemblyVersion` maps `8.4.0-pre.0.4` to `8.4.0.4` (`package-helpers.psm1:164-188`), whereas `VersionPrefix` stamping yields `8.4.0.0`, so the build height the `ASSEMBLY_VERSION` argument carries would be silently dropped in exchange.
...
One property falls out of the ordering and is worth naming, because it is the one DMS-1434's mechanism does not have: the host registers `PluginAuditInput` **after** every hook has already run, so a plugin that registered its own cannot displace it, since the host's is the last registration for that service type and a single-service resolve takes the last.
That is why [Contract Cardinality](#contract-cardinality)'s note about a plugin displacing a captured collection is offered to the custom validation story and not to this one.
...
| **Having `build-dms.ps1`'s `DockerBuild` run `SetDMSAssemblyInfo` before the image build**, so the local image stamps through `Directory.Build.props` the way the release lane does | **Rejected at the fourth panel review.** It was the previous decision here | `SetDMSAssemblyInfo` regenerates the **tracked** `src/dms/Directory.Build.props` into a different shape and writes unconditionally, while `DockerBuild` is reached from `E2ETests` and `InstanceE2ETests` unless `-SkipDockerBuild` is passed, so every local end-to-end run would dirty `git status`. It would also execute only on developer machines, since every CI end-to-end leg passes `-SkipDockerBuild` and CI's own image comes from `docker/build-push-action` with no build arguments. And it stamps a different value from the argument it replaces: `Convert-ToAssemblyVersion` maps `8.4.0-pre.0.4` to `8.4.0.4` while `VersionPrefix` stamping yields `8.4.0.0`. The stamping is removed and nothing replaces it; the local image's DMS assemblies carry the committed props version |
...
The published overlay is unchanged and is run exactly as committed; what the test adds is a feed, which is the deployment's side of the recipe in every real deployment too.
...
- `build-dms.ps1:210-244`, `SetDMSAssemblyInfo` - regenerates the tracked `src/dms/Directory.Build.props` with `VersionPrefix` equal to the release version, called only from `BuildAndPublish` (`:1868`, `:2006`); it must **not** reach `src/plugins/Directory.Build.props`, whose version is the contract's own, and it is deliberately not called from `DockerBuild`
- `build-dms.ps1:870-871`, `:976-977`, `package-helpers.psm1:164-188` - the `E2ETests` and `InstanceE2ETests` paths that reach `DockerBuild` unless `-SkipDockerBuild` is passed, and `Convert-ToAssemblyVersion`, which maps a prerelease version to a build height that `VersionPrefix` stamping would drop
