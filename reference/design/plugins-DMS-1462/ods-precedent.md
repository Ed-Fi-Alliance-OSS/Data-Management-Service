...
| Module construction and registration failures caught, logged, skipped | `OdsStartupBase.cs:379-386`, and again in the Implementation repo at `EdFi.Ods.WebApi/Program.cs:82-90`, which wraps each `ConfigureHost` call in its own `try`/`catch` and writes the failure to `Console.Error` before continuing to the next activity | **Refused.** Fatal. A silently absent secrets, identity, or validation plugin changes correctness or security posture. Both ODS seams do this, not just the DI one, so the refusal is against a consistent ODS position rather than against one lapse |
...
| The pre-host seam's invocation lives in a different repository from its definition | Implementation repo `EdFi.Ods.WebApi/Program.cs:78-85` | **Refused.** Both phases are invoked from `Program.cs` in this repository, from one load |
