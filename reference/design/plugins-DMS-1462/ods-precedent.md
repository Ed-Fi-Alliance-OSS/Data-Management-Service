...
One seam's invocation lives in the separate **Ed-Fi-ODS-Implementation** repository and is marked where it appears.
...
| Folder resolution, four fallbacks | `EdFi.Ods.Api/Helpers/AssemblyLoaderHelper.cs:90-120` - rooted at `:97`, project-relative at `:104`, executable-relative at `:115` |
...
| Replace semantics by module name sorting | `EdFi.Ods.Api/Helpers/TypeHelper.cs:22-40` |
...
| Its invocation | **Ed-Fi-ODS-Implementation**, `EdFi.Ods.WebApi/Program.cs:78-84`, via `TypeHelper.GetAssemblyTypes<IHostConfigurationActivity>()` and `Activator.CreateInstance`, before the host is built |
...
| Folder resolution tries rooted, then project-relative, then executable-relative | `AssemblyLoaderHelper.cs:90-120` | **Refused.** Multiple fallbacks make "which directory did it actually use" a diagnostic question. DMS resolves relative paths against `AppContext.BaseDirectory` and nothing else |
...
| Replace semantics by module ordering, `ICustomModule` last, then `Override`-prefixed names | `TypeHelper.cs:22-40` | **Refused.** Two plugins claiming one replace-contract is fatal and names both, rather than resolved by class-name sorting |
...
Sorting module class names to decide which registration wins makes the answer depend on how two vendors happened to name their classes.
