...
| C08 | `[DMS-1218] Return structured errors from tenant resolution` | 4 | INV-19…22 (`TenantResolutionMiddleware` 4 branches → `FailureResponseWriter` + `FailureResponse.ForBadRequest`/`ForUnknown`; statuses 400/400/500/400 preserved per D-06; `SanitizeForLog`/bypass/context behavior preserved) | C02, C07 | TenantResolutionMiddlewareTests | ready for review | recorded in C09 update (self-SHA rule) | INV-19…22 done; body-contract asserted per producer; `FailureUnknown` sentinel absent; S3398 helpers scoped into fixture; V-08 green; scoped V-02 shows no tenant ad-hoc bodies; C07 follow-ups applied | — | — |
...
| V-08 | 4/C08 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~TenantResolutionMiddlewareTests" --nologo --verbosity minimal` | Pass; body-shape asserted; statuses preserved | passed | Green (part of a 65/65 combined run). Missing/empty header → 400 `bad-request` "The 'Tenant' header is required when multi-tenancy is enabled"; not-found → 400 "Invalid tenant: …"; lookup failure → 500 `internal-server-error` (DB sentinel absent); unexpected result → 400 "Failed to validate tenant"; XSS tenant name → sanitized, no markup in body; each asserts content type, exact detail, type/title, known `TraceIdentifier` as `correlationId`, empty extension members, no legacy `error`/`message` | 2026-07-23 | C08 (pre-commit) | Existing fixtures extended (not replaced); statuses preserved | Yes |
...
| V-20 | 4/C07 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~PipelineExceptionBoundaryTests\|FullyQualifiedName~RequestLoggingMiddleware\|FullyQualifiedName~ClaimsManagementModuleTests" --nologo --verbosity minimal` | Pass; throwing tenant resolution → full Ed-Fi 500 | passed | 56/56 (C07) and re-green in C08's combined run. Tenant repo throws → 500 `internal-server-error` contract, sentinel absent, `TraceId` header == body correlationId, repo reached, exactly one HttpRequestFailed logged. `PipelineExceptionBoundaryTests` refactored to a nested `Given_…` fixture (FQN still matches) | 2026-07-23 | C07/C08 | Combined filter matches the reported run | Yes |
