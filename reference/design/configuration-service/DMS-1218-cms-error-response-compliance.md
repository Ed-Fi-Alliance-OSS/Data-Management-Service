...
| C06 | `[DMS-1218] Convert claims-management non-success DTOs to Ed-Fi errors` | 3 | INV-08…15 (reload/upload non-success DTOs → `FailureResults` per §12.2.1 deny-by-default; new `FailureResults.BadRequest`/`DataValidation` helpers; `LogError` + success responses preserved) | C05 | ClaimsManagementModuleTests, FailureResultsTests | ready for review | recorded in C07 update (self-SHA rule) | INV-08…15 done; deny-by-default helper (`ToUploadValidationFailureResult`); 13 new fixtures (grouping, both structure literals, database/pathless/future/mixed sentinels, all catch branches); V-07 65/65 green; scoped audit: no `ex.Message`/non-success DTO in module (only 200 `Results.Json` remains) | — | — |
