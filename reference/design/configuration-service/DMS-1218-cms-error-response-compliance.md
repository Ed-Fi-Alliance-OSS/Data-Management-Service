...
| C17  | *(committed post-rebase)* | *recorded in this commit's summary (self-SHA)* |
...
| V-23 | 8/C16-C17 | CMS Config E2E (Reqnroll) — CI jobs "Run E2E Tests (self-contained)" and "Run E2E Tests (keycloak)" on PR #1125; locally requires the full DMS+CMS+DB(+Keycloak) Docker stack | All pass; `ClaimsManagement.feature` `_05` asserts the exact Ed-Fi data-validation contract | **run (baseline) — original failure recorded; rerun pending** | **This suite was NOT run during C01–C15a** (only unit + focused suites were), which is why the shape change went undetected until PR CI. **Original PR CI (commits through C15a):** both Postgres jobs failed on the single scenario `_05 Upload invalid claims returns error` (self-contained 193/196 pass, 1 fail; keycloak 191/196 pass, 1 fail) — `errors.Count > 0` asserted the pre-compliance shape. All four **MSSQL** E2E jobs passed but run only a 17-test smoke subset excluding `_05`. **C16** aligned the assertion (too permissive); **C17** strengthened it to the exact contract. **CI rerun after C17 is pending — not yet marked passed.** | 2026-07-24 | C16/C17 (CI) | Root-cause suite for the PR failure; must be in DoD going forward | Yes |
...
- [ ] **CMS Config E2E suite (Reqnroll) green (V-23).** Full PostgreSQL E2E jobs ("self-contained" + "keycloak") and the Config CI Gate must pass on PR #1125. **Open — CI rerun pending after C17.** (Gap: this suite was not run during C01–C15a; it caught the `_05` assertion mismatch only at PR CI.)
...
*End of R4. Status: Approved — 2026-07-23; on PR #1125 (rebased base `b12c5522`). Implementation delivered across commits C01–C17 (test-convention follow-ups C10a/C11a/C12a/C15a; final-review hardening C15; post-PR-CI E2E-assertion fixes C16/C17), each behind an architect approval gate. All inventory items resolved; the `FailureResponseWriter` `Content-Length` defect closed with a regression test; full CMS frontend suite 603/603 and backend `FailureResponseTests` 30/30 green. The one outstanding gate is the CMS Config E2E suite (V-23): the `_05` assertion is corrected (C16) and strengthened to the exact contract (C17); the PostgreSQL E2E CI rerun is pending — not yet green.*
