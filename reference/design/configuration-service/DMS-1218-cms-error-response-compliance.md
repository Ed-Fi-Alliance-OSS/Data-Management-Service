...
| C05 | `[DMS-1218] Return structured 500 for current-claims exceptions` | 2 | INV-05/06 (`GetCurrentClaims` JsonException/InvalidOperationException catches → `FailureResults.Unknown`; `LogError` preserved) | C02 | ClaimsManagementModuleTests | ready for review | recorded in C06 update (self-SHA rule) | INV-05/06 done; two exception fixtures assert full 500 contract + secret-sentinel absence + no legacy `error`/`message`; V-07 green (24/24); reload/upload DTOs untouched (C06) | — | — |
...
| V-07 | 2-3/C05-C06 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~ClaimsManagementModuleTests" --nologo --verbosity minimal` | Pass; 500/400 Ed-Fi bodies; no `ex.Message`/DTO shape | passed (C05 scope) | 24/24 passed; both `GetCurrentClaims` exception fixtures assert 500 + `application/problem+json` + empty detail + type `internal-server-error` + title "Internal Server Error" + status 500 + non-empty correlationId + empty validationErrors/errors + secret-sentinel absent + no legacy `error`/`message` keys | 2026-07-23 | C05 (pre-commit) | INV-05/06 done; reload/upload DTO conversion re-runs this at C06 | Yes |
