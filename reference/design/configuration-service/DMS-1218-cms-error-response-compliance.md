...
| C11 | `[DMS-1218] Make bad HTTP request handling status-aware` | 5 | INV-30 (`GlobalExceptionHandler` `BadHttpRequestException` status-aware via `FailureResponseWriter`: 400→`ForBadRequest`, 415→`ForUnsupportedMediaType`, else `ForUnclassifiedStatus`/`about:blank` D-08; new `FailureResponse.ForUnclassifiedStatus`; `TraceId` header preserved; no message leak) | C02, C10 | GlobalExceptionHandlerTests, FailureResponseTests | ready for review | recorded in C12 update (self-SHA rule) | INV-30 done; reachability audit (400/413 reachable, 415 defensive); `GlobalExceptionHandlerTests` refactored to nested `Given_` fixtures (400/415/413-about:blank/validation/500) with sentinel-absence + TraceId-match; V-12 + backend `FailureResponseTests` (12/12) + Pipeline/Framework regressions green (42/42) | — | — |
...
| V-12 | 5/C11 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~GlobalExceptionHandlerTests" --nologo --verbosity minimal` | Pass; malformed body → 400 `bad-request`; 415 → `unsupported-media-type` (Ed-Fi URI); any reachable status with no ticket/KB/platform URI (413) → `about:blank` + reason phrase; body `status` == HTTP status | passed | Run within a 42/42 combined regression (GlobalExceptionHandlerTests + PipelineExceptionBoundaryTests + FrameworkErrorResponseTests). GlobalExceptionHandlerTests fixtures: 400→`bad-request` "The request was malformed or invalid."; 415→`unsupported-media-type`; **413→`about:blank`, title = `ReasonPhrases.GetReasonPhrase(413)` ("Payload Too Large")**; validation→`bad-request:data` with grouped validationErrors; generic→500 `internal-server-error`. Each asserts exact status/type/title/detail, `application/problem+json`, correlationId == `TraceIdentifier` == `TraceId` header, empty extensions where appropriate, and sentinel absence | 2026-07-23 | C11 (pre-commit) | Status-aware `BadHttpRequestException`; D-08 `about:blank` | Yes |
