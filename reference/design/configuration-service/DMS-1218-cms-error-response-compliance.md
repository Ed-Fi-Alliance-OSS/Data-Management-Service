...
| Branch | `DMS-1218-V2` |
| Original audit/implementation base | `5922d4fb403ac2fe23510ad4110fb305e71a4d70` (all line numbers, greps, and per-commit reviews below were performed against this commit) |
| Rebased PR base | `b12c5522` (`origin/main` at push time; the series was rebased onto it before opening PR #1125 — one non-overlapping commit, [DMS-1250], touching only `.github/workflows/scheduled-build.yml`, `AGENTS.md`, `src/dms/Directory.Build.targets`). The rebase rewrote every commit SHA — see the §26 reconciliation table for the current-branch hashes |
...
- **415 URI classification [Corrected 2026-07-27].** `urn:ed-fi:api:unsupported-media-type` (415, "Unsupported Media Type") **is KB-documented** — the live Knowledge Base has a "415 Unsupported Media Type" section using this exact URI — **and** is independently established in DMS Core `src/dms/core/EdFi.DataManagementService.Core/Response/FailureResponse.cs:47` (`_unsupportedMediaTypeType`). CMS reuses this established URI for 415. (405 is likewise both KB-documented and a DMS platform convention — `FailureResponse.cs:29`.)
...
- **2026-07-23 — C14 (final phase).** Committed `92ad82ccab38d81ce85f851fa5d870f3469e1dd1`. Static audits re-run (V-01/V-02 acceptance met: 0 bare + 0 ad-hoc; V-03 no obsolete URI; V-14 no dead refs); `csharpier format src/config` reformatted 0 C# files (V-17); full frontend suite 600/600; backend `FailureResponseTests` 12/12; `git diff --check` clean (V-18); duplicate C14 row removed. Submitted for final review.
- **2026-07-23 — Final review (C14). Final revision required.** One **blocking** defect: `FailureResponseWriter` wrote the body without correcting a stale `Content-Length` — framework bodiless errors (e.g. 404/405) carry `Content-Length: 0`, and Kestrel rejects a write exceeding the declared length, so a shaped response could throw instead of returning the Ed-Fi body (TestServer masked this in the integration tests). Plus residual test-convention items (two `FailureResponseWriterTests` scenarios and the three new backend factory tests still lacked `[SetUp]`-arrange/act + `It_`) and tracking-document truthfulness (C14 row still `in progress`; V-15 recorded only the interim C13 run; V-10 `not run` despite V-12's identical filter; the provider-detail checkbox conflicted with the retained IdP parser). Directed to open **C15**.
- **2026-07-23 — C15.** `FailureResponseWriter` now serializes to a UTF-8 byte array and sets `Content-Length` to that exact length, replacing any stale value before writing via `Response.Body.WriteAsync` — Kestrel accepts the write regardless of the pre-existing `Content-Length: 0`. Added the `Given_a_response_with_a_stale_zero_content_length` regression fixture (asserts the length is reset to the body byte count and the full body is emitted); reworked the two flagged writer scenarios and the three backend factory tests into `Given_`/`[SetUp]`/`It_` (legacy flat tests untouched). Corrected V-10 (superseded by V-12), V-15 (formal Phase-7 603/603), V-16 (30/30, factories now nested fixtures), the C14 row (committed + SHA), and the DoD provider-detail wording (§14 caveat). Evidence: V-22 focused 47/47, V-16 backend 30/30, V-15 full frontend 603/603, `csharpier` 0 C# reformatted, `git diff --check` clean. Submitted for final re-review.
- **2026-07-23 — Final review (C15). Production approved; test-only revision required.** The `FailureResponseWriter` `Content-Length` fix, regression coverage, factory-test restructuring, and specification corrections are sound. One conformance item: `FailureResponseWriterTests` declared `private Exception? _exception;`, but AGENTS.md requires non-nullable variable declarations. Directed to open **C15a** (test + spec only): change to `private Exception _exception = null!;`, preserve the capture and `BeOfType<ArgumentException>()` assertion. C15 committed `4247bb79911def9c1f86ead0496bf3eb732bedd0`.
- **2026-07-23 — C15a.** `_exception` made non-nullable (`private Exception _exception = null!;`); exception capture and `It_throws_an_argument_exception` assertion unchanged; no production change. Evidence: CSharpier 0 C# reformatted, `FailureResponseWriterTests` green, `git diff --check` clean, worktree clean. Submitted for final re-review.
- **2026-07-24 — PR #1125 CI. E2E regression found and fixed (C16).** After the series was approved for PR, GitHub CI failed the **Config CI Gate** — an aggregator that failed only because `run-e2e-tests` failed. The two **PostgreSQL** E2E jobs (self-contained + keycloak) each reported a single failure, `ClaimsManagement.feature:79` `_05 Upload invalid claims returns error` (193/191 passed, 1 failed); all four **MSSQL** E2E variants passed because they run only a 17-test smoke subset that excludes `_05`. Root cause: the step `ThenTheResponseBodyContainsValidationErrors` asserted `errors.Count > 0` — the pre-compliance shape. For the invalid body `{ "claims": { "claimSets": [{"invalidProperty":"test"}], "claimsHierarchy": [] } }` the claims JSON schema rejects the claimSet (missing required `claimSetName`/`isSystemReserved`, `additionalProperties:false`) and the empty hierarchy (`minItems:1`), yielding `"Validation"` failures with non-whitespace paths (`$.claimSets[0]`, `$.claimsHierarchy`). Per §12.2.1 those are surfaced in **`validationErrors`** with `errors: []` — the production response is already contract-compliant. Fix (C16, test only): the step now requires validation detail under `validationErrors` or `errors`. This exposed a process gap — the CMS E2E suite was not run locally during C01–C15a (only unit + focused suites were); the DoD command set should include the config E2E going forward.
- **2026-07-27 — Documentation correction (post-PR review).** Re-fetching the live Ed-Fi Error Response Knowledge Base found a "415 Unsupported Media Type" section documenting `urn:ed-fi:api:unsupported-media-type` verbatim, contradicting this spec's R2/R3-era classification of that URI as "not KB-documented." Corrected §3.1, the §11 taxonomy matrix, and §14's factory description to state the URI is both KB-documented and an established DMS platform convention. No production behavior changed — CMS already emits this exact URI for 415. The R3 revision note, decision log (§24 D-08), and §29.7b are left as an accurate historical record of what was believed at the time; they are not rewritten.
...
| C14 | `[DMS-1218] Format and final regression cleanup` | 7 | Final static audit, formatting, regression, DoD, and duplicate-row cleanup | all | full CMS frontend suite + backend `FailureResponseTests` | committed | `92ad82ccab38d81ce85f851fa5d870f3469e1dd1` | Final review 2026-07-23 found a **latent `Content-Length` defect** in `FailureResponseWriter` (bodiless framework errors carry `Content-Length: 0`; Kestrel rejects the body write — TestServer masked it) plus residual test-convention/tracking items — all addressed in C15 | — | 2026-07-23 |
| C15 | `[DMS-1218] Harden framework error writing and finalize verification` | 7 | Final-review follow-up: `FailureResponseWriter` now serializes to UTF-8 bytes and sets the exact `Content-Length`, replacing any stale `0` so Kestrel accepts the write; new zero-`Content-Length` regression fixture; refactor the two flagged writer scenarios (no-status, already-started) and the three backend factory tests (405/415/about:blank) into `Given_`/`[SetUp]`/`It_`; V-10/V-15/V-16 evidence corrected; DoD provider-detail wording aligned to §14 | C14 | FailureResponseWriterTests, FailureResponseTests (backend) | committed | `4247bb79911def9c1f86ead0496bf3eb732bedd0` | Production approved 2026-07-23 (final review): Content-Length fix + regression sound. Test-only follow-up required (`_exception` declared nullable) — addressed in C15a. Focused 47/47, backend 30/30, full frontend 603/603 | — | 2026-07-23 |
| C15a | `[DMS-1218] Align writer test state with repository conventions` | 7 | C15 follow-up: `FailureResponseWriterTests` `_exception` field made non-nullable (`private Exception _exception = null!;`) per AGENTS.md; exception capture + `BeOfType<ArgumentException>()` assertion preserved (no production change) | C15 | FailureResponseWriterTests | committed | `38d47844c5fd6a2d57b8d508090357d143db31e9` | Architect-approved 2026-07-23 (final re-review); test/spec only; non-nullable field convention; `FailureResponseWriterTests` 12/12. **Series approved for PR** | — | 2026-07-23 |
| C16 | `[DMS-1218] Align claims-upload E2E assertion with the Ed-Fi contract` | 8 (post-PR CI) | PR-CI follow-up: `ClaimsManagement.feature:79` (`_05 Upload invalid claims`) failed on both Postgres E2E runs — the step `ThenTheResponseBodyContainsValidationErrors` asserted the **pre-compliance** shape (`errors.Count > 0`). Under the approved contract (§12.2.1) the invalid body's schema-validation failures are surfaced in **`validationErrors`** (keyed by path) with `errors: []` (test only; no production change — the production response is already compliant) | C15a | ClaimsManagementStepDefinitions (E2E) | committed | `5e75ff414df83d497196f45fc24644a05b993285` | Root cause: MSSQL E2E runs only a 17-test smoke subset (passed); the Postgres E2E runs the full 196 incl. `_05`. **Review found the C16 assertion too permissive** (either-location `detailCount`, tolerated legacy `success`) — strengthened in C17 | — | 2026-07-24 |
| C17 | `[DMS-1218] Strengthen claims-upload E2E contract verification` | 8 (post-PR CI) | C16 follow-up: rewrite the `_05` step to assert the exact data-validation contract — `type=urn:ed-fi:api:bad-request:data`, title, detail, status 400, non-empty `correlationId`, **`validationErrors` present and non-empty**, **`errors` present and empty**, and legacy `success`/`Success` members absent; remove the permissive either-location `detailCount`; non-nullable locals per AGENTS.md (test only; no production change) | C16 | ClaimsManagementStepDefinitions (E2E) | committed | `e06ea4a9e617ba931b61d18f4781297fa7b4ed0b` | Architect-approved 2026-07-24; exact-contract assertion; a pre-compliance body can no longer pass. E2E project builds clean (0 warn/0 err) | — | 2026-07-24 |
| C18 | `[DMS-1218] Record green PR CI in verification ledger` | 8 (post-PR CI) | Spec only: mark V-23 and the E2E DoD item passed and update the closing status after CI went green on HEAD `e06ea4a9` (whole PR 55 pass / 2 skip / 0 fail) | C17 | (spec only) | committed | recorded post-commit (self-SHA) | Truthful record of the passing rerun; no code/test change | — | 2026-07-24 |

### 26.1 Commit SHA reconciliation (post-rebase)

The **Actual SHA** column above records the **as-reviewed, pre-rebase** hash for each commit C01–C15a (the exact object the architect approved at that gate; base `5922d4fb`). Opening PR #1125 required rebasing the series onto `origin/main` at `b12c5522`, which rewrote every SHA. The table below is the authoritative pre-rebase → **current-branch** mapping. C16/C17 were committed after the rebase, so their Actual-SHA entries above are already current.

| Commit | Pre-rebase (as-reviewed, base `5922d4fb`) | Current branch (base `b12c5522`, PR #1125) |
|---|---|---|
| C01  | `7e6d52401b4fe92eec25985ff9dbff0aec151f74` | `a602f968ff47d2a7545c46e782264ce1485654c2` |
| C02  | `ded5a4fae4f2c904391f56a2bf8631e609ec3b07` | `dea514ac0da5569f9f7f6b7f8a944e0bac3825d2` |
| C03  | `ff18145dac82f21cde872be50196a27081e6a040` | `a14b32b41895e5d5a85f53270113f5cf1bb37bc8` |
| C04  | `6d90dedbff2e1ec7c4aac1ae5b4ea3d753fe24f2` | `3be9e26235ab25bddb5369429b481c02ac9bf539` |
| C05  | `0b642bf3a4549da3f2f0f83e46ab3cd42ff0a216` | `9bd6f0f05647aae7ce9d66dbe0bd669c21184bad` |
| C06  | `5d207987063d7074d977ed6c11fc75250b8d6fb9` | `68d094510762e876ecd951337dccf94e9afcb253` |
| C07  | `b0ceb7e2623160b0d409db0efc32c9539f3616c4` | `f671e69ee18d5c8a651fd777f4d8de3cbb2d64f0` |
| C08  | `46a40c50887934b247dcfbc26e1459160a9897cb` | `71c9ff9269138af58004d2dc0c114fd9f040938a` |
| C09  | `496983352573fff551d9ff3ee9644d35aa0cd9c4` | `34904a499707115dede8ba6bd35ac87492b66168` |
| C10  | `8d5586948e77286fdabe84bdf6a1e60c1df1f0a5` | `d4ac308d000c2cc338b73fef7ed921608ca01274` |
| C10a | `acdcc27a3aa32919d8b76e6eea1ed47713004154` | `7288d6c49ee24e1e4a84dbc085de94e81a15a5be` |
| C11  | `b5a7e92a15214e7966ded36d21e0f41a8649d621` | `c8a5e655b64f688f725f095860c7878fff2415c6` |
| C11a | `934e1ad4e70864c23821bc70f1972199cf4b0cb8` | `ac02cb291925982caa22785748fd0a72ede46654` |
| C12  | `c5d1528343d7ad3780722b0e2d78027cd86e057f` | `4a87e3b77376cc15fb48ee7d4edc37d4682e64dc` |
| C12a | `75844da3747a58bf3e583af696937733d67fd2fd` | `793a0c8a7a7febaac18bc5a5d1401a2766861f83` |
| C13  | `5395822e12850ea095d546234fa35907b6be7307` | `3c5164e83bd5496b169f1f1fbb1b8e238518fa07` |
| C14  | `92ad82ccab38d81ce85f851fa5d870f3469e1dd1` | `515d297cbd93c9e38eb23fe9d12f19ede00fcc69` |
| C15  | `4247bb79911def9c1f86ead0496bf3eb732bedd0` | `453e9f44c466407dccc97ee0989f976553b7d340` |
| C15a | `38d47844c5fd6a2d57b8d508090357d143db31e9` | `b9270f000f64b997027cd92a5bcd9f3168840425` |
| C16  | *(committed post-rebase)* | `5e75ff414df83d497196f45fc24644a05b993285` |
| C17  | *(committed post-rebase)* | `e06ea4a9e617ba931b61d18f4781297fa7b4ed0b` |
| C18  | *(committed post-rebase)* | *recorded in this commit's summary (self-SHA)* |
...
| V-10 | 4-5 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~GlobalExceptionHandlerTests" --nologo --verbosity minimal` | Pass | superseded by V-12 | **Superseded** — this command is byte-for-byte identical to V-12's, which executed it (C11a: **18/18**) and again as part of the C15 focused re-run (**47/47** across writer/framework/handler). No separate execution is needed; see V-12 for the enumerated result | 2026-07-23 | C11a / C15 | Regression + INV-30 (covered by V-12) | Yes |
...
| V-15 | 7 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --nologo --verbosity minimal` | All pass | passed | **Formal Phase-7 acceptance run (C15): 603/603 passed** — full CMS frontend unit suite green after the `FailureResponseWriter` `Content-Length` hardening and the writer test-convention rework (+3 vs the C13 interim: 2 new zero-`Content-Length` assertions, 1 from splitting the already-started scenario into two `It_`). (Interim history: C02 538, C10a 594, C13 600.) | 2026-07-23 | C15 (pre-commit) | Full CMS frontend unit suite | Yes |
| V-16 | 7 | `dotnet test src/config/backend/EdFi.DmsConfigurationService.Backend.Tests.Unit/EdFi.DmsConfigurationService.Backend.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseTests" --nologo --verbosity minimal` | Pass | passed | **C15 re-run: 30/30 passed.** The three new factories (`ForMethodNotAllowed` 405, `ForUnsupportedMediaType` 415, `ForUnclassifiedStatus` → `about:blank`) are now nested `Given_`/`[SetUp]`/`It_` fixtures (21 `It_` methods); the legacy flat factory tests (`ForUnauthorized`/`ForForbidden`/`ForBadRequest`/`ForNotFound`/`ForConflict`/`ForDataValidation`/`ForNonUniqueIdentity`/`ForBadGateway`/`ForUnknown`, incl. F7 `…:bad-request:data`) are untouched | 2026-07-23 | C15 | Narrowly-necessary backend test; new factories converted to convention | No |
...
| V-19 | 7/C15a | `git status --porcelain` | Only in-scope files; clean worktree after commit | passed | Every commit C01–C15a ended with a clean worktree; all changes confined to `src/config` (frontend, datamodel, backend tests) + the tracked spec. Final clean worktree confirmed after the C15a commit | 2026-07-23 | C15a (post-commit) | DoD | No |
...
| V-22 | 7/C15 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseWriterTests\|FullyQualifiedName~FrameworkErrorResponseTests\|FullyQualifiedName~GlobalExceptionHandlerTests" --nologo --verbosity minimal` | Pass; writer replaces a stale `Content-Length: 0` with the body's UTF-8 byte length and writes the full body | passed | **47/47 passed.** New `Given_a_response_with_a_stale_zero_content_length` fixture proves the writer sets `Content-Length` to the exact UTF-8 byte count (not `0`) and emits the complete Ed-Fi body — closing the Kestrel body-write defect that TestServer masked. The two previously-flagged writer scenarios (missing-status → `ArgumentException`; already-started → no status/content-type write) reworked to `Given_`/`[SetUp]`/`It_`; framework + handler regressions unchanged | 2026-07-23 | C15 (pre-commit) | Content-Length regression (Kestrel body-write) | Yes |
| V-23 | 8/C16-C17 | CMS Config E2E (Reqnroll) — CI jobs "Run E2E Tests (self-contained)" and "Run E2E Tests (keycloak)" on PR #1125; locally requires the full DMS+CMS+DB(+Keycloak) Docker stack | All pass; `ClaimsManagement.feature` `_05` asserts the exact Ed-Fi data-validation contract | **passed** | **This suite was NOT run during C01–C15a** (only unit + focused suites were), which is why the shape change went undetected until PR CI. **Original PR CI (commits through C15a):** both Postgres jobs failed on the single scenario `_05 Upload invalid claims returns error` (self-contained 193/196 pass, 1 fail; keycloak 191/196 pass, 1 fail) — `errors.Count > 0` asserted the pre-compliance shape. All four **MSSQL** E2E jobs passed but run only a 17-test smoke subset excluding `_05`. **C16** aligned the assertion (too permissive); **C17** strengthened it to the exact contract. **Rerun on HEAD `e06ea4a9`: `Run E2E Tests (self-contained)` pass (3m52s), `Run E2E Tests (keycloak)` pass (3m18s), `Config CI Gate` pass — whole PR 55 pass / 2 skip / 0 fail.** | 2026-07-24 | C16/C17 (CI, HEAD `e06ea4a9`) | Root-cause suite for the PR failure; must be in DoD going forward | Yes |
