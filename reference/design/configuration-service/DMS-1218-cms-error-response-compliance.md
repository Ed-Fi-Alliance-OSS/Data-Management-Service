...
| V-04 | 1/C02 | `dotnet test src/config/backend/EdFi.DmsConfigurationService.Backend.Tests.Unit/EdFi.DmsConfigurationService.Backend.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseTests" --nologo --verbosity minimal` | Pass (incl. `…:data`) | passed | 9/9 passed (F7 `…:bad-request:data` intact) | 2026-07-23 | C02 (pre-commit) | Regression guard F7 | Yes |
...
| V-15 | 7 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --nologo --verbosity minimal` | All pass | passed (interim @ C02) | 538/538 passed — content-type change to `NotFound`/`Unknown` caused no module regressions | 2026-07-23 | C02 (pre-commit) | Full CMS frontend unit suite; formal acceptance re-run at Phase 7 | Yes |
