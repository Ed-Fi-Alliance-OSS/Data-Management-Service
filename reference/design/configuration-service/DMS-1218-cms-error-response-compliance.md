...
- `Authorization(string detail, string correlationId, string[] errors)` — **D-02**: builds a 403 `security:authorization` body with an explicit `errors` array and **no IdP-JSON parsing** (so INV-01 yields `errors: ["Registration is disabled."]`, not the mangled `["Forbidden. …"]` produced by the existing `GetIdentityErrorDetails`). Add analogous explicit-`errors` overloads only where a *changed* branch needs them.
...
|---|---|---|---|---|---|---|---|---|---|---|
| C01 | `[DMS-1218] Add CMS error-response compliance implementation spec` | 0 | This spec (approved), tracked path | — | n/a | approved (committing) | recorded in C02 update (self-SHA rule) | Approved R4 2026-07-23; commit then stop for review | — | 2026-07-23 |
| C02 | `[DMS-1218] Centralize CMS error helpers, writer, and content type` | 1 | `FailureResults` problem+json + `Authorization` overload + `FailureResponseWriter` | C01 | FailureResponseTests, new helper/writer tests | not started | — | — | — | — |
| C03 | `[DMS-1218] Return structured 403 for disabled client registration` | 2 | INV-01 | C02 | RegisterEndpointTests | not started | — | — | — | — |
| C04 | `[DMS-1218] Return structured 404 for disabled claims endpoints` | 2 | INV-02/03/04 | C02 | ClaimsManagementModuleTests | not started | — | — | — | — |
| C05 | `[DMS-1218] Return structured 500 for current-claims exceptions` | 2 | INV-05/06 | C02 | ClaimsManagementModuleTests | not started | — | — | — | — |
| C06 | `[DMS-1218] Convert claims-management non-success DTOs to Ed-Fi errors` | 3 | INV-08…15 | C05 | ClaimsManagementModuleTests | not started | — | — | — | — |
| C07 | `[DMS-1218] Move exception boundary ahead of tenant/config middleware` | 4 | INV-36 + pipeline test | C02 | pipeline test, GlobalExceptionHandlerTests | not started | — | — | — | — |
| C08 | `[DMS-1218] Return structured errors from tenant resolution` | 4 | INV-19…22 | C02, C07 | TenantResolutionMiddlewareTests | not started | — | — | — | — |
| C09 | `[DMS-1218] Return structured 500 for invalid configuration` | 4 | INV-23 | C02, C07 | invalid-config test | not started | — | — | — | — |
| C10 | `[DMS-1218] Shape framework 401/403/404/405/415 responses` | 5 | INV-25…29 (scheme-independent) | C02 | AuthorizationTests + framework tests | not started | — | — | — | — |
| C11 | `[DMS-1218] Make BadHttpRequestException handling status-aware` | 5 | INV-30 | C02 | GlobalExceptionHandlerTests + new | not started | — | — | — | — |
| C12 | `[DMS-1218] Convert OAuth/OIDC error responses to Ed-Fi contract` | 6 | INV-16/17/18 | C02 | IdentityModuleTests | not started | — | — | — | — |
| C13 | `[DMS-1218] Remove unwired OpenIddict error middleware` | 6 | INV-24 | C12 | delete OpenIddictErrorHandlingMiddlewareTests | not started | — | — | — | — |
| C14 | `[DMS-1218] Format and final regression cleanup` | 7 | formatting/regression | all | full CMS unit suite | not started | — | — | — | — |
...
|---|---|---|---|---|---|---|---|---|---|
| V-01 | 0/7 | `rg -n "Results\.(Forbid\|NotFound\|BadRequest\|Unauthorized\|Conflict\|StatusCode)\|TypedResults\.(Forbid\|NotFound\|BadRequest\|Unauthorized\|Conflict\|StatusCode)\|Results\.Json\(" src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore -g "*.cs"` | Post-impl acceptance: every match is a compliant helper/writer or preserved-compliant inline body | run (baseline) | **Complete baseline enumeration:** non-compliant matches found by this command = INV-01 (`Results.Forbid`), INV-02/03/04 (`Results.NotFound`), INV-05/06 (`Results.Json{error,message}`), INV-08…15 (`Results.Json` DTOs), INV-16/17/18 (`Results.Json` OAuth). Compliant matches = inline `Results.Json(FailureResponse.ForX)` across the 12 CRUD modules + `FailureResults.*` (INV-33) and success `Results.Ok/Json` (INV-34/35). This command does **not** surface middleware/framework producers (INV-19…29) — see V-02. | 2026-07-23 | 5922d4fb | Drafting sweep (Grep-tool equivalent of the `rg`); acceptance met only after Phases 2/3/6; canonical `rg` re-run in Phase 7 | Yes |
| V-02 | 0/7 | `rg -n "WriteAsJsonAsync\|WriteAsync\|Response\.StatusCode\|new \{ error\|new \{ active\|error_description" src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore -g "*.cs"` | Post-impl acceptance: no ad-hoc `{error,message}`/`{error,error_description}` error bodies remain | run (baseline) | **Complete baseline enumeration:** `TenantResolutionMiddleware` ×4 (`WriteAsJsonAsync` + `Response.StatusCode`, INV-19…22); `ReportInvalidConfigurationMiddleware:19` (`Response.StatusCode = 500`, INV-23); `ClaimsManagementModule:284/289` (`new { error`, INV-05/06); `IdentityModule` OAuth (`error_description`, INV-16/17/18); `OpenIddictErrorHandlingMiddleware` (`WriteAsync` + `error_description`, dead, INV-24). Non-producer/compliant matches also surface: `GlobalExceptionHandler` `WriteAsync` (INV-32, compliant), `FailureResults.cs` `GetIdentityErrorDetails` (`error_description` **parser**, INV-33 helper, compliant), `MetadataModule:195` `WriteAsync` (INV-35, OpenAPI 200 success), `RequestLoggingMiddleware:149` `Response.StatusCode` (read-only), `IdentityModule` `new { active }` (INV-34, success). | 2026-07-23 | 5922d4fb | Acceptance met only after Phases 3/4/6; re-run each phase | Yes |
...
| V-04 | 1/2 | `dotnet test src/config/backend/EdFi.DmsConfigurationService.Backend.Tests.Unit/EdFi.DmsConfigurationService.Backend.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseTests" --nologo --verbosity minimal` | Pass (incl. `…:data`) | not run | | | | Regression guard F7 | Yes |
| V-05 | 2/C03 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~RegisterEndpointTests.When_allow_registration_is_disabled" --nologo --verbosity minimal` | Pass; 403 `security:authorization` body | not run | | | | Extend to assert body | Yes |
| V-06 | 2/C04 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~ClaimsManagementModuleTests" --nologo --verbosity minimal` | Pass; 404 `not-found` body | not run | | | | 404 body-shape fixtures | Yes |
| V-07 | 2-3/C05-C06 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~ClaimsManagementModuleTests" --nologo --verbosity minimal` | Pass; 500/400 Ed-Fi bodies; no `ex.Message`/DTO shape | not run | | | | Exception + DTO conversion | Yes |
| V-08 | 4/C08 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~TenantResolutionMiddlewareTests" --nologo --verbosity minimal` | Pass; body-shape asserted; statuses preserved | not run | | | | Add body assertions | Yes |
| V-09 | 4/C09 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~ReportInvalidConfiguration" --nologo --verbosity minimal` | Pass; 500 Ed-Fi body | not run | | | | New test | Yes |
...
| V-11 | 5/C10 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~AuthorizationTests|FullyQualifiedName~FrameworkErrorResponseTests" --nologo --verbosity minimal` | Pass; 401/403/404/405/415 bodies; 2xx untouched; `WWW-Authenticate` preserved | not run | | | | Scheme-independent; new fixture name TBD | Yes |
| V-12 | 5/C11 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~GlobalExceptionHandlerTests" --nologo --verbosity minimal` | Pass; malformed body → 400 `bad-request`; 415 → `unsupported-media-type` (Ed-Fi URI); any reachable status with no ticket/KB/platform URI (e.g. 413 if reachable) → `about:blank` + reason phrase; body `status` == HTTP status | not run | | | | D-08 rule fixed; Phase 5 audit sizes cases | Yes |
| V-13 | 6/C12 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~IdentityModuleTests" --nologo --verbosity minimal` | Pass; token/introspect/revoke errors = Ed-Fi contract; introspection `{active:false}`=200 | not run | | | | Includes TokenEndpointTests | Yes |
| V-14 | 0/6 | `rg -n "UseEnhancedOpenIddict|UseOpenIddictErrorHandling|AddEnhancedOpenIddict|OpenIddictErrorHandlingMiddleware" src/config` | Baseline: only definition + tests (not wired); post-C13 acceptance: no references | run (baseline) | Confirmed not wired: matches only in `OpenIddictIntegrationExtensions.cs` (definition), `OpenIddictErrorHandlingMiddleware.cs`, and `OpenIddictErrorHandlingMiddlewareTests.cs`; no pipeline invocation | 2026-07-23 | 5922d4fb | Dead-code confirmation; acceptance met after C13 | Yes |
| V-15 | 7 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --nologo --verbosity minimal` | All pass | not run | | | | Full CMS frontend unit suite | Yes |
| V-16 | 7 | `dotnet test src/config/backend/EdFi.DmsConfigurationService.Backend.Tests.Unit/EdFi.DmsConfigurationService.Backend.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseTests" --nologo --verbosity minimal` | Pass | not run | | | | Narrowly-necessary backend test | Yes |
| V-17a | 7 | `dotnet csharpier format src/config` | Formatting completes with exit 0 | not run | | | | TC-10 mitigation (step 1) | Yes |
| V-17b | 7 | `git diff --name-only` | Only in-scope files listed; revert any unrelated formatter-only change | not run | | | | TC-10 mitigation (step 2) | Yes |
| V-18 | 7 | `git diff --check` | No whitespace errors | not run | | | | | Yes |
| V-19 | 7 | `git status --porcelain` | Only in-scope files; clean worktree after commit | not run | | | | DoD | Yes |
| V-20 | 4/C07 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~PipelineExceptionBoundaryTests" --nologo --verbosity minimal` | Pass; throwing tenant resolution → full Ed-Fi 500 | not run | | | | New pipeline test (INV-36) | Yes |

---

## 28. Definition-of-done checklist

- [ ] Confirmed findings F1–F6 fixed; F7 verified already-compliant with regression coverage.
- [ ] `FailureResponse.ForDataValidation(...)` emits `urn:ed-fi:api:bad-request:data` (verified, unchanged).
- [ ] Disabled registration → structured 403 (`security:authorization`), `errors: ["Registration is disabled."]`.
- [ ] Disabled claims endpoints → structured 404 (`not-found`).
- [ ] Current-claims exceptions → structured 500 (`internal-server-error`), no `ex.Message`.
- [ ] ClaimsManagement non-success DTOs converted (INV-08…15); statuses preserved; `validationErrors` populated where field-level data exists; no leaks.
- [ ] Tenant-resolution + invalid-config middleware emit contract bodies; exception boundary precedes them (no uncontrolled 500).
- [ ] Framework 401/403/404/405/415 shaped scheme-independently for every bodiless response (no route exclusions, no re-execution); `WWW-Authenticate` preserved; 2xx/204 untouched.
- [ ] `BadHttpRequestException` status-aware; ticket/KB/platform statuses mapped; unclassified reachable statuses use RFC 9457 `about:blank` (D-08); no URI invented.
- [ ] OAuth/OIDC non-success responses converted; protocol success preserved; dead OpenIddict middleware removed.
- [ ] No bare framework results, ad-hoc `{error,message}`, or `{error,error_description}` remain for CMS non-success responses.
- [ ] No exception/provider/DB/config/token/secret detail in any error body.
- [ ] `FailureResults`/new writers use `application/problem+json`; existing compliant inline bodies unchanged.
- [ ] Unit tests assert full contract per changed branch; compliant behavior preserved.
- [ ] Targeted + full CMS frontend unit suites pass.
- [ ] `dotnet csharpier format src/config` run; no unrelated diffs; `git diff --check` clean.
- [ ] Final changed-file inspection confirms scope; worktree clean.
...
*End of R4. Status: Approved — 2026-07-23. Committed as C01; production work proceeds phase-by-phase behind per-commit approval gates, stopping for architect review after each commit (or tightly-related series).*
