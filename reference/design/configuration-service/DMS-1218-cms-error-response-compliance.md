...
| C10 | `[DMS-1218] Shape framework 401, 403, 404, 405, and 415 responses` | 5 | INV-25…29 (`FrameworkErrorResponseMiddleware` after `UseRouting`, before CORS/authn/authz; single non-re-executing status-code shaper; `FailureResponse.ForMethodNotAllowed`/`ForUnsupportedMediaType` added) | C02 | FailureResponseTests, AuthorizationTests, FrameworkErrorResponseTests | ready for review | recorded in C11 update (self-SHA rule) | INV-25…29 done; every bodiless 401/403/404/405/415 shaped, no route exclusions, no re-execution, `WWW-Authenticate`/`Allow` preserved, structured-404/health-200/CORS-204 untouched; V-11 (31/31) + backend `FailureResponseTests` (11/11) + full frontend suite 586/586 (R-02 gate) green; C09 `is not null` follow-up applied | — | — |
...
| V-15 | 7 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --nologo --verbosity minimal` | All pass | passed (interim @ C10) | Latest interim R-02 regression gate at C10: **586/586 passed** — the framework-shaping middleware caused no regressions (structured errors, success, and 204 all untouched). (Earlier interim at C02: 538/538.) Formal acceptance re-run at Phase 7 | 2026-07-23 | C10 (pre-commit) | Full CMS frontend unit suite; re-run at Phase 7 | Yes |
