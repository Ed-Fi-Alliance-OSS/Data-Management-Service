...
| C11a | `[DMS-1218] Align exception handler tests with repository conventions` | 5 | C11 follow-up: move the `handled` assertion out of `[SetUp]` into an `It_reports_the_exception_as_handled` per fixture (no production change) | C11 | GlobalExceptionHandlerTests | ready for review | recorded in C12 update (self-SHA rule) | `HandleAsync` returns `handled`; each fixture asserts it in an `It_`; **V-12 isolated 18/18**; Pipeline+Framework regression (separate) 29/29 | — | — |
