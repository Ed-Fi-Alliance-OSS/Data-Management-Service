...
| C10a | `[DMS-1218] Align framework error tests with repository conventions` | 5 | C10 follow-up: refactor `FrameworkErrorResponseTests` to nested `Given_` fixtures + `[SetUp]` + `It_` (no production change) | C10 | FrameworkErrorResponseTests | ready for review | recorded in C11 update (self-SHA rule) | 9 scenarios preserved as nested fixtures; shared helpers scoped in the outer container (multi-user → no S3398); V-11 27/27; full frontend suite 594/594 | — | — |
...
| V-04 | 1/C02, 5/C10 | `dotnet test src/config/backend/EdFi.DmsConfigurationService.Backend.Tests.Unit/EdFi.DmsConfigurationService.Backend.Tests.Unit.csproj --filter "FullyQualifiedName~FailureResponseTests" --nologo --verbosity minimal` | Pass (incl. `…:data`; + 405/415 factories) | passed | C02: 9/9 (F7 `…:bad-request:data` intact). C10: **11/11** — added `ForMethodNotAllowed` (405, `method-not-allowed`) and `ForUnsupportedMediaType` (415, `unsupported-media-type`) with full contract + empty extension members | 2026-07-23 | C10 (pre-commit) | Regression guard F7 + new 405/415 factories | Yes |
...
| V-11 | 5/C10 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --filter "FullyQualifiedName~AuthorizationTests|FullyQualifiedName~FrameworkErrorResponseTests" --nologo --verbosity minimal` | Pass; 401/403/404/405/415 bodies; 2xx untouched; `WWW-Authenticate` preserved | passed | 31/31 passed (AuthorizationTests + FrameworkErrorResponseTests). Real-JWT 401 (+ `WWW-Authenticate: Bearer`); TestAuth 401 & 403 (scheme-independent); health-lookalike 404 (no route exclusion); wrong-method 405 (+ `Allow: GET`); text/plain → 415; existing structured 404 unchanged; health 200 unchanged; CORS preflight 204 empty/unchanged; each shaped response asserts content type + exact taxonomy/title/detail/status + non-empty correlationId + empty extension members. C10a re-ran after the `Given_`/`It_` refactor: **27/27** (same coverage, more granular `It_` methods) | 2026-07-23 | C10a (pre-commit) | Scheme-independent; nested `Given_` fixtures | Yes |
...
| V-15 | 7 | `dotnet test src/config/frontend/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit/EdFi.DmsConfigurationService.Frontend.AspNetCore.Tests.Unit.csproj --nologo --verbosity minimal` | All pass | passed (interim @ C10a) | Latest interim R-02 regression gate at C10a: **594/594 passed** (C10 was 586/586; count rose as framework scenarios split into more granular `It_` methods). No regressions. Formal acceptance re-run at Phase 7 | 2026-07-23 | C10a (pre-commit) | Full CMS frontend unit suite; re-run at Phase 7 | Yes |
