...
| C04 | `[DMS-1218] Return structured 404 for disabled claims endpoints` | 2 | INV-02/03/04 (`ClaimsManagementModule` reload/upload/current-claims disabled → `FailureResults.NotFound`; `HttpContext` added to `ReloadClaims`/`UploadClaims`) | C02 | ClaimsManagementModuleTests | ready for review | recorded in C05 update (self-SHA rule) | INV-02/03/04 done; three full-access 404 tests assert full contract + route-specific detail; auth (401/403) preserved; V-06 green (22/22) | — | — |
