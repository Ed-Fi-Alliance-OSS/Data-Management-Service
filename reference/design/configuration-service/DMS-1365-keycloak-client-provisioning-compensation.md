...
3. Effective configuration per lane **[R3, corrected in the Step 3.1 review]**: the step derives the expected claim type **from the provider under test**, read from `DMS_CONFIG_IDENTITY_PROVIDER`: Keycloak lane → `DMS_CONFIG_IDENTITY_ROLE_CLAIM_TYPE`; self-contained lane → `Authentication__RoleClaimAttribute` if set, otherwise the OpenIddict default URI, and **never** the Keycloak setting. The expected role is `DMS_CONFIG_IDENTITY_CLIENT_ROLE` on both lanes because both populate their role store from `IdentitySettings.ClientRole`. Fallbacks match the checked-in environment files so a bare `dotnet test` still compares correctly.
