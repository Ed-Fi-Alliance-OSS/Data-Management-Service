...
2. Files: `Features/Token.feature` (new `Scenario: 05 A newly created client's token carries the configured client role`); `StepDefinitions/StepDefinitions.cs` (new step `Then the token carries the configured client role claim`); `JwtTokenValidator.cs` (`TryGetClaimValues(token, claimType, out IReadOnlyList<string>)`); `build-config.ps1` `E2ETests` (publish the expected role settings, assigned unconditionally like the neighbors; names and source in item 3).
3. Effective configuration per lane **[R3, corrected in the Step 3.1 review]**: the step derives the expected claim type **from the provider under test**, read from `CMS_E2E_EXPECTED_IDENTITY_PROVIDER`: Keycloak lane → `CMS_E2E_EXPECTED_ROLE_CLAIM_TYPE`; self-contained lane → `CMS_E2E_EXPECTED_SELF_CONTAINED_ROLE_CLAIM_TYPE` if set, otherwise the OpenIddict default URI, and **never** the Keycloak setting. The expected role is `CMS_E2E_EXPECTED_CLIENT_ROLE` on both lanes because both populate their role store from `IdentitySettings.ClientRole`. Fallbacks match the checked-in environment files so a bare `dotnet test` still compares correctly.

   **`build-config.ps1` publishes those four values by reading them back from the Configuration Service container** (`AppSettings__IdentityProvider`, `IdentitySettings__RoleClaimType`, `IdentitySettings__ClientRole`, `Authentication__RoleClaimAttribute`), not by copying the environment file. Copying the file was the original plan and is **wrong**: Compose resolves a shell-provided value ahead of the file, so a shell override would leave the scenario expecting a role the container never received; and a key no compose file forwards would change the expectation without changing the provider. A failed or provider-less inspect fails the run rather than silently falling back. Only those four settings are read, so no unrelated container value is exported. Evidence for each lane run is the same inspect output, recorded in "As implemented"; matching defaults are *observed*, not assumed.

   **Expectation names [corrected after the pre-push review]**: the four values are published under test-only names — `AppSettings__IdentityProvider` → `CMS_E2E_EXPECTED_IDENTITY_PROVIDER`, `IdentitySettings__RoleClaimType` → `CMS_E2E_EXPECTED_ROLE_CLAIM_TYPE`, `IdentitySettings__ClientRole` → `CMS_E2E_EXPECTED_CLIENT_ROLE`, `Authentication__RoleClaimAttribute` → `CMS_E2E_EXPECTED_SELF_CONTAINED_ROLE_CLAIM_TYPE` — that neither Compose nor the service reads. The first implementation reused the configuration names (`DMS_CONFIG_IDENTITY_*`, `Authentication__RoleClaimAttribute`), so a run's read-back became a shell value that Compose then preferred over the next run's environment file, and a container without `Authentication__RoleClaimAttribute` cleared a developer's own value. The container read-back itself is unchanged and still the source of truth; only the destination names moved. There is no fallback read from the old names.
