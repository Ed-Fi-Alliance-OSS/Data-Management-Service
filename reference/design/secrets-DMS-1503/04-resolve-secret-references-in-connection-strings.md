...
- A grammar table test carries one case per permitted punctuation character and one per excluded character.
- The table covers an empty name, an embedded space, an embedded brace, and an unterminated `${secret:`, each left verbatim and failing nothing.
...
- A resolver returning an incomplete `ValueTask` and ignoring the token fails within the window.
- A resolver blocking before returning its `ValueTask` fails within the window.
...
- A resolver that throws fails the read.
- A resolver that cancels fails the read.
- A resolver that returns null fails the read.
- A resolver that returns an empty string fails the read.
...
- The same unresolvable token fails through `GET /v3/dataStoreDerivatives/` (`Config.Frontend/Modules/DataStoreDerivativeModule.cs:21`).
- The same unresolvable token fails through `GET /v3/dataStoreDerivatives/{id}` (`:22`).
...
- `docs/CONFIGURATION.md` documents both keys with their defaults.
- Both compose files carry `DMS_CONFIG_*` overrides for both keys.
