...
Ten changed a decision rather than a wording, and each is recorded where the decision is made rather than in a revision history: builder-mediated substitution, the builder taken from the validator's own registration, decrypting every row, materializing every projection the seam runs in, the derivative containment rule and its boundary, the host-raced resolve timeout, the keyed-registration refusal, the retrieval-path statement in the trust notes, the compose variable the identity bootstrap consumes, and DMS-1552's dependency on the plugin foundations.
...
| `src/config/Dockerfile` build stage: the `pluginsource` filtering stage, the plugin project and lock file copies, the `WORKDIR` move, the extended restore, and the post-restore source copy | Small | Mechanical but load-bearing, and `src/dms/Dockerfile` carries a worked version of it. It also carries the contract project's own entries and the removal of this image's version stamping, which DMS-1552 specifies. |
