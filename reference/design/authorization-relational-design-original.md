...
- **Mandatory authorization table** (single, unpartitioned) that explodes all document-to-EdOrg pathways, carries `ResourceName`/`CreatedAt`, and is indexed for ordered scans (avoids bitmap merges).
...
### 5) Document-to-EdOrg Authorization Table (required)
Union of all pathways, persisted in a base table and indexed for ordered scans. Includes `ResourceName` and `CreatedAt` to satisfy collection paging without combining indexes. The table is maintained transactionally from `Document*` and `*Link` changes so authorization is near-real-time and strongly consistent.

```sql
CREATE TABLE dms.DocumentEdOrgAuthorization (
  DocumentPartitionKey smallint NOT NULL,
  DocumentId bigint NOT NULL,
  ResourceName text NOT NULL,
  CreatedAt timestamptz NOT NULL,
  Pathway text NOT NULL, -- DirectEdOrg, StudentSchool, StaffEdOrg, ContactStudentSchool
  EdOrgId bigint NOT NULL,
  PRIMARY KEY (DocumentPartitionKey, DocumentId, Pathway, EdOrgId)
);
```

**Logical definition (used for backfill and verification)**

The table rows correspond to the union below; this SELECT can be used to backfill or validate the table contents:
...
-- If table size or refresh cost grows, add partial indexes per partition key:
-- CREATE INDEX CONCURRENTLY IF NOT EXISTS IX_DocumentEdOrgAuth_p00
--   ON dms.DocumentEdOrgAuthorization (Pathway, EdOrgId, ResourceName, CreatedAt, DocumentPartitionKey, DocumentId)
--   WHERE DocumentPartitionKey = 0;
-- (repeat for each partition key in use)
```
...
In this pattern, the `Document` collection query uses a single index on `Document(ResourceName, CreatedAt, Id)` to drive ordering and paging, while the authorization table index on `(Pathway, EdOrgId, ResourceName, CreatedAt, DocumentPartitionKey, DocumentId)` is used to satisfy the correlated `EXISTS` predicate. This keeps the planner from needing bitmap merges on `Document` itself to enforce both authorization and `ORDER BY CreatedAt`.
...
- **Mandatory authorization table**: `DocumentEdOrgAuthorization` is a base table, maintained transactionally from `Document*` and `*Link` changes so authorization stays near-real-time and strongly consistent. The UNION query in the data model section is used for backfill/verification, not on the hot path. If the table grows too large, partial indexes per partition key keep scans narrow without introducing bitmap merges.
- **Order-preserving covering index**: `IX_DocumentEdOrgAuth_Pathway_EdOrg_Resource_CreatedAt` lets the planner satisfy the auth predicate in `EXISTS` using a single index on the authorization table, while a separate single index on `Document(ResourceName, CreatedAt, Id)` preserves collection ordering without bitmap merges during deep paging.
- **Optional doc-level cache**: If profiling shows the authorization table is still hot, add `DocumentAuthorizationCache(DocumentPartitionKey, DocumentId, Pathway, EdOrgId, ResourceName, CreatedAt)` maintained with the same triggers/refresh logic.
...
- **Authorization table maintenance**: Triggers (or lightweight write-path logic) on `Document*` and `*Link` changes maintain `DocumentEdOrgAuthorization` transactionally so reads see up-to-date authorization facts while still using a single-index pattern.
- **Migration plan**: Retire JSON auth columns (`StudentSchoolAuthorizationEdOrgIds`, `ContactStudentSchoolAuthorizationEdOrgIds`, `StaffEducationOrganizationAuthorizationEdOrgIds`, `StudentEdOrgResponsibilityAuthorizationIds`) and their GIN indexes after backfilling the new tables and authorization table. This removes competing indexes that would otherwise force bitmap scans.
...
- Replaced search-engine-shaped denormalization with relational link tables and a mandatory, order-preserving authorization table.
...
3. **Authorization = EXISTS**: Join authorized sets to the order-preserving authorization table in one SQL round-trip.
