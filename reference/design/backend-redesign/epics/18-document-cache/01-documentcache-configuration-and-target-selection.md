...
1. 18-01 should not infer the target provider solely from process `AppSettings:Datastore`, and it should not map CMS `DataStoreType` to a relational provider. Make provider compatibility taskable by adding or reading an explicit normalized relational provider token on resolved data-store metadata, with values `postgresql` or `sqlserver`. A missing or unknown token leaves the target resolved but projection/cache-ineligible with a provider-metadata diagnostic; a token that differs from the process `AppSettings:Datastore` is the provider-mismatch classification. The target execution context and physical-source fingerprint should use the normalized token only after it matches the process provider. Until CMS exposes this metadata, real CMS provider-mismatch integration is not taskable; classifier tests may use fake resolved metadata.
   ...
   process `AppSettings:Datastore`, and CMS data-store target provider metadata are expected
   to match. The provider-mismatch classification remains a DMS guard for invalid, stale,
   or test-supplied metadata; it is not a supported mixed-provider CMS deployment mode.
...
1. What exact JSON shape should 18-01 use for the normalized target key in administrative request/result DTOs: a nested `targetKey` object or flat `tenantKey`/`dataStoreId` properties, an empty string or `"default"` for the default tenant, and numeric or string `dataStoreId`?
2. Should JSON-facing enum values for lifecycle command results, target resolution states, downstream-publication status, provider prerequisites, and rejection classifications use the C# design tokens as written (`Tracking`, `InternalOnly`, etc.) or a normalized wire style such as lower camel case?
3. When a DocumentCache target refresh calls CMS and receives an auth/connectivity/server error rather than a successful response without the configured target, should DMS startup/refresh continue with that target marked `Unresolved` and retry diagnostics, or should startup fail outside malformed local configuration?

### Answers 3

1. Use a nested `targetKey` object in every JSON-facing administrative request/result DTO: `"targetKey": { "tenantKey": "", "dataStoreId": 1 }`. Use the normalized non-default tenant key string for non-default tenants, use empty string for the default/single-tenant target, and serialize `dataStoreId` as a positive JSON number. Do not use `"default"` in these DTOs because it can collide with a real tenant key; the CDC binding record's deployment-owned flat/string shape remains separate.
2. Use explicit lower-camel JSON strings for all JSON-facing enum values, with `System.Text.Json` tests pinning the values. C# models may keep PascalCase enum member names such as `Tracking` and `InternalOnly`, but the wire values should be `tracking`, `internalOnly`, `providerPrerequisiteFailed`, and so on. Do not serialize numeric enum values or depend on the default enum-name serializer.
3. Treat CMS auth, connectivity, timeout, invalid-response, and server errors as operational target-resolution failures, not malformed local configuration. Startup and refresh should continue, classify an unresolved or not-yet-resolved target as `Unresolved` with sanitized retry diagnostics, and leave unrelated targets plus ordinary canonical API routing unaffected. For an already resolved target, a failed replacement refresh should keep the current target-context generation and attach refresh-failure diagnostics until a successful refresh proves removal or replacement; do not tear down a usable context based only on a transient CMS failure. Startup should fail only for malformed local `DocumentCache` options or other process-wide configuration errors.
