...
- The connector login and credentials are deployment-supplied. Under the amended
  [SQL Server provider contract](../../design-docs/cdc/cdc-streaming.md#sql-server), initial
  setup may create a missing database user for an existing same-name SQL login; existing
  users must exact-match the expected login SID and pass principal/access checks.
  Validate-only execution never creates or remaps a user. Implementation and qualification
  belong to [DMS-1326](07-ops-docs-runbooks.md#sql-server-initial-connector-user-mapping),
  without reopening DMS-1320. Production setup validates required CDC read/metadata access
  and known disallowed elevated privileges. It may create database membership for the named
  principal but does not create logins or create or rotate credentials. It grants CDC
