...
### Questions 2
  ...
  8. Is it acceptable for GET-by-id _etag to move to a different format now even though write responses still use the older behavior and If-Match is deferred to DMS-1005? That affects
...
### Answers 2
  ...
  8. No. Do not ship a split contract where relational GET returns one `_etag` format while write responses use a different format.
     ...
     responses must emit the same composed `_etag` format: `{ContentVersion}-{variantKey}`.
...
### Questions 3
  ...
  2. What exact format do you want for relational read metadata? We know _etag should come from the stored representation stamp plus representation selector, but I still need the concrete string form, and whether _lastModifiedDate should
 ...
 ### Answers 3
  ...
  2. Use `_etag = "{ContentVersion}-{variantKey}"` and `_lastModifiedDate` sourced from
     `dms.Document.ContentLastModifiedAt`. The server composes `_etag` from the stored representation stamp and the request's representation `variantKey`; it does not hash or read back the reconstituted JSON document.
     `_lastModifiedDate` formatting follows the update-tracking formatter for the stored UTC stamp.
