...
- `dms.DocumentChangeEvent` and `dms.IdentityChangeEvent` are derived artifacts.
...
- When `dms.Document` is inserted, triggers emit:
  ...
  - one `dms.IdentityChangeEvent` row.
- When `ContentVersion` changes, triggers emit a `dms.DocumentChangeEvent` row.
- When `IdentityVersion` changes, triggers emit:
  - a `dms.IdentityChangeEvent` row,
  - and a `dms.DocumentChangeEvent` row.
