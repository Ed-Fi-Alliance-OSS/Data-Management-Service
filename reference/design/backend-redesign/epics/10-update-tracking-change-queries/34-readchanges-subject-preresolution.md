...
- Re-run the DMS-1185 A/B benchmark shapes on PostgreSQL at 10M tombstones and assert plan shapes: no nested-loop or join-filter flips on full-window `/keyChanges`, narrow-window `/deletes`, or full-window `/deletes` with district claims (the flip accepted by the Tier-1 story must be gone), with wins on selective claims and view evaluation retained as same-run relative comparisons.
- Run the equivalent plan-shape verification on SQL Server and record the results.
- Benchmark portability note: timing absolutes are environment- and CPU-architecture-sensitive (the spike's numbers come from an arm64 macOS Docker host; the team develops on Windows/amd64 and Linux); gate on plan shapes and same-run relative factors only, never on absolute times.
