...
- The check constraint requires ordinal equality including length in both engines, so equality is exact rather than merely case-sensitive. Case variants such as `SNAPSHOT` and `readreplica` are rejected even under a case-insensitive SQL Server collation, and whitespace variants such as `Snapshot ` are rejected even though SQL Server's SQL-92 string padding makes them compare equal to `Snapshot` under `=` and `IN` at any collation. `LEN` is not a usable length test on SQL Server because it ignores trailing spaces.
...
- Invalid-type diagnostics identify the offending `(Id, DataStoreId, DerivativeType)` tuples, including case and whitespace variants. The preflight comparison is itself padding-exact, so a value such as `Snapshot ` is reported rather than passing an `IN` scan silently on SQL Server and surviving into a runtime where DMS's ordinal type matching ignores it.
