...
- Cover inventory derivation, SQL Server DDL and manifest output, upgrade behavior, deterministic naming, and golden regeneration; when emitting, add this index to the core-DDL ownership carve-out list in `compiled-mapping-set.md` and update `change-queries.md` § "`*_RefKey` index ordering for `/deletes`" to record the implemented decision. The `RelationalMappingVersion` bump this physical change requires is deferred to a dedicated mapping-version ticket rather than owned here; the first such ticket is filed with the follow-on tickets after spike approval and must land no later than the first story that ships a physical index change, and each later independently released batch of physical index changes requires its own bump ticket for that release.
...
- Follow `22-spike-findings.md` § "Tracked-index evidence protocol" for environment pinning (including both supported SQL Server database compatibility levels), measurement and noise controls, and storage measurement; additionally pin descriptor cardinality and value distributions and the exact read and write parameters before running the A/B gates.
