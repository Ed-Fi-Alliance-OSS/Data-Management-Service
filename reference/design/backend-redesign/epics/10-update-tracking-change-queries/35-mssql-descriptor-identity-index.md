...
- Cover inventory derivation, SQL Server DDL and manifest output, upgrade behavior, deterministic naming, and golden regeneration; the `RelationalMappingVersion` bump this physical change requires is deferred to a dedicated mapping-version ticket filed with the follow-on tickets after spike approval, and that ticket must land no later than the first story that ships a physical index change.
