...
The Tier-1 indexes are proposed separately in `33-tracked-change-index-emission.md` and carry one bounded PostgreSQL candidate exception that this story is expected to remove.
...
This story depends on Story 33's evidence phase for its checked-in harness and result matrix, but not on successful Tier-1 production emission.
...
- Re-run Story 33's checked-in DMS-1185 read matrix without adding the deferred per-resource indexes. Use Story 33's final production Tier-1 disposition as the baseline: when Tier 1 is emitted, PostgreSQL must remove its narrow-window candidate exception; when Tier 1 is blocked, evaluate the selected query shape against the unchanged production DDL and do not treat candidate Tier-1 DDL as authorized. Neither provider's selected shape may exceed the general 1.20 median regression gate. Record result counts and plans so the downstream EdOrg/person index story starts from a fixed runtime baseline.
