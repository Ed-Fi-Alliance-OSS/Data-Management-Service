---
jira: DMS-1358
jira_url: https://edfi.atlassian.net/browse/DMS-1358
---
...
- Preserve the distinction between dynamic emptiness and static unrepresentability:
  ...
  - only-array-nested and mixed root/array configurations exercise that security-configuration outcome on both providers, for EdOrg securables and for each person securable kind (Student, Contact, Staff), so an unrepresentable AND-composed subject is never silently dropped; a valid root subject with a dynamically empty resolved set exercises the successful-empty outcome;
...
- Re-run Story 33's checked-in DMS-1185 read matrix without adding the deferred per-resource indexes: always against the production DDL (Story 33's emitted categories present) to hold the general 1.20 median regression gate for the selected shape on both providers, and, only if Story 33's disposition left the PA category blocked, in a second configuration with Story 33's pinned five-PA candidate overlay applied to demonstrate that the narrow-window `/deletes` anti-join flip is gone on all five PrimaryAssociation fixtures. Passing that conditional candidate-overlay rerun triggers the dedicated PA re-evaluation ticket defined by Story 33's closure rule, which reruns Story 33's recorded eligibility, combined-interaction, emission, and post-implementation gates; the overlay itself remains a benchmark input, unauthorized for production, until those gates pass. If Story 33 emitted the PA category, the production-DDL configuration already contains the five PA indexes, no overlay rerun applies, and no re-evaluation ticket is triggered. Record result counts and plans so the downstream EdOrg/person index story starts from a fixed runtime baseline.
...
- Measure performance evidence for each of the five relationship strategy envelopes (`RelationshipsWithEdOrgsOnly`, `RelationshipsWithEdOrgsOnlyInverted`, `RelationshipsWithEdOrgsAndPeopleIncludingDeletes`, `RelationshipsWithStudentsOnlyIncludingDeletes`, `RelationshipsWithStudentsOnlyThroughResponsibilityIncludingDeletes`) on both `/deletes` and `/keyChanges` in the decision checkpoint and the rerun; a regression on a strategy absent from the rerun matrix would bake into every downstream story's cumulative baseline ungated.
- Run the decision checkpoint and the rerun at both supported SQL Server database compatibility levels (170 for newly created databases, 160 for databases restored from SQL Server 2022-built templates), or record an explicit evidence-backed narrowing, mirroring the evidence protocol's environment rule.
