...
3. Should Story 20 follow Story 26 and exclude `SchoolYearType` from generated `/deletes` and `/keyChanges` OpenAPI paths, despite the later `change-queries.md` section that says MetaEd emits those paths for `SchoolYearType`?
...
2. For OpenAPI emission, treat Change Query support as a generated-resource capability, not as claim/action metadata. MetaEd should advertise `/deletes` and `/keyChanges` for every non-composite generated resource and descriptor in the resources/descriptors OpenAPI documents, excluding `SchoolYearType`. DMS runtime authorization remains responsible for enforcing whether the caller has the `ReadChanges` action and strategies for the resolved resource.
3. Follow Story 26: exclude `SchoolYearType` from generated `/deletes` and `/keyChanges` OpenAPI paths. The conflicting `change-queries.md` paragraph should later be corrected to match Story 26 and the ODS-compatible OpenAPI behavior; crafted `SchoolYearType` requests remain a runtime authorization case, not an advertised path.
4. Omit the ODS `Use-Snapshot` header and snapshot-specific response entries from new DMS Change Query OpenAPI definitions, including live GET-many, `/deletes`, `/keyChanges`, and `/availableChangeVersions`. Snapshot support is deferred for DMS v1.0, and the OpenAPI should not advertise snapshot behavior that DMS only ignores at runtime.
...
2. Emit `projectSchema.openApiBaseDocuments.changeQueries` only from the core/data-standard ApiSchema package. Extension packages should continue to carry only their resource/descriptor OpenAPI fragments. DMS should source the Change Queries document only from the selected core ApiSchema and ignore any extension-side `openApiBaseDocuments.changeQueries` if present.
3. Generate strongly typed per-resource key schemas for `/deletes` and `/keyChanges`. The key schemas should contain only public identity fields derived from `identityJsonPaths` and `queryFieldMapping`, including descriptor-reference fields composed as public descriptor URI strings. A generic `keyValues` object schema is not sufficient for the advertised discovery contract.
