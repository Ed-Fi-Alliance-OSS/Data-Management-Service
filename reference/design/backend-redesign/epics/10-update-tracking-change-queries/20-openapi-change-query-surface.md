...
- The temporary dev-only `CoreEndpointModule` route-level stub for resource and descriptor `/deletes` and
  `/keyChanges` endpoints is removed when model-driven runtime route handling for these paths is implemented.
