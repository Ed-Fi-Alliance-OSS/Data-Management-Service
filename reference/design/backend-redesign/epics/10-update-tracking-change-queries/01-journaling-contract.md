...
- Successful no-op updates emit no journal row because `ContentVersion` does not change.
...
2. Add integration smoke tests validating trigger behavior on insert/update token changes and no-op update paths.
