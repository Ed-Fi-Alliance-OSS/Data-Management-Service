...
- Regenerate the affected DDL/manifests/goldens and pass PostgreSQL and SQL Server generated-DDL integration smoke tests; the `RelationalMappingVersion` bump this physical change requires is tracked by its own open ticket rather than owned here.
- After emission, verify that the exact generated DDL for every adopted predicate is semantically identical to its pinned overlay (per-table key columns, operator classes, deduplication outcomes, and shortened identifiers), then rerun the applicable representative read and write matrix against that generated DDL. A mismatch or failed post-implementation gate blocks acceptance rather than being described as run-to-run noise.
