...
- Release notes call out that `Use-Snapshot: true` on a non-`GET` resource or descriptor request changes from ignored in DMS v1.0 to the snapshot `405` with `Allow: GET`, so a `POST`, `PUT`, or `DELETE` that succeeds today is rejected after the upgrade. State that API Publisher is unaffected because it applies the header to its read-only source client only, and that any client setting the header once on a shared connection it also writes through is affected.
