...
- Provider cleanup preserves that ownership rule. PostgreSQL evicts and disposes its owned `NpgsqlDataSource` after the last owner and in-flight lease are gone. SQL Server keeps its existing per-operation `SqlConnection` construction and driver-managed pooling, but DMS constructs every derivative connection through `SqlConnectionStringBuilder` inside the connection-acquisition boundary and sets its `PoolBlockingPeriod` to `PoolBlockingPeriod.NeverBlock` in the effective provider connection string. This derivative-only policy overrides an operator-supplied pool-blocking value so a failed login or timeout is not replayed from SqlClient's blocking period on the next request; primary connection strings are passed through unchanged. The effective provider string, including the forced derivative setting, is the SQL Server pool and ownership identity. Consequently, a primary and derivative with otherwise identical stored text use distinct SqlClient pools, while derivatives with the same effective string can share one. After the last configured owner and in-flight lease for a retired effective string are gone, DMS calls `SqlConnection.ClearPool` for that exact pool. It never calls `SqlConnection.ClearAllPools`, which would disrupt unrelated targets, and it does not leave retired derivative pools solely to driver idle cleanup.
...
Because both responses are `405`, this precedence is invisible to a status-only assertion. It is a deliberate, test-visible change to the response for the three invalid mutation shapes when the header parses `true`, and must be asserted on type, title, detail, content type, and the exact `Allow` value — `Allow: GET` on the snapshot response versus the route's supported-method set on the generic response.
...
2. Selection before `ValidateRouteSemanticsMiddleware`, so a `POST`, `PUT`, or `DELETE` against a known resource or descriptor carrying `Use-Snapshot: true` receives the snapshot `405` whether or not its route shape is valid.
...
`Use-Snapshot: true` is rejected on the supported mutation verbs — resource and descriptor `POST`, `PUT`, and `DELETE` — whether or not the route shape is valid, so a collection `DELETE`, a collection `PUT`, and an item `POST` are rejected by snapshot policy rather than by route semantics. `OPTIONS` remains unaffected, because it neither reads nor modifies data and is outside the `Use-Snapshot` contract, and so is every verb DMS does not map at all (`HEAD`, `PATCH`, and any other unsupported method): those dispatch to the method-not-allowed pipelines, which open no database, run no target selection, and keep their existence-then-method `404`/`405` contract regardless of the header.
...
| --- | --- | --- | --- | --- | --- |
| `Use-Snapshot: true` on a resource or descriptor `POST`, `PUT`, or `DELETE`, including an invalid route shape | `urn:ed-fi:api:snapshots:method-not-allowed` | `Method Not Allowed with Snapshots` | `405` | `An attempt was made to modify data in a Snapshot, but this data is read-only.` | `Allow: GET` |
...
- cancellation attributable to a supplied caller or request `CancellationToken` propagates unchanged. It is never wrapped as `DatabaseConnectionUnavailableException`, treated as a cache miss, or translated to Snapshot Not Found, even though it is raised inside an acquisition boundary. Six of the seven baseline seams below already accept a token and pass it into the open call, so the exclusion must be explicit: an aborted or timed-out caller is not evidence that the snapshot is missing, and translating it would also diverge from primary and read-replica cancellation behavior.
...
The wrap must cover construction and parsing rather than the open call alone, because at every baseline seam the provider parses the connection string on a statement that precedes the open: PostgreSQL parses while the pooled data source is built, and SQL Server parses while `SqlConnectionStringBuilder` constructs a derivative effective string and again in the `SqlConnection` constructor. A boundary drawn around the open call alone would let a provider-invalid connection string escape as an unhandled provider argument failure at all seven baseline seams.

There are seven baseline read-path seams, and all seven must be covered:
...
When E18 cache-backed reads are present, the provider cache-lookup adapter is an optional acquisition caller in front of those baseline seams. An expected connection-establishment failure during cache data-source or connection construction, connection-string parsing, or open is treated as an unavailable cache read and falls through to the relational path on the same selected target. The fallback then uses the wrapped baseline seam, so an unavailable selected snapshot still produces Snapshot Not Found `404`, an unavailable read replica retains its normal database-availability contract, and neither can fall back to the primary. Caller cancellation and unexpected or programming exceptions are not cache misses and propagate unchanged. This conditional adapter rule prevents E18 from opening an unwrapped path without making the cache optimization authoritative for database availability.
...
- a resource or descriptor `POST`, `PUT`, or `DELETE` with `Use-Snapshot: true` returns `405` before route-semantics, content-type, body, profile, coercion, document, or fingerprint validation and without opening the primary or derivative database; the snapshot policy intentionally preempts the generic route-semantics `405` as well as the `415` or `400` responses those later mutation validations would otherwise produce. A verb DMS does not map dispatches to the method-not-allowed pipelines and keeps its existing contract regardless of the header;
...
- Provider-specific pool-lifecycle tests prove PostgreSQL disposes only the retired `NpgsqlDataSource`; SQL Server forces `PoolBlockingPeriod.NeverBlock` only for derivative effective strings, leaves primary strings unchanged, and calls `SqlConnection.ClearPool` only for the exact retired effective string after its final owner and in-flight lease are gone. `SqlConnection.ClearAllPools` is never used, an unrelated SqlClient pool remains usable, and a primary and derivative with otherwise identical stored text use their distinct effective pools.
...
- Tests proving failed, missing, and malformed derivative validation results are not cached, so the next request revalidates instead of reusing the failure. SQL Server coverage makes the first derivative open fail with a login or timeout error and restores availability at the same effective connection string before the immediately following request, proving SqlClient does not replay a driver-blocked exception and the second request actually reaches the server.
...
- When E18 cache-backed reads and derivative routing are present together, integration tests use distinguishable primary, snapshot, and read-replica databases and prove that cache lookup, freshness comparison, authorization, hydration, and relational fallback remain on the selected target; primary cache JSON is never returned for a derivative request, and derivative requests perform no direct fill write. With cache reads enabled, an expected cache-adapter acquisition failure falls through to relational acquisition on the same selected snapshot and produces the exact Snapshot Not Found `404` when that acquisition also fails; a supplied cancellation from the cache adapter is not swallowed as a cache miss.
- Tests proving an unknown resource path with `Use-Snapshot: true` returns its existing `404` rather than the snapshot `405`.
- Tests proving an invalid mutation route with `Use-Snapshot: true` — collection `DELETE`, collection `PUT`, and item `POST` — returns the snapshot `405` rather than the existing route-semantics `405`, asserted on type, title, detail, content type, and the exact `Allow` value, since both responses share the status code and both carry `Allow`.
- Tests proving the same three invalid mutation routes without the header, and with `Use-Snapshot: false`, still return the existing generic route-semantics `405` with its detail, `application/json; charset=utf-8`, and the route-specific `Allow` set, so the *substitution of the snapshot body* is confined to requests carrying a parsed `true`. These run against a provisioned, reachable database and therefore say nothing about the reordering's own precedence changes, which are covered by the next bullet.
...
9. **E18 cache-backed reads never weaken request-scoped target selection when the two features are present together.** Cache lookup, lifecycle reads, canonical-version comparison, authorization, hydration, and relational fallback all stay on the selected physical database. A derivative request uses only cache state from that same database or bypasses cache acceleration; it never reads the parent primary's cache. Expected connection-establishment failure in the optional cache adapter falls through to relational acquisition on that same target, while cancellation and unexpected exceptions propagate. Direct fill is bypassed for snapshots and read replicas because it writes `dms.DocumentCache`, and E18 configuration enables cache execution without changing API routing. Either story remains independently closeable before the other exists; whichever implementation lands second owns this integration and its tests, and the features cannot ship together without them.

## Follow-on Ticket Plan

The following implementation tickets were created and linked after this proposal was approved. Each slice's Jira key is recorded in two places: the story file's `jira` / `jira_url` front matter and the matching `EPIC.md` § Follow-on Stories row.
...
The cache integration in Resolved Decision 9 is conditional on both features being present,
and whichever implementation lands second owns it. If E18 `05-cache-backed-read-path.md`
already exists when Story 39 lands, Story 39 owns the selected-target integration and tests;
if Story 39 already exists when E18 Story 05 lands, E18 owns them. Either story remains
independently closeable before the other exists, but the two features cannot ship together
without the selected-database-or-bypass behavior, same-target acquisition fallback, and
distinct-database integration coverage.
...
- Both deferred Snapshot ProblemDetails are specified above, including the `Allow: GET` header, the reused `404` factory, and the new `405` factory. The `405` applies to the supported mutation verbs — resource and descriptor `POST`, `PUT`, and `DELETE` — including the invalid route shapes, and triggers on a successfully parsed `true`; verbs DMS does not map stay on the method-not-allowed pipelines, outside target selection.
...
- The required implementation-ticket slices are defined above and are tracked by the Jira keys recorded in each story file and in `EPIC.md` § Follow-on Stories.
