...
- when the data-store configuration cache is disabled or non-expiring, the derivative TTL stands alone and remains bounded. Derivative routing does not require data-store refresh to be enabled. It does, however, require that refresh for any *change* to derivative configuration to be observed at all: with refresh disabled or non-expiring, the derivative rows loaded at startup are fixed for the process lifetime, and a CMS edit is picked up only on restart. The bounded TTL guarantees a fresh verdict about the connection DMS already knows, not a fresh copy of the CMS row;
