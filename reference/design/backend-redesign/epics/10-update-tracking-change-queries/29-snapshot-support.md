...
- Tests proving the same three invalid mutation routes without the header, and with `Use-Snapshot: false`, still return the existing generic route-semantics `405` with its detail, `application/json; charset=utf-8`, and no `Allow` header, so the change is confined to requests carrying a parsed `true`.
