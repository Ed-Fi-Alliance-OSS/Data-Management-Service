...
- A successfully parsed `Use-Snapshot: true` on a resource or descriptor `POST`, `PUT`, or `DELETE`, including an invalid route shape such as a collection `DELETE`, a collection `PUT`, or an item `POST`, returns:
...
- The snapshot `405` covers only the three supported mutation verbs. `OPTIONS` and every verb DMS does not map at all (`HEAD`, `PATCH`, and any other unsupported method) dispatch to the method-not-allowed pipelines, which run no target selection and keep their existence-then-method `404`/`405` contract regardless of `Use-Snapshot`, per `39-snapshot-read-replica-runtime-routing.md` § Endpoint coverage.
...
- The snapshot `405` does displace the generic route-semantics `405` when the header parses `true`. A collection `DELETE`, a collection `PUT`, or an item `POST` carrying a parsed `true` returns the snapshot `405` with `Allow: GET` and `application/problem+json`. The same request without the header, or with `Use-Snapshot: false`, keeps the response `ValidateRouteSemanticsMiddleware` already produces through `FailureResponse.ForMethodNotAllowed`: its own type, title, and detail, content type `application/json; charset=utf-8`, and the route-specific `Allow` set (`GET, POST`, `GET, PUT, DELETE`, or `GET`).
- Because both are `405`, tests distinguishing them assert type, title, detail, content type, and the exact `Allow` value - `Allow: GET` on the snapshot response versus the route's supported-method set on the generic response. On a partitions path the two `Allow` values coincide at `GET`, so the ProblemDetails fields and content type are what distinguish them there. A status-code assertion alone cannot tell the two apart and does not satisfy this criterion.
