...
Implement the Tier-1 index set proposed by spike DMS-1185 and specified in `change-queries.md` § "Indexes on the `tracked_changes*` tables": the five tracked PrimaryAssociation covering indexes and the shared-descriptor `(Discriminator, ChangeVersion)` index, derived by the tracked-change extension to the existing `DeriveIndexInventoryPass`.
...
This story's evidence phase creates the reproducible two-provider harness and checked-in result package, and runs the mandatory implementation-scale read/write/storage matrix defined below before product code or goldens change.
...
- Reposition the existing `DeriveIndexInventoryPass` immediately after `DeriveTrackedChangeInventoryPass` and before `DeriveAuthHierarchyPass`; extend it with tracked-change index derivation while preserving its existing PK/UK/FK-support and content-version output.
- The pass takes the same strictness flag as `DeriveAuthorizationIndexInventoryPass` (`throwOnMissingPaLiteral` semantics), wired from `CreateStrict`/`CreateDefault`; pass-order tests pin both pipelines and prove that the reposition-only inventory is otherwise byte-for-byte unchanged.
...
- Unit tests (`DeriveIndexInventoryPassTests`, using `CommonInventoryTestSchemaBuilder`): PA table present/absent, missing mapped column under strict (throws) and default (skips), shared-descriptor emission and its absence when the model set has no descriptor resources, missing `Discriminator` system column under strict and default, no per-resource emission, existing-inventory preservation after repositioning, deterministic ordering, and identifier-shortening interplay for long tracked-change table names.
...
- Phase 1 creates and checks in a reproducible harness and dated raw/result package, executes the mandatory implementation-scale read/write/storage matrix against the exact generated DDL on both providers, and records the disposition of each category. The implementation artifact demonstrates that both providers pass a category's gates before any production code or golden changes for that category begin; the story grants no regression exceptions.
- After emitting a category, rerun that harness against the exact implemented DDL and require that category's provider-specific gates to pass. A failure blocks acceptance rather than being described as additional run-to-run noise.
