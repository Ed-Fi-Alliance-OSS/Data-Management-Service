...
- Specify the trigger behavior when the feature is disabled: are the `*_Stamp` triggers stripped of their `TriggerKindParameters.ChangeTracking` tombstone/key-change branches, or are the triggers removed entirely (in which case mirror maintenance has to be reattached)?
