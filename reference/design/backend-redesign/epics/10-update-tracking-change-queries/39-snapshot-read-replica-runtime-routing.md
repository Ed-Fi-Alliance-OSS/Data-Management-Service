...
- The reordering's own behavior changes are covered, and there are three of them. The reorder moves the endpoint phase — and, on the mutation pipelines, `ValidateRouteSemanticsMiddleware` — ahead of `ValidateDatabaseFingerprintMiddleware`, so against an unprovisioned or unreachable database: an unroutable request now returns the endpoint `404` instead of the fingerprint `503`; an invalid mutation route shape now returns the route-semantics `405` instead of the fingerprint `503`; and a request arriving while `IApiSchemaProvider.IsSchemaValid` is false now returns the ApiSchema failure instead of the fingerprint `503`. **None of the three depends on `Use-Snapshot` being present**, so they are distinct from the header-driven precedence change below and need their own coverage. Existing pipeline and integration tests that assert the previous ordering are updated deliberately rather than adjusted to whatever the new code emits.
...
- Tests cover the three reordering consequences, each against an unprovisioned or unreachable database and each with no `Use-Snapshot` header: an unroutable request returns the endpoint `404` rather than the fingerprint `503`; an invalid mutation route shape — collection `DELETE`, collection `PUT`, item `POST` — returns the route-semantics `405` rather than the fingerprint `503`; and an invalid ApiSchema returns the ApiSchema failure rather than the fingerprint `503`. Without this bullet the reorder is only exercised where the database happens to be healthy, which is exactly where it is invisible.
