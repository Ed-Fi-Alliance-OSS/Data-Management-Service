...
- Target selection's insertion point is specified per pipeline, because two pipelines perform no endpoint validation. Selection runs after the validations whose existing response precedence must be preserved — common authentication and parent resolution, and endpoint validation where present — and before the first database connection. It does not wait for every validation that happens not to use the database, and on mutation pipelines it deliberately precedes route-semantics validation:
  - `CreateGetByIdPipeline`, `CreateQueryPipeline`, and `CreateGetPartitionsPipeline`: after `ValidateEndpointMiddleware`, before `ValidateDatabaseFingerprintMiddleware`. The partitions pipeline landed with the partitioned-cursor-paging work after this story was first written; it shares every step before its paging swap with GET-many by construction, so it takes the same selection step in the same position. Omitting it is not an option: it is a ninth database-opening pipeline, and without selection every `/partitions` request would read the write-once effective target before assignment.
...
- Snapshot and read-replica eligibility apply to resource and descriptor GET-many and GET-by-id, their profile-shaped variants, resource and descriptor `/deletes` and `/keyChanges` including their profile-shaped variants, `/changeQueries/v1/availableChangeVersions`, and resource and descriptor `/partitions`. A partition boundary set describes the authorized candidate relation a page of the same request would be selected from, so it must be computed against the same database those pages are read from; routing `/partitions` keeps partition sizing and page reads on one selected target. It does not by itself promise snapshot-consistent export, which `partitioned-cursor-paging.md` keeps a non-goal.
...
- Resource and descriptor `POST`, `PUT`, and `DELETE` are snapshot-`RejectedAsMutation`, whether or not the route shape is valid, so a collection `DELETE`, a collection `PUT`, and an item `POST` are rejected by snapshot policy rather than by route semantics when the header parses `true`. `OPTIONS` is unaffected, and so is every verb DMS does not map at all (`HEAD`, `PATCH`, and any other unsupported method): those dispatch to the two method-not-allowed pipelines, which open no database, run no target selection, and keep their existence-then-method `404`/`405` contract regardless of `Use-Snapshot`. `RejectedAsMutation` is a verdict about the three supported mutation verbs, not a substitute for the unsupported-method terminal — a `HEAD` carrying the header is not a mutation, and answering it as one would misstate both what the request would do and which methods the route supports.
...
- Pipeline-composition tests assert the selection step's position in all nine pipelines — the eight originally enumerated plus `CreateGetPartitionsPipeline` — including that `CreateGetAvailableChangeVersionsPipeline` gains selection but gains no ApiSchema, endpoint, path, or resource-key step, that `CreateGetTokenInfoPipeline` runs selection and resolves `Primary`, and that the two method-not-allowed pipelines carry no selection step at all.
