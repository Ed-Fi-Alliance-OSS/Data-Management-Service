...
- For a route-valid mutation, target selection runs before `ValidateContentTypeMiddleware`, `ParseBodyMiddleware`, profile resolution, coercion, and document validation. `Use-Snapshot: true` therefore returns the snapshot `405` even if one of those later validations would otherwise return `415` or `400`; this precedence is intentional and must be tested.
...
- Tests prove a route-valid resource or descriptor mutation carrying `Use-Snapshot: true` returns the snapshot `405` before later mutation validation. Cover an invalid or missing content type that would otherwise return `415`, malformed or invalid body input that would otherwise return `400`, and profile/document validation failures; assert the exact snapshot response and prove no database connection is opened.
