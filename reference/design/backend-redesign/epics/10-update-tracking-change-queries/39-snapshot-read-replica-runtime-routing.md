...
- Keep `ValidateRouteSemanticsMiddleware` in the mutation pipelines, positioned after selection, so its response is unchanged for every request that does not carry a parsed `true`. Do not hoist route semantics into shared routed-resource steps; doing so would couple read pipelines to future additions to write-route validation.
