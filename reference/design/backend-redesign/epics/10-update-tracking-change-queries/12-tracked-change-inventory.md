...
The inventory describes per-resource tracked-change tables, the shared descriptor tracked-change table, tracked old/new value columns, descriptor and person joins, and the `DocumentStamping.ChangeTracking` attachment on document-stamping triggers.
...
- `DocumentStamping.ChangeTracking` is attached to the affected `TriggerKindParameters.DocumentStamping` entries.
