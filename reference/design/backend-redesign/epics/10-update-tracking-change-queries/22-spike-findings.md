...
7. Write shapes additionally run every warm-up and measured sample against the same pinned pre-state, restored between executions by reverting to a pinned physical snapshot or template (a PostgreSQL template-database or file-level restore; a SQL Server database-snapshot revert or `RESTORE`), with the mechanism and any post-restore statistics preparation pinned in the artifact. Transaction rollback is not a sufficient restore: aborted inserts leave dead heap and index entries on PostgreSQL until `VACUUM`, and SQL Server page splits performed by system transactions survive rollback. Every sample of both variants must start from identical table size, page fill, index state, and statistics. A write shape that cannot be restored this way is measured only under a documented deterministic state-advancement plan that applies identically to both variants, recorded in the artifact.
...
- Seek use: verified per category or predicate on its measured tables. Every adopted category or predicate is exercised as a seek by at least one applicable shape on each of its measured tables. On PostgreSQL the index appears in an Index Scan or Index Only Scan with its leading column in the index condition rather than a filter; on SQL Server an Index Seek carries seek predicates on the key columns, with no scan of that index and no key lookup where the design claims covering. A candidate that is unused or scan-only on its measured tables fails its category or predicate even when other shapes improved. Indexes emitted deterministically to tables absent from the benchmark schema are covered by this predicate-level verification rather than per-index measurement.
- Write: the write ceilings are per-table cumulative budgets across every adopted tracked-change secondary index, not per-story increments. Each measured table's bulk-write median elapsed-time ratio compares the table carrying all previously adopted indexes plus the candidate against the pinned pre-tracked-index baseline (the same deterministic provisioning with no tracked-change secondary indexes), and is at or below `1.85` on PostgreSQL and `2.00` on SQL Server; for the first story on a table this is identical to the current-baseline comparison.
- Storage, measured per table with baseline denominators, never aggregated across tables and never against candidate-state sizes: PostgreSQL compares `pg_relation_size` of each added index against `pg_table_size` of its table captured on the baseline before candidate DDL; SQL Server compares the added index's used pages (`SUM(used_page_count)` from `sys.dm_db_partition_stats` across all partitions of that index's `index_id`; `used_page_count` already totals in-row, LOB, and row-overflow pages) against the table's baseline used pages (the same sum across all partitions where `index_id` is 0 or 1, captured before candidate DDL), so the denominator is the heap or clustered index alone, excluding nonclustered indexes, matching the PostgreSQL `pg_table_size` denominator. Each table's added index storage, cumulative across every adopted tracked-change secondary index on that table including the candidate, is at or below 40% (PostgreSQL) or 50% (SQL Server) of that table's baseline size; the storage ceilings are per-table budgets shared by successive stories, not per-story allowances.
- Provider blocking: one provider's failure blocks the category or predicate on both providers pending reviewed redesign; failures are not waived as run-to-run noise. A category the design explicitly scopes to a single provider (the SQL Server-only live descriptor identity index) is gated only on its target provider; the out-of-scope provider is not measured against the gates and must remain unchanged, per the recorded per-provider disposition.
...
- Cumulative baselines: every phase's baseline is the current generated DDL and the current shipped runtime query shapes at measurement time, including all previously adopted categories, predicates, and query-shape changes from earlier stories. Stories whose indexes can appear in the same query plans land sequentially, so the later story's isolated, combined, and post-emission gates capture cross-story interaction by construction; combined overlays within a story remain required for co-emission inside that story. Sequential landing is enforced at merge time rather than assumed: a story's evidence is valid only when measured against a baseline containing every interacting change already merged, and when an interacting story merges after this story's evidence was captured, the applicable read and write matrix reruns against the updated cumulative baseline before this story merges. That rerun covers the interacting shapes of both stories: when the merging story's changes intersect plans covered by an earlier story's shipped cells, including combined-authorization cells where predicates AND-compose (for example tracked namespace indexes already emitted before a relationship rewrite or new relationship indexes land), those shipped cells rerun as part of the merging story's matrix.
...
The write and storage ceilings are cumulative per-table budgets because the 1.69x/30% evidence came from the historical combined candidate; successive stories indexing the same table share one budget rather than each consuming a fresh one, so per-story passes cannot compound past the ceiling.
