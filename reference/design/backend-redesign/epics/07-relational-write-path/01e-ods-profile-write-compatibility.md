...
DMS-1229's "accept and ignore hidden submitted data" rule is scoped to **schema-valid** submitted data. Profile shaping (`ProfileWritePipelineMiddleware` / `WritableRequestShaper`) runs *after* document/decimal validation but *before* `ExtractDocumentInfoMiddleware` / relational reference extraction in the `ApiService` upsert/update pipelines. Profile shaping does not mutate the raw submitted body — it stores the shaped result on `BackendProfileWriteContext` — so both document/decimal validation and `ExtractDocumentInfoMiddleware` operate on the raw `ParsedBody` and are profile-unaware. The compatibility boundary is therefore:
...
- **Malformed hidden references can still fail extraction.** Relational reference extraction (`ExtractDocumentInfoMiddleware` / `ReferenceExtractor` in `RelationalWriteValidation` mode) runs *after* shaping but still reads the raw `ParsedBody`, so a reference object missing identifying values is rejected before the backend shaped-reference filter runs.
