...
- Authorization execution remains out of scope for this story. The seam leaves two ordered hooks: stored-value
  authorization before full current-state loading/request-reference validation, and proposed-value authorization after
  the finalized resolved merge but before terminal DML. `DMS-983` does not introduce backend authorization behavior.
...
- The flattener consumes the existing `ResolvedReferenceSet` at the repository boundary and adapts ordinary maps plus
  any instance-scoped certified same-statement overrides into one flattener-local hot-loop lookup rather than introducing
  a second request-wide reference-resolution contract. This initial non-persisting slice supports
  `PrestatementLookupOnly`; a binding requiring the certified update-only policy remains explicitly fenced until the
  current-state/transactional DMS-1258 path lands.
...
- Populate scalar columns from strict JSON value reads, FK columns from resolved `DocumentId` and `DescriptorId` values,
  demanded identity-lineage anchor columns from the complete binding-instance resolution (ordinary target map or
  certified future override), and all
  non-storage-assigned precomputed bindings, including key-unification values and synthetic presence flags.
  Storage-assigned root `DocumentId` values and new `CollectionItemId` values remain unresolved for create flows.
...
- Relational writes call the real `IReferenceResolver`, preserve existing reference-failure semantics, and short-circuit
  before flattening and terminal-stage handoff when normal resolution fails. In this slice, a plan whose binding policy
  requires certified same-statement resolution returns the explicit slice fence rather than reusing a stored id.
- Authorization remains out of scope for `DMS-983`; the relational seam preserves the stored-before-request-work and
  proposed-after-final-merge hooks without reshaping the flattener contract.
...
- Flattening populates all demanded lineage-anchor bindings and non-storage-assigned precomputed bindings required by the
  compiled write plan, including key-unification values and synthetic presence flags; storage-assigned root and
  collection identities remain unresolved for create flows.
...
Authorization is out of scope for this story, but the repository seam must support a minimal stored-value gate before
full current-state/profile/request-reference work and a proposed-value gate after the exact merged write set is built,
before terminal DML. The stored gate must short-circuit request-dependent work on denial. These hooks must not require a
later redesign of `FlatteningInput`, `FlattenedWriteSet`, or terminal-stage contracts. See
`reference/design/backend-redesign/design-docs/auth.md` and `transactions-and-concurrency.md`.
...
6. Adapt `ResolvedReferenceSet` into flattener-local hot-loop lookups for document/descriptor FK binding by ordinal path
   and lineage-anchor binding from the same complete instance result. Accept the instance-override shape, but fence its
   execution in this non-persisting slice until current-state locking and post-write verification are available.
