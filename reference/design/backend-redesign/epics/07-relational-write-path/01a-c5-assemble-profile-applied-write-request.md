---
jira: DMS-1117
jira_url: https://edfi.atlassian.net/browse/DMS-1117
---
...
5. Implement the stored-side existence lookup: run C1's address derivation against the stored document, classify each scope using C3's shared visibility classification primitive (scope-level visibility + item-level value filtering for collection rows), and produce a lookup keyed by address that C4 consumes. A stored collection row that fails the profile's item value filter must not appear in the lookup. Preserve the intermediate classified-scope results for C6.
