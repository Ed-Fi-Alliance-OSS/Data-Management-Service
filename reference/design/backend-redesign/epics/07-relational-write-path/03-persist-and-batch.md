...
- For non-collection scopes (root-adjacent, nested/common-type, and extension scopes), use normal visible-present / visible-absent semantics:
  ...
  - preserve hidden scopes and hidden inlined columns when the writable profile excludes them.
- For collection/common-type/extension collection tables, use stable-identity merge semantics:
  - determine the visible stored rows for the scope instance,
  ...
  - insert only new visible rows marked creatable by Core, and
  - preserve hidden rows and hidden columns.
...
- Hidden collection rows, hidden columns on matched rows, hidden non-collection scopes, and hidden extension data are preserved under writable profiles.
...
4. Implement profile-aware non-collection scope handling for separate-table 1:1/extension scopes and inlined parent-row common-type/root-column data using `ProfileAppliedWriteContext`.
5. Implement stable-identity collection/common-type merge execution, including matched-row update, visible-row delete, and batched `CollectionItemId` reservation for inserts.
...
8. Add integration tests that:
   ...
   - exercise profile-scoped non-collection handling for one separate-table scope and one inlined scope,
