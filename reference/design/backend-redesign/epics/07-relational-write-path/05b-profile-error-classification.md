...
- Core/backend contract mismatches such as unknown `JsonScope`, ancestor-chain mismatch, or stored-side visibility metadata that cannot be lined up to the compiled plan shape.
...
- Core/backend profile-contract mismatches in emitted scope/row addresses or stored-side visibility metadata map to deterministic non-DB failures and stop before merge DML proceeds.
...
- Unit or integration tests cover representative cases for invalid usage, forbidden submitted data, contract-mismatch diagnostics, `ProfileRootCreateRejectedWhenNonCreatable`, `ProfileVisibleScopeOrItemInsertRejectedWhenNonCreatable`, and the required update-allowed/create-denied pairing from the shared profile scenario matrix.

## Tasks

1. Define or align the typed failure contract emitted across Core, backend, and the API layer for invalid profile definitions, invalid profile usage, writable-profile validation failures, creatability violations, and Core/backend contract mismatches.
...
3. Add tests covering the shared matrix scenario names where the failure case maps to an existing profile scenario:
  ...
  - invalid Core/backend contract address behavior.
