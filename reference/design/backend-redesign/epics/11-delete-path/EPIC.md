...
Authorization remains out of scope; however, the delete transaction structure should accommodate future authorization checks being batched into the same DB roundtrip. See `reference/design/backend-redesign/design-docs/auth-redesign.md` §"Performance improvements over ODS" for the expected roundtrip layout for DELETE.
