...
Implement the ResolveSecurableElementColumnPath helper function that, given a source resource and a securable element, resolves the chain of table joins needed to reach the authorization column in the relational model.
...
- When the securableElement type is a person (Student, Contact, or Staff) and the person is referenced directly from the source resource, the function returns a single-entry collection pointing to the person's DocumentId column on the source table.
...
NOTE: The `ResolveSecurableElementColumnPath(sourceResourceFullName, targetResourceFullName)` overload (for the View-based auth strategy) will be implemented in [DMS-1061](https://edfi.atlassian.net/browse/DMS-1061).
