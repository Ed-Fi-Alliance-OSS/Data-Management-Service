...
- When the securableElement type is EducationOrganization or Namespace, the function returns a single-entry collection where targetTable and targetColumnName are null — the column is resolved directly from whichever table the reference lives on, using the Derived Relational Model, with no traversal of intermediate references. For non-nested paths the column is on the root resource table; for array-nested paths (emitted by older ApiSchema artifacts — corrected artifacts declare Namespace securables at resource-root scope only — e.g. `$.requiredAssessments[*].assessmentReference.namespace` on `GraduationPlan`) the column is on the child collection table that owns the reference (e.g. `edfi.GraduationPlanRequiredAssessment.RequiredAssessmentAssessment_Namespace`). The single-step shape (target table/column null) holds in both cases; only `sourceTable` differs. See `auth.md` § "ResolveSecurableElementColumnPath" for the canonical statement.
