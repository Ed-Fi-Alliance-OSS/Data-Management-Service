...
- Re-enable `@relational-backend` and the appropriate `@relational-ci-shard-*` tag on the People-involved E2E scenarios temporarily excluded during the DMS-1056 EdOrg-only slice because they require People relational GET-by-id, POST, PUT, or DELETE authorization. At minimum, restore both tags on:
...
- Re-enable `@relational-backend` and the appropriate `@relational-ci-shard-*` tag on the People-involved E2E scenarios temporarily excluded during DMS-1162 because they require People-involved relational POST/PUT authorization. Restore both tags on:
  ...
  - `Features/Security/TokenIntrospection.feature`: scenario 02, "Missing token in request body returns bad request error".
  - `Features/Security/TokenIntrospection.feature`: scenario 03, "Token mismatch between Authorization header and request body returns bad request error".
  - `Features/References/StudentAssessmentRegistrationBatteryPartAssociation.feature`: scenario 01, "Create two StudentAssessmentRegistrationBatteryPartAssociation with only different AssigningEducationOrganizationIds".
