...
- Existing broad E2E scenarios whose effective authorization uses `NamespaceBased` or People-involved relationship strategies remain excluded from relational CI during Slice 2; restoring `@relational-backend` and the appropriate `@relational-ci-shard-*` tags belongs to the owning strategy stories, including DMS-1057 for `NamespaceBased` and DMS-1158 for People-involved relationship CRUD.
