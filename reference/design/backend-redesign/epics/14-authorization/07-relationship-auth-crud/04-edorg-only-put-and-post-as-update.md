...
5. Yes. Unauthorized PUT and POST-as-update assertions should include no `dms.DocumentChangeEvent` or other change-query journal rows for the attempted update, no `dms.ReferentialIdentity` insert/update/replacement for the attempted identity, unchanged `dms.Document` content stamps, and unchanged resource root/child/extension rows. Apply this to stored-value denials, proposed-value denials, and unauthorized no-op attempts. Do not assert that global sequences or identity generators did not advance; assert only persisted rows and derived artifacts that would indicate the write progressed.
