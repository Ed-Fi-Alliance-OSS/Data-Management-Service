...
direct-SQL bootstrap intent with API loading through the repo-pinned BulkLoadClient resolution path. Because
the planned new seed-file format will not land in time, the DMS-916 seed payload contract is legacy ODS XML
interchange files. The target state remains replacement of the direct-SQL path, not supplementation.

The slice includes seed-source selection, the dedicated DMS-dependent `SeedLoader` credential flow for the
seed-delivery phase, per-year loading for the existing `-SchoolYearRange` workflow, and the bootstrap-side
...
depends on, and the pass-through result handling. It also owns the repo-local `Minimal` and `Populated` XML
developer seed assets used by `-SeedTemplate`. It does not broaden DMS-916 into owning BulkLoadClient product
design, published seed package distribution, or non-bootstrap runtime behavior.

## Implementation Readiness Gates

This story has no external seed-format blocker. End-to-end implementation is gated by DMS-owned compatibility work:
...
- Repo-local XML seed assets must be added for the built-in `Minimal` and `Populated` templates.

These gates do not introduce a second normative seed mode. The direct-SQL path is **not** the design target
state; it is the current gap this story closes through API-based XML loading. No DMS-916 artifact normalizes
the legacy direct-SQL path as an ongoing or permanent alternative.
...
- Bootstrap also fails fast when a built-in seed source is selected but the required repo-local XML directory
  cannot be resolved.
- Supported seed inputs match the design:
  - repo-local Ed-Fi XML seed templates selected by `-SeedTemplate`,
...
- Bootstrap stages the selected XML interchange files into one flat repo-local BulkLoadClient data directory
...
- When `-SchoolYearRange` is used, bootstrap prepares the seed workspace once and invokes BulkLoadClient once
  per year against the route-qualified DMS base URL and matching OAuth URL for that year.
...
4. Implement seed-workspace creation, XML interchange materialization for both built-in artifacts and
   `-SeedDataPath`, deterministic collision-safe file naming or pre-invocation failure, and cleanup behavior.
   Ordering remains owned by BulkLoadClient dependency/interchange metadata rather than bootstrap filename
   prefixes.
...
6. Implement the BulkLoadClient invocation path, including `-BootstrapManifestPath` handling,
   ...
   creation, broad target-selection policy, or non-selector-driven discovery during seed delivery.
