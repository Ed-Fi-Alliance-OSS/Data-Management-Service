...
- Serves `_lastModifiedDate/ChangeVersion` from stored `dms.Document` stamps and computes `_etag` from deterministic serialized JSON (no read-time dependency derivation).
...
- computing `_etag` from the serialized JSON representation while serving `_lastModifiedDate/ChangeVersion` from stored stamps (no dependency-token expansion).
