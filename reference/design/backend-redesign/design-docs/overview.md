...
4. **Bounded cascades with stable FKs**: Store relationships as stable surrogate keys (`DocumentId`) for referential integrity, but also persist the referenced identity parts needed for query/reconstitution at the reference site (the `{RefBaseName}_{IdentityPart}` *binding* columns). These values stay consistent via composite FKs plus dialect-specific propagation: PostgreSQL uses `ON UPDATE CASCADE` for abstract targets and concrete targets with `allowIdentityUpdates=true` (`ON UPDATE NO ACTION` otherwise), while SQL Server uses **foreign-key pruning** — `ON UPDATE CASCADE` on the one surviving (live) edge into each referenced table and `ON UPDATE NO ACTION` on pruned edges, with a `TriggerKindParameters.MssqlIdentityPropagationTrigger` fallback only for pruned edges that remain live, and fail-fast derivation when no safe pruning exists (see [mssql-cascading.md](mssql-cascading.md); this supersedes the earlier all-`NO ACTION`-plus-trigger rule). When `equalityConstraints` unify identity parts across sites/paths on the same row, those per-site/per-path bindings may be generated/persisted aliases of a canonical stored column (presence-gated when optional); see [key-unification.md](key-unification.md).
...
- **Polymorphic references use abstract identity tables and abstract union views**: For abstract reference targets (e.g., `EducationOrganization`), provision an `{AbstractResource}Identity` table (`DocumentId` + abstract identity fields + `Discriminator` (NOT NULL)) and reference it with composite FKs (including identity columns). PostgreSQL uses `ON UPDATE CASCADE`; SQL Server uses foreign-key pruning (`ON UPDATE CASCADE` on surviving edges, `ON UPDATE NO ACTION` on pruned edges, `TriggerKindParameters.MssqlIdentityPropagationTrigger` fallback only where a pruned edge remains live) — see [mssql-cascading.md](mssql-cascading.md). Also provision `{AbstractResource}_View` union views for query/diagnostics; they are no longer required for reference identity projection. See [data-model.md](data-model.md).
...
- In this redesign, canonical storage is relational (tables per resource). Referencing relationships are stored as stable `DocumentId` FKs, so:
  ...
  - responses can reconstitute reference identity values directly from local reference-identity binding columns. Propagation to those columns is database-driven: PostgreSQL uses `ON UPDATE CASCADE` for eligible edges, and SQL Server uses foreign-key pruning: `ON UPDATE CASCADE` on surviving edges and `ON UPDATE NO ACTION` on pruned edges, with `TriggerKindParameters.MssqlIdentityPropagationTrigger` fallback only where a pruned edge remains live (see [mssql-cascading.md](mssql-cascading.md)). Under key unification, those binding columns may be presence-gated aliases of canonical stored columns, preserving “absent optional reference ⇒ `NULL` at the binding columns”.
- Identity/URI changes do not rewrite `..._DocumentId` foreign keys, but **do** propagate into the local canonical stored identity columns that underpin reference-identity bindings (PostgreSQL cascades; on SQL Server via foreign-key pruning — cascade on surviving edges, `MssqlIdentityPropagationTrigger` fallback where a pruned edge remains live; see [mssql-cascading.md](mssql-cascading.md)). Cascades still exist for derived artifacts, but they are handled row-locally:
...
1. **Cascade feasibility (SQL Server)**: SQL Server “multiple cascade paths” / cycle restrictions (confirmed: error 1785) are handled by **foreign-key pruning** rather than by blanket `ON UPDATE NO ACTION`: cascade is kept on one surviving edge per table, redundant edges are pruned to `NO ACTION`, a `TriggerKindParameters.MssqlIdentityPropagationTrigger` fallback covers pruned-but-live edges, and derivation fails fast when no safe pruning exists. See [mssql-cascading.md](mssql-cascading.md).
2. **Operational fan-out**: an identity update on a “hub” document can synchronously update many referencing rows (via PostgreSQL FK cascades or SQL Server `MssqlIdentityPropagationTrigger` triggers), increasing deadlock and latency risk.
