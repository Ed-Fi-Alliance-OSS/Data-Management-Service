...
4. **Bounded cascades with stable FKs**: Store relationships as stable surrogate keys (`DocumentId`) for referential integrity, but also persist the referenced identity parts needed for query/reconstitution at the reference site (the `{RefBaseName}_{IdentityPart}` *binding* columns). Derivation first maps bindings through key-unification storage, then canonicalizes and deduplicates the resulting full-composite physical FK candidates. A cross-engine, statement-scoped `ValueFlowAnalysis` derives facts and proof obligations for every identity update and every FK that reads a potentially cascade-written canonical receiver column. Those obligations include exact component lineage, origin-row correlation, reference co-presence, and trigger statement boundaries; shared columns or table reachability alone are not sufficient. PostgreSQL evaluates its fixed action assignment (`ON UPDATE CASCADE` for candidates whose target can change transitively, `NO ACTION` otherwise) against those obligations. SQL Server jointly selects `NativeCascade` / `NoPropagation` modes that satisfy both error 1785 and the value-flow obligations. Only the resulting dialect assignment is certified; derivation fails when no certifiable assignment exists. There is no `DocumentId`-only shape or identity-value propagation-trigger fallback; see [mssql-cascading.md](mssql-cascading.md). When `equalityConstraints` unify identity parts across sites/paths on the same row, those per-site/per-path bindings may be generated/persisted aliases of a canonical stored column (presence-gated when optional); see [key-unification.md](key-unification.md).
...
- **Polymorphic references use abstract identity tables and abstract union views**: For abstract reference targets (e.g., `EducationOrganization`), provision an `{AbstractResource}Identity` table (`DocumentId` + abstract identity fields + `Discriminator` (NOT NULL)) and reference it with full-composite FKs (identity columns first, then `DocumentId`). Abstract-identity maintenance occurs in a later trigger statement, so `ValueFlowAnalysis` records that boundary rather than treating the table as an ordinary same-statement cascade source. PostgreSQL evaluates its fixed actions with that obligation; SQL Server includes it while jointly selecting modes for value-flow safety and error 1785. Also provision `{AbstractResource}_View` union views for query/diagnostics; they are no longer required for reference identity projection. See [data-model.md](data-model.md) and [mssql-cascading.md](mssql-cascading.md).
...
- In this redesign, canonical storage is relational (tables per resource). Referencing relationships are stored as stable `DocumentId` FKs, so:
  ...
  - responses can reconstitute reference identity values directly from local reference-identity binding columns. Under key unification, those binding columns may be presence-gated aliases of canonical stored columns, preserving “absent optional reference ⇒ `NULL` at the binding columns”. `ValueFlowAnalysis` derives the obligations consumed by PostgreSQL's fixed-action evaluation and SQL Server's joint mode selection; see [mssql-cascading.md](mssql-cascading.md).
- Identity/URI changes do not rewrite `..._DocumentId` foreign keys, but **do** propagate into the local canonical stored identity columns that underpin reference-identity bindings. The certified final assignment uses database FK cascades where required; SQL Server may use a full-composite `NO ACTION` edge only when the joint solution proves its `NoPropagation` coverage. Cascades still exist for derived artifacts, but they are handled row-locally:
...
1. **Identity-update feasibility (cross-engine + SQL Server)**: Key unification can make several physical FKs read one canonical receiver column. Independent parents are not assumed safe. `ValueFlowAnalysis` derives component-lineage, origin-row-correlation, reference-co-presence, and statement-boundary obligations. PostgreSQL certifies or rejects its fixed action assignment. SQL Server jointly selects modes satisfying those obligations and error 1785, then emits coverage certificates from the final assignment. Any assignment that cannot be certified fails derivation. There is no `DocumentId`-only or propagation-trigger fallback. See [mssql-cascading.md](mssql-cascading.md).
