...
- DocumentCache projection and relational CDC (authoritative integration/deployment design):
  ...
  relationship.
- CDC decisions (projector/source and topic/message): [cdc/](cdc/)
...
1. **Cascade feasibility (SQL Server)**: SQL Server multiple-cascade-path and cycle restrictions require the focused selection and fail-fast rules in [SQL Server foreign-key pruning](sql-server-pruning.md).
2. **Operational fan-out**: an identity update on a “hub” document can synchronously update many referencing rows through native FK cascades, increasing deadlock and latency risk.
