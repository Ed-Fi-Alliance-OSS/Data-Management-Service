...
- `reference/design/backend-redesign/design-docs/auth-redesign.md` (authorization is token-dependent and is integrated with the read/write paths, but is not the primary focus of mapping packs)
...
Authorization note:
- Mapping packs do not embed token-dependent authorization SQL (which depends on caller context and configured strategy sets). Authorization is applied at runtime using the `auth.*` companion objects and token-derived context described in `auth-redesign.md`.
