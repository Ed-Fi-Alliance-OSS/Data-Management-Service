...
- `reference_target_anchor_reads`: ordered by target `(project_name, resource_name)`; anchor columns remain in target
  transitive-lineage order.
...
message MappingPackPayload {
  ...
  repeated ReferenceTargetAnchorRead reference_target_anchor_reads = 3;
}
...
message ReferenceTargetAnchorRead {
  ...
  repeated DbColumnName ordered_anchor_columns = 4;
}
...
Notes:
- This overview schema is abbreviated; `mpack-format-v1.md` is the normative wire contract.
...
public sealed class MappingPackBuilder
{
    public async Task BuildAsync(BuildOptions options, CancellationToken ct)
    {
        ...
        // Serialize the effective-schema target inventory once. This includes explicit abstract identity tables;
        // do not infer target reads from the per-resource incoming bindings above.
        payload.ReferenceTargetAnchorReads.AddRange(
            SerializeReferenceTargetAnchorReads(docs, options.Dialect));
