...
- Relationships are stored as stable `DocumentId` foreign keys, with referenced identity natural-key fields available locally for query/reconstitution and kept consistent via dialect-specific propagation rules (no FK rewrites): PostgreSQL uses `ON UPDATE CASCADE` for abstract targets and concrete targets whose identity can change transitively (`ON UPDATE NO ACTION` otherwise), while SQL Server uses foreign-key pruning — `ON UPDATE CASCADE` on eligible edges (including independent parents into a shared receiver), pruning a covered edge to `ON UPDATE NO ACTION` only where one update would otherwise reach a table by two cascade paths (a diamond), and fail-fast when no safe pruning exists (a cascade cycle/SCC, or diamonds that cannot be jointly broken — a single uncovered diamond, or globally infeasible overlapping diamonds); every SQL Server reference FK keeps the full composite key, with no `DocumentId`-only trigger fallback (see [mssql-cascading.md](mssql-cascading.md)). Under key unification, equality-constrained per-site/per-path bindings may be generated/persisted, presence-gated aliases of canonical stored columns (see `key-unification.md`).
...
- SQL Server and PostgreSQL must both be supported (the design must be implementable — DDL + CRUD + query — on both), with shared behavior where practical and explicit engine-specific behavior where the engines diverge. Parity is **not** forced where the engines differ: SQL Server cascade pruning / unsafe-graph detection / fail-fast are SQL-Server-only, and PostgreSQL is left as-is (full-composite `ON UPDATE CASCADE` on eligible edges, never pruned or failed by DMS); see [mssql-cascading.md](mssql-cascading.md).
...
- **Representation dependency** (1 hop): any referenced non-descriptor document whose identity values are embedded in the full resource-state representation before readable profile projection. Indirect representation changes are realized as database-driven propagation updates to canonical stored identity columns that back the local bindings (PostgreSQL FK cascades; SQL Server native `ON UPDATE CASCADE` on eligible edges), including presence-gated aliases that preserve “absent ⇒ `NULL` at the binding columns”, which trigger normal stamping of stored `_etag/_lastModifiedDate/ChangeVersion`.
...
- Root table `{schema}.{ResourceName}`:
  ...
  - Reference FK columns:
    - for each document reference site: store `..._DocumentId` and the identity-part bindings, with a composite FK to the target identity key `(<IdentityParts...>, DocumentId)` (identity parts first, `DocumentId` last). PostgreSQL uses `ON UPDATE CASCADE` for abstract targets and concrete targets whose identity can change transitively (`ON UPDATE NO ACTION` otherwise). SQL Server uses foreign-key pruning: `ON UPDATE CASCADE` on eligible edges, pruning a covered edge to `ON UPDATE NO ACTION` only at a diamond (where one update would otherwise reach a table by two cascade paths), failing fast on cycles/SCC or diamonds that cannot be jointly broken, incl. globally infeasible overlapping — see [mssql-cascading.md](mssql-cascading.md). Under key unification, composite FKs are built over canonical stored identity columns (single source of truth), while per-site/per-path identity-part bindings can remain as generated/persisted aliases.
    - polymorphic targets: composite FK to `{schema}.{AbstractResource}Identity(<AbstractIdentityParts...>, DocumentId)` with the same dialect-specific update behavior (`ON UPDATE CASCADE` on PostgreSQL; on SQL Server, foreign-key pruning — cascade on eligible edges, pruning a covered edge to `NO ACTION` (full composite) only at a diamond, fail-fast on cycles/SCC or diamonds that cannot be jointly broken, incl. globally infeasible overlapping; see [mssql-cascading.md](mssql-cascading.md)),
...
3. **DB-enforced identity propagation**
   - Composite foreign keys keep canonical stored identity columns consistent when referenced identities change (PostgreSQL `ON UPDATE CASCADE` for abstract targets and concrete targets whose identity can change transitively; on SQL Server via foreign-key pruning — cascade on eligible edges, pruning a covered edge to `ON UPDATE NO ACTION` (full composite) only at a diamond, fail-fast on cycles/SCC or diamonds that cannot be jointly broken, incl. globally infeasible overlapping; see [mssql-cascading.md](mssql-cascading.md)). Per-site/per-path identity bindings may be generated/persisted (and presence-gated) aliases of those canonical columns under key unification.
...
5. **Strict identity maintenance (row-local triggers)**
   ...
   - Identity changes therefore propagate transitively via DB-driven propagation (PostgreSQL FK cascades; SQL Server native `ON UPDATE CASCADE` on eligible edges), without application-managed closure traversal.
...
- **Cascade feasibility and fan-out**
  - SQL Server “multiple cascade paths” / cycle restrictions (error 1785) are handled by foreign-key pruning analyzed in propagation direction: cascade on eligible edges (including independent parents into a shared receiver), pruning a covered edge to `ON UPDATE NO ACTION` (full composite) only where one update would otherwise reach a table by two cascade paths (a diamond), and fail-fast when no safe pruning exists (a cascade cycle/SCC, or diamonds that cannot be jointly broken — a single uncovered diamond, or globally infeasible overlapping diamonds) — no `DocumentId`-only trigger fallback (see [mssql-cascading.md](mssql-cascading.md), including its [diagrams](mssql-cascading.md#diagrams)).
