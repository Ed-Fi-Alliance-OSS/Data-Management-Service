...
- Relationships use full-composite vectors containing referenced public identity values, complete transitive lineage
  anchors, and stable target `DocumentId`. After key unification, DMS promotes canonical identity overlaps atomically and
  rejects cycles in that effective dependency graph. PostgreSQL assigns fixed actions mechanically and is never pruned or
  classified for broader origin-terminal physical topology. SQL Server reports broader physical
...
- SQL Server and PostgreSQL must both be supported, with explicit engine-specific behavior where they diverge. SQL
  ...
  broader physical topology. Ordinary provider-independent validation, including effective identity-cycle rejection and
  terminal-edge certification, still
  applies; see
...
3. **DB-enforced identity propagation**
   - Complete-vector FKs keep public identity and stable lineage anchors consistent. Provider-independent validation
     rejects authored and storage-promoted effective identity cycles. PostgreSQL uses fixed actions; SQL Server rejects
     broader physical cycles from its normal
...
5. **Strict identity maintenance (row-local triggers)**
   - Per-resource triggers recompute `dms.ReferentialIdentity` whenever the document's actual identity projection changes,
     including a canonical update arriving through a storage-promoted effective dependency.
...
- **Cascade feasibility and fan-out**
  - Authored and post-key-unification storage-promoted effective identity cycles are rejected provider-independently.
    Broader SQL Server physical cycles fail its normal topological
