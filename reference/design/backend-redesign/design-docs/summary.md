...
- SQL Server foreign-key pruning: `reference/design/backend-redesign/design-docs/sql-server-pruning.md`
...
- DocumentCache role, projector, freshness, backfill, failure, and lifecycle separation:
  `reference/design/backend-redesign/design-docs/document-cache/`
- CDC/Kafka (DocumentCache upserts, `dms.Document` deletes, topic/message contract, connector deployment): `reference/design/backend-redesign/design-docs/cdc/`
...
- Relational Debezium/Kafka CDC, when enabled, uses one connector to capture
  `dms.DocumentCache` create/update/snapshot events as upserts and authoritative
  `dms.Document` deletes as tombstones. It publishes a compacted topic-per-instance
  document-state stream keyed by `DocumentUuid`; `dms.DocumentCache` remains optional
  when CDC/Kafka is not enabled.
- Cache deletion never represents domain deletion. The connector ignores cache deletes,
  so API deletion does not depend on projection health and cache truncation/rebuild does
  not publish mass tombstones. The detailed projector and lifecycle-separation contract
  is defined in `design-docs/document-cache/`.
...
- Root table `{schema}.{ResourceName}`:
  ...
  - Reference FK columns:
    - for each document reference site: store `..._DocumentId` and the identity-part bindings, with a composite FK to the target identity key `(DocumentId, <IdentityParts...>)`. PostgreSQL uses `ON UPDATE CASCADE` for abstract targets and transitively mutable concrete targets (`ON UPDATE NO ACTION` otherwise). SQL Server assigns native cascade or a safe full-composite `NO ACTION` cut under `sql-server-pruning.md`. Under key unification, composite FKs are built over canonical stored identity columns (single source of truth), while per-site/per-path identity-part bindings can remain as generated/persisted aliases.
    - polymorphic targets: composite FK to `{schema}.{AbstractResource}Identity(DocumentId, <AbstractIdentityParts...>)` with the same dialect-specific update behavior (native cascade on PostgreSQL; SQL Server action selected by `sql-server-pruning.md`),
...
- **Cascade feasibility and fan-out**
  - SQL Server multiple-cascade-path and cycle restrictions require the focused selection and fail-fast rules in `sql-server-pruning.md`.
