...
public sealed record TrackedChangeColumnInfo(
    ...
    // The canonical storage column when the source participates in key unification, or DocumentId
    // when the column is a zero-hop self-person projection (PersonJoinName = null); null otherwise.
...
- Index/trigger/tracked-change inventories are dialect-aware (“SQL-free DDL intent”), derived deterministically from the derived tables/constraints plus the policies in `ddl-generation.md` and `change-queries.md`.
  ...
  - Scope: schema-derived project objects only (resource/extension/abstract-identity tables). This includes authorization-required indexes on resource tables derived from `securableElements` (see `auth.md`) and tracked-change tables/views derived from Change Query metadata (see `change-queries.md`). Core `dms.*` / `auth.*` objects (and their indexes/triggers) are owned by core DDL emission, with the following carve-outs:
    ...
    - the shared `dms.Descriptor` change-version index (`IX_Descriptor_Discriminator_ContentVersion`) is derived into the index inventory by `DeriveIndexInventoryPass` rather than emitted by core DDL, because its existence depends on the content-version mirror feature and on descriptor resources being present in the effective schema (see [change-queries.md](change-queries.md) § "Concrete-resource `ContentVersion` / `ContentLastModifiedAt` mirror").
    - the SQL Server-only live descriptor identity index (`(Discriminator, Namespace, CodeValue)` on `dms.Descriptor`), selected by DMS-1185 for future adoption, is likewise derived through the dialect-aware index inventory when its story lands (see [change-queries.md](change-queries.md) § "`*_RefKey` index ordering for `/deletes`").
