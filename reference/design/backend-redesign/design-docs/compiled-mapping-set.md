...
- Index/trigger/tracked-change inventories are dialect-aware (“SQL-free DDL intent”), derived deterministically from the derived tables/constraints plus the policies in `ddl-generation.md` and `change-queries.md`.
  ...
  - For key-change rows, dialect emitters use the same null-safe old/new value-diff workset already required for key-change eligibility. Under key unification, this includes the presence-gated canonical expressions defined in `key-unification.md`.
