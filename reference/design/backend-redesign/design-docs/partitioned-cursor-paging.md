...
1. **Resource filters.** The same unknown-query-field and filter-value-type rules GET-many applies,
   over the same candidate set. The five reserved paging names and `partitionCount` are excluded
   from filter matching before this phase runs, so a supplied `limit` is not reported as an unknown
   query field.
2. **Change-version window.** The same `minChangeVersion`/`maxChangeVersion` parsing GET-many
   applies.
3. **`partitionCount` syntax and range.** A malformed or out-of-range `partitionCount` produces the
...
- `?partitionCount=abc&notAField=1` answers with the unknown-query-field error alone. Both are
  client mistakes, and answering the field first keeps this operation's unknown-field behavior
  identical to GET-many's.
- `?partitionCount=abc&minChangeVersion=bogus` answers with the change-version error alone.
