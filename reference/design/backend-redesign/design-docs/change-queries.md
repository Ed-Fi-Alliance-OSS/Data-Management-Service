...
The accepted envelope, which implementation verification gates against on both dialects: narrow-window `/deletes` against a PrimaryAssociation resource may regress at most 5x relative and 3 seconds absolute at the 10M-tombstone benchmark scale (calibrated to the measured 4.7x), and no other measured request shape may regress beyond run-to-run noise.
...
Person columns are selected by `Role = PersonDocumentId`; EdOrg and namespace columns both surface as `TrackedChangeColumnInfo.Origin` securable-element flag with `Role = Scalar`, so the pass must distinguish them with the runtime planner's dual check: a column is a namespace column when its `SourceJsonPath` matches one of the resource's `SecurableElements.Namespace` paths, or when its `CanonicalStorageColumn` matches the live column such a path resolves to through `SecurableElementLocationResolver` (the rule `ReadChangesAuthorizationPlanner.ResolveTrackedColumnForSecurable` applies, preserving the both-sides-agree contract).
