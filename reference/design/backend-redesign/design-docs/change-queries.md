...
6. On SQL Server, before a nonempty `affectedDocs` workset updates `dms.Document` in lifecycle `Tracking`, `Resetting`, or `Rebuilding`, the generated trigger MUST require the `sys.configurations` row named `nested triggers` to be readable with `value_in_use = 1` and throw otherwise. This fail-closed guard is inactive for root/descriptor inserts that only copy an already-enqueued `dms.Document` default stamp and for lifecycle `Disabled`. See [update-tracking.md](update-tracking.md#projection-work-follows-every-committed-content-stamp).
...
BEGIN
    ...
    -- Before executing this UPDATE for a nonempty affectedDocs workset, the generated
    -- SQL Server trigger performs the enqueue-enabled nested-trigger guard described
    -- above. The renderer may persist the workset or emit an equivalent set-based shape
    -- so the check and UPDATE use the same affected DocumentIds.
    UPDATE d
...
- SQL Server DB-behavior: with projection lifecycle enqueue-enabled, disabling `nested triggers` makes every nonempty indirect `affectedDocs` stamp fail and roll back its complete canonical transaction; root/descriptor inserts that only copy the already-enqueued initial `dms.Document` stamp and all writes in lifecycle `Disabled` retain their documented behavior.
