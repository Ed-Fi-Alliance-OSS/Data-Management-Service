...
- DB-behavior: no separate document-level identity counter exists on every in-scope root table or on `dms.Descriptor`; identity-projection changes are reflected through `ContentVersion` and `ContentLastModifiedAt`.
