...
| **Failure conditions** | Runtime-contract preflight failure before any stack-lifecycle mutation (the same validation `-PreflightOnly` runs; an isolated read-only validator container is a support operation, not a stack-lifecycle change), when `docker compose config` cannot resolve or the Compose-resolved runtime contract is invalid: a DMS or Configuration Service runtime provider (`AppSettings__Datastore`) that is not exactly `postgresql`/`mssql` or does not equal the selected engine; a Configuration Service connection string the exact runtime provider rejects, or one that targets the wrong configuration database (the DMS datastore anchor in shared topology, or the dedicated `edfi_configurationservice` under `-SeparateConfigDatabase`); a Configuration Service connection whose ENDPOINT does not reach the local database container when the CMS must be local (a full-stack or self-contained-OpenIddict run) - a foreign or host-loopback host, the host-published port instead of the container-internal port, a multi-host or named-instance endpoint, or a SQL Server Failover Partner (a standalone Keycloak Configuration Service is exempt and may use an external database); a blank or non-concrete topology datastore anchor; a separate topology whose datastore and configuration databases are not distinct physical databases; or a blank SQL Server SA password. Docker compose start failure; health-wait timeout for any service; malformed or incomplete bootstrap manifest when present |
...
| **Inputs** | `-EnvironmentFile <path>` (select local settings for CMS URL, auth, tenant scope, and database defaults); `-NoDataStore` (narrow reuse escape hatch: valid only when exactly one existing instance is present); `-SchoolYearRange <range>` (school-year path); `-AddSmokeTestCredentials` (creates CMS-only test application); `-SeparateConfigDatabase` (separate-topology awareness so an explicit `-DataStoreDatabaseName` replacement cannot collide with the dedicated `edfi_configurationservice` configuration database) |
...
| **Failure conditions** | Zero matching instances found; multiple matching instances found without an explicit `-DataStoreId` or `-SchoolYear` selector; SchemaTools/runtime provisioning exits non-zero, including when target stored schema state is incompatible with the staged schema set; connection to target database fails; a target's CMS-stored connection string is not accepted by the effective environment's `DMS_DATASTORE` provider (the provisioning dialect is the explicit `DMS_DATASTORE`, never inferred from the connection string; a connection the selected provider rejects is classified a stale data store only when the other engine accepts it, and simply invalid otherwise) |
...
orchestration target the same local environment. Dialect: `pgsql`|`mssql`, derived from the explicit
`DMS_DATASTORE` (canonicalized through the single engine-token boundary, so a case variant resolves and
an unsupported token fails fast) and NEVER inferred from the connection string.
`Resolve-ExpectedProvisioningDialect` selects it. The provisioning flow resolves the SchemaTools tool once up
front and passes it into `New-ProvisionTarget`, which calls `Convert-CmsConnectionStringToHostSideTarget` to
parse each CMS-stored connection with the exact runtime provider via the SchemaTools `connection inspect`
verb (never a generic keyword builder), so a provider-valid
alias (e.g. the Npgsql `Server=`/`User Id=` aliases) is accepted exactly as the runtime accepts it. When the
selected provider rejects a stored connection, the other engine is probed: valid under the other engine is
classified a stale data store from a previous run with that engine; invalid under both is simply invalid for
the selected provider (no stale claim); provider-valid but database-less is incomplete. `New-ProvisionTarget`
fails fast with data-store-scoped remediation guidance, preserving the specific message, while a
tool-contract/version failure propagates as a tool error rather than a datastore signal. A recognized
Docker-internal endpoint is translated to the host-side mapped host/port through the `ddl provision`
overrides, and the original connection string is otherwise passed through unchanged.
...
| `start-local-dms.ps1` | `-EnvironmentFile <path>`, `-Rebuild`/`-r`, `-IdentityProvider`, `-EnableConfig` (legacy compat), `-EnableKafkaUI`, `-EnableSwaggerUI`, `-DatabaseEngine` (`postgresql`/`mssql`; selects the database compose file), `-d`/`-v`, `-AddExtensionSecurityMetadata` (no-manifest startup only; bootstrap mode activates staged claims from manifest), split-startup switches `-InfraOnly`, `-DmsOnly`, and `-DbOnly` (database container plus readiness only), `-DmsBaseUrl <url>` (valid only with `-InfraOnly`; not valid with `-DbOnly`), `-SeparateConfigDatabase` (dedicated `edfi_configurationservice` topology; orthogonal to the engine), `-PreflightOnly` (resolve and validate the runtime contract, then stop before starting the stack; not valid with teardown `-d`) |
| `configure-local-data-store.ps1` | `-EnvironmentFile <path>`, `-DatabaseEngine postgresql\|mssql`, `-NoDataStore`, `-SchoolYearRange`, `-AddSmokeTestCredentials`, `-SeparateConfigDatabase` (topology awareness so an explicit `-DataStoreDatabaseName` cannot collide with the dedicated `edfi_configurationservice` database) |
