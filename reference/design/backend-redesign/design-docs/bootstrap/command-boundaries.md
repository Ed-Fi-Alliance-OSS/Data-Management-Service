...
**DMS-1271 (delivered):** guarded replacement exists, restore-mode-only, via `-RestoreTemplate` on both
wrappers. The restore branch builds the workspace as a private candidate through the unchanged prepare
phases (root override), cross-checks it against the trusted package's restore manifest, proves the stack
stopped, and commits the whole `.bootstrap` tree at once - schema, claims, and seed handoff state
regenerate together, never schema alone. Non-restore runs keep the DMS-1255 fail-fast on a mismatched
workspace by design; reviews should continue to treat that as intentional, not as a missing automatic
