...
4. Standard mode means package-backed staging (omit `-ApiSchemaPath`). With `-EnvironmentFile`, the effective
   `SCHEMA_PACKAGES` list selects core plus any published extensions; without it, direct invocation uses the
   catalog-pinned core-only fallback. Expert mode means `-ApiSchemaPath`; expert mode is the path for unpublished or custom schema sets, narrows
...
5. Implementation is split: Story 00 delivers the direct filesystem `-ApiSchemaPath` path; Story 06 delivers
   package-backed standard mode after Story 05.
6. DDL provisioning and seed loading are separate phases. DMS startup is never the authoritative schema
   deployment path.
7. Wrapper standard mode stages the effective `SCHEMA_PACKAGES` set. The default DS 5.2 profile is core +
   TPDM; DS 6.1 is core-only because TPDM is folded into core. Unpublished/custom schema sets use expert
   `-ApiSchemaPath`.
...
- **Standard mode**: package-backed staging (omit `-ApiSchemaPath`); `-EnvironmentFile` supplies the effective
  `SCHEMA_PACKAGES`, while direct invocation without it uses the catalog-pinned core-only fallback. There is
  no `-Extensions` parameter.
...
- **Explicit selectors**: `-DataStoreId <long[]>` and `-SchoolYear <int[]>` flags passed to downstream
...
| Phase 6: Database Provisioning | Drop/create/migrate Admin, Security, ODS databases per engine and install type | Covered by the DMS-916 design for Docker-managed PostgreSQL: `postgresql.yml` starts the engine, `Add-DataStore` / `Add-DmsSchoolYearInstances` in `Dms-Management.psm1` identify the target instances, and the explicit `provision-dms-schema.ps1` SchemaTools/runtime-owned provisioning and validation path performs the authoritative pre-start schema work for those targets. See [Backend Redesign Impact and DDL Provisioning](#11-backend-redesign-impact-and-ddl-provisioning). |
| Phase 7: Data Seeding | Load bootstrap/sample data through an API host | Covered by the phase-owned `load-dms-seed-data.ps1` API-based path (XML interchange in Phase 1; see §6 "Phased delivery" for the Phase 2 JSONL target). The historical start-script `-LoadSeedData` path called `setup-database-template.psm1` directly and has been removed from both start scripts. |
...
| 11 | Reset populated template database | `Reset-TestPopulatedTemplateDatabase` | `!NoDeploy` | **Covered by API seed delivery** | Bootstrap seed delivery replaces this ODS initdev step. The historical start-script `-LoadSeedData` flag ran `setup-database-template.psm1` directly; both start scripts have since removed it, and the wrapper-level opt-in invokes `load-dms-seed-data.ps1`. See [API-Based Seed Data Loading](#6-api-based-seed-data-loading). |
| 12 | Full database deployment | `Initialize-DeploymentEnvironment` | `!NoDeploy` | **Covered by DMS-916 design, pending implementation** | `Add-DataStore` and `Add-DmsSchoolYearInstances` in `Dms-Management.psm1` identify the target instances, and the DMS-916 design then requires the explicit `provision-dms-schema.ps1` SchemaTools/runtime-owned provisioning and validation path before DMS startup. The remaining work here is implementation delivery of that designed path, not a missing design contract. See [Backend Redesign Impact and DDL Provisioning](#11-backend-redesign-impact-and-ddl-provisioning) and Story 01. |
...
- **Standard mode** (omit `-ApiSchemaPath`): package-backed — stage the effective `SCHEMA_PACKAGES` set when
  `-EnvironmentFile` is supplied, or the catalog-pinned core package for direct invocation without one.
...
Omit `-ApiSchemaPath` (standard mode). In the wrapper flow, bootstrap resolves every package in the effective
`SCHEMA_PACKAGES` list host-side (core plus any published extensions), extracts each into an
...
normalized staged schema set. Direct preparation without `-EnvironmentFile` resolves only the catalog-pinned
core Data Standard package. The target package payload is asset-only as defined in
...
The wrapper intentionally retains a `SCHEMA_PACKAGES`-driven local profile: default DS 5.2 stages core + TPDM,
while DS 6.1 stages core only because TPDM is folded into core. Published extension packages may be selected
through `SCHEMA_PACKAGES`; unpublished or custom schema sets use expert `-ApiSchemaPath`.
...
- `-SeedDataPath` - supplies custom seed files (XML interchange in Phase 1, JSONL in Phase 2 per §6); without this, no seed data is loaded
...
> implements that direct filesystem path only; package-backed standard mode belongs to Story 06.
...
- Standard mode: the schema and claims phase commands resolve their owned artifact types for the effective
  package set. The seed phase resolves seed artifacts from its seed catalog. If any selected package cannot be
...
On startup, CMS loads this metadata into its configured datastore using one of two modes controlled by
`DMS_CONFIG_CLAIMS_SOURCE`. In the default local topology, CMS uses the `dmscs` schema in the same physical
PostgreSQL or SQL Server database as DMS:
...
1. **Core security metadata is always loaded.** The embedded `Claims.json` (Embedded mode) covers all
   core Ed-Fi Data Standard resources. No extra configuration is needed when the staged set is core only.
...
- **Standard mode**: claims mode follows the staged package set; core-only remains Embedded, while selected
  extensions with bootstrap-managed fragments resolve to Hybrid - no extra flag needed.
...
- *Phase 1 — XML interchange (DMS-1152, current):* DMS-1152 delivers seed loading against the
  repo-pinned `EdFi.BulkLoadClient` package's XML-mode CLI surface. `load-dms-seed-data.ps1` resolves
  the pinned package, validates the short-flag XML surface (`-b`, `-d`, `-w`, `-k`, `-s`, `-o`, `-x`)
  via `Assert-BulkLoadClientXmlInterface`, and stages XML interchange files from the pinned
  Ed-Fi-Data-Standard repository tag into a per-tier workspace.
  See [`02-api-seed-delivery.md`](../../epics/16-bootstrap/02-api-seed-delivery.md) for the
  Phase 1 acceptance criteria, built-in `Minimal`/`Populated` inventory, and per-tier staging shape
  actually shipped under DMS-1152.
- *Phase 2 — JSONL interchange (post-ODS-6738):* The long-term BulkLoadClient input format is JSONL.
  ODS-6738 tracks the cross-team extension that adds JSONL input support to BulkLoadClient. Once
  that surface is delivered and verified against DMS, the bootstrap-side seed phase migrates from
  XML staging to JSONL staging. The `-SeedTemplate`, `-SeedDataPath`, and
  `-AdditionalNamespacePrefix` bootstrap-developer-facing parameters are stable across both phases.
  [Section 14.3](#143-blocking-cross-team-dependencies) tracks the cross-team blocker; the sections
  below document the Phase 2 JSONL consumption contract as the design's long-term target.

**Pre-flight check:** When `load-dms-seed-data.ps1` runs, it resolves the pinned BulkLoadClient
package and fails immediately if the package cannot be downloaded/resolved or if the CLI surface
required by the active phase is not available — the XML short-flag surface in Phase 1, the JSONL
surface defined in [Section 6.1](#61-bulkloadclient-bootstrap-consumption-contract) in Phase 2.
See [Pinned BulkLoadClient Resolution](#633-pinned-bulkloadclient-resolution).

**Operational gate:** Phase 2 migration is gated on BulkLoadClient JSONL support being delivered
and verified against DMS. Phase 1 is the current shipped seed path under DMS-1152, not a temporary
bridge. The legacy direct-SQL seeding path (`setup-database-template.psm1`) retention during
implementation sequencing is a separate operational bridge governed by Section 6.4 and is not
part of the DMS-916 design contract.
...
> **Phase reminder:** The JSONL CLI surface in the table below is the Phase 2 (post-ODS-6738) target
> contract. DMS-1152 Phase 1 invokes the same tool's XML-mode short-flag surface
> (`-b`, `-d`, `-w`, `-k`, `-s`, `-o`, `-x`), validated by `Assert-BulkLoadClientXmlInterface` in
> `load-dms-seed-data.ps1`. The two surfaces are not active at once; the live surface is the one
> belonging to the phase the bootstrap implementation targets.
...
DMS-916 defines only the **developer bootstrap consumption contract** for seed data. The built-in
core developer templates selected by `-SeedTemplate` are bootstrap-owned developer assets — Phase 1
fetches XML interchange files from the pinned Ed-Fi-Data-Standard repository tag (see
[`02-api-seed-delivery.md`](../../epics/16-bootstrap/02-api-seed-delivery.md)); Phase 2 will source
JSONL assets via the same `-SeedTemplate` selectors. Neither form is a published deployment package. Broader artifact distribution concerns - package naming,
...
data store IDs, credentials, URLs, Docker or container state, environment settings, seed file paths, phase
...
4. **`Add-Application`** — creates an application bound to the vendor, a claim set (`EdFiSandbox`), a set of education organization IDs, and the DMS data store ID; returns `Key` and `Secret`.
...
runtime authorization strategy by inheriting from the claim hierarchy (`NamespaceBased`,
...
There is **no `-Extensions` parameter**. Standard mode (omit `-ApiSchemaPath`) is package-backed: with
`-EnvironmentFile`, it resolves and stages the effective `SCHEMA_PACKAGES` set; without an env file, direct
invocation falls back to the catalog-pinned core package. Unpublished/custom schema sets use the expert
`-ApiSchemaPath` filesystem path, which stages every `ApiSchema*.json` file present in the supplied directory.

```powershell
pwsh eng/docker-compose/prepare-dms-schema.ps1                                    # direct core-only fallback
pwsh eng/docker-compose/prepare-dms-schema.ps1 -EnvironmentFile eng/docker-compose/.env.bootstrap.ds52 # package-driven core + TPDM
pwsh eng/docker-compose/prepare-dms-schema.ps1 -ApiSchemaPath "C:\dev\my-schema"  # expert: core + any extensions in the directory
```

Wrapper standard mode intentionally follows its effective `SCHEMA_PACKAGES` baseline. Built-in seed package
availability is seed-catalog-driven (Story 02), and custom seed payloads flow through `-SeedDataPath`.
...
**Note - the two modes:** standard mode (omit `-ApiSchemaPath`) stages the effective package set, or the
catalog core-only fallback when invoked directly without an env file; expert `-ApiSchemaPath` supplies a
complete filesystem schema set. There is no `-Extensions` parameter. In expert
...
the default no-argument package-backed standard-mode flow and keycloak-backed runs. `-EnableConfig` therefore remains
...
| `-NoDataStore`, no `-SchoolYearRange`, multiple existing instances found in the current tenant scope | "Error: -NoDataStore requires exactly one existing DMS instance in the current tenant scope, but multiple instances were found. Tear down the extra instances or manually prepare the environment so one intended target remains before rerunning." |
| `-NoDataStore` with `-SchoolYearRange` | "Error: -NoDataStore with -SchoolYearRange is not supported in DMS-916. Tear down and recreate the environment for multi-year runs." |
...
that owns it. It does not expose `-DataStoreId`; explicit ID targeting is phase-command-only. `-EnvironmentFile`
...
- `eng/docker-compose/.bootstrap/seed/` - staged seed workspace for the current seed-loading run (XML interchange in Phase 1; JSONL in Phase 2)
...
`$StartYear` to `$EndYear`, calls `Add-DataStore` for each year, then calls `Add-DataStoreContext`
...
### 11.2 Historical State: Bundled DDL and Seed Data

Before the API-based seed cut-over, `start-local-dms.ps1 -LoadSeedData` called
`setup-database-template.psm1`, which:
...
That former single SQL file bundled two distinct concerns:
...
Bundling these two concerns created several problems:
...
**Current state**: A `-NoDataStore` flag exists on the script, but it does not suppress DMS container
startup. The current script still includes `local-dms.yml` in the compose file list and starts the normal
DMS service in non-teardown flows; `-NoDataStore` only changes whether bootstrap creates instance records
...
**Relationship with `-NoDataStore`**: `-NoDataStore` skips creating DMS instance records in the Config Service but still starts the DMS container in the normal flow. `-InfraOnly` skips the DMS container entirely. When `-InfraOnly` is used, `-NoDataStore` is typically omitted because the IDE-hosted DMS process still reads instance records from the Config Service just as the containerized DMS would. If `-NoDataStore` is used in this workflow, the same narrow reuse rule applies: exactly one existing instance must already be present, `-SchoolYearRange` is not supported, and later phases consume that selected target rather than rediscovering it.
...
`start-local-dms.ps1` does not call `Get-DataStores` again and does not own the post-health handoff
...
| Package-backed standard schema selection | [`../../epics/16-bootstrap/06-package-backed-standard-schema-selection.md`](../../epics/16-bootstrap/06-package-backed-standard-schema-selection.md) | Story 06 (DMS-1156) delivers package-backed standard mode (no `-Extensions` parameter): direct invocation without `-EnvironmentFile` resolves the catalog-pinned core package, while env-driven invocation resolves the effective `SCHEMA_PACKAGES` set into the same normalized ApiSchema workspace, ApiSchema asset manifest, and root bootstrap manifest schema section used by Story 00. Unpublished/custom schema sets use expert `-ApiSchemaPath`. |
...
- Story 06 (DMS-1156) owns the standard developer schema-selection path: package-backed standard mode driven
  by the effective `SCHEMA_PACKAGES` set, with a catalog core-only direct-invocation fallback. There is no
  `-Extensions` parameter; unpublished/custom schema sets use the expert `-ApiSchemaPath` path
...
|---|------|--------------|----------------------|------------------|
| 1 | Default standard-mode schema profile | Early DMS-916 design narrowed standard mode to the core package only | Wrapper standard mode stages the effective `SCHEMA_PACKAGES` set (default DS 5.2: core + TPDM; DS 6.1: core because TPDM is folded in); direct preparation without `-EnvironmentFile` retains the catalog-pinned core-only fallback | Select published package sets through the effective env file; use expert `-ApiSchemaPath` for unpublished/custom schemas |
