...
| `configure-local-data-store.ps1` | [`command-boundaries.md` Section 3.4](command-boundaries.md#34-configure-local-data-storeps1--instance-setup) | Story 03 |
...
| Credential bootstrapping | [`../../epics/16-bootstrap/02-api-seed-delivery.md`](../../epics/16-bootstrap/02-api-seed-delivery.md), [`../../epics/16-bootstrap/03-entry-point-and-ide-workflow.md`](../../epics/16-bootstrap/03-entry-point-and-ide-workflow.md) | `start-local-dms.ps1`, `configure-local-data-store.ps1`, `load-dms-seed-data.ps1` |
...
| Runtime ApiSchema content loading | [`../../epics/16-bootstrap/04-apischema-runtime-content-loading.md`](../../epics/16-bootstrap/04-apischema-runtime-content-loading.md) | **Delivered (DMS-1154).** `ContentProvider` reads discovery/specification JSON and XSD content from the normalized filesystem workspace via `bootstrap-api-schema-manifest.json`; Docker activation emits `USE_API_SCHEMA_PATH=true`, mounts `.bootstrap/ApiSchema`, and clears `SCHEMA_PACKAGES`; staged claims activate per manifest `claims.mode`; all as one startup boundary |
