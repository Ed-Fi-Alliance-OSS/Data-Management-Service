...
- Profile-constrained collection merges additionally require the request/body-shaping and visibility-projection contract described in section 5.2.4.
...
### 5.2.4 Profile write request and context assembly
...
1. Traverse the incoming JSON and build logical collection-item candidates:
   - each candidate records its `JsonScope`, `OrdinalPath`, requested sibling order, scalar values, resolved FK values,
     complete lineage-anchor values, and any extension subtrees
...
- complete-vector reference FKs and cascades target canonical public-identity storage columns, lineage anchors, and the
  terminal target `DocumentId` (binding columns are read-only when unified), and
...
/// - complete-vector lineage-anchor columns (stable target DocumentIds, dedicated unless exact safe reuse applies)
