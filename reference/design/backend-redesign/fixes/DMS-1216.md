...
The Coverlet and ReportGenerator tool-manifest change is part of DMS-1216 because this fix depends on repeatable CI validation of the backend unit tests and relational E2E scenarios listed above. The failing PR build was not exposing a runtime defect in the relational write executor; it was failing in the coverage/reporting command path used to validate the DMS-1216 changes.

Pinning `coverlet.console` and `dotnet-reportgenerator-globaltool` in `.config/dotnet-tools.json` keeps the coverage tools under repository control, alongside the source and test changes for this fix. The GitHub Actions jobs should restore those local tools with `dotnet tool restore`, and the build scripts should invoke them through `dotnet tool run`, so the DMS-1216 PR is not affected by unpinned global tool updates on the runner.
