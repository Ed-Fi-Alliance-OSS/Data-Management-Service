...
for the run. `provision-dms-schema.ps1` uses that staged schema context as the authoritative provisioning
handoff; the expected `EffectiveSchemaHash` from Story 00 is diagnostic metadata for logging or comparison,
not a required SchemaTools input or a second schema-readiness decision table.
...
- Before any DMS host starts, `provision-dms-schema.ps1` consumes the selected `ApiSchema*.json` files
  already staged by the schema-selection slice for the run.
...
- `provision-dms-schema.ps1` passes the staged schema paths and target connection details into that shared
  path. The precomputed `EffectiveSchemaHash` may be used for logging or comparison, but it is not a
  required provisioning input or bootstrap's final serviceability decision.
- `provision-dms-schema.ps1` accepts `-EnvironmentFile` and uses the shared local-settings helper to resolve
  CMS URL, auth, tenant scope, and database connection defaults. Direct invocation therefore targets the same
  local environment as the wrapper and does not depend on process environment variables left behind by
  `start-local-dms.ps1`.
...
- Bootstrap keeps `AppSettings__DeployDatabaseOnStartup=false` and disables or removes the legacy
  `NEED_DATABASE_SETUP` / `EdFi.DataManagementService.Backend.Installer.dll` startup provisioning path for
  the DMS-916 flow. DMS startup does not perform schema work through any application setting, container
  entrypoint, or pre-launch hook.
...
2. Inside `provision-dms-schema.ps1`, consume the staged schema paths produced by Story 00 and pass them,
   with target connection details, into the authoritative provisioning helper. Read the expected
   `EffectiveSchemaHash` only as diagnostic metadata for logging or comparison.
...
4. Keep the committed default `AppSettings__DeployDatabaseOnStartup=false` in place, and explicitly disable
   or remove the legacy Docker startup provisioning path that defaults `NEED_DATABASE_SETUP=true` and invokes
   `EdFi.DataManagementService.Backend.Installer.dll` before the app starts. Surface authoritative
   provisioning diagnostics directly rather than routing schema work through DMS startup side effects or
   inventing bootstrap-owned readiness rules. Treat SchemaTools as a black-box authority for success/failure:
   rely on invocation shape, exit code, and pass-through diagnostics rather than parsing undocumented
