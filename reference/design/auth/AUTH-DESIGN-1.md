# Authorization Design v1 (Post‑JSONB Subject/EdOrg Model)

## 1. Purpose and Goals

This document consolidates the initial authorization redesign for the DMS
(PostgreSQL backend) into a single coherent specification. It replaces the
current JSONB/trigger‑driven model with a relational, strategy‑friendly design
that:
...
- Integrates with the planned `dms.DocumentIndex` query/indexing design so that
  ...
  `dms.Document`.
- Keeps all authorization enforcement in application code (no PostgreSQL RLS in
  this phase) while using narrow relational tables for performance.

Non‑goals:

- Changing claimset or strategy metadata semantics from ODS.
- Introducing ODS‑style `auth.*` views or O/RM‑specific implementations.
- Implementing per‑tenant or per‑client physical isolation (handled at other
  layers).
...
1. **`dms.DocumentIndex`**  
   Narrow, hash‑partitioned index table for efficient filtering and paging:
...
4. **EducationOrganization hierarchy**  
   ...
   recursive function to compute ancestor EdOrgs. The old
   `EducationOrganizationHierarchyTermsLookup` table is retired.
...
The `dms.Document` table remains the canonical store for resource payloads
...
- The `SecurityElements` JSONB column is removed from `dms.Document`. The
  ...
  from `EdfiDoc` or the request body when needed (see §9).
- Any GIN or expression indexes built solely to support those columns are also
  removed.
...
- 32 hash partitions by `(ProjectName, ResourceName)` so each `/data` query
...
- Per‑partition GIN on `QueryFields` (using `jsonb_path_ops`) for high‑selectivity
  query filtering.
...
- `SubjectKey` is the canonical identifier for the subject in that type:
  ...
  - Staff → `StaffUniqueId`.
  - EdOrg → `EducationOrganizationId::text`.
  - Ownership or other subject dimensions can be added later.
...
- `EducationOrganization` (existing Ed‑Fi table).
- `EducationOrganizationRelationship` (or equivalent) representing parent‑child
  relationships between EdOrgs.
...
BEGIN
    ...
    WITH RECURSIVE OrganizationHierarchy AS (
        SELECT eoh.Id, eoh.EducationOrganizationId, eoh.ParentId
        FROM dms.EducationOrganizationHierarchy eoh
        WHERE eoh.EducationOrganizationId = p_educationOrganizationId

        UNION ALL

        SELECT parent.Id, parent.EducationOrganizationId, parent.ParentId
        FROM dms.EducationOrganizationHierarchy parent
        JOIN OrganizationHierarchy child ON child.ParentId = parent.Id
    )
...
- `EducationOrganizationHierarchyTermsLookup` and its triggers are removed.
...
### 4.3 Mapping Ed‑Fi Resources to Subjects and Pathways
...
- Specialized tables and triggers:
  ...
  - Triggers on `dms.Document` and `EducationOrganizationHierarchy` that
    maintain those tables and JSONB arrays.
...
The existing `AddAuthorizationFilters` logic is currently tied to old JSONB
arrays and `dms.Document`. It is reworked to target `DocumentIndex` plus
...
- **Recomputation over incremental updates**  
  ...
  documents for that subject. This is simple and robust:
...
on those EdOrgs. Two approaches are possible:
...
- **Deferred reconciliation (initial implementation)**  
  Since hierarchy changes are expected to be rare after initial load, the
  initial implementation may:
  - Recompute memberships for a broader set of subjects or
  - Defer to a background job that periodically rederives `SubjectEdOrg` from
    the current snapshot of relationship documents and hierarchy.

The initial design leaves this choice open, preferring correctness and
simplicity over eager micro‑optimization.
...
- `totalCount` (if requested) uses the same `WHERE` (including the `EXISTS`)
  against `DocumentIndex`, without the `OFFSET/LIMIT`.
...
There is no need for namespace‑specific rows in `DocumentSubject` or
`SubjectEdOrg`.

Other non‑relationship strategies (e.g., ownership tokens) can be represented
either as:
...
`QueryFields` on `DocumentIndex` is the **sole** basis for:
...
- Based on `ResourceSchema.QueryFields`.
- For each query field:
  - Evaluate supplied JSONPaths against `EdfiDoc`.
  - Canonicalize values (boolean, number, date, date‑time, time, string).
  - Deduplicate and sort where multiple values are possible.
  - Encode as scalar or array according to configured policy.

This ensures that query‑time JSON `@>` predicates match index contents
...
1. **Introduce new tables and functions**
   - Create `dms.DocumentSubject`.
   - Create `dms.SubjectEdOrg`.
   - Ensure `DocumentIndex` is in place and populated.
   - Ensure `GetEducationOrganizationAncestors` supports the desired hierarchy.

2. **Implement C# helpers**
   - `SubjectMembershipWriter` for `DocumentSubject` and `SubjectEdOrg`.
   - New `PostgresqlAuthorizationRepository` implementation using `SubjectEdOrg`.
   - Redesigned `AddAuthorizationFilters` targeting `DocumentIndex` +
     `DocumentSubject` + `SubjectEdOrg`.

3. **Backfill**
   - From existing JSONB auth arrays and specialized tables:
     - Derive `DocumentSubject` rows for all documents.
     - Derive `SubjectEdOrg` rows for all subjects and pathways.
   - Validate that results match existing authorization behavior via targeted
     scenarios and integration tests.

4. **Switch read and write paths**
   - Enable new `SubjectMembershipWriter` in Upsert/Update/Delete code paths.
   - Switch `/data` queries to use `DocumentIndex` plus the new
     authorization predicates.
   - Switch `IAuthorizationRepository` to the new implementation.

5. **Remove old structures**
   - Drop JSONB auth arrays from `dms.Document` and associated GIN indexes.
   - Drop pathway‑specific authorization tables and triggers.
   - Drop `EducationOrganizationHierarchyTermsLookup` and its trigger.
   - Remove legacy `AddAuthorizationFilters` code that targets `dms.Document`.

6. **Refine indexing**
   - Monitor performance and adjust B‑tree indexes on `DocumentSubject` and
     `SubjectEdOrg` as needed.
   - Consider partial indexes if specific SubjectType/Pathway combinations are
     extremely hot.
...
For non‑implementation stakeholders, the benefits of this redesign are:

- **Performance**
  - Query authorization is performed over narrow, indexed tables using exist
    checks, not large JSONB arrays or multi‑index bitmap scans.
  - Paging is handled by `DocumentIndex`, allowing the database to fetch only
    the requested page of JSONB documents.

- **Simplicity**
  - A single generic subject model (`DocumentSubject` + `SubjectEdOrg`)
    replaces multiple pathway‑specific tables and complex triggers.
  - Authorization behavior aligns closely with the proven ODS authorization
    model.

- **Maintainability**
  - All authorization logic is in application code (C#) and shared repositories,
    not in triggers.
  - New authorization pathways can be added by configuring subject types and
    pathways rather than by creating new tables and triggers.

- **Correctness**
  - Strategy semantics remain compatible with ODS and claimset metadata.
  - Relationship logic is recomputed from authoritative relationship documents,
    reducing risk of drift.

---

## 12. Open Questions and Future Enhancements

The initial design intentionally keeps the model simple and generic. Areas for
future refinement include:

- **Enum governance**
  - How `SubjectType` and `Pathway` identifiers are managed and documented
    (central enum vs descriptor tables).

- **Hierarchy change handling**
  - Exact strategy for reconciling `SubjectEdOrg` after EdOrg hierarchy changes
    (on‑change vs periodic job vs hybrid).

- **Incremental membership updates**
  - If recomputation proves too expensive in high‑churn scenarios, introduce
    incremental update logic for simple cases.
...
- **RLS**
  - Once the relational model is stable and well‑understood, evaluate whether
    parts of the authorization logic could safely move into PostgreSQL Row‑Level
    Security (e.g., for analytics workloads) without sacrificing performance or
    flexibility.

This document serves as the authoritative baseline for implementing and
iterating on the new authorization system.
