...
- For claim-set content changes, when `AppSettings:EnableClaimsetReload` is enabled, `POST /management/{tenant}/reload-claimsets` (multitenant) or `POST /management/reload-claimsets` (single-tenant) refreshes the addressed entry on that replica. `CachedClaimSetProvider.GetCacheKey` preserves tenant spelling for lookup, locking, and invalidation: `/North/...` and `/north/...` can warm separate entries, and reloading `North` does not invalidate `north`. A successful reload on every replica therefore does not establish revocation for every casing. The single-tenant route has one default key. Reload does not clear application contexts or change token claims.
- To clear every cached tenant-spelling variant, drain requests/fills and restart every serving replica, clearing the process-local claim cache before traffic resumes, or wait for all previously cached entries to expire. Reloading known spellings can refresh those entries but is not a complete purge when other variants may have been used. Verifying only the canonical spelling is insufficient. Cache-key normalization is a possible shared-provider follow-up; this epic preserves existing claim-cache keys and documents their operational limitation.
...
- If access must stop before those windows elapse, the operator must stop the affected traffic at its deployment access boundary while changing CMS/provider policy and refreshing or restarting affected replicas. For claim changes requiring revocation across all tenant spellings, use the full drain/restart procedure above; successful exact-spelling reloads are not a substitute. Verify denial using the original token and case-variant tenant routes on each serving replica before resuming traffic. That verification supplements the full cache reset rather than proving untested entries are gone. A restart still requires normal host startup prerequisites to succeed.
...
The claim-revocation test warms both `North` and `north`, removes identity permission, and reloads only `North`: that spelling is denied while the previously cached `north` entry may still authorize. After expiry of the remaining entry, or a drained process restart clearing all claim entries, both spellings are denied. This demonstrates the documented limitation and recovery procedure; it does not normalize keys or weaken the provider context-equality contract.
...
- warmed application/claim caches expose client deletion and claim-action removal only after the applicable expiry/invalidation, tested with the original still-valid token; verify replica-local, exact-spelling claim reload with `North`/`north`, denial for both after remaining-entry expiry or full process restart, and distinguish changes to token-carried claim-set assignment;
