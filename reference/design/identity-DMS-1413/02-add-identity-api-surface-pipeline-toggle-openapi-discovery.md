...
- Every host-generated response for an enabled, mapped identity operation carries `Cache-Control: no-store`, including pending/complete polls, `202`, no-match, early authorization/validation/capability rejection, and all provider failure types. Install the frontend policy before body extraction/Core execution and preserve it through error-response replacement. HTTP tests cover early failures and successful/error polls. Identity metadata, unrelated routes, feature-off fallback, and server-level pre-application rejection are outside this policy.
