...
- A unit test proves `OnPath` throws `ArgumentException` for a null path, `""`, `"$"`, `"$."`, and any path not `"$."`-prefixed, and for a null or empty message, and that `OnResource` throws for a null or empty message. `"$"` and `"$."` are asserted explicitly because each is `$`-rooted yet names no member, which is `OnResource`'s case; a prefix check alone would admit `"$."`.
