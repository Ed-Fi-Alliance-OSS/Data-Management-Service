...
5. Remove the unused `Microsoft.CodeAnalysis` `PackageReference` from `EdFi.DataManagementService.Core.External.csproj` and regenerate the lock file, confirming the build still succeeds. Do this before adding the package target, so the first packed output already carries the intended dependency set.
...
9. Build the scratch consumer library against the produced package to prove self-sufficiency, and record the result in the pull request.
