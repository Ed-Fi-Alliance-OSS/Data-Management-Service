...
Everything `ValidateAsync` receives is either a BCL type or a type from that assembly, so nothing in the contract obliges a validator to compile against `Backend.External` or any other host assembly.
...
`Core.External.csproj` declares `IsPackable=true`, but nothing packs or publishes it today (`build-dms.ps1:1801-1867` packs only the API package and SchemaTools, and no `Core.External` nuspec exists), so adding the pack-and-publish step would be prerequisite work for publishing. Deferred; see **Packaging deferral**.
...
Removing the Roslyn reference is therefore prerequisite work for publishing rather than for the contract itself, and whichever story publishes asserts the resulting dependency list rather than trusting it. With nothing published it is ordinary cleanup; see **Packaging deferral**.
...
- Packaging and publishing the contract. See **Packaging deferral** immediately below.

### Packaging deferral

The abstractions-contract story ships the contract as public types in `Core.External` and packs nothing.
This design originally had that story add the pack-and-publish path in the same change, and every passage below that says so is superseded by this one.

Two reasons, neither of them a change of mind about publishing eventually.
Nothing can consume the contract until the fan-in step lands, so a package published now is one no host can run.
And a package id cannot be changed after first publication without breaking every consumer that compiled against it, so publishing is the one step here that cannot be walked back.

Publishing also has an unresolved fork inside it, which deferring keeps open.
Publishing `Core.External` whole - the `EdFi.Api.Core.External` id this design first specified - relocates nothing, because the contract and every model type its signatures name already live in that assembly and it declares `IsPackable=true` with no `ProjectReference`.
What it does do is put that assembly's entire public surface under semver, roughly 174 public types, so that an implementer can name five.
Publishing a contract-only package instead keeps the published surface to the contract, but then the model types the signatures name have to move into the published assembly, because a published package cannot depend on an unpublished one.
Neither option is obviously right, and the choice does not have to be made to ship the contract.

Deferring costs little. The namespaces are `EdFi.DataManagementService.Core.External.*` either way, so whichever option is taken, the consuming code in this repository is unaffected and the relocation is no more expensive later than now.
...
1. **Abstractions contract** - five public top-level types in `Core.External`. The pack-and-publish path that does not exist today (a `build-dms.ps1` package target, prerelease pack/push jobs, and a release promote step) is deferred out of this surface; see **Packaging deferral**.
