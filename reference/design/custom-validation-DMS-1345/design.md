...
`FrontendRequest` carries no token today (`Core.External/Frontend/FrontendRequest.cs:14`), so one is added as a non-positional `init`-only property defaulting to `CancellationToken.None`; a positional addition would regenerate `Deconstruct` at a new arity and regenerate the primary constructor, breaking external consumers in source and binary respectively.
The one carve-out is that a new backing field enters the record's synthesized `Equals`, `GetHashCode`, and `PrintMembers`, which no production code depends on.
The catch chain has no cancellation-specific arm, so an `OperationCanceledException` becomes the same logged 500 as any other exception, built for a client that has already disconnected; whether cancellation deserves distinct handling is an open decision assigned to the fan-in ticket.
