...
- The guard is an `IDmsStartupTask` whose `Order` falls in 200-299, so it runs inside an executed `RunByOrderRangeAsync` window and after `LoadAndBuildEffectiveSchemaTask`, whose effective ApiSchema the `AppliesTo` warning reads.
