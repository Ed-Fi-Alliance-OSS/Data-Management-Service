...
- **Note, not a required change: consider capturing the collection in a closure rather than resolving it from DI.** design.md's mechanism is `services.AddSingleton<IServiceCollection>(services)`, and the guard then resolves `IServiceCollection` to read the final descriptor set. `IServiceCollection` is declared in `Microsoft.Extensions.DependencyInjection.Abstractions`, so under the plugin spine it is a `Microsoft.Extensions.*` service type that a plugin is explicitly permitted to register, and a plugin registering its own `IServiceCollection` displaces the collection this guard reads. Nothing about that is an isolation hole - a loaded plugin has full process trust either way - but the alternative costs nothing: a guard that closes over the collection it was handed cannot be displaced at all, and it removes a public DI registration that exists only for the guard's own benefit. The decision belongs to this story's owners; the plugin spine records the exposure and does not change this mechanism on its own.
