...
- The guard is an `IDmsStartupTask` whose `Order` falls in 101-299, so it runs inside an executed `RunByOrderRangeAsync` window and after `LoadAndBuildEffectiveSchemaTask`, whose effective ApiSchema the `AppliesTo` warning reads.
...
- A registration that is not transient aborts startup, proven by a test registering a singleton `ICustomResourceValidator` and asserting the host build throws rather than serving any request.
- A descriptor carrying an `ImplementationInstance` aborts startup.
...
- A validator with an unsatisfiable constructor dependency aborts startup rather than failing the first matching write, proven by asserting the host build throws before any request is served.
...
5. Write the startup-abort tests in `src/dms/frontend/EdFi.DataManagementService.Frontend.AspNetCore.Tests.Unit`, registering fixture validators through the test host's `ConfigureServices` seam. That seam reaches the guard because the guard is a post-container startup task.
