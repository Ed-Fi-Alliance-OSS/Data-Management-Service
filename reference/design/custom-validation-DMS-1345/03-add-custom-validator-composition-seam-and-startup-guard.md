...
Core's contribution is the guard, not the registration. Core ships an extension the frontend calls once, unconditionally, which registers a startup guard and closes over the live `IServiceCollection` so the guard can read the final descriptor set after the container is built.
...
- A Core-owned `IServiceCollection` extension registers the startup guard and captures the live service collection in a closure, and the frontend calls it exactly once, unconditionally, from `WebApplicationBuilderExtensions.AddServices`, following the call-site shape `AddJwtAuthentication` already uses.
- **The capture is a closure the guard holds, and no `IServiceCollection` is registered as a service.** The extension hands the live collection to the guard by closing over it; `services.AddSingleton<IServiceCollection>(services)` is specifically not the mechanism, and a test asserts the built container carries no `IServiceCollection` descriptor. `IServiceCollection` is declared in `Microsoft.Extensions.DependencyInjection.Abstractions`, so under the plugin spine it is a `Microsoft.Extensions.*` service type that a plugin is explicitly permitted to register, and a plugin registering its own `IServiceCollection` would displace the collection a DI-resolving guard reads. Nothing about that is an isolation hole - a loaded plugin has full process trust either way - but closure capture cannot be displaced at all, and it removes a public DI registration that existed only for the guard's own benefit.
...
1. Add the Core-owned composition extension beside `DmsCoreServiceExtensions`, modeled on `AddJwtAuthentication`'s shape and call-site pattern. It registers the startup guard and captures the live `IServiceCollection` in a closure so the guard can read final descriptors, registering no `IServiceCollection` service.
