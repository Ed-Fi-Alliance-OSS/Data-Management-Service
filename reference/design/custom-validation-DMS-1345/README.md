...
The extension point is a public abstractions contract that a custom validator implements, delivered by referencing the contract's assembly and registering the validator into DMS's composition at build time, guarded by an unconditional fail-loud startup check. Custom validators are document-oriented and async, and they run only on the write path, on POST and PUT requests. They receive the request document, its resource identity, the write verb, the scope the write belongs to (tenant and route qualifiers), a trace id, and the request's cancellation token; this version's contract gives them no access to stored data.
...
| Packaging and publishing the contract | "### Packaging deferral" | The contract ships as public types in `Core.External`. Publishing is the one step that cannot be walked back, nothing can consume the contract until the fan-in step lands, and the choice between publishing `Core.External` whole and publishing a contract-only package is left open |
