...
The step is an internal `IPipelineStep` named `CustomResourceValidationMiddleware`. It resolves `IEnumerable<ICustomResourceValidator>` from the current request's own DI scope, filters by `AppliesTo`, invokes `Validate` sequentially against a deep-cloned copy of the profile-effective document, and merges every returned `ValidationFailure` into the same `validationErrors`/`errors` buckets and the same factory-selection rule `ValidateDocumentMiddleware` already uses. It is added to both write pipelines immediately after `ProvideAuthorizationFiltersMiddleware` and immediately before the terminal handler, so authorization is enforced and the request's authorization context is populated before any validator runs. GET and DELETE are untouched.
...
- A unit test proves `CreateGetByIdPipeline` and `CreateQueryPipeline` contain no `CustomResourceValidationMiddleware` step.
...
- A unit test proves the fan-in step throws on a `ValidationFailure` subtype other than `OnPath`/`OnResource`, constructed via a test-only derived record chaining the copy constructor, surfacing as a 500 through the catch chain.
- A unit test proves a custom-validator 400 is byte-identical in its `detail` member to the corresponding core schema-validation 400, asserted for both the `errors` arm and the `validationErrors` arm, so an implementation that substitutes its own `detail` string fails.
...
7. Implement the exhaustive switch over `ValidationFailure` that throws on any subtype other than the two constructible cases.
