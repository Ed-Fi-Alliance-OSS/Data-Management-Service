...
Write-Host "Starting performance test for simple original function..."
Write-Host "Running sp_STUDENT_SIMPLE_Postgres() 10 times on northridge-original"
...
for ($i = 1; $i -le 10; $i++) {
    ...
    $output = docker exec postgres-northridge-original psql -U postgres -d "northridge-original" -c "SELECT COUNT(*) FROM sp_STUDENT_SIMPLE_Postgres();" 2>&1
...
$results | ConvertTo-Json -Depth 3 | Out-File -FilePath "performance-simple-original-results.json"
Write-Host "`nResults saved to performance-simple-original-results.json"