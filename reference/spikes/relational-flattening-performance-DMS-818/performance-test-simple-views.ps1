...
Write-Host "Starting performance test for simple views function..."
Write-Host "Running sp_STUDENT_SIMPLE_Postgres_Views() 10 times on northridge-flattened"
...
for ($i = 1; $i -le 10; $i++) {
    ...
    $output = docker exec postgres-northridge-flattened psql -U postgres -d "northridge-flattened" -c "SELECT COUNT(*) FROM sp_STUDENT_SIMPLE_Postgres_Views();" 2>&1
...
$results | ConvertTo-Json -Depth 3 | Out-File -FilePath "performance-simple-views-results.json"
Write-Host "`nResults saved to performance-simple-views-results.json"