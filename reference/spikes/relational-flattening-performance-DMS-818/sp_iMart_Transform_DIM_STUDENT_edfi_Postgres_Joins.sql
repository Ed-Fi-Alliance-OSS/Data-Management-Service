-- PostgreSQL version of sp_iMart_Transform_DIM_STUDENT_edfi (Joins Version)
CREATE OR REPLACE FUNCTION sp_iMart_Transform_DIM_STUDENT_edfi_Postgres_Joins_Fixed(
...
BEGIN
    ...
    -- JOINS VERSION: studentusi and schoolid no longer exist on association table, get from base tables
    ...
    WITH enrollment_priority_base AS (
        SELECT
            ...
            CASE WHEN EXISTS(SELECT 1 FROM edfi.school sc2 WHERE sc2.schoolid = sc.schoolid
                            AND sc2.schoolid::text LIKE '%Service%' OR sc2.schoolid < 100)
        ...
        -- JOINS VERSION: Get studentusi from student table since it's no longer on association
        JOIN edfi.student e ON a.student_surrogateid = e.surrogateid
        -- JOINS VERSION: Get schoolid from school table since it's no longer on association
        JOIN edfi.school sc ON a.school_surrogateid = sc.surrogateid
    ...
    -- JOINS VERSION: studentusi and educationorganizationid no longer exist on association, get from base tables
    DROP TABLE IF EXISTS temp_race_complex;
    CREATE TEMP TABLE temp_race_complex AS
    SELECT st.studentusi, eo.schoolid as educationorganizationid,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           JOIN edfi.descriptor d ON r.racedescriptor_surrogateid = d.surrogateid
                           JOIN edfi.studenteducationorganizationassociation seoa2 ON r.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           JOIN edfi.student st2 ON seoa2.student_surrogateid = st2.surrogateid
                           WHERE st2.studentusi = st.studentusi AND d.codevalue = 'White') THEN 'White'
                WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           JOIN edfi.descriptor d ON r.racedescriptor_surrogateid = d.surrogateid
                           JOIN edfi.studenteducationorganizationassociation seoa2 ON r.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           JOIN edfi.student st2 ON seoa2.student_surrogateid = st2.surrogateid
                           WHERE st2.studentusi = st.studentusi AND d.codevalue = 'Black - African American') THEN 'Black'
                WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           JOIN edfi.descriptor d ON r.racedescriptor_surrogateid = d.surrogateid
                           JOIN edfi.studenteducationorganizationassociation seoa2 ON r.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           JOIN edfi.student st2 ON seoa2.student_surrogateid = st2.surrogateid
                           WHERE st2.studentusi = st.studentusi AND d.codevalue = 'Asian') THEN 'Asian'
                ELSE 'Other' END as race_code
    FROM edfi.studenteducationorganizationassociation seoa
    -- JOINS VERSION: Get studentusi and educationorganizationid from base tables
    JOIN edfi.student st ON seoa.student_surrogateid = st.surrogateid
    JOIN edfi.school eo ON seoa.educationorganization_surrogateid = eo.surrogateid;
    temp_count := temp_count + 1;

    -- TEMP TABLE 5: Gender Complex
    -- JOINS VERSION: studentusi no longer exists on association, get from student table
    ...
    SELECT st.studentusi, COUNT(*) as char_count,
    ...
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON seoa.student_surrogateid = st.surrogateid
    LEFT JOIN edfi.studenteducationorganizationassociationstudentcharacteristic sc ON sc.studenteducationorganizationassociation_surrogateid = seoa.surrogateid
    ...
    SELECT st.studentusi, spa.begindate,
           ROW_NUMBER() OVER (PARTITION BY st.studentusi ORDER BY spa.begindate DESC) as program_sequence
    FROM edfi.studentprogramassociation spa
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON spa.student_surrogateid = st.surrogateid
    WHERE EXISTS(SELECT 1 FROM temp_calendar_complex tcc
                WHERE tcc.studentusi = st.studentusi AND tcc.enrollment_priority = 1);
    temp_count := temp_count + 1;
    ...
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON spa.student_surrogateid = st.surrogateid
    JOIN edfi.generalstudentprogramassociation gspa ON gspa.student_surrogateid = spa.student_surrogateid
    ...
    SELECT st.studentusi,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.studentspecialeducationprogramassociation ssepa
                           JOIN edfi.student st2 ON ssepa.student_surrogateid = st2.surrogateid
                           WHERE st2.studentusi = st.studentusi) THEN 'Y' ELSE 'N' END as disability_ind
    FROM edfi.studenteducationorganizationassociation seoa
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON seoa.student_surrogateid = st.surrogateid;
    temp_count := temp_count + 1;
    ...
    SELECT st.studentusi, d.codevalue as grade_code,
           CASE d.codevalue
    ...
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON ssa.student_surrogateid = st.surrogateid
    ...
    SELECT st.studentusi, COUNT(*) as indicator_count,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationstudentindicator si2
                           JOIN edfi.studenteducationorganizationassociation seoa2 ON si2.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           JOIN edfi.student st2 ON seoa2.student_surrogateid = st2.surrogateid
                           WHERE st2.studentusi = st.studentusi) THEN 'Y' ELSE 'N' END as has_indicators
    FROM edfi.studenteducationorganizationassociation seoa
    -- JOINS VERSION: Get studentusi from student table
    JOIN edfi.student st ON seoa.student_surrogateid = st.surrogateid
    LEFT JOIN edfi.studenteducationorganizationassociationstudentindicator si ON si.studenteducationorganizationassociation_surrogateid = seoa.surrogateid
    GROUP BY st.studentusi;
    temp_count := temp_count + 1;
    ...
    SELECT tsb.studentusi,
           ...
           COALESCE(
               (SELECT ssa.schoolyear FROM edfi.studentschoolassociation ssa
                JOIN edfi.student st ON ssa.student_surrogateid = st.surrogateid
                WHERE st.studentusi = tsb.studentusi ORDER BY ssa.entrydate DESC LIMIT 1), 2018
           ) as school_year,
