-- PostgreSQL version of sp_iMart_Transform_DIM_STUDENT_edfi using Views
CREATE OR REPLACE FUNCTION sp_iMart_Transform_DIM_STUDENT_edfi_Postgres_Views(
...
BEGIN
    ...
    WITH enrollment_priority_base AS (
        SELECT
            ...
            a.lastmodifieddate
        FROM edfi.vw_studentschoolassociation a
    ...
    SELECT seoa.studentusi, seoa.educationorganizationid,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           ...
                           WHERE r.StudentEducationOrganizationAssociation_SurrogateId = seoa.surrogateid AND d.codevalue = 'White') THEN 'White'
                WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           JOIN edfi.descriptor d ON r.RaceDescriptor_SurrogateId = d.surrogateid
                           WHERE r.StudentEducationOrganizationAssociation_SurrogateId = seoa.surrogateid AND d.codevalue = 'Black - African American') THEN 'Black'
                WHEN EXISTS(SELECT 1 FROM edfi.studenteducationorganizationassociationrace r
                           JOIN edfi.descriptor d ON r.RaceDescriptor_SurrogateId = d.surrogateid
                           WHERE r.StudentEducationOrganizationAssociation_SurrogateId = seoa.surrogateid AND d.codevalue = 'Asian') THEN 'Asian'
    ...
    SELECT seoa.studentusi, COUNT(DISTINCT sc.studentcharacteristicdescriptor_surrogateid) as char_count,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.vw_studenteducationorganizationassociationstudentcharacteristic sc2
                           JOIN edfi.vw_studenteducationorganizationassociation seoa2 ON sc2.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           WHERE seoa2.studentusi = seoa.studentusi
                           AND EXISTS(SELECT 1 FROM edfi.descriptor d2 WHERE d2.surrogateid = sc2.studentcharacteristicdescriptor_surrogateid AND d2.codevalue LIKE '%Homeless%'))
                THEN 'Y' ELSE 'N' END as homeless_ind
    FROM edfi.vw_studenteducationorganizationassociation seoa
    LEFT JOIN edfi.vw_studenteducationorganizationassociationstudentcharacteristic sc ON sc.studenteducationorganizationassociation_surrogateid = seoa.surrogateid
    ...
    SELECT seoa.studentusi,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.vw_studentspecialeducationprogramassociation ssepa
                           WHERE ssepa.student_surrogateid = seoa.surrogateid) THEN 'Y' ELSE 'N' END as disability_ind
    FROM edfi.vw_studenteducationorganizationassociation seoa;
    temp_count := temp_count + 1;
    ...
    SELECT seoa.studentusi, COUNT(DISTINCT si.surrogateid) as indicator_count,
           CASE WHEN EXISTS(SELECT 1 FROM edfi.vw_studenteducationorganizationassociationstudentindicator si2
                           JOIN edfi.vw_studenteducationorganizationassociation seoa2 ON si2.studenteducationorganizationassociation_surrogateid = seoa2.surrogateid
                           WHERE seoa2.studentusi = seoa.studentusi) THEN 'Y' ELSE 'N' END as has_indicators
    FROM edfi.vw_studenteducationorganizationassociation seoa
    LEFT JOIN edfi.vw_studenteducationorganizationassociationstudentindicator si ON si.studenteducationorganizationassociation_surrogateid = seoa.surrogateid
    GROUP BY seoa.studentusi;
    temp_count := temp_count + 1;
    ...
    SELECT tsb.studentusi,
           ...
           COALESCE(
               (SELECT ssa.schoolyear FROM edfi.vw_studentschoolassociation ssa
