...
public class ClaimsFragmentComposer(ILogger<ClaimsFragmentComposer> logger) : IClaimsFragmentComposer
{
    ...
    public ClaimsLoadResult ComposeClaimsFromFragments(ClaimsDocument baseClaimsNodes, string fragmentsPath)
    {
        try
        {
            ...
            var fragmentFiles = DiscoverFragmentFiles(fragmentsPath);
            if (fragmentFiles.Count == 0)
            {
                _logger.LogInformation("No fragment files found - returning base claims");
            ...
            var transformedHierarchy = ConvertFromTransformationModel(baseClaims);
        ...
        catch (Exception ex)
        {
            var failure = new ClaimsFailure(
                "FragmentComposition",
                "Failed to compose claims from fragments",
                fragmentsPath,
                ex
            );
    ...
    public List<string> DiscoverFragmentFiles(string fragmentsPath)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Fragment path does not exist: {FragmentsPath}", fragmentsPath);
                return [];
            }
        ...
        {
            _logger.LogError(ex, "Failed to discover fragment files in path: {FragmentsPath}", fragmentsPath);
            return [];
        }
    }
    ...
    )
    {
        ...
        {
            _logger.LogWarning("Fragment file {FilePath} has no resource claims", fragmentFilePath);
            return existingClaims;
        }

        foreach (var resourceClaim in claimSetData.ResourceClaims)
        {
    ...
    )
    {
        ...
        var existingClaim = existingClaims.SelectMany(x => SearchRecursive(x, singularName)).FirstOrDefault();

        if (existingClaim != null)
        {
            var actions = new List<TransformationClaimSetAction>();
            var actionsAndAuthStrategies = resourceClaim.AuthorizationStrategyOverridesForCRUD;

            if (actionsAndAuthStrategies != null)
            {
                foreach (var actionAuthStrategy in actionsAndAuthStrategies.Where(x => x != null))
                {
