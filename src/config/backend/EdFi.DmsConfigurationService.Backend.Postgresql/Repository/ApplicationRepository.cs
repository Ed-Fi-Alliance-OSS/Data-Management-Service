...
public class ApplicationRepository(IOptions<DatabaseOptions> databaseOptions) : IApplicationRepository
{
    ...
    public async Task<ApplicationsByVendorResult> GetApplicationsByVendorId(long vendorId)
    {
        await using var connection = new NpgsqlConnection(databaseOptions.Value.DatabaseConnection);
        try
        {
            ...
            if (!vendorExists)
            {
                return new ApplicationsByVendorResult.FailureVendorNotFound();
            }

            return new ApplicationsByVendorResult.Success(response.Values.OrderBy(a => a.ApplicationName));
        }
        catch (Exception ex)
        {
            return new ApplicationsByVendorResult.FailureUnknown(ex.Message);
        }
    }
}
