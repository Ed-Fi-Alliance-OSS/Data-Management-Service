...
) : IOwnershipTokenRepository
{
    ...
    private static string BuildOrderByClause(OwnershipTokenQuery query)
    {
        ...
        {
            return PostgresqlIdentifier.OrderBy(column, query.IsDescending, "ot");
        }

        return PostgresqlIdentifier.OrderBy("Id", isDescending: false, "ot");
    }
    ...
    public async Task<OwnershipTokenUpdateResult> UpdateOwnershipToken(OwnershipTokenUpdateCommand command)
    {
        ...
        {
            string sql = $"""
                UPDATE "dmscs"."OwnershipToken" ot
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                ...
                && !await AllOwnershipTokensInTenant(connection, transaction, tokenIdsToValidate)
            )
            {
                await transaction.RollbackAsync();
            ...
            if (command.OwnershipTokenIds.Length > 0)
            {
                await connection.ExecuteAsync(
                    ...
                    SELECT @ApiClientId, token_id::smallint, @CreatedBy
                    FROM unnest(@OwnershipTokenIds) AS token_id;
                    """,
                    new
                    {
                        command.ApiClientId,
                        command.OwnershipTokenIds,
                        CreatedBy = auditContext.GetCurrentUser(),
                    },
                    transaction
                );
    ...
    private async Task<bool> AllOwnershipTokensInTenant(
        ...
        int[] ownershipTokenIds
    )
    {
        string sql = $"""
            SELECT COUNT(1)
            FROM "dmscs"."OwnershipToken" ot
            WHERE ot."Id" = ANY(@OwnershipTokenIds) AND {TenantContext.TenantWhereClause("ot")};
            """;
        int count = await connection.ExecuteScalarAsync<int>(
            sql,
            new { OwnershipTokenIds = ownershipTokenIds, TenantId },
            transaction
        );
        return count == ownershipTokenIds.Length;
    }
