...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'PK_Vendor'
          AND conrelid = '"dmscs"."Vendor"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."Vendor" ADD CONSTRAINT "PK_Vendor" PRIMARY KEY ("Id");
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'PK_VendorNamespacePrefix'
          AND conrelid = '"dmscs"."VendorNamespacePrefix"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."VendorNamespacePrefix" ADD CONSTRAINT "PK_VendorNamespacePrefix" PRIMARY KEY ("VendorId", "NamespacePrefix");
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'FK_VendorNamespacePrefix_Vendor'
          AND conrelid = '"dmscs"."VendorNamespacePrefix"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."VendorNamespacePrefix" ADD CONSTRAINT "FK_VendorNamespacePrefix_Vendor" FOREIGN KEY ("VendorId") REFERENCES "dmscs"."Vendor"("Id") ON DELETE CASCADE;
    END IF;
END$$;
