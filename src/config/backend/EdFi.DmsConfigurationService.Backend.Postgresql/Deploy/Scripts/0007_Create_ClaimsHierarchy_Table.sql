...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'PK_ClaimsHierarchy'
          AND conrelid = '"dmscs"."ClaimsHierarchy"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."ClaimsHierarchy" ADD CONSTRAINT "PK_ClaimsHierarchy" PRIMARY KEY ("Id");
    END IF;
END$$;
