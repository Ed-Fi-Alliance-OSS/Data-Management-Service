...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'PK_OpenIddictToken'
          AND conrelid = '"dmscs"."OpenIddictToken"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."OpenIddictToken" ADD CONSTRAINT "PK_OpenIddictToken" PRIMARY KEY ("Id");
    END IF;
END$$;
...
CREATE INDEX IF NOT EXISTS "IX_OpenIddictToken_ApplicationId" ON "dmscs"."OpenIddictToken" ("ApplicationId");
CREATE INDEX IF NOT EXISTS "IX_OpenIddictToken_Subject" ON "dmscs"."OpenIddictToken" ("Subject");
CREATE INDEX IF NOT EXISTS "IX_OpenIddictToken_ReferenceId" ON "dmscs"."OpenIddictToken" ("ReferenceId");
CREATE INDEX IF NOT EXISTS "IX_OpenIddictToken_ExpirationDate" ON "dmscs"."OpenIddictToken" ("ExpirationDate");
