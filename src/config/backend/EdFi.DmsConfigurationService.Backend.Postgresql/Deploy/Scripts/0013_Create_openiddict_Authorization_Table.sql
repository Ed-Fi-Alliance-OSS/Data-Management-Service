...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'PK_OpenIddictAuthorization'
          AND conrelid = '"dmscs"."OpenIddictAuthorization"'::regclass
    ) THEN
        ALTER TABLE "dmscs"."OpenIddictAuthorization" ADD CONSTRAINT "PK_OpenIddictAuthorization" PRIMARY KEY ("Id");
    END IF;
END$$;
