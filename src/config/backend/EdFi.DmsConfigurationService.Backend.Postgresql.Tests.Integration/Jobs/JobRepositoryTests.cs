...
public class JobRepositoryTests
{
    ...
    public abstract class JobRepositoryTestBase : JobSchemaTestBase
    {
        protected static JobRepository Repository(TenantContext? tenant = null) =>
            Repository(Configuration.DatabaseOptions, tenant);
        ...
        protected static PostgresqlCmsTransactionFactory TransactionFactory() =>
            new(Configuration.DatabaseOptions);

        protected static string JobIdOf(JobEnqueueResult result) =>
            result.Should().BeOfType<JobEnqueueResult.Success>().Subject.JobId;

        protected async Task<StoredJob?> StoredJobAsync(string jobId) =>
            await Connection!.QuerySingleOrDefaultAsync<StoredJob>(
                """
                SELECT "JobId", "TenantId", "JobType", "PayloadVersion", "Payload", "Status", "CreatedAt",
                       "NextAttemptAt", "AttemptCount", "FencingToken", "CreatedBy", "SourceScheduleId",
                       "ScheduledOccurrence", "FinishedAt", "LeaseExpiresAt", "LeaseOwner", "ErrorMessage"
                FROM "dmscs"."Job"
                WHERE "JobId" = @JobId;
                """,
                new { JobId = jobId }
    ...
    public class Given_a_job_enqueued_inside_a_caller_transaction_that_commits : JobRepositoryTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            _beforeCommit = await StoredJobAsync(JobIdOf(_result));
    ...
    [TestFixture]
    public class Given_jobs_enqueued_inside_caller_transactions_that_do_not_commit : JobRepositoryTestBase
    {
        private JobEnqueueResult _rolledBack = null!;
        private JobEnqueueResult _disposed = null!;
        ...
        [Test]
        public void It_reports_each_insert_as_done_inside_its_transaction()
        {
            _rolledBack.Should().BeOfType<JobEnqueueResult.Success>();
            _disposed.Should().BeOfType<JobEnqueueResult.Success>();
        }

        [Test]
        public void It_leaves_no_job_after_a_rollback_or_a_dispose_without_commit() =>
    ...
    public class Given_caller_transactions_that_have_already_completed : JobRepositoryTestBase
    {
        ...
        {
            DiagnosticOf(_results["committed"])
                .Should()
                .Be(new JobFailureDiagnostic("System.InvalidOperationException", null, "EnqueueJob"));
            DiagnosticOf(_results["rolled back"])
                .Should()
                .Be(new JobFailureDiagnostic("System.InvalidOperationException", null, "EnqueueJob"));
            DiagnosticOf(_results["disposed"])
                .Should()
                .Be(new JobFailureDiagnostic("System.ObjectDisposedException", null, "EnqueueJob"));
        }

        [Test]
        public void It_stores_no_job() => _jobCount.Should().Be(0);

        private static JobFailureDiagnostic DiagnosticOf(JobEnqueueResult result) =>
            result.Should().BeOfType<JobEnqueueResult.FailureUnknown>().Subject.Diagnostic;
    }
    ...
    public class Given_a_finished_job : JobRepositoryTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            await Connection!.ExecuteAsync(
                """
                UPDATE "dmscs"."Job"
                SET "Status" = 'Error', "FinishedAt" = '2026-09-23 10:15:30', "NextAttemptAt" = NULL,
    ...
    public class Given_database_errors : JobRepositoryTestBase
    {
        ...
        [Test]
        public void It_reports_an_enqueue_failure_with_the_type_chain_and_SqlState() =>
            _tooLongType
                .Should()
                .BeOfType<JobEnqueueResult.FailureUnknown>()
                .Which.Diagnostic.Should()
                .Be(new JobFailureDiagnostic("Npgsql.PostgresException", "22001", "EnqueueJob"));

        [Test]
        public void It_reports_a_rejected_payload_by_code_without_its_content()
        {
            ...
            diagnostic
                .Should()
                .Be(new JobFailureDiagnostic("Npgsql.PostgresException", "23514", "EnqueueJob"));
            diagnostic.ToString().Should().NotContain(PayloadLiteral);
        }

        [Test]
        public void It_reports_a_read_failure_with_the_type_chain_and_SqlState() =>
            _missingDatabase
                .Should()
                .BeOfType<JobStatusQueryResult.FailureUnknown>()
                .Which.Diagnostic.Should()
                .Be(new JobFailureDiagnostic("Npgsql.PostgresException", "3D000", "GetJobStatus"));
    }
