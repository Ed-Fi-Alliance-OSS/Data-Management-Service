...
public class JobLeaseRepositoryTests
{
    ...
    public abstract class JobLeaseTestBase : JobSchemaTestBase
    {
        protected static JobLeaseRepository Repository(JobLeaseTimings? timings = null) =>
            new(Configuration.DatabaseOptions, timings ?? _timings);

        protected static PostgresqlJobFenceFactory FenceFactory(JobLeaseTimings? timings = null) =>
            new(Configuration.DatabaseOptions, timings ?? _timings);
        ...
        protected async Task<long> SeedJobAsync(
            ...
            await Connection!.ExecuteScalarAsync<long>(
                ...
                INSERT INTO "dmscs"."Job" (
                    ...
                    "LeaseOwner", "LeaseExpiresAt", "FencingToken", "FinishedAt")
                VALUES (@JobId, @JobType, 1, '{"dataStoreId":1}', @Status,
                    ...
                    @FencingToken,
                    CASE WHEN @Status IN ('Completed', 'Error') THEN (now() AT TIME ZONE 'UTC') END)
                RETURNING "Id";
                """,
                new
                {
        ...
        protected async Task SeedOverLimitJobsAsync(int count) =>
            await Connection!.ExecuteAsync(
                ...
                INSERT INTO "dmscs"."Job" (
                    "JobId", "JobType", "PayloadVersion", "Payload", "Status", "NextAttemptAt", "AttemptCount")
                SELECT replace(gen_random_uuid()::text, '-', ''), 'DataStore.RefreshEducationOrganizations', 1,
                    '{"dataStoreId":1}', 'Pending', (now() AT TIME ZONE 'UTC'), @MaxAttempts
        ...
        protected async Task<LeaseRow> RowAsync(long id) =>
            await Connection!.QuerySingleAsync<LeaseRow>(
                """
                SELECT "Status", "AttemptCount", "FencingToken", "LeaseOwner", "LeaseExpiresAt", "NextAttemptAt",
                       "FinishedAt", "ErrorMessage", "ModifiedBy"
        ...
        protected async Task ExpireLeaseAsync(long id) =>
            await Connection!.ExecuteAsync(
                """
                UPDATE "dmscs"."Job"
                SET "LeaseExpiresAt" = (clock_timestamp() AT TIME ZONE 'UTC') - interval '1 second'
                WHERE "Id" = @Id;
                """,
                new { Id = id }
            );
        ...
        protected async Task<long> CountAsync(string where) =>
            await Connection!.ExecuteScalarAsync<long>(
                $"""SELECT count(*) FROM "dmscs"."Job" WHERE {where};"""
            );
    ...
    /// <summary>Another transaction holding <c>FOR UPDATE</c> on one job row until it is released.</summary>
    public sealed class RowLock : IAsyncDisposable
    {
        ...
        {
            NpgsqlConnection connection = new(Configuration.DatabaseOptions.Value.DatabaseConnection);
    ...
    public class Given_an_ownership_write_whose_deadline_ends_during_the_lock_wait : JobLeaseTestBase
    {
        ...
        public void It_reports_that_nothing_was_written()
        {
            ...
            failure.Diagnostic.ProviderErrorCode.Should().NotBe("55P03");
    ...
    public class Given_an_exhaust_sweep_stopped_after_its_first_batch : JobLeaseTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            _exhaustedRows = await CountAsync("\"Status\" = 'Error'");
