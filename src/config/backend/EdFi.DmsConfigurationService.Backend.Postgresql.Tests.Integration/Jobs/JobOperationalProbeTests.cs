...
/// <remarks>
/// <para>
/// The PostgreSQL job runs this assembly unfiltered, so <c>[Explicit]</c> on every concrete fixture is what keeps
/// these probes off pull-request CI. Run them with
/// <c>dotnet test … --filter "Category=OperationalProbe"</c>. The server comes from
/// <c>ConnectionStrings__JobProbePostgresql</c> when set, otherwise from <c>DatabaseSettings:DatabaseConnection</c>;
/// the probe database <c>edfi_cms_job_probe</c> is created on that server when absent. Set
...
public abstract class JobOperationalProbeBase
{
    ...
    /// <summary>
    /// The candidate RenewalTimeout (RenewalInterval / 2). It must exceed the 5 s WriteLockWait: with the two equal,
    /// the client-side timeout races the server's lock timeout and Npgsql's cancellation wait (CancellationTimeout,
    /// 2 s by default) delays or replaces the lock-timeout error.
    /// </summary>
    ...
    {
        NpgsqlConnectionStringBuilder server = new(ServerConnectionString);
        NpgsqlConnectionStringBuilder admin = new(server.ConnectionString)
        {
            Database = "postgres",
            Pooling = false,
        };
        ...
        await using (NpgsqlConnection connection = new(admin.ConnectionString))
        {
            ...
            exists.Parameters.AddWithValue("Name", ProbeDatabaseName);
            if (await exists.ExecuteScalarAsync() is null)
            {
                await using NpgsqlCommand create = new(
                    $"CREATE DATABASE \"{ProbeDatabaseName}\"",
                    connection
                );
        ...
        NpgsqlConnectionStringBuilder probe = new(server.ConnectionString)
        {
    ...
    protected Task AnalyzeAsync() =>
        ExecuteAsync("ANALYZE \"dmscs_probe\".\"Job\"; ANALYZE \"dmscs_probe\".\"JobSchedule\";");

    protected static NpgsqlCommand Command(
    ...
    /// <c>clock_timestamp()</c> is evaluated after the lock is held.
    /// </summary>
    ...
    )
    {
        ...
        {
            await using (
                NpgsqlCommand lockWait = Command(
                    connection,
                    transaction,
    ...
    protected static void Report(string probe, string metric, object value)
    {
        ...
        TestContext.Progress.WriteLine($"PROBE postgresql {probe} {metric} {text}");

        if (Environment.GetEnvironmentVariable("CMS_JOB_PROBE_RESULTS") is { Length: > 0 } path)
        {
            lock (_resultsFileLock)
            {
                File.AppendAllText(path, $"postgresql\t{probe}\t{metric}\t{text}{Environment.NewLine}");
            }
        }
    }
...
internal static class JobProbeSql
{
    ...
    public const string SeedJobs = """
        INSERT INTO "dmscs_probe"."Job" (
            ...
            "FinishedAt", "LeaseExpiresAt", "ErrorMessage", "AttemptCount", "LeaseOwner", "FencingToken", "CreatedBy")
        SELECT
    ...
    private const string ClaimUpdate = """
        ...
        RETURNING j."Id", j."FencingToken", j."AttemptCount", j."LeaseExpiresAt", (now() AT TIME ZONE 'UTC') AS "DbNow";
        """;

    /// <summary>D-4 step 1 (the fixed 5 s WriteLockWait).</summary>
    public const string SetWriteLockWait = "SET LOCAL lock_timeout = '5000ms';";

    /// <summary>D-4 step 2.</summary>
    public const string LockJobRow = """SELECT "Id" FROM "dmscs_probe"."Job" WHERE "Id" = @Id FOR UPDATE;""";

    private const string OwnershipPredicate = """
        "Id" = @Id AND "LeaseOwner" = @Owner AND "FencingToken" = @Token AND "Status" = 'InProgress'
    ...
    public const string Renew = $"""
        ...
        SET "LeaseExpiresAt" = (clock_timestamp() AT TIME ZONE 'UTC') + @LeaseSeconds * interval '1 second',
    ...
    /// <summary>
    /// D-9 coalescing expression: estimate <c>j0</c> from the elapsed seconds, then choose the boundary if it is in
    ...
    {
        string interval = $"({intervalMinutes} * interval '1 minute')";
        string boundary =
            $"({nextRunAt} + GREATEST(floor(extract(epoch from ({now} - {nextRunAt})) / ({intervalMinutes} * 60)), 0) * {interval})";
        return $"CASE WHEN {boundary} > {now} THEN {boundary} ELSE {boundary} + {interval} END";
    }
    ...
    public const string LockDueSchedule = """
        SELECT "Id", "TenantId", "JobType", "PayloadVersion", "Payload", "IntervalMinutes", "NextRunAt", "FencingToken"
        FROM "dmscs_probe"."JobSchedule"
        WHERE "Enabled"
          AND "NextRunAt" <= (clock_timestamp() AT TIME ZONE 'UTC')
    ...
    public const string LeaseSchedule = """
        ...
        SET "LeaseOwner" = @Owner,
            "LeaseExpiresAt" = (clock_timestamp() AT TIME ZONE 'UTC') + @LeaseSeconds * interval '1 second',
    ...
    public static readonly string AdvanceSchedule = $"""
        ...
        UPDATE "dmscs_probe"."JobSchedule" AS s
    ...
    : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        _claimedRowsInDatabase = await ScalarAsync(
            """SELECT count(*) FROM "dmscs_probe"."Job" WHERE "Status" = 'InProgress' AND "LeaseOwner" LIKE 'probe-claimer-%'"""
        );
        _claimedRowsWithFirstAttemptTokens = await ScalarAsync(
            """SELECT count(*) FROM "dmscs_probe"."Job" WHERE "Status" = 'InProgress' AND "FencingToken" = 1 AND "AttemptCount" = 1"""
        );

        await ExecuteAsync(
            """
            UPDATE "dmscs_probe"."Job"
            SET "Status" = 'InProgress', "LeaseOwner" = 'probe-busy', "AttemptCount" = 1, "FencingToken" = 1,
                "LeaseExpiresAt" = (now() AT TIME ZONE 'UTC') + interval '1 hour'
    ...
    private async Task<string> ExplainClaimAsync()
    {
        ...
        explain.Parameters.AddWithValue("MaxAttempts", MaxAttempts);
        List<string> nodes = [];
        await using NpgsqlDataReader reader = await explain.ExecuteReaderAsync();
        while (await reader.ReadAsync())
        {
            string line = reader.GetString(0).Trim().TrimStart('-', '>', ' ');
            if (!line.Contains(':'))
            {
                nodes.Add(line);
            }
...
public class Given_renewals_waiting_on_a_job_row_lock_held_by_another_session : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        _expiredRowsRenewed = await ScalarAsync(
            """SELECT count(*) FROM "dmscs_probe"."Job" WHERE "LeaseExpiresAt" > (clock_timestamp() AT TIME ZONE 'UTC')"""
        );
    ...
    {
        List<(NpgsqlConnection Holder, NpgsqlConnection Renewer)> pairs = [];
        try
        {
            foreach (long _ in ids)
            {
                pairs.Add((await DataSource.OpenConnectionAsync(), await DataSource.OpenConnectionAsync()));
            }
            ...
            if (leaseSecondsBeforeLock is { } leaseSeconds)
            {
                await ExecuteAsync(
                    """
                    UPDATE "dmscs_probe"."Job"
                    SET "LeaseExpiresAt" = (clock_timestamp() AT TIME ZONE 'UTC') + @Seconds * interval '1 second'
        ...
        {
            foreach ((NpgsqlConnection holder, NpgsqlConnection renewer) in pairs)
            {
    ...
    private static async Task<RenewalSample> ContendOneAsync(
        long id,
        NpgsqlConnection holder,
        NpgsqlConnection renewer,
        TimeSpan stagger,
        TimeSpan hold
    )
    {
        ...
        await using (
            NpgsqlCommand lockRow = Command(
                ...
                holderTransaction,
                """
                SELECT "LeaseExpiresAt" > (clock_timestamp() AT TIME ZONE 'UTC')
                FROM "dmscs_probe"."Job" WHERE "Id" = @Id FOR UPDATE;
                """
            )
        )
        {
            lockRow.Parameters.AddWithValue("Id", id);
            leaseLive = (bool)(await lockRow.ExecuteScalarAsync())!;
        }
...
public class Given_an_exhaust_sweep_over_100000_jobs(ClaimShape shape) : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        _exhaustedRows = await ScalarAsync(
            ...
            WHERE "Status" = 'Error' AND "ErrorMessage" = @Message AND "LeaseOwner" IS NULL
...
public class Given_retention_batches_over_100000_finished_jobs : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        {
            await using NpgsqlCommand batch = Command(
                connection,
                null,
        ...
        _remainingExpired = await ScalarAsync(
            """
            SELECT count(*) FROM "dmscs_probe"."Job"
            WHERE "Status" IN ('Completed', 'Error') AND "FinishedAt" <= (now() AT TIME ZONE 'UTC') - interval '7 days'
            """
        );
        ...
        _remainingActive = await ScalarAsync(
            """SELECT count(*) FROM "dmscs_probe"."Job" WHERE "Status" IN ('Pending', 'InProgress')"""
        );
    ...
    : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        _completedRowsInDatabase = await ScalarAsync(
            """
            SELECT count(*) FROM "dmscs_probe"."Job"
            WHERE "Status" = 'Completed' AND "AttemptCount" = 1 AND "FencingToken" = 1 AND "LeaseOwner" IS NULL
            """
        );
        _rowsClaimedMoreThanOnce = await ScalarAsync(
            """SELECT count(*) FROM "dmscs_probe"."Job" WHERE "AttemptCount" > 1 OR "FencingToken" > 1"""
        );
    ...
    private async Task RunSessionAsync(int session)
    {
        ...
        while (Interlocked.Read(ref _operations) < TargetOperations)
        {
            try
            {
                ...
                when (exception.SqlState == PostgresErrorCodes.DeadlockDetected)
            {
                Interlocked.Increment(ref _deadlocks);
            }
            catch (PostgresException exception)
            {
                _unexpectedErrors.Add(exception.SqlState);
            }
        }
    }
...
/// fixed vectors (the review's worked example, boundaries 100 ms before, at, and after, sub-millisecond boundaries,
/// multi-interval downtime, the maximum interval, and a not-yet-due row). Part two runs the whole D-8
/// materialization transaction against due schedules and compares every advance with the reference computed from
/// the database time the statement returned.
/// </summary>
[TestFixture]
[Explicit("Operational probe (DMS-1437 step 0.2); run manually with --filter Category=OperationalProbe.")]
public class Given_schedule_coalescing_vectors_and_live_materializations : JobOperationalProbeBase
{
    ...
    public async Task Setup()
    {
        ...
        {
            await using NpgsqlCommand advance = Command(connection, null, JobProbeSql.AdvanceWithInjectedNow);
            advance.Parameters.Add(
                new NpgsqlParameter("NextRunAt", NpgsqlDbType.Timestamp) { Value = vector.NextRunAt }
            );
            advance.Parameters.Add(new NpgsqlParameter("Now", NpgsqlDbType.Timestamp) { Value = vector.Now });
        ...
        for (int i = 0; i < _liveSchedules.Length; i++)
        {
            ...
            await using NpgsqlCommand seed = Command(
                connection,
                null,
                """
                INSERT INTO "dmscs_probe"."JobSchedule" (
                    ...
                    "NextRunAt", "CreatedBy")
                VALUES (NULL, @ScheduleType, 'DataStore.RefreshEducationOrganizations', 1, '{"dataStoreId":1}',
                    @IntervalMinutes, TRUE,
                    (clock_timestamp() AT TIME ZONE 'UTC') - @OverdueMicroseconds * interval '1 microsecond', 'probe');
                """
            );
        ...
        _occurrencesMatchingSchedules = await ScalarAsync(
            """
            SELECT count(*) FROM "dmscs_probe"."Job" j
            JOIN "dmscs_probe"."JobSchedule" s ON s."Id" = j."SourceScheduleId"
            WHERE j."ScheduledOccurrence" = s."LastEnqueuedOccurrence" AND j."Status" = 'Pending' AND j."CreatedBy" = 'system'
            """
        );
        _schedulesWithRecordedOccurrence = await ScalarAsync(
            """
            SELECT count(*) FROM "dmscs_probe"."JobSchedule"
    ...
    /// sample, commit.
    /// </summary>
    private static async Task<Materialization?> MaterializeNextAsync(NpgsqlConnection connection)
    {
        ...
        int intervalMinutes;
        DateTime nextRunAt;
        await using (NpgsqlCommand lockDue = Command(connection, transaction, JobProbeSql.LockDueSchedule))
        await using (NpgsqlDataReader reader = await lockDue.ExecuteReaderAsync(deadline.Token))
        {
        ...
        long token;
        await using (NpgsqlCommand lease = Command(connection, transaction, JobProbeSql.LeaseSchedule))
        {
        ...
        int inserted;
        await using (NpgsqlCommand insert = Command(connection, transaction, JobProbeSql.InsertOccurrence))
        {
            insert.Parameters.AddWithValue("JobId", Guid.NewGuid().ToString("N"));
            insert.Parameters.Add(
                new NpgsqlParameter("TenantId", NpgsqlDbType.Bigint)
                {
            ...
            insert.Parameters.Add(
                new NpgsqlParameter("Occurrence", NpgsqlDbType.Timestamp) { Value = nextRunAt }
            );
        ...
        DateTime advanced;
        DateTime databaseNow;
        await using (NpgsqlCommand advance = Command(connection, transaction, JobProbeSql.AdvanceSchedule))
        {
            ...
            await using NpgsqlDataReader reader = await advance.ExecuteReaderAsync(deadline.Token);
            if (!await reader.ReadAsync(deadline.Token))
            {
