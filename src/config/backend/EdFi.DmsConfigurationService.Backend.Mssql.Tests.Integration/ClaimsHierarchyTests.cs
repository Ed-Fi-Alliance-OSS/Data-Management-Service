...
public class ClaimsHierarchyTests : DatabaseTest
{
    ...
    {
        // Arrange
        string connectionString = MssqlTestConfiguration.DatabaseConnectionString;

        await using var conn = new SqlConnection(connectionString);

        // Insert 2 hierarchies
        int recordsAffected = await conn.ExecuteAsync(
            "INSERT INTO dmscs.claimshierarchy (hierarchy, lastmodifieddate) VALUES ('{}', SYSUTCDATETIME())"
        );
    ...
    public async Task Should_return_success_when_single_hierarchy_exists()
    {
        ...
        int recordsAffected = await conn.ExecuteAsync(
            "INSERT INTO dmscs.claimshierarchy (hierarchy, lastmodifieddate) VALUES (@HierarchyJson, SYSUTCDATETIME())",
