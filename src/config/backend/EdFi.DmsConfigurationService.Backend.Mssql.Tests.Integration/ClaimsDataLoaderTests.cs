...
public class ClaimsDataLoaderTests : DatabaseTestBase
{
    ...
    public void Setup()
    {
        ...
        // Set up dependencies
        var databaseOptions = Options.Create(MssqlTestConfiguration.DatabaseOptions.Value);
        ...
        // Configure ClaimsProvider to use the embedded resource from the Mssql test assembly
        ...
        _claimsTableValidator = new ClaimsTableValidator(
            Options.Create(MssqlTestConfiguration.DatabaseOptions.Value),
    ...
    private static async Task<IReadOnlyList<string>> SnapshotResourceClaimNamesAsync()
    {
    ...
    private async Task RestoreResourceClaimBaselineAsync()
    {
        ...
        await connection.ExecuteAsync(
            "DELETE FROM dmscs.ResourceClaim WHERE ClaimName NOT IN @Baseline",
    ...
    public async Task Given_ClaimSet_has_data_It_should_return_false_for_AreClaimsTablesEmpty()
    {
        ...
        await using var connection = await OpenConnectionAsync();
        await connection.ExecuteAsync(
            "INSERT INTO dmscs.ClaimSet (ClaimSetName, IsSystemReserved) VALUES ('TestClaim', 1)"
        );
    ...
    public async Task Given_ClaimsHierarchy_has_data_It_should_return_false_for_AreClaimsTablesEmpty()
    {
        // Arrange
        await using var connection = await OpenConnectionAsync();
        await connection.ExecuteAsync("INSERT INTO dmscs.ClaimsHierarchy (Hierarchy) VALUES ('[]')");
    ...
    public async Task Given_resource_claims_already_seeded_It_should_insert_only_missing_rows()
    {
        ...
        await using (var arrangeConnection = await OpenConnectionAsync())
        {
        ...
        // (skipped via NOT EXISTS on ClaimName), so existing rows are never duplicated.
    ...
    public async Task Given_upload_introduces_a_new_claim_It_should_seed_resource_claim_metadata_through_the_loader()
    {
        ...
        await using (var arrange = await OpenConnectionAsync())
        {
            bool alreadySeeded = await arrange.ExecuteScalarAsync<bool>(
                "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ResourceClaim WHERE ClaimName = @ClaimName) THEN 1 ELSE 0 END AS BIT)",
    ...
    public async Task Given_loaded_claim_sets_It_should_preserve_system_reserved_flags()
    {
        ...
        var systemReservedCount = await connection.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM dmscs.ClaimSet WHERE IsSystemReserved = 1"
        );
    ...
    public class Given_Upload_and_Reload_Integration : ClaimsDataLoaderTests
    {
        ...
        public async Task It_should_allow_upload_in_embedded_mode()
        {
            ...
            var customClaimExists = await connection.ExecuteScalarAsync<bool>(
                "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'CustomTestClaimSet') THEN 1 ELSE 0 END AS BIT)"
            );
        ...
        public async Task It_should_restore_to_embedded_baseline_on_reload_in_embedded_mode()
        {
            ...
            var customExists = await connection.ExecuteScalarAsync<bool>(
                "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'TempCustomClaimSet') THEN 1 ELSE 0 END AS BIT)"
            );
            ...
            var standardClaimExists = await connection.ExecuteScalarAsync<bool>(
                "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'SISVendor') THEN 1 ELSE 0 END AS BIT)"
            );
        ...
        public async Task It_should_allow_upload_in_hybrid_mode()
        {
            ...
            await using var connection = await OpenConnectionAsync();
            var customExists = await connection.ExecuteScalarAsync<bool>(
                "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'HybridCustomClaimSet') THEN 1 ELSE 0 END AS BIT)"
            );
            Assert.That(customExists, Is.True);
        }
        ...
        public async Task It_should_restore_to_composed_baseline_on_reload_in_hybrid_mode()
        {
            ...
            try
            {
                ...
                var tempCustomExists = await connection.ExecuteScalarAsync<bool>(
                    "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'TempHybridClaimSet') THEN 1 ELSE 0 END AS BIT)"
                );
        ...
        private static async Task<bool?> GetIsSystemReservedAsync(string claimSetName)
        {
            await using var connection = await OpenConnectionAsync();
        ...
        public async Task It_should_handle_mode_switching_gracefully()
        {
            ...
            try
            {
                ...
                var filesystemClaimExists = await connection.ExecuteScalarAsync<bool>(
                    "SELECT CAST(CASE WHEN EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = 'FilesystemOnlyClaimSet') THEN 1 ELSE 0 END AS BIT)"
                );
