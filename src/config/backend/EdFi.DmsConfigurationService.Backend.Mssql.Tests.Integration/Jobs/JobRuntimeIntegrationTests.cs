...
using LeaseRow = EdFi.DmsConfigurationService.Backend.Mssql.Tests.Integration.Jobs.JobLeaseRepositoryTests.LeaseRow;
...
public class JobRuntimeIntegrationTests
{
    ...
    public abstract class RuntimeTestBase : JobLeaseRepositoryTests.JobLeaseTestBase
    {
        ...
        ) => new(MssqlTestConfiguration.DatabaseOptions, settings.LeaseTimings) { SessionHooks = hooks };

        protected static MssqlJobFenceFactory HookedFenceFactory(
            JobOptions settings,
            JobDatabaseSessionHooks hooks
        ) => new(MssqlTestConfiguration.DatabaseOptions, settings.LeaseTimings) { SessionHooks = hooks };
        ...
        protected async Task<DateTime> DatabaseUtcNowAsync() =>
            await Connection!.ExecuteScalarAsync<DateTime>("SELECT SYSUTCDATETIME();");
        ...
        protected static async Task ReconcilingFencedWriteAsync(
            ...
            await transaction.Connection!.ExecuteAsync(
                new CommandDefinition(
                    ...
                    SELECT N'fenced-write'
                    WHERE NOT EXISTS (SELECT 1 FROM dmscs.OwnershipToken WHERE Description = N'fenced-write');
                    """,
    ...
    public class Given_a_due_schedule_run_by_the_dispatcher_and_the_worker : RuntimeTestBase
    {
        ...
        public async Task Setup()
        {
            ...
            long scheduleId = await Connection!.ExecuteScalarAsync<long>(
                ...
                OUTPUT inserted.Id
                VALUES (N'Runtime.Nightly', @JobType, 1, N'{"dataStoreId":1}', 60, 1,
            ...
            using (JobWorkerService worker = _harness.Worker())
            {
                ...
                await EventuallyAsync(
                    async () =>
                        await CountAsync($"SourceScheduleId = {scheduleId} AND Status = N'Completed'") == 1,
