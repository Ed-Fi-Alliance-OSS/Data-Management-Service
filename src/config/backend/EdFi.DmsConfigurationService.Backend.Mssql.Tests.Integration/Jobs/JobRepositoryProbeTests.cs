...
/// from <c>ConnectionStrings__MssqlAdmin</c>.
...
public abstract class JobRepositoryProbeBase
{
    ...
    protected const string LockTimeout = "1222";
    ...
    private static string ServerConnectionString =>
        MssqlTestConfiguration.AdminConnectionString is { Length: > 0 } configured
            ? configured
            : throw new InvalidOperationException(
                "SQL Server repository probes require the ConnectionStrings__MssqlAdmin environment variable."
            );

    /// <summary>When set, every claim records the locks it holds just before its commit.</summary>
    protected ConcurrentBag<ProbeLockFootprint>? ClaimLocks { get; set; }

    /// <summary>When set, every <c>Exhaust</c> batch records the locks it holds just before its commit.</summary>
    protected ConcurrentBag<ProbeLockFootprint>? ExhaustBatchLocks { get; set; }
    ...
    public async Task DeployProbeDatabase()
    {
        ...
        SqlConnectionStringBuilder deploy = new(ServerConnectionString)
        {
    ...
    {
        SqlConnectionStringBuilder master = new(ServerConnectionString)
        {
        ...
        await connection.ExecuteAsync(
            $"""
            IF DB_ID(N'{ProbeDatabaseName}') IS NOT NULL
            BEGIN
                ALTER DATABASE [{ProbeDatabaseName}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
                DROP DATABASE [{ProbeDatabaseName}];
            END;
            """
        );
    }

    /// <summary>The session's table, partition, or page locks other than intent locks, and its key locks.</summary>
    private static async Task<ProbeLockFootprint> HeldLocksAsync(
        SqlConnection connection,
        DbTransaction transaction
    ) =>
        await connection.QuerySingleAsync<ProbeLockFootprint>(
            """
            SELECT
    ...
    protected JobLeaseRepository LeaseRepository(Action<int>? afterExhaustBatch = null) =>
        new(DatabaseOptions, Timings)
        {
            AfterExhaustBatch = afterExhaustBatch,
            BeforeClaimCommit = async (connection, transaction) =>
            {
                if (ClaimLocks is { } claimLocks)
                {
                    claimLocks.Add(await HeldLocksAsync(connection, transaction));
                }
            },
            BeforeExhaustBatchCommit = async (connection, transaction) =>
            {
                if (ExhaustBatchLocks is { } batchLocks)
                {
                    batchLocks.Add(await HeldLocksAsync(connection, transaction));
                }
            },
        };

    protected MssqlJobFenceFactory FenceFactory(JobDatabaseSessionHooks? hooks = null) =>
    ...
    protected Task ResetAsync() => ExecuteAsync("TRUNCATE TABLE dmscs.Job; DELETE FROM dmscs.JobSchedule;");

    protected Task AnalyzeAsync() =>
        ExecuteAsync("UPDATE STATISTICS dmscs.Job; UPDATE STATISTICS dmscs.JobSchedule;");
    ...
    protected Task SeedJobsAsync(
        ...
        ExecuteAsync(
            ...
            INSERT INTO dmscs.Job (
                JobId, JobType, PayloadVersion, Payload, Status, CreatedAt, NextAttemptAt, FinishedAt, LeaseExpiresAt,
                ErrorMessage, AttemptCount, LeaseOwner, FencingToken, CreatedBy)
            SELECT
    ...
    protected static void Report(string probe, string metric, object value)
    {
        ...
        TestContext.Progress.WriteLine($"PROBE repository-mssql {probe} {metric} {text}");

        if (Environment.GetEnvironmentVariable("CMS_JOB_PROBE_RESULTS") is { Length: > 0 } path)
        {
            lock (_resultsFileLock)
            {
                File.AppendAllText(path, $"repository-mssql\t{probe}\t{metric}\t{text}{Environment.NewLine}");
            }
        }
    }
    ...
    public sealed class RowLockHolder(SqlConnection connection, DbTransaction transaction) : IAsyncDisposable
    {
...
public class Given_three_claimers_draining_a_backlog_through_the_lease_repository : JobRepositoryProbeBase
{
    ...
    private ProbeLockFootprint _claimLocks = null!;
    ...
    public async Task RunProbe()
    {
        ...
        ClaimLocks = [];
        ClaimedOf(
            await repository.ClaimNext("probe-footprint", LeaseSeconds, MaxAttempts, CancellationToken.None)
        );
        _claimLocks = ClaimLocks.Single();
        ClaimLocks = null;
        Report("claim", "locks_held_by_one_claim", _claimLocks);
    ...
    [Test]
    public void It_holds_row_locks_only_for_a_claim_at_the_backlog()
    {
        _claimLocks.CoarseLocks.Should().Be(0);
        _claimLocks.KeyLocks.Should().BeLessThanOrEqualTo(2);
    }
}
...
public class Given_renewals_waiting_on_a_row_lock_through_the_lease_repository : JobRepositoryProbeBase
{
    ...
    public async Task RunProbe()
    {
        ...
        _expiringLeasesExtended = await ScalarAsync<int>(
            """
            SELECT COUNT(*) FROM dmscs.Job WHERE LeaseExpiresAt > SYSUTCDATETIME();
            """
        );
    ...
    private async Task<string> ExpiringAsync(JobLeaseRepository repository, long id)
    {
        await using RowLockHolder holder = await HoldRowLockAsync(
            id,
            """
            UPDATE dmscs.Job SET LeaseExpiresAt = DATEADD(second, 2, SYSUTCDATETIME()) WHERE Id = @Id;
            """
        );
        Task<JobWriteResult> renewal = repository.Renew(id, Owner, 1, LeaseSeconds, CancellationToken.None);
...
public class Given_bounded_exhaust_sweeps_through_the_lease_repository : JobRepositoryProbeBase
{
    ...
    private ProbeLockFootprint[] _batchLocks = [];
    ...
    public async Task RunProbe()
    {
        ...
        _batchLocks = [.. ExhaustBatchLocks];
        ExhaustBatchLocks = null;
        Report(
            "exhaust",
            "locks_held_by_one_batch",
            $"max_coarse={_batchLocks.Max(locks => locks.CoarseLocks)} max_key={_batchLocks.Max(locks => locks.KeyLocks)}"
        );

        _liveLeasesLeft = await ScalarAsync<int>(
            """
            SELECT COUNT(*) FROM dmscs.Job WHERE Status = N'InProgress' AND LeaseExpiresAt > SYSUTCDATETIME();
            """
        );
    ...
    [Test]
    public void It_holds_row_locks_only_for_each_batch()
    {
        _batchLocks.Should().HaveCount(10);
        _batchLocks.Should().OnlyContain(locks => locks.CoarseLocks == 0 && locks.KeyLocks < 5_000);
    }
}
...
public class Given_retention_batches_through_the_retention_repository : JobRepositoryProbeBase
{
    ...
    public async Task RunProbe()
    {
        ...
        _activeLeft = await ScalarAsync<int>("SELECT COUNT(*) FROM dmscs.Job WHERE Status = N'Pending';");
    ...
    : JobRepositoryProbeBase
{
    ...
    public async Task RunProbe()
    {
        ...
        _claimedTwice = await ScalarAsync<int>(
            "SELECT COUNT(*) FROM dmscs.Job WHERE AttemptCount > 1 OR FencingToken > 1;"
        );
        _completed = await ScalarAsync<int>("SELECT COUNT(*) FROM dmscs.Job WHERE Status = N'Completed';");
    ...
    public async Task RunProbe()
    {
        ...
        for (int index = 0; index < _schedules.Length; index++)
        {
            ...
            await ExecuteAsync(
                ...
                VALUES (@ScheduleType, N'DataStore.RefreshEducationOrganizations', 1, N'{"dataStoreId":1}', @IntervalMinutes, 1,
                    DATEADD(microsecond, -CAST(ROUND((@Overdue - FLOOR(@Overdue)) * 1000000, 0) AS INT),
                        DATEADD(second, -CAST(FLOOR(@Overdue) AS INT), SYSUTCDATETIME())));
                """,
                new
                {
        ...
        _eligibleFromCreation = await ScalarAsync<int>(
            """
            SELECT COUNT(*) FROM dmscs.Job WHERE SourceScheduleId IS NOT NULL AND NextAttemptAt = CreatedAt;
            """
        );
    ...
    : JobRepositoryProbeBase
{
    ...
    public async Task RunProbe()
    {
        ...
        _untouched = await ScalarAsync<int>(
            """
            SELECT COUNT(*) FROM dmscs.Job
            WHERE (JobId = N'live_lease' AND LeaseOwner = N'probe-seed')
               OR (JobId = N'retry_in_backoff' AND Status = N'Pending')
               OR (JobId = N'at_limit' AND Status = N'Pending');
            """
        );
    }
    ...
    private Task InsertAsync(
        ...
        ExecuteAsync(
            """
            INSERT INTO dmscs.Job (
                JobId, JobType, PayloadVersion, Payload, Status, NextAttemptAt, AttemptCount, FencingToken, LeaseOwner,
                LeaseExpiresAt)
            VALUES (@JobId, N'DataStore.RefreshEducationOrganizations', 1, N'{"dataStoreId":1}', @Status,
                DATEADD(millisecond, CAST(@NextAttemptOffset * 1000 AS INT), SYSUTCDATETIME()),
                @AttemptCount, @AttemptCount,
                CASE WHEN @Status = N'InProgress' THEN N'probe-seed' END,
...
/// <summary>The locks a SQL Server session held at one point, as <c>sys.dm_tran_locks</c> reports them.</summary>
public sealed class ProbeLockFootprint
{
    public int CoarseLocks { get; set; }
    public int KeyLocks { get; set; }

    public override string ToString() =>
        string.Create(CultureInfo.InvariantCulture, $"coarse={CoarseLocks} key={KeyLocks}");
}
