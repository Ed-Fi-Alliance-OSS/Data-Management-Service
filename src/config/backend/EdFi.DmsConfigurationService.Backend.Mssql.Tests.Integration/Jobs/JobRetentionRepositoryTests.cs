...
public class JobRetentionRepositoryTests
{
    ...
    public abstract class JobRetentionTestBase : JobSchemaTestBase
    {
        protected static JobRetentionRepository Repository(IOptions<DatabaseOptions>? options = null) =>
            new(options ?? MssqlTestConfiguration.DatabaseOptions);
        ...
        protected async Task<long> SeedAsync(
            ...
            await Connection!.ExecuteScalarAsync<long>(
                ...
                INSERT INTO dmscs.Job (
                    JobId, JobType, PayloadVersion, Payload, Status, NextAttemptAt, FinishedAt, LeaseExpiresAt,
                    LeaseOwner, ErrorMessage, AttemptCount)
                OUTPUT inserted.Id
                VALUES (@JobId, N'DataStore.RefreshEducationOrganizations', 1, N'{"dataStoreId":1}', @Status,
                    ...
                    DATEADD(millisecond, CAST(@FinishedOffset * 1000 AS INT), SYSUTCDATETIME()),
    ...
    public class Given_a_database_failure : JobRetentionTestBase
    {
        ...
        [Test]
        public void It_reports_the_failure_with_the_type_chain_and_error_number() =>
            _result
                .Should()
