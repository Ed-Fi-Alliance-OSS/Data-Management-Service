...
public class ClaimSetTests : DatabaseTest
{
    ...
    protected async Task EnsureClaimsDataLoaded()
    {
        ...
        var claimsOptions = Options.Create(
            new ClaimsOptions
            {
                UseClaimsPath = false, // Use embedded resource
                ClaimsPath = null,
            }
        );
        ...
        // Create a temporary ClaimsDataLoader for the ClaimsProvider constructor
        var tempClaimsDataLoader = new Backend.ClaimsDataLoader.ClaimsDataLoader(
            null!,
            _repository,
