...
/// <summary>
/// SQL Server enqueue and status read for <c>dmscs.Job</c> (spec D-14, D-16a, D-18). A failure is returned
...
) : IJobRepository
{
    ...
    {
        // CreatedAt and NextAttemptAt come from one SYSUTCDATETIME() sample, so a new job is eligible from the
        // moment it was created (A1).
        const string Sql = """
            DECLARE @Now DATETIME2 = SYSUTCDATETIME();
            INSERT INTO dmscs.Job (
                JobId, TenantId, JobType, PayloadVersion, Payload, Status, CreatedAt, NextAttemptAt, CreatedBy)
            VALUES (@JobId, @TenantId, @JobType, @PayloadVersion, @Payload, @Status, @Now, @Now, @CreatedBy);
            """;
        ...
        try
        {
            ...
            else
            {
                ...
                // transaction must fail rather than let the insert run outside it: reading IsolationLevel
                // throws once the transaction has been committed, rolled back, or disposed.
        ...
        {
            return new JobEnqueueResult.FailureUnknown(MssqlJobDiagnostics.From(exception, EnqueueOperation));
        }
    }
    ...
    public async Task<JobStatusQueryResult> GetJobStatus(string jobId, CancellationToken cancellationToken)
    {
        try
        {
            // An exact, parameterized match (D-14), scoped to the current tenant. JobId uses a binary
            // collation, so the match is case-sensitive; the DATALENGTH guard rejects an identifier that
            // differs only by trailing spaces, which SQL Server pads before comparing.
            string sql = $"""
                SELECT JobId, Status, CreatedAt, FinishedAt, ErrorMessage
                FROM dmscs.Job
                WHERE JobId = @JobId AND DATALENGTH(JobId) = DATALENGTH(@JobId)
                    AND {TenantContext.TenantWhereClause()};
                """;
        ...
        catch (Exception exception) when (!IsCallerCancellation(exception, cancellationToken))
        {
            return new JobStatusQueryResult.FailureUnknown(
                MssqlJobDiagnostics.From(exception, GetStatusOperation)
            );
        }
    }
