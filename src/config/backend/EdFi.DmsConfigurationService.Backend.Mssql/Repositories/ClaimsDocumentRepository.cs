...
) : IClaimsDocumentRepository
{
    ...
    {
        await using SqlConnection connection = new SqlConnection(databaseOptions.Value.DatabaseConnection);
    ...
    )
    {
        ...
        const string InsertSql =
            @"
            IF NOT EXISTS (SELECT 1 FROM dmscs.ClaimSet WHERE ClaimSetName = @ClaimSetName)
    ...
    )
    {
        ...
        try
        {
            ...
            parameters.Add("Hierarchy", newHierarchyJson);
            parameters.Add("Id", existingHierarchyId);
