...
) : IProfileRepository
{
    ...
    public async Task<ProfileInsertResult> InsertProfile(ProfileInsertCommand command)
    {
        ...
        {
            string sql =
                @"INSERT INTO dmscs.Profile (ProfileName, Definition, CreatedBy, TenantId) OUTPUT INSERTED.Id VALUES (@Name, @Definition, @CreatedBy, @TenantId);";
        ...
        catch (SqlException ex) when (ex.IsUniqueViolation("UX_Profile_TenantId_ProfileName"))
        {
            logger.LogWarning(
                ex,
    ...
    public async Task<ProfileUpdateResult> UpdateProfile(ProfileUpdateCommand command)
    {
        ...
        {
            string sql =
                $@"UPDATE dmscs.Profile SET ProfileName=@Name, Definition=@Definition, LastModifiedAt=SYSUTCDATETIME(), ModifiedBy=@ModifiedBy WHERE Id=@Id AND {TenantContext.TenantWhereClause()};";
    ...
    public async Task<ProfileDeleteResult> DeleteProfile(int id)
    {
        ...
        try
        {
            string sql = $@"DELETE FROM dmscs.Profile WHERE Id=@Id AND {TenantContext.TenantWhereClause()};";
