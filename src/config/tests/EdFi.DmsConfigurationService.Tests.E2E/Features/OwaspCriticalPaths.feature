Feature: OWASP critical attack path protections
        ...
        Scenario: 04 X-Forwarded-Host spoofing does not make an invalid token acceptable
            Given token signature manipulated
             When a security GET request is made to "/v3/vendors" with header "X-Forwarded-Host" value "trusted.local"
             Then it should respond with 401

        Scenario: 05 X-Forwarded-Proto spoofing does not make an invalid token acceptable
            Given token signature manipulated
             When a security GET request is made to "/v3/vendors" with header "X-Forwarded-Proto" value "https"
             Then it should respond with 401

        Scenario: 06 Origin spoofing does not make an invalid token acceptable
            Given token signature manipulated
             When a security GET request is made to "/v3/vendors" with header "Origin" value "https://malicious.attacker"
             Then it should respond with 401
        ...
        Scenario: 08 Allowed CORS origin receives Access-Control-Allow-Origin
             When a security GET request is made to "/v3/vendors" with header "Origin" value "http://localhost:8082"
        ...
        Scenario: 12 Disallowed CORS preflight does not return allow-origin header
             When an "OPTIONS" request is made to "/v3/vendors" with headers
                  | Key                           | Value                |
        ...
        Scenario: 13 Unsupported TRACE method is not enabled
             When an "TRACE" request is made to "/v3/vendors" with headers
                  | Key    | Value |
                  | Accept | */*   |
        ...
        Scenario: 15 CSRF style forged browser cookie request without bearer token is rejected
             When an unauthenticated POST request is made to "/v3/vendors" with header "Cookie" value "sessionid=forged-session-id" and
                  """
                  {
        ...
        Scenario: 17 CSRF style browser form post without bearer token is rejected
             When an unauthenticated Form URL Encoded POST request is made to "/v3/vendors" with
                  | Key                 | Value                 |
