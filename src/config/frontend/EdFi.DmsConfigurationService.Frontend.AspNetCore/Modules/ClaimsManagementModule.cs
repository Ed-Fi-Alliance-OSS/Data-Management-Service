...
public class ClaimsManagementModule : IEndpointModule
{
    ...
    )
    {
        ...
        if (!appSettings.Value.DangerouslyEnableDynamicClaimsLoading)
        {
            logger.LogWarning(
                "Claims reload requested but DangerouslyEnableDynamicClaimsLoading is disabled"
            );
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                "Claims upload requested but DangerouslyEnableDynamicClaimsLoading is disabled"
            );
            return Results.NotFound();
        }
        ...
        {
            logger.LogError(ex, "Unexpected error during claims upload");
            return Results.Json(
                UploadClaimsResponse.Failed(
                    new List<ClaimsUploadError>
                    {
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                "Current claims requested but DangerouslyEnableDynamicClaimsLoading is disabled"
            );
            return Results.NotFound();
        }
        ...
        try
        {
            // Get current claims nodes
            var claimsNodes = claimsProvider.GetClaimsDocumentNodes();
            ...
            var originalFormat = new JsonObject
            {
                ["claimSets"] = JsonNode.Parse(claimsNodes.ClaimSetsNode.ToJsonString())!,
                ["claimsHierarchy"] = JsonNode.Parse(claimsNodes.ClaimsHierarchyNode.ToJsonString())!,
            };
        ...
        {
            logger.LogError(ex, "Failed to retrieve current claims");
            return Results.Json(
                new { error = "Failed to retrieve current claims", message = ex.Message },
                statusCode: 500
            );
        }
    }
