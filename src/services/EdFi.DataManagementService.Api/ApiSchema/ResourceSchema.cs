...
public class ResourceSchema(JsonNode _resourceSchemaNode, ILogger _logger)
{
    private readonly Lazy<MetaEdResourceName> _resourceName =
        new(() =>
        {
            return new MetaEdResourceName(
                _resourceSchemaNode.SelectRequiredNodeFromPathAs<string>("$.resourceName", _logger)
            );
        });
    ...
    private readonly Lazy<JsonNode> _jsonSchemaForUpdate =
        new(() =>
        {
            return _resourceSchemaNode.SelectRequiredNodeFromPath("$.jsonSchemaForUpdate", _logger);
        });
