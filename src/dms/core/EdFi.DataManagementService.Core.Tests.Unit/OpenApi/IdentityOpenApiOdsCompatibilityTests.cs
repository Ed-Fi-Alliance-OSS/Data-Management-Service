...
public class IdentityOpenApiOdsCompatibilityTests
{
    ...
    [TestCaseSource(nameof(SharedComponentNames))]
    public void Every_ODS_property_exists_in_DMS_with_equal_type_and_format(string componentName)
    {
        ...
        foreach ((string propertyName, JsonNode? odsPropertyNode) in odsProperties)
        {
            ...
            if (odsRefTarget is not null)
            {
                ExtractRefTargetName(dmsProperty)
                    .Should()
                    .Be(odsRefTarget, $"{componentName}.{propertyName} must reference the same component");
            }
        }
    }

    [Test]
    public void Every_remaining_difference_is_explained_by_the_encoded_ledger()
    {
    ...
    )
    {
        ...
        differences.AddRange(
            dmsStatuses.Except(odsStatuses).Select(status => $"response-code-added:{path}:{method}:{status}")
        );
    ...
    )
    {
        ...
        differences.AddRange(dmsSchemaNames.Except(odsSchemaNames).Select(name => $"component-added:{name}"));
    ...
    private static void AddRootExtensionDifferences(List<string> differences, JsonObject ods, JsonObject dms)
    {
    ...
    private static string? ExtractRefTargetName(JsonObject propertySchema)
    {
        ...
        if (candidate["$ref"] is JsonValue directRef && directRef.TryGetValue(out string? directReference))
        {
