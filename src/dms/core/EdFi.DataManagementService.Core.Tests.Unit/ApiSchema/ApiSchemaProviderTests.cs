...
[TestFixture]
[NonParallelizable]
public class Given_bundled_ApiSchema_package_content_namespace_securable_elements
{
    /// <summary>
    /// The collection-scoped Namespace paths that stale Data Standard 5.2 ApiSchema packages
    /// declare in <c>securableElements.Namespace</c>. Loaded schema nodes must never expose
    /// them as Namespace securable elements, while their reference metadata stays intact.
    /// </summary>
    private static readonly (string Resource, string Path)[] _staleDs52CollectionPaths =
    [
    ...
    private ApiSchemaDocumentNodes _nodes = null!;
    private List<(string Resource, string Path)> _namespaceSecurablePaths = null!;
    ...
    public void Setup()
    {
        ...
        _namespaceSecurablePaths = [];
        JsonNode[] allSchemaNodes = [_nodes.CoreApiSchemaRootNode, .. _nodes.ExtensionApiSchemaRootNodes];
        foreach (var schemaNode in allSchemaNodes)
        {
            if (schemaNode["projectSchema"]?["resourceSchemas"] is not JsonObject resourceSchemas)
            {
                continue;
            }
            ...
            foreach (var (resourceName, resourceSchema) in resourceSchemas)
            {
                ...
                {
                    _namespaceSecurablePaths.Add((resourceName, pathNode!.GetValue<string>()));
                }
            }
        }
    }
    ...
    [Test]
    public void It_exposes_no_collection_scoped_namespace_securable_elements()
    {
        _namespaceSecurablePaths
            .Where(entry => entry.Path.Contains("[*]", StringComparison.Ordinal))
            .Should()
            .BeEmpty("Namespace authorization applies only to resource-root fields");
    }

    [Test]
    public void It_exposes_no_extension_namespace_securable_elements()
    {
        _namespaceSecurablePaths
            .Where(entry => entry.Path.Contains("._ext.", StringComparison.Ordinal))
            .Should()
            .BeEmpty("fields beneath an _ext container must not be Namespace securable elements");
    }

    [Test]
    public void It_exposes_none_of_the_stale_ds52_collection_namespace_paths()
    {
        _namespaceSecurablePaths.Should().NotContain(_staleDs52CollectionPaths);
    }

    [Test]
    public void It_retains_document_paths_mapping_reference_metadata_for_the_stale_paths()
    {
        var resourceSchemas = _nodes.CoreApiSchemaRootNode["projectSchema"]!["resourceSchemas"]!.AsObject();

        foreach (var (resource, path) in _staleDs52CollectionPaths)
        {
            CollectStringValues(resourceSchemas[resource]?["documentPathsMapping"])
                .Should()
                .Contain(path, $"'{resource}' must keep the reference metadata for '{path}'");
        }
    }

    private static HashSet<string> CollectStringValues(JsonNode? node)
    {
