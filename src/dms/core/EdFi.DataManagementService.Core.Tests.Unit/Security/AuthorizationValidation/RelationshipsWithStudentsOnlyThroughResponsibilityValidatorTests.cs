...
public class RelationshipsWithStudentsOnlyThroughResponsibilityValidatorTests
{
    private readonly IAuthorizationRepository _authorizationRepository = A.Fake<IAuthorizationRepository>();

    [TestFixture]
    [Parallelizable]
    public class Given_Request_Has_No_Student_Info
        : RelationshipsWithStudentsOnlyThroughResponsibilityValidatorTests
    {
        ...
        {
            var validator = new RelationshipsWithStudentsOnlyThroughResponsibilityValidator(
        ...
        : RelationshipsWithStudentsOnlyThroughResponsibilityValidatorTests
    {
        ...
        {
            A.CallTo(() =>
                    _authorizationRepository.GetEducationOrganizationsForStudentResponsibility("12345")
                )
                .Returns([255901L]);
            var validator = new RelationshipsWithStudentsOnlyThroughResponsibilityValidator(
                _authorizationRepository
    ...
    [TestFixture]
    [Parallelizable]
    public class Given_No_Student_Securable_Info
        : RelationshipsWithStudentsOnlyThroughResponsibilityValidatorTests
    {
        private ResourceAuthorizationResult? _expectedResult;
    ...
    [TestFixture]
    [Parallelizable]
    public class Given_Multiple_Students_Some_Authorized
        : RelationshipsWithStudentsOnlyThroughResponsibilityValidatorTests
    {
        private ResourceAuthorizationResult? _expectedResult;
        ...
        public async Task Setup()
        {
            A.CallTo(() =>
                ...
                .Returns([255902L]); // First student doesn't have access to 255901
            A.CallTo(() =>
                    _authorizationRepository.GetEducationOrganizationsForStudentResponsibility("67890")
                )
                .Returns([255901L]); // Second student has access, but only first is checked
            var validator = new RelationshipsWithStudentsOnlyThroughResponsibilityValidator(
                _authorizationRepository
            );
            _expectedResult = await validator.ValidateAuthorization(
                new DocumentSecurityElements(
                    [],
                    [],
                    [new StudentUniqueId("12345"), new StudentUniqueId("67890")],
                    [],
                    []
                ),
                [new AuthorizationFilter.EducationOrganization("255901")],
        ...
        [Test]
        public void Should_Return_NotAuthorized_Result()
        {
            _expectedResult.Should().NotBeNull();
            _expectedResult!
                .GetType()
