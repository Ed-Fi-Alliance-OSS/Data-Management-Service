...
public class IdentityProviderBoundaryTests
{
    ...
    [Test]
    public void Activate_returns_null_and_sets_500_when_the_registration_factory_throws()
    {
        ...
        IIdentityService? provider = boundary.Activate(requestInfo);

        provider.Should().BeNull();
        AssertProviderConfiguration500(requestInfo);
    }

    [Test]
    public void Activate_returns_null_and_sets_500_when_the_constructor_throws()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        IServiceProvider services = new ServiceCollection()
    ...
    [Test]
    public void Activate_returns_null_and_sets_500_when_a_scoped_dependency_throws()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        IServiceProvider services = new ServiceCollection()
    ...
    [Test]
    public void ReadCapabilities_returns_null_and_sets_500_when_the_getter_throws()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        RequestInfo requestInfo = CreateRequestInfo();
        var provider = new ThrowingCapabilitiesIdentityService(new InvalidOperationException("getter boom"));

        IdentityCapabilities? capabilities = boundary.ReadCapabilities(provider, requestInfo);
    ...
    [Test]
    public async Task InvokeAsync_returns_null_and_sets_502_when_the_operation_throws()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        RequestInfo requestInfo = CreateRequestInfo();

        IdentityResult? result = await boundary.InvokeAsync<IdentityResult>(
            () => throw new InvalidOperationException("operation boom"),
            requestInfo
        );

        result.Should().BeNull();
        requestInfo.FrontendResponse.StatusCode.Should().Be(502);
        requestInfo.FrontendResponse.ContentType.Should().Be("application/problem+json");
        requestInfo.FrontendResponse.Body!["type"]!
            .ToString()
            .Should()
    ...
    [Test]
    public async Task A_nested_sentinel_never_reaches_the_response_body_or_any_event_above_Debug_but_reaches_Debug()
    {
        ...
        IdentityResult? result = await boundary.InvokeAsync<IdentityResult>(() => throw outer, requestInfo);

        result.Should().BeNull();
        requestInfo.CaughtException.Should().BeNull();
        requestInfo.FrontendResponse.Body!.ToJsonString().Should().NotContain(Sentinel);

        sink.Events.Should().NotBeEmpty();
        foreach (LogEvent logEvent in sink.Events.Where(e => e.Level > LogEventLevel.Debug))
        {
            RenderEvent(logEvent).Should().NotContain(Sentinel);
        }

        sink.Events.Where(e => e.Level == LogEventLevel.Debug)
            .Select(RenderEvent)
            .Should()
            .Contain(rendered => rendered.Contains(Sentinel));
    }

    [Test]
    public void Activation_failure_never_logs_the_exception_message_above_Debug()
    {
        (IdentityProviderBoundary boundary, CapturingSerilogSink sink) = CreateBoundary();
        IServiceProvider services = new ServiceCollection()
    ...
    [Test]
    public void Activate_propagates_cancellation_without_a_problem_response_when_the_request_token_is_cancelled()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        ...
        Action act = () => boundary.Activate(requestInfo);

        act.Should().Throw<OperationCanceledException>().Which.Message.Should().NotContain(Sentinel);
        requestInfo.FrontendResponse.Should().BeSameAs(No.FrontendResponse);
    }

    [Test]
    public void ReadCapabilities_propagates_cancellation_without_a_problem_response_when_the_request_token_is_cancelled()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        using var cts = new CancellationTokenSource();
        cts.Cancel();
        RequestInfo requestInfo = CreateRequestInfo(cancellationToken: cts.Token);
        var provider = new ThrowingCapabilitiesIdentityService(new InvalidOperationException("unreachable"));

        Action act = () => boundary.ReadCapabilities(provider, requestInfo);

        act.Should().Throw<OperationCanceledException>().Which.Message.Should().NotContain(Sentinel);
    }

    [Test]
    public async Task InvokeAsync_propagates_cancellation_without_a_problem_response_when_the_request_token_is_cancelled()
    {
        (IdentityProviderBoundary boundary, _) = CreateBoundary();
        using var cts = new CancellationTokenSource();
    ...
    [Test]
    public async Task InvokeAsync_sanitizes_a_provider_OperationCanceledException_thrown_after_request_cancellation()
    {
        (IdentityProviderBoundary boundary, CapturingSerilogSink sink) = CreateBoundary();
        using var cts = new CancellationTokenSource();
        RequestInfo requestInfo = CreateRequestInfo(cancellationToken: cts.Token);
        ...
        Func<Task> act = async () =>
            await boundary.InvokeAsync<IdentityResult>(
                () =>
                {
                    // Cancel only after the boundary's pre-check has already passed, so the exception
                    // is thrown and caught inside the guarded try, not by the pre-check.
                    cts.Cancel();
