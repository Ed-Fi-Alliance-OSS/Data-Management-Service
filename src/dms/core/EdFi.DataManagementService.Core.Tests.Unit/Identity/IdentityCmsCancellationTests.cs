...
public class IdentityCmsCancellationTests
{
    ...
    /// <summary>
    /// Mid-flight orchestration: each scenario must observe the gate being reached before cancelling
    /// and releasing it, so the gated TaskCompletionSource cannot be moved into SetUp.
    /// </summary>
    [TestFixture]
    public class Given_An_Identity_Request_Blocked_Inside_A_Cms_Lookup : IdentityCmsCancellationTests
    {
        [Test]
        public async Task It_propagates_cancellation_when_blocked_at_token_acquisition() =>
            await AssertCancellationPropagatesAndPairedWaiterCompletes("connect/token");

        [Test]
        public async Task It_propagates_cancellation_when_blocked_at_tenant_retrieval() =>
            await AssertCancellationPropagatesAndPairedWaiterCompletes("v3/tenants/");

        [Test]
        public async Task It_propagates_cancellation_when_blocked_at_application_lookup() =>
            await AssertCancellationPropagatesAndPairedWaiterCompletes("v3/apiClients/");

        [Test]
        public async Task It_propagates_cancellation_when_blocked_at_claim_retrieval() =>
            await AssertCancellationPropagatesAndPairedWaiterCompletes("v3/authorizationMetadata");

        /// <summary>
        /// Drives one gate scenario end to end: a cancelled caller observes propagation and never
        /// touches the identity service, then a paired live waiter on the same lookup completes
        /// normally once the gate opens.
        /// </summary>
        private static async Task AssertCancellationPropagatesAndPairedWaiterCompletes(
            string gateUrlSubstring
        )
        {
            (ApiService apiService, GatedCmsHandler handler, IIdentityService identityService) =
                BuildApiService(gateUrlSubstring);
            ...
            // A short real-time settle so the live waiter has genuinely joined before the gate is
            // cancelled and released, matching the interleaving-wait precedent in
            // CachedApplicationContextProviderTests and IdentityTenantSnapshotTests.
            ...
            // The cancelled caller never reached the capability gate, so the identity service was
            // never touched - not even its Capabilities getter.
        ...
        /// <summary>
        /// Builds a real ApiService over a ServiceCollection carrying every service the identity
        /// pipeline resolves while running, wired to a real CMS provider chain over a gated transport
        /// (the shape ApiServiceWriteCancellationTests.BuildApiService uses for the write pipelines).
        /// </summary>
        private static (
        ...
        /// <summary>
        /// The canned success body for every CMS call this suite's identity request makes, keyed by
        /// path. Every call that is not gated answers immediately with this, so only the one chosen
        /// lookup ever blocks.
        /// </summary>
        ...
        /// <summary>
        /// A CMS transport double that blocks every request whose path contains a chosen gate
        /// substring until the test releases it, and answers every other CMS request immediately from
        /// a canned responder. Modeled on BlockingDataStoreHttpMessageHandler /
        /// BlockingTenantsHttpMessageHandler (ConfigurationServiceDataStoreProviderTests) and
        /// GatedRecordingHandler (ConfigurationServiceClaimSetProviderHeaderTests), generalized to a
        /// single caller-chosen gate and an explicit release so a paired live waiter can be proven to
        /// complete normally once it opens.
        /// </summary>
