...
public class ValidateDatabaseFingerprintMiddlewareMissingTableTests
{
    ...
    [TestFixture]
    [Parallelizable]
    public class Given_DmsInstance_Has_Null_ConnectionString
        : ValidateDatabaseFingerprintMiddlewareMissingTableTests
    {
        private RequestInfo _requestInfo = No.RequestInfo();
        private bool _nextCalled;
        private IDatabaseFingerprintReader _fingerprintReader = null!;
        ...
        [Test]
        public void It_includes_no_connection_string_message()
        {
            _requestInfo
                .FrontendResponse.Body!.ToString()
                .Should()
                .Contain("no connection string configured");
        }
