...
public class CoreExceptionLoggingMiddlewareSnapshotTests
{
    ...
    ) => ExecuteWith(new DatabaseConnectionUnavailableException(kind, ProviderFailure()));
    ...
    /// read replica keeps the generic 500, and keeps being captured for the structured
    /// request-failure event, because for those kinds this is still an unexplained fault.
    /// </summary>
    ...
    public async Task It_keeps_the_existing_500_for_a_non_snapshot(EffectiveTargetKind kind)
    {
        ...
        requestInfo.CaughtException.Should().BeOfType<DatabaseConnectionUnavailableException>();
    }
