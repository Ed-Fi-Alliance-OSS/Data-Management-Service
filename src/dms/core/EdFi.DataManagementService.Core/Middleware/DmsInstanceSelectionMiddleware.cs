...
) : IPipelineStep
{
    ...
    public async Task Execute(RequestInfo requestInfo, Func<Task> next)
    {
        ...
        // Set the selected DMS instance and connection string in RequestInfo
        requestInfo.DmsInstanceId = selectedDmsInstanceId;
        requestInfo.ConnectionString = dmsInstance.ConnectionString;

        // Set connection string in scoped provider for repository access
        requestConnectionStringProvider.SetConnectionString(
            dmsInstance.ConnectionString,
            selectedDmsInstanceId
        );
