...
) : IPipelineStep
{
    ...
    public async Task Execute(RequestInfo requestInfo, Func<Task> next)
    {
        ...
        catch (DatabaseConnectionUnavailableException ex) when (ex.TargetKind == EffectiveTargetKind.Snapshot)
        {
            ...
            //
            // The inner exception's type and the target kind only, for the reason the generic catch
            // below spells out - and here the provider exception is carried as InnerException, whose
            // message is exactly where a connection string appears.
