...
`VersionPrefix`, `AssemblyVersion`, `FileVersion` and `InformationalVersion` itself, so a nupkg
packed from this repository is
...
DMS-relevant path and unconditionally in the merge queue, and the prerelease lane asserts it on
every `dms-` prerelease. The sibling
