...
try {
    ...
    try {
        ...
        if ($SkipDockerBuild) {
            ./start-local-dms.ps1 -EnableKafkaUI -EnableConfig -EnvironmentFile ./.env.routeContext.e2e -IdentityProvider self-contained -NoDataStore -AddExtensionSecurityMetadata -SkipConnectorSetup
        }
        else {
            ./start-local-dms.ps1 -EnableKafkaUI -EnableConfig -EnvironmentFile ./.env.routeContext.e2e -r -IdentityProvider self-contained -NoDataStore -AddExtensionSecurityMetadata -SkipConnectorSetup
        }
