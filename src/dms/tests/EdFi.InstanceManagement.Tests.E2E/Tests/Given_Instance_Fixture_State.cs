...
public class Given_A_Valid_Instance_Fixture_Environment
{
    ...
    [Test]
    public void It_parses_the_three_routes_with_ownership()
    {
        _state
            .RoutesForTenant("Tenant_255901")
            .Select(r => r.RouteQualifier)
            .Should()
            .BeEquivalentTo(new List<string> { "255901/2024", "255901/2025" });
        _state
    ...
    public void It_exposes_the_immutable_fixture_id_guard_sets()
    {
        ...
        _state.DataStoreIds.Should().BeEquivalentTo(new List<int> { 201, 202, 203 });
        _state.DataStoreContextIds.Should().BeEquivalentTo(new List<int> { 401, 402, 403, 404, 405, 406 });
    }
