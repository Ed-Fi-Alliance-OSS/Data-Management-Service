...
| `CursorPartitionContract` | `src/dms/backend/EdFi.DataManagementService.Backend.IntegrationFixtures/cursor-partition-contract/` | The descriptor-runtime core schema plus one standalone extension resource, giving a regular, an extension, and a descriptor collection in one fixture for cursor and partition walk coverage. The extension resource declares a string query field `label` and a numeric one `itemNumber`, which is what makes the both-operations filter proof executable: one field filters identically on the collection GET and on `/partitions`. Neither name may collide with a DMS-reserved query parameter, or the fixture would be refused at ApiSchema load. |
