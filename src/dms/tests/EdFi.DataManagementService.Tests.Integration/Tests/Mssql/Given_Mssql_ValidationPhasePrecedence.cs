...
public abstract class MssqlValidationPhasePrecedenceTestBase : MssqlApiIntegrationTestBase
{
    ...
    private const string UnknownResourcePartitionsEndpoint =
        "/data/ed-fi/nonexistentThings/partitions?number=2";
    ...
    /// <summary>
    /// The partitions surface obeys the same ordering as every other database-reading pipeline: it
    /// reaches the database phase for a routable request, so a broken later stage answers it.
    /// </summary>
    [Test]
    public async Task It_answers_503_when_a_partitions_request_reaches_the_broken_later_stage()
    {
        BreakLaterStageAfterSetup();

        using HttpResponseMessage response = await Harness.HttpClient.GetAsync(PartitionsEndpoint);

        response.StatusCode.Should().Be(DatabaseValidationStatus);
    }

    /// <summary>
    /// And the endpoint phase still outranks it: an unroutable partitions request is a 404 rather than
    /// the database-phase answer, which is the whole point of the reordering.
    /// </summary>
    [Test]
    public async Task It_answers_the_endpoint_404_for_an_unroutable_partitions_request()
    {
        BreakLaterStageAfterSetup();

        using HttpResponseMessage response = await Harness.HttpClient.GetAsync(
            UnknownResourcePartitionsEndpoint
        );
    ...
    [Test]
    public async Task It_answers_the_api_schema_failure_for_a_partitions_request()
    {
        BreakLaterStageAfterSetup();
