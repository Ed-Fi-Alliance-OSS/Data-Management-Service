...
/// on a SQL Server read. Four of the seven seams have SQL-Server-specific code, so the coverage is not
/// a duplicate of the PostgreSQL fixture.
/// </summary>
public sealed class Given_Mssql_SnapshotUnavailable : MssqlApiIntegrationTestBase
{
    ...
    public Task It_does_not_translate_a_failure_after_a_successful_acquisition() =>
        SnapshotUnavailableScenario.It_does_not_translate_a_failure_after_a_successful_acquisition(
            Harness,
            OpenAssertionConnectionAsync,
            "EXEC sp_rename 'edfi.Student', 'Student_hidden'",
            DerivativeConnectionString(DataStoreDerivativeType.Snapshot)
        );
...
/// not reading one.
/// </summary>
public sealed class Given_Mssql_SnapshotUnavailable_With_Read_Authorization_Denied
...
public sealed class Given_Mssql_SnapshotUnavailable_With_Cache_Acceleration : MssqlApiIntegrationTestBase
{
    private MutableInstanceProvider _provider = null!;
