...
) : IDocumentStoreRepository, IQueryHandler
{
    ...
    private static string? ExtractIfMatchEtag(Dictionary<string, string> headers)
    {
        ...
        var value = headers
            ...
            .Value;
        // RFC 7232 §3.1: a field-value of "" is not a valid entity-tag list.
        // Treat a blank header as absent so callers receive null and skip the If-Match path,
        // rather than activating the path with a value that can never match and returning 412.
