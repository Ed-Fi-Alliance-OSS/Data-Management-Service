...
/// <summary>
/// Parity tests that verify the SQL Server dms.uuidv5() helper function produces
...
public abstract class Uuidv5ParityTestBase
{
    ...
    internal static void InitializeConnectionString(string connectionString) =>
        _connectionString = connectionString;

    protected Guid CoreResult { get; set; }
    protected Guid MssqlResult { get; set; }
    ...
    public static async Task<Guid> ComputeMssql(Guid namespaceUuid, string name)
    {
        await using SqlConnection connection = new(ConnectionString);
        await connection.OpenAsync();
        await using SqlCommand cmd = new("SELECT dms.uuidv5(@ns, @name)", connection);
        cmd.Parameters.AddWithValue("@ns", namespaceUuid);
...
public class Given_Simple_Ascii_Name : Uuidv5ParityTestBase
{
    ...
    public async Task Setup()
    {
        ...
        MssqlResult = await ComputeMssql(EdFiNamespace, Name);
    }

    [Test]
    public void It_should_produce_matching_uuids()
    {
        MssqlResult.Should().Be(CoreResult);
    }
...
public class Given_Empty_Name
{
    ...
    /// the MSSQL function still produces a deterministic result for empty input,
    ...
    [Test]
    public async Task It_should_still_produce_a_result_in_mssql()
    {
        var result = await Uuidv5ParityTestBase.ComputeMssql(
...
public class Given_Alternate_Namespace : Uuidv5ParityTestBase
{
    ...
    public async Task Setup()
    {
        ...
        MssqlResult = await ComputeMssql(DnsNamespace, Name);
    }
...
public class Given_Long_Name : Uuidv5ParityTestBase
{
    ...
    public async Task Setup()
    {
        ...
        MssqlResult = await ComputeMssql(EdFiNamespace, _longName);
    }
...
public class Given_Same_Input_Called_Twice
{
    ...
    public async Task Setup()
    {
        _firstCall = await Uuidv5ParityTestBase.ComputeMssql(Uuidv5ParityTestBase.EdFiNamespace, Name);
        _secondCall = await Uuidv5ParityTestBase.ComputeMssql(Uuidv5ParityTestBase.EdFiNamespace, Name);
    }
