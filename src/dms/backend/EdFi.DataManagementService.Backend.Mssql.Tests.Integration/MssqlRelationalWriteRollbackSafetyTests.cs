...
file sealed class FailOnAlignedExtensionAddressInsertMssqlRelationalWriteSessionFactory(
...
) : IRelationalWriteSession
{
    ...
    // transaction all roll back. The closing bracket keeps SchoolExtensionAddressSponsorReference
    // commands from matching.
    private const string AlignedExtensionAddressInsertSql = "INSERT INTO [sample].[SchoolExtensionAddress]";
...
file static class RollbackSafetyIntegrationTestSupport
{
    ...
    public static async Task<NoProfileAtomicRollbackAssertions.FullSurfaceRollbackSnapshot> ReadFullSurfaceSnapshotAsync(
        ...
        new(
            ...
            SchoolExtensionAddressCount: await CountRowsAsync(database, "[sample].[SchoolExtensionAddress]"),
    ...
    ) => database.ExecuteScalarAsync<long>($"SELECT COUNT_BIG(*) FROM {qualifiedTableName};");
}
...
public class Given_A_Mssql_Relational_Write_Create_Failure_After_Early_Writes_With_A_Focused_Stable_Key_Fixture
{
    ...
    private async Task ExecuteCreateAsync()
    {
        ...
        scope
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "MssqlRelationalWriteRollbackSafety",
        ...
        await repository.UpsertDocument(
            RollbackSafetyIntegrationTestSupport.CreateFullSurfaceCreateRequest(
                _mappingSet,
                SchoolDocumentUuid,
                "mssql-rollback-safety"
            )
        );
    }

    // Translates the recorded SQL Server command text into the provider-neutral ordered write steps
    // consumed by the shared rollback assertion. Provider dialect SQL stays here, not in the contract.
    // Bracket quoting keeps a table name that prefixes another (School/SchoolAddress,
    // SchoolExtension/SchoolExtensionAddress) from matching its extensions.
