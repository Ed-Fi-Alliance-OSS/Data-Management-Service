...
file static class MultiBatchCollectionsIntegrationTestSupport
{
    ...
    // Translate recorded SQL Server commands into the provider-neutral batch summaries the shared
    // contract asserts over. Dialect command text stays in this adapter, not in Common. SQL Server
    // reserves collection ids via a recursive CTE over NEXT VALUE FOR (no generate_series), reading
    // the same @count parameter the shared partition assertion checks.
    public static IReadOnlyList<int> ReservationRowCounts(MultiBatchCommandRecorder recorder) =>
        recorder
            .Commands.Where(command =>
                command.CommandText.Contains(
                    "NEXT VALUE FOR [dms].[CollectionItemIdSequence] OVER",
    ...
    > ReadSchoolAddressesWithDescriptorAsync(MssqlGeneratedDdlTestDatabase database, long documentId)
    {
...
public class Given_A_Mssql_Relational_Write_Multi_Batch_Collection_Create_With_A_Focused_Stable_Key_Fixture
    : MultiBatchCollectionsGeneratedDdlFixtureTestBase
{
    ...
    public void It_partitions_collection_id_reservation_and_insert_commands_using_the_compiled_batch_limit() =>
        NoProfileMultiBatchCollectionScenarios.AssertCreateBatchPartitions(
            ...
            // SQL Server reaches the parameter cap first, and its per-table row cap is derived from that
            // same cap. Inlining the collection key frees one parameter per row, so the whole rowset now
            // fits a single statement inside one command.
            expectedInsertStatementCount: 1,
        ...
        scope
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "MssqlRelationalWriteMultiBatchCollections",
...
[Category("MssqlIntegration")]
[Category(MssqlCiShards.Shard4)]
public class Given_A_Mssql_Relational_Write_Multi_Batch_Collection_Delete_Update_With_A_Focused_Stable_Key_Fixture
    : MultiBatchCollectionsGeneratedDdlFixtureTestBase
{
    private static readonly DocumentUuid SchoolDocumentUuid = new(
        ...
        scope
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "MssqlRelationalWriteMultiBatchCollectionDeletes",
                    ConnectionString: _database.ConnectionString,
                    RouteContext: []
                )
            );
...
[Category("MssqlIntegration")]
[Category(MssqlCiShards.Shard4)]
public class Given_A_Mssql_Relational_Write_Multi_Batch_Collection_Aligned_Extension_Create_With_A_Focused_Stable_Key_Fixture
    : MultiBatchCollectionsGeneratedDdlFixtureTestBase
{
    private static readonly DocumentUuid SchoolDocumentUuid = new(
    ...
    public void It_partitions_collection_aligned_extension_insert_commands_using_the_compiled_batch_limit() =>
        NoProfileMultiBatchCollectionScenarios.AssertAlignedExtensionInsertBatchPartitions(
            ...
            // An aligned scope binds a reserved collection key rather than inlining it, so its rows are one
            // parameter wider and the 2098-parameter budget genuinely needs a second command. That is the
            // budget, not a per-table round trip: the count does not grow with the number of tables.
            expectedInsertStatementCount: 2,
            expectedInsertCommandCount: 2
        );
        ...
        scope
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "MssqlRelationalWriteMultiBatchCollectionAlignedExtensions",
                    ConnectionString: _database.ConnectionString,
                    RouteContext: []
                )
            );
...
[Category("MssqlIntegration")]
[Category(MssqlCiShards.Shard4)]
public class Given_A_Mssql_Relational_Write_Multi_Batch_Collection_Changed_Descriptor_Update_With_A_Focused_Stable_Key_Fixture
    : MultiBatchCollectionsGeneratedDdlFixtureTestBase
{
    private static readonly DocumentUuid SchoolDocumentUuid = new(
    ...
    private async Task ExecuteCreateAsync()
    {
        ...
        scope
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "MssqlRelationalWriteMultiBatchCollectionChangedDescriptorUpdate",
                    ConnectionString: _database.ConnectionString,
                    RouteContext: []
                )
            );
        ...
        var createResult = await repository.UpsertDocument(
            MultiBatchCollectionsIntegrationTestSupport.CreateCreateRequestWithDescriptor(
                ...
                MultiBatchCollectionsIntegrationTestSupport.OriginalAddressTypeDescriptorUri,
                SchoolDocumentUuid,
                "mssql-multi-batch-collection-changed-descriptor-create"
            )
        );
    ...
    private async Task<UpdateResult> ExecuteUpdateAsync()
    {
        ...
        return await repository.UpdateDocumentById(
            MultiBatchCollectionsIntegrationTestSupport.CreateUpdateRequestWithDescriptor(
                ...
                MultiBatchCollectionsIntegrationTestSupport.ReplacementAddressTypeDescriptorUri,
                SchoolDocumentUuid,
                "mssql-multi-batch-collection-changed-descriptor-update"
            )
        );
    }
}
