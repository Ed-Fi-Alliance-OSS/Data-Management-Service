...
public class Given_A_Mssql_DescriptorRead_Test_Support
{
    ...
    public void It_builds_get_requests_for_external_and_stored_document_modes()
    {
        ...
        var documentUuid = new DocumentUuid(Guid.Parse("20000000-0000-0000-0000-000000000001"));

        var externalRequest = DescriptorReadIntegrationTestSupport.CreateGetRequest(
            documentUuid,
            resourceInfo,
            _mappingSet,
            new TraceId("mssql-descriptor-external")
        );
        var storedRequest = DescriptorReadIntegrationTestSupport.CreateGetRequest(
            documentUuid,
            resourceInfo,
            _mappingSet,
            new TraceId("mssql-descriptor-stored"),
    ...
    public async Task It_resolves_descriptor_resource_key_ids_from_the_mapping_set()
    {
        var expectedSchoolTypeDescriptorResourceKeyId = await _database.ExecuteScalarAsync<short>(
            ...
            new SqlParameter("@resourceName", "SchoolTypeDescriptor")
        );
    ...
    public async Task It_seeds_valid_descriptor_rows_with_mixed_case_values()
    {
        var seed = new DescriptorReadSeed(
            DocumentUuid: new DocumentUuid(Guid.Parse("20000000-0000-0000-0000-000000000101")),
        ...
        var descriptorRow = await MssqlDescriptorReadTestSupport.ReadDescriptorRowAsync(
            _database,
            documentId
        );
