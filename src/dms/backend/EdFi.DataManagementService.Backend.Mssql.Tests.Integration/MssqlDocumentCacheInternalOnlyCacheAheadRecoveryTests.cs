...
public class Given_A_Mssql_DocumentCacheInternalOnlyCacheAheadRecovery_Command
{
    ...
    public async Task OneTimeSetUp()
    {
        ...
        _baseline = await MssqlGeneratedDdlBaselineDatabaseFactory.CreateAsync(
            $"{nameof(Given_A_Mssql_DocumentCacheInternalOnlyCacheAheadRecovery_Command)}:{_fixture.MappingSet.Key.EffectiveSchemaHash}",
    ...
    public async Task SetUp()
    {
        ...
        {
            Assert.Ignore("SQL Server cache-ahead recovery tests require nested triggers to be enabled.");
        }
    }
        ...
        )
        {
            ...
            return Task.FromResult(
                new DocumentCacheDownstreamPublicationHistoryObservation(
                    ...
                    status,
                    evidenceSource: "mssql-cache-ahead-recovery-test",
                    evidenceGenerationIdentifier: null,
                    ObservedAtOffset,
                    "SQL Server cache-ahead recovery test downstream publication proof."
                )
            );
        }
    }
