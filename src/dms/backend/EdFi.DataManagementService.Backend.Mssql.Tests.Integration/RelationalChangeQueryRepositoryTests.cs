...
public class Given_A_Mssql_Generated_Ddl_RelationalChangeQueryRepository
{
    ...
    private const string SeekMidtermGradeTypeDescriptorUri =
        "uri://ed-fi.org/GradeTypeDescriptor#Seek Midterm";
    ...
    private async Task<long> InsertGradeRootAsync(
        ...
        string studentUniqueId,
        string gradeTypeDescriptorUri = SeekGradeTypeDescriptorUri
    )
    {
        long documentId = await InsertDocumentAsync(
            Guid.NewGuid(),
        ...
        await _database.ExecuteNonQueryAsync(
            ...
            new SqlParameter("@studentUniqueId", studentUniqueId),
            new SqlParameter("@gradeTypeDescriptorUri", gradeTypeDescriptorUri),
    ...
    [Test]
    public async Task ReadChanges_custom_view_descriptor_basis_filters_deleted_grades_by_the_live_grade_type_descriptor()
    {
        await SeedGradeChainReferenceDataAsync();
        await SeedSchoolTypeDescriptorsAsync();
        await SeedDescriptorAsync(
        ...
        SectionChainSeed chain = await InsertSectionChainAsync();
        long studentDocumentId = await InsertStudentAsync("STU-CV-GradeType");
        long associationDocumentId = await InsertStudentSectionAssociationRootAsync(
            chain,
            studentDocumentId,
            "STU-CV-GradeType"
        );
        long finalGradeDocumentId = await InsertGradeRootAsync(
            chain,
            associationDocumentId,
            "STU-CV-GradeType"
        );
        long midtermGradeDocumentId = await InsertGradeRootAsync(
            chain,
            associationDocumentId,
            "STU-CV-GradeType",
            SeekMidtermGradeTypeDescriptorUri
        );
        // GradeTypeDescriptor is part of Grade's identity, so the Grade tombstone carries the descriptor's
        // old Namespace and CodeValue. Descriptors share dms.Descriptor, so the seek matches those two
        // values under the GradeTypeDescriptor discriminator and checks the found DocumentId against the view.
        await CreateCustomAuthViewAsync(
            "GradeTypeDescriptorWithSeekFinal",
            GradeTypeDescriptorViewSql("Seek Final")
        );

        long windowStart = await GetNewestChangeVersionAsync() + 1;
        await DeleteEdfiRootAsync("Grade", finalGradeDocumentId);
        await DeleteEdfiRootAsync("Grade", midtermGradeDocumentId);
        ChangeVersionRange window = new(windowStart, await GetNewestChangeVersionAsync());
        ...
        result.TotalCount.Should().Be(1);
        result.Items.Should().ContainSingle();
        result.Items[0]!["keyValues"]!["gradeTypeDescriptor"]!
            .GetValue<string>()
            .Should()
            .Be(SeekGradeTypeDescriptorUri);
    }

    [Test]
    public async Task ReadChanges_custom_view_descriptor_basis_probes_the_shared_descriptor_tombstone_only_with_the_suffix()
    {
        await SeedGradeChainReferenceDataAsync();
        await SeedSchoolTypeDescriptorsAsync();
        SectionChainSeed chain = await InsertSectionChainAsync();
        long studentDocumentId = await InsertStudentAsync("STU-CV-GradeTypeGone");
        long associationDocumentId = await InsertStudentSectionAssociationRootAsync(
            chain,
            studentDocumentId,
            "STU-CV-GradeTypeGone"
        );
        long gradeDocumentId = await InsertGradeRootAsync(
            chain,
            associationDocumentId,
            "STU-CV-GradeTypeGone"
        );
        ...
        // Every deleted descriptor of every type lands in the one shared descriptor tombstone table; the
        // tombstone arm narrows it to the deleted Seek Final descriptor by its old code value.
        await CreateCustomAuthViewAsync(
            "GradeTypeDescriptorWithSeekFinalIncludingDeletes",
            GradeTypeDescriptorViewSql("Seek Final")
                + """

                UNION
                SELECT tombstone.[DocumentId]
                FROM [tracked_changes_edfi].[Descriptor] tombstone
                WHERE tombstone.[OldCodeValue] = 'Seek Final'
                """
        );

        long windowStart = await GetNewestChangeVersionAsync() + 1;
        await DeleteEdfiRootAsync("Grade", gradeDocumentId);
        ChangeVersionRange beforeDescriptorDelete = new(windowStart, await GetNewestChangeVersionAsync());

        // While the descriptor is live, the seek finds it and the Grade tombstone is authorized.
        TrackedChangeQueryResult whileDescriptorLive = await QueryCustomViewChangesAsync(
            ChangeQueryEndpointOperation.Deletes,
            GradeResource,
            ["GradeTypeDescriptorWithSeekFinal"],
            claimEdOrgIds: [],
            beforeDescriptorDelete
        );
        whileDescriptorLive.AuthorizationFailure.Should().BeNull();
        whileDescriptorLive.TotalCount.Should().Be(1);
        ...
        long finalDescriptorDocumentId = await _database.ExecuteScalarAsync<long>(
            ...
            new SqlParameter("@uri", SeekGradeTypeDescriptorUri)
        );
        await DeleteDescriptorAsync(finalDescriptorDocumentId);
        ChangeVersionRange afterDescriptorDelete = new(windowStart, await GetNewestChangeVersionAsync());

        // Once the descriptor is gone the live seek finds nothing: without the IncludingDeletes suffix the
        // Grade tombstone is denied, matching ODS behavior for a view over live tables.
        TrackedChangeQueryResult unsuffixed = await QueryCustomViewChangesAsync(
            ChangeQueryEndpointOperation.Deletes,
            GradeResource,
            ["GradeTypeDescriptorWithSeekFinal"],
            claimEdOrgIds: [],
            afterDescriptorDelete
        );
        unsuffixed.AuthorizationFailure.Should().BeNull();
        unsuffixed.TotalCount.Should().Be(0);
        unsuffixed.Items.Should().BeEmpty();

        // The suffix makes DMS probe the shared descriptor tombstone table under the GradeTypeDescriptor
        // discriminator, so the deleted descriptor's DocumentId is found and the view's tombstone arm
        // authorizes it.
        TrackedChangeQueryResult suffixed = await QueryCustomViewChangesAsync(
            ChangeQueryEndpointOperation.Deletes,
            GradeResource,
            ["GradeTypeDescriptorWithSeekFinalIncludingDeletes"],
            claimEdOrgIds: [],
        ...
        suffixed.AuthorizationFailure.Should().BeNull();
        suffixed.TotalCount.Should().Be(1);
        suffixed.Items.Should().ContainSingle();
        suffixed.Items[0]!["keyValues"]!["studentUniqueId"]!
            .GetValue<string>()
            .Should()
