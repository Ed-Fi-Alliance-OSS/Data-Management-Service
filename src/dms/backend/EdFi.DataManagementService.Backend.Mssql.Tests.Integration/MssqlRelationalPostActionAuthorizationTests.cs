...
/// on real SQL Server, for an ordinary resource and a descriptor. Every denial asserts the persisted state did
...
public class Given_A_Mssql_Post_With_Distinct_Create_And_Update_Authorization
{
    ...
    public async Task It_never_commits_a_create_that_loses_to_a_concurrent_create()
    {
        ...
        // SQL Server's capture waits on the uncommitted row rather than reading past it, so once the blocker
        // commits it observes the existing record and applies Update in the same attempt.
        postResult
            .Should()
            .Be(new UpsertResult.UpsertFailureTargetActionNotPermitted(UpsertTargetAction.Update));
    ...
    public async Task It_reselects_the_update_policy_after_a_descriptor_create_loses_to_a_concurrent_create()
    {
        ...
        // SQL Server's lookup waits on the uncommitted row rather than reading past it, so once the blocker
        // commits it resolves the existing descriptor and applies Update in the same attempt.
        postResult
            .Should()
            .Be(new UpsertResult.UpsertFailureTargetActionNotPermitted(UpsertTargetAction.Update));
    ...
    public async Task It_reselects_the_create_policy_after_a_descriptor_is_deleted_under_the_lock()
    {
        ...
        deleteResult.Should().BeOfType<DeleteResult.DeleteSuccess>();
        // SQL Server's lookup waits on the row the delete holds, so once the delete commits it resolves no target
        // and applies Create in the same attempt, rather than updating under the stale Update policy.
        postResult
            .Should()
            .Be(new UpsertResult.UpsertFailureTargetActionNotPermitted(UpsertTargetAction.Create));
