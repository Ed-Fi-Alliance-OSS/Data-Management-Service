...
public class Given_A_Mssql_Relational_Query_With_The_Authoritative_Sample_School_Fixture
{
    ...
    /// <summary>
    /// Updates the first seeded school's name through the enabled stamping triggers so its
    /// ContentVersion becomes the largest, making ContentVersion order diverge from DocumentId
    /// order, then returns fresh state.
    /// </summary>
    private async Task<IReadOnlyList<PersistedQuerySchool>> UpdateFirstSchoolAndReadStateAsync()
    {
        ...
        var affectedRows = await _database.ExecuteNonQueryAsync(
            ...
            """,
            new SqlParameter("@nameOfInstitution", $"{seed.NameOfInstitution} (updated)"),
