...
/// <summary>Administrative invocation may select an unconfigured target. CDC must prove configured membership.</summary>
public enum DocumentCacheRuntimeTargetSelection
{
    InvocationTarget,
    RequireConfiguredMembership,
}

/// <summary>Shared non-HTTP runtime composition. Only the selected target executes; no hosted service is started.</summary>
public static class DocumentCacheRuntimeServiceCollectionExtensions
{
    public static IServiceCollection AddDocumentCacheRuntimeServices(
        this IServiceCollection services,
        IConfiguration configuration,
        ILogger logger,
        DocumentCacheTargetKey invocationTarget,
        DocumentCacheRuntimeTargetSelection targetSelection
    )
    {
        ...
        if (!Enum.IsDefined(targetSelection))
        {
            throw new ArgumentOutOfRangeException(nameof(targetSelection));
        }
        ...
        services
            ...
            .Configure(options =>
                ConfigureDocumentCacheOptions(configuration, invocationTarget, targetSelection, options)
            )
            .ValidateOnStart();
    ...
    {
        if (targetSelection == DocumentCacheRuntimeTargetSelection.RequireConfiguredMembership)
        {
            DocumentCacheOptions configuredOptions = new();
            configuration.GetSection(DocumentCacheOptions.SectionName).Bind(configuredOptions);
            var validation = new DocumentCacheOptionsValidator(configuration).Validate(
                Options.DefaultName,
                configuredOptions
            );
            ...
            if (!configuredOptions.GetTargetKeys().Contains(invocationTarget))
            {
                throw new InvalidOperationException(
                    "Selected CDC target is not configured in DocumentCache:Targets."
                );
            }
        }
