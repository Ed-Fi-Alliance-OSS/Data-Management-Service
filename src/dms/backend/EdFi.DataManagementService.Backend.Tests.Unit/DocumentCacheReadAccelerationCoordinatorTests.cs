...
public class Given_DocumentCacheReadAccelerationCoordinator
{
    ...
    public async Task It_returns_complete_get_selection_result_before_runtime_target_resolution(
    ...
    public async Task It_returns_complete_query_selection_result_before_runtime_target_resolution(
    ...
    [Test]
    public async Task It_does_not_select_a_get_candidate_when_the_exact_target_is_not_configured()
    {
        var lookupAdapter = new RecordingLookupAdapter();
        var fallbackResult = new GetResult.GetFailureNotExists();
        var selectionAttempts = 0;
                ...
                selectAuthorizedCandidate: _ =>
                {
                    ...
                    return Task.FromResult<DocumentCacheReadAccelerationGetByIdSelectionResult>(
                        new DocumentCacheReadAccelerationGetByIdSelectionResult.Complete(
                            new GetResult.UnknownFailure("candidate selection should not run")
                        )
                    );
                }
            )
    ...
    [Test]
    public async Task It_does_not_select_a_query_candidate_page_when_the_exact_target_is_not_configured()
    {
        var lookupAdapter = new RecordingLookupAdapter();
        var fallbackResult = new QueryResult.QuerySuccess(
            [new JsonObject { ["id"] = DocumentUuid.Value.ToString() }],
        ...
        QueryResult result = await sut.QueryAsync(
            CreateQueryRequest(
                ...
                selectAuthorizedCandidatePage: _ =>
                {
                    selectionAttempts++;
                    return Task.FromResult<DocumentCacheReadAccelerationQuerySelectionResult>(
                        new DocumentCacheReadAccelerationQuerySelectionResult.Complete(
                            new QueryResult.QueryFailureKnownError("candidate selection should not run")
                        )
                    );
                }
            )
    ...
    [Test]
    public async Task It_selects_a_get_candidate_before_runtime_resolution_for_a_configured_target()
    {
        var lookupAdapter = new RecordingLookupAdapter();
        var fallbackResult = new GetResult.GetFailureNotExists();
        var selectionAttempts = 0;
        ...
        lookupAdapter.GetByIdAttempts.Should().Be(0);
        registry.CurrentSnapshotAccesses.Should().Be(1);
        registry.CurrentRuntimeSnapshotAccesses.Should().Be(1);
    }

    [Test]
    public async Task It_selects_a_query_candidate_page_before_runtime_resolution_for_a_configured_target()
    {
        var lookupAdapter = new RecordingLookupAdapter();
        var fallbackResult = new QueryResult.QuerySuccess(
            [new JsonObject { ["id"] = DocumentUuid.Value.ToString() }],
    ...
    public async Task It_direct_fills_get_by_id_when_raw_rebuilding_is_observed_with_a_stale_target_snapshot(
    ...
    [TestCase(DocumentCacheLifecycleState.Resetting, false)]
    [TestCase(DocumentCacheLifecycleState.Tracking, true)]
    public async Task It_direct_fills_surviving_query_candidates_when_raw_rebuilding_is_observed_with_a_stale_target_snapshot(
        DocumentCacheLifecycleState lifecycleState,
        bool cacheAheadRecoveryRequired
    )
    {
    ...
    [TestCaseSource(nameof(NonRebuildingRawLookupOutcomesThatSkipDirectFill))]
    public async Task It_skips_tracking_direct_fill_for_non_rebuilding_raw_lookup_outcomes(
        object rawLookupOutcomeValue
    )
    {
        var rawLookupOutcome = (DocumentCacheReadLookupOutcome)rawLookupOutcomeValue;
        var fallbackResult = new GetResult.GetSuccess(
            DocumentUuid,
            new JsonObject { ["id"] = DocumentUuid.Value.ToString() },
            new DateTime(2026, 8, 1, 12, 0, 0, DateTimeKind.Utc),
    ...
    [TestCaseSource(nameof(FencedLookupOutcomesExceptRebuilding))]
    public async Task It_skips_get_by_id_direct_fill_in_rebuilding_when_raw_lookup_outcome_is_not_rebuilding(
        object rawLookupOutcomeValue
    )
    {
        var rawLookupOutcome = (DocumentCacheReadLookupOutcome)rawLookupOutcomeValue;
    ...
    [TestCaseSource(nameof(FencedLookupOutcomesExceptRebuilding))]
    public async Task It_skips_query_direct_fill_in_rebuilding_when_raw_lookup_outcome_is_not_rebuilding(
        object rawLookupOutcomeValue
    )
    {
        var rawLookupOutcome = (DocumentCacheReadLookupOutcome)rawLookupOutcomeValue;
    ...
    [Test]
    public async Task It_direct_fills_only_rebuilding_query_candidates_that_survive_relational_fallback()
    {
        DocumentCacheReadAccelerationCandidate first = Candidate(documentId: 345, contentVersion: 91);
        DocumentCacheReadAccelerationCandidate second = Candidate(
            documentId: 346,
    ...
    [TestCase(DocumentCacheLifecycleState.Resetting, false)]
    [TestCase(DocumentCacheLifecycleState.Tracking, true)]
    public async Task It_direct_fills_get_by_id_when_live_tracking_lookup_is_observed_with_a_stale_target_snapshot(
        DocumentCacheLifecycleState lifecycleState,
        bool cacheAheadRecoveryRequired
    )
    {
    ...
    [TestCase(DocumentCacheLifecycleState.Resetting, false)]
    [TestCase(DocumentCacheLifecycleState.Tracking, true)]
    public async Task It_direct_fills_query_when_live_tracking_lookup_is_observed_with_a_stale_target_snapshot(
        DocumentCacheLifecycleState lifecycleState,
        bool cacheAheadRecoveryRequired
    )
    {
    ...
    {
        yield return DocumentCacheReadLookupOutcome.LifecycleDisabled;
        yield return DocumentCacheReadLookupOutcome.LifecycleResetting;
        yield return DocumentCacheReadLookupOutcome.CacheAheadRecoveryRequired;
        yield return DocumentCacheReadLookupOutcome.MissingLifecycleState;
        yield return DocumentCacheReadLookupOutcome.InvalidLifecycleState;
    ...
    private static IEnumerable<DocumentCacheReadLookupOutcome> NonRebuildingRawLookupOutcomesThatSkipDirectFill()
    {
        foreach (DocumentCacheReadLookupOutcome outcome in FencedLookupOutcomesExceptRebuilding())
        {
            yield return outcome;
        }

        yield return DocumentCacheReadLookupOutcome.CacheUnavailable;
        yield return DocumentCacheReadLookupOutcome.DeterministicInvariantFailure;
    }
    ...
    private static StaticTargetRegistry CreateConfiguredOnlyRegistry()
    {
        DocumentCacheTargetExecutionContext executionContext = ExecutionContext();
        DocumentCacheTargetRegistrySnapshot snapshot = new(
            [DocumentCacheTargetObservation.Configured(TargetKey, executionContext.EffectiveSettings)],
            ObservedAt
        );
