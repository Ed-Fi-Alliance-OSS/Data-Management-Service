...
[TestFixture]
[Parallelizable]
public class Given_PostgresqlTokenInfoEducationOrganizationLookup
{
    ...
    public async Task It_executes_the_relational_hierarchy_query_with_a_postgresql_array_parameter()
    {
        ...
        command
            .CommandText.Should()
            .Contain("WHERE c.\"EducationOrganizationId\" = ANY(@ClaimEducationOrganizationIds)");
        command
            .CommandText.Should()
        ...
        command
            .Parameters[0]
            .Value.Should()
            .BeEquivalentTo(new long[] { 255900L, 255901L }, options => options.WithStrictOrdering());
        ...
        npgsqlParameter.NpgsqlDbType.Should().Be(_claimEducationOrganizationIdsParameterDbType);
    }
