...
public class Given_MssqlReferenceLookupSmallListStrategy
{
    ...
    public async Task It_projects_found_missing_alias_and_descriptor_membership_rows_for_supported_small_lookup_sets()
    {
        ...
        var schoolClassificationReferentialId = CreateReferentialId(5);
        ...
        var result = await sut.ResolveAsync(
            new ReferenceLookupRequest(
                ...
                [
                    ...
                    RelationalAccessTestData.CreateSchoolClassificationLookup(
                        schoolClassificationReferentialId
                    ),
                ]
            )
        );
        ...
        executor.Commands[0].CommandText.Should().Contain("(@p5, 5)");
        ...
        executor.Commands[0].CommandText.Should().Contain("FROM [edfi].[SchoolClassification_View] source");
        ...
        executor.Commands[0].Parameters[5].Name.Should().Be("@p5");
        executor.Commands[0].Parameters[5].Value.Should().Be(schoolClassificationReferentialId.Value);
