...
public sealed class Given_MessageContractSqlServer
{
    ...
    public async Task Setup()
    {
        ...
        await using CdcConnectorTemplatePinnedImageFixture fixture =
            await CdcConnectorTemplatePinnedImageFixture.StartAsync(CdcProvider.SqlServer, token);
        ...
        rendered.Config["data.query.mode"].Should().Be("function");
        rendered.Config["snapshot.isolation.mode"].Should().Be("snapshot");
        rendered.Config["time.precision.mode"].Should().Be("isostring");
        rendered.Config["database.names"].Should().Be("edfi_datastore");
        rendered
        ...
        // Cache delete and canonical create/update must have no public output.
    ...
    private async Task RetainEvidenceAsync(CancellationToken token)
    {
        ...
        TestContext.AddTestAttachment(
            path,
            "Bounded SQL Server source schemas and broker observation summaries; payloads omitted"
        );
    }
    ...
    {
        _fences.Add(await fixture.FenceSqlServerSourceAsync(_request, phase, token));
    ...
    [Test]
    [Property("ScenarioId", "MC-SQLSERVER-SNAPSHOT-LIVE")]
    public void It_publishes_complete_shared_snapshot_and_live_envelopes_on_fixed_key_partitions()
    {
        ...
        cache
            .Where(r => Operation(r) == "u")
            .Should()
            .OnlyContain(r => r.GetProperty("timeHasSevenFractionalDigits").GetBoolean());
        cache
            .Where(r => Operation(r) == "u")
            .Should()
            .OnlyContain(r => r.GetProperty("beforeJsonState").GetString() == "present");
        foreach (JsonElement input in cache)
        {
            AssertUuidKey(input);
            AssertSchema(input.GetProperty("jsonSchema"), "STRING", "", 0);
            AssertSchema(input.GetProperty("timeSchema"), "STRING", "io.debezium.time.IsoTimestamp", 1);
            AssertSchema(input.GetProperty("versionSchema"), "INT64", "", 0);
        }
    }

    [Test]
    [Property("ScenarioId", "MC-SQLSERVER-DELETE-EXCLUSION")]
    public void It_emits_one_true_tombstone_per_canonical_delete_and_drops_excluded_operations()
    {
        ...
        _public["EXCLUDED"].Records.Count.Should().Be(0);
        ...
        foreach (JsonElement input in deletes)
        {
            AssertUuidKey(input);
            input
                .GetProperty("beforeUuidState")
                .GetString()
                .Should()
                .Be("uuid", "this live canonical delete retains its non-LOB UUID before image");
        ...
        _source.Select(Operation).Should().NotContain("t", "SQL Server CDC exposes row changes");
        _source
    ...
    [Test]
    [Property("ScenarioId", "MC-SQLSERVER-PROGRESS")]
    public void It_publishes_structured_and_native_heartbeats_with_the_fixed_key_only_on_progress()
    {
        ...
        foreach (JsonElement input in _source)
        {
            ...
            input.GetProperty("partitionFields").GetInt32().Should().Be(2);
            input.GetProperty("databaseMatches").GetBoolean().Should().BeTrue();
            input.GetProperty("sourceDatabaseMatches").GetBoolean().Should().BeTrue();
        }
        _public
            .Values.SelectMany(s => s.Records)
    ...
    [Test]
    [Property("ScenarioId", "MC-SQLSERVER-WORK-EXCLUSION")]
    public void It_excludes_work_activity_from_capture_and_fenced_public_and_progress_observations()
    {
        _public["WORK"].Records.Count.Should().Be(0);
        _progress["WORK"]
            .Records.Should()
            .NotBeEmpty("a retained heartbeat crosses the post-work captured heartbeat fence");
        _source
            .Select(Kind)
            .Should()
    ...
    private static void AssertUuidKey(JsonElement input)
    {
        ...
        AssertSchema(input.GetProperty("keyUuidSchema"), "STRING", "", 0);
    }
