...
Only the Connect image is required. It must exactly match the shipped
`CdcQualifiedWorkerImage.json` reference; another immutable digest is rejected before
Docker starts. Successor image publication and telemetry qualification belong to DMS-1323. There is one isolated, network-disabled, short-lived
