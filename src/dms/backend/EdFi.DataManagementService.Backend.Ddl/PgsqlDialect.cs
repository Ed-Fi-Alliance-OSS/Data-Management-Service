...
public sealed class PgsqlDialect : SqlDialectBase
{
    ...
    )
    {
        ...
        return $"CREATE {uniqueKeyword}INDEX IF NOT EXISTS {quotedIndex} ON {QualifyTable(table)} ({columnList}){includeClause}{filterClause};";
    }
    ...
    public override string CreateGetMaxChangeVersionFunction(DbSchemaName schema)
    {
        ...
        return $"""
            CREATE OR REPLACE FUNCTION {qualifiedName}() RETURNS bigint AS
            $GetMaxChangeVersion$
            DECLARE
                result bigint;
            BEGIN
