...
CREATE OR REPLACE FUNCTION "edfi"."TF_TR_ParentResourceParentChildren_Stamp"()
RETURNS TRIGGER AS $func$
BEGIN
    IF TG_OP = 'DELETE' THEN
        WITH stamped AS (
...
CREATE TRIGGER "TR_ParentResourceParentChildren_Stamp"
BEFORE INSERT OR UPDATE OR DELETE ON "edfi"."ParentResourceParentChildren"
FOR EACH ROW
EXECUTE FUNCTION "edfi"."TF_TR_ParentResourceParentChildren_Stamp"();

CREATE OR REPLACE FUNCTION "rootext"."TF_TR_ParentResourceExtension_Stamp"()
RETURNS TRIGGER AS $func$
BEGIN
    IF TG_OP = 'DELETE' THEN
        WITH stamped AS (
            ...
            SET "ContentVersion" = nextval('"dms"."ChangeVersionSequence"'), "ContentLastModifiedAt" = now()
            WHERE "DocumentId" = OLD."DocumentId"
            AND EXISTS (SELECT 1 FROM "edfi"."ParentResource" r WHERE r."DocumentId" = OLD."DocumentId")
            RETURNING "DocumentId", "ContentVersion", "ContentLastModifiedAt"
        )
        UPDATE "edfi"."ParentResource" r
        SET "ContentVersion" = stamped."ContentVersion", "ContentLastModifiedAt" = stamped."ContentLastModifiedAt"
        ...
        RETURN OLD;
    END IF;
    IF TG_OP = 'UPDATE' AND NOT (OLD."DocumentId" IS DISTINCT FROM NEW."DocumentId" OR OLD."RootExtHiddenScalar" IS DISTINCT FROM NEW."RootExtHiddenScalar" OR OLD."RootExtVisibleScalar" IS DISTINCT FROM NEW."RootExtVisibleScalar") THEN
        RETURN NEW;
    END IF;
    WITH stamped AS (
        UPDATE "dms"."Document"
        SET "ContentVersion" = nextval('"dms"."ChangeVersionSequence"'), "ContentLastModifiedAt" = now()
        WHERE "DocumentId" = NEW."DocumentId"
        AND EXISTS (SELECT 1 FROM "edfi"."ParentResource" r WHERE r."DocumentId" = NEW."DocumentId")
        RETURNING "DocumentId", "ContentVersion", "ContentLastModifiedAt"
    )
    UPDATE "edfi"."ParentResource" r
    SET "ContentVersion" = stamped."ContentVersion", "ContentLastModifiedAt" = stamped."ContentLastModifiedAt"
...
CREATE TRIGGER "TR_ParentResourceExtension_Stamp"
BEFORE INSERT OR UPDATE OR DELETE ON "rootext"."ParentResourceExtension"
FOR EACH ROW
EXECUTE FUNCTION "rootext"."TF_TR_ParentResourceExtension_Stamp"();

CREATE OR REPLACE FUNCTION "rootext"."TF_TR_ParentResourceExtensionRootExtChildren_Stamp"()
RETURNS TRIGGER AS $func$
BEGIN
    IF TG_OP = 'DELETE' THEN
        WITH stamped AS (
        ...
        RETURN OLD;
    END IF;
    IF TG_OP = 'UPDATE' AND NOT (OLD."CollectionItemId" IS DISTINCT FROM NEW."CollectionItemId" OR OLD."Ordinal" IS DISTINCT FROM NEW."Ordinal" OR OLD."ParentResource_DocumentId" IS DISTINCT FROM NEW."ParentResource_DocumentId" OR OLD."RootExtChildCode" IS DISTINCT FROM NEW."RootExtChildCode" OR OLD."RootExtChildValue" IS DISTINCT FROM NEW."RootExtChildValue") THEN
        RETURN NEW;
    END IF;
    WITH stamped AS (
        UPDATE "dms"."Document"
...
CREATE TRIGGER "TR_ParentResourceExtensionRootExtChildren_Stamp"
BEFORE INSERT OR UPDATE OR DELETE ON "rootext"."ParentResourceExtensionRootExtChildren"
FOR EACH ROW
EXECUTE FUNCTION "rootext"."TF_TR_ParentResourceExtensionRootExtChildren_Stamp"();
