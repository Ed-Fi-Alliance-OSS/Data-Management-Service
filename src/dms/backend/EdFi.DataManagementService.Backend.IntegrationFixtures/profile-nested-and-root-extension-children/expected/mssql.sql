...
BEGIN
    ...
    IF @preflight_stored_hash IS NOT NULL AND @preflight_stored_hash <> N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59'
    BEGIN
        DECLARE @preflight_msg nvarchar(500) = CONCAT(N'EffectiveSchemaHash mismatch: database has ''', @preflight_stored_hash, N''' but expected ''', N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59', N'''');
...
IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = N'rootext')
    EXEC('CREATE SCHEMA [rootext]');
...
IF OBJECT_ID(N'rootext.ParentResourceExtension', N'U') IS NULL
CREATE TABLE [rootext].[ParentResourceExtension]
(
    [DocumentId] bigint NOT NULL,
    [RootExtHiddenScalar] nvarchar(100) NULL,
    [RootExtVisibleScalar] nvarchar(100) NULL,
    CONSTRAINT [PK_ParentResourceExtension] PRIMARY KEY ([DocumentId])
);

IF OBJECT_ID(N'rootext.ParentResourceExtensionRootExtChildren', N'U') IS NULL
CREATE TABLE [rootext].[ParentResourceExtensionRootExtChildren]
(
    ...
    [RootExtChildCode] nvarchar(30) NULL,
    [RootExtChildValue] nvarchar(100) NULL,
    CONSTRAINT [PK_ParentResourceExtensionRootExtChildren] PRIMARY KEY ([CollectionItemId]),
    CONSTRAINT [UX_ParentResourceExtensionRootExtChildren_Ordinal_ParentResource_DocumentId] UNIQUE ([ParentResource_DocumentId], [Ordinal]),
    CONSTRAINT [UX_ParentResourceExtensionRootExtChildren_RootExtChildCode_ParentResource_DocumentId] UNIQUE ([ParentResource_DocumentId], [RootExtChildCode])
);
...
IF OBJECT_ID(N'edfi.ParentResourceParentChildren', N'U') IS NULL
CREATE TABLE [edfi].[ParentResourceParentChildren]
(
    [CollectionItemId] bigint NOT NULL DEFAULT (NEXT VALUE FOR [dms].[CollectionItemIdSequence]),
    [Ordinal] int NOT NULL,
    ...
    CONSTRAINT [PK_ParentResourceParentChildren] PRIMARY KEY ([CollectionItemId]),
    CONSTRAINT [UX_ParentResourceParentChildren_ChildCode_ParentCollectionItemId] UNIQUE ([ParentCollectionItemId], [ChildCode]),
    CONSTRAINT [UX_ParentResourceParentChildren_Ordinal_ParentCollectionItemId] UNIQUE ([ParentCollectionItemId], [Ordinal])
);
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_ParentResourceExtension_ParentResource' AND parent_object_id = OBJECT_ID(N'rootext.ParentResourceExtension')
)
ALTER TABLE [rootext].[ParentResourceExtension]
ADD CONSTRAINT [FK_ParentResourceExtension_ParentResource]
FOREIGN KEY ([DocumentId])
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_ParentResourceExtensionRootExtChildren_ParentResourceExtension' AND parent_object_id = OBJECT_ID(N'rootext.ParentResourceExtensionRootExtChildren')
)
ALTER TABLE [rootext].[ParentResourceExtensionRootExtChildren]
ADD CONSTRAINT [FK_ParentResourceExtensionRootExtChildren_ParentResourceExtension]
FOREIGN KEY ([ParentResource_DocumentId])
REFERENCES [rootext].[ParentResourceExtension] ([DocumentId])
ON DELETE CASCADE
ON UPDATE NO ACTION;
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_ParentResourceParentChildren_ParentResourceParent' AND parent_object_id = OBJECT_ID(N'edfi.ParentResourceParentChildren')
)
ALTER TABLE [edfi].[ParentResourceParentChildren]
ADD CONSTRAINT [FK_ParentResourceParentChildren_ParentResourceParent]
...
CREATE OR ALTER TRIGGER [edfi].[TR_ParentResourceParentChildren_Stamp]
ON [edfi].[ParentResourceParentChildren]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER TRIGGER [rootext].[TR_ParentResourceExtension_Stamp]
ON [rootext].[ParentResourceExtension]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[DocumentId] = i.[DocumentId]
        WHERE del.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[RootExtHiddenScalar] AS varbinary(max)) <> CAST(del.[RootExtHiddenScalar] AS varbinary(max)) OR (i.[RootExtHiddenScalar] IS NULL AND del.[RootExtHiddenScalar] IS NOT NULL) OR (i.[RootExtHiddenScalar] IS NOT NULL AND del.[RootExtHiddenScalar] IS NULL)) OR (CAST(i.[RootExtVisibleScalar] AS varbinary(max)) <> CAST(del.[RootExtVisibleScalar] AS varbinary(max)) OR (i.[RootExtVisibleScalar] IS NULL AND del.[RootExtVisibleScalar] IS NOT NULL) OR (i.[RootExtVisibleScalar] IS NOT NULL AND del.[RootExtVisibleScalar] IS NULL))
        UNION
        SELECT del.[DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[DocumentId] = del.[DocumentId]
        WHERE i.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[RootExtHiddenScalar] AS varbinary(max)) <> CAST(del.[RootExtHiddenScalar] AS varbinary(max)) OR (i.[RootExtHiddenScalar] IS NULL AND del.[RootExtHiddenScalar] IS NOT NULL) OR (i.[RootExtHiddenScalar] IS NOT NULL AND del.[RootExtHiddenScalar] IS NULL)) OR (CAST(i.[RootExtVisibleScalar] AS varbinary(max)) <> CAST(del.[RootExtVisibleScalar] AS varbinary(max)) OR (i.[RootExtVisibleScalar] IS NULL AND del.[RootExtVisibleScalar] IS NOT NULL) OR (i.[RootExtVisibleScalar] IS NOT NULL AND del.[RootExtVisibleScalar] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
CREATE OR ALTER TRIGGER [rootext].[TR_ParentResourceExtensionRootExtChildren_Stamp]
ON [rootext].[ParentResourceExtensionRootExtChildren]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[CollectionItemId] = i.[CollectionItemId]
        WHERE del.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[ParentResource_DocumentId] <> del.[ParentResource_DocumentId] OR (i.[ParentResource_DocumentId] IS NULL AND del.[ParentResource_DocumentId] IS NOT NULL) OR (i.[ParentResource_DocumentId] IS NOT NULL AND del.[ParentResource_DocumentId] IS NULL)) OR (CAST(i.[RootExtChildCode] AS varbinary(max)) <> CAST(del.[RootExtChildCode] AS varbinary(max)) OR (i.[RootExtChildCode] IS NULL AND del.[RootExtChildCode] IS NOT NULL) OR (i.[RootExtChildCode] IS NOT NULL AND del.[RootExtChildCode] IS NULL)) OR (CAST(i.[RootExtChildValue] AS varbinary(max)) <> CAST(del.[RootExtChildValue] AS varbinary(max)) OR (i.[RootExtChildValue] IS NULL AND del.[RootExtChildValue] IS NOT NULL) OR (i.[RootExtChildValue] IS NOT NULL AND del.[RootExtChildValue] IS NULL))
        UNION
        SELECT del.[ParentResource_DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[CollectionItemId] = del.[CollectionItemId]
...
IF NOT EXISTS (SELECT 1 FROM [dms].[EffectiveSchema] WHERE [EffectiveSchemaSingletonId] = 1)
    ...
    VALUES (1, N'1.0.0', N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59', 1, 0xAA4516A2188A393B97F346BD6483E8A82E57AB430F5377D00B6409E307A812DC);
...
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59' AND [ProjectEndpointName] = N'ed-fi')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59', N'ed-fi', N'Ed-Fi', N'1.0.0', 0);
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59' AND [ProjectEndpointName] = N'root_ext')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59', N'root_ext', N'RootExt', N'1.0.0', 1);
...
SELECT @sc_actual_count = COUNT(*) FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59';
...
WHERE sc.[EffectiveSchemaHash] = N'd5ef03614de79ee4fb420fa29855da446a652984587c3ffe656267cd6bcc8a59'
AND NOT EXISTS (
    SELECT 1 FROM (VALUES
        (N'ed-fi', N'Ed-Fi', N'1.0.0', 0),
        (N'root_ext', N'RootExt', N'1.0.0', 1)
