...
BEGIN
    ...
    IF @preflight_stored_hash IS NOT NULL AND @preflight_stored_hash <> N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46'
    BEGIN
        DECLARE @preflight_msg nvarchar(500) = CONCAT(N'EffectiveSchemaHash mismatch: database has ''', @preflight_stored_hash, N''' but expected ''', N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46', N'''');
...
IF OBJECT_ID(N'edfi.Sponsor', N'U') IS NULL
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_ParentResourceExtensionParent_Sponsor_RefKey' AND parent_object_id = OBJECT_ID(N'aligned.ParentResourceExtensionParent')
)
ALTER TABLE [aligned].[ParentResourceExtensionParent]
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_Sponsor_Document' AND parent_object_id = OBJECT_ID(N'edfi.Sponsor')
)
ALTER TABLE [edfi].[Sponsor]
...
IF NOT EXISTS (
    ...
    JOIN sys.tables t ON i.object_id = t.object_id
    JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = N'aligned' AND t.name = N'ParentResourceExtensionParent' AND i.name = N'IX_ParentResourceExtensionParent_BaseCollectionItemId_ParentResource_DocumentId'
)
CREATE INDEX [IX_ParentResourceExtensionParent_BaseCollectionItemId_ParentResource_DocumentId] ON [aligned].[ParentResourceExtensionParent] ([BaseCollectionItemId], [ParentResource_DocumentId]);
...
CREATE OR ALTER TRIGGER [edfi].[TR_Sponsor_Stamp]
ON [edfi].[Sponsor]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    ...
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[DocumentId] = i.[DocumentId]
        WHERE del.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[SponsorName] AS varbinary(max)) <> CAST(del.[SponsorName] AS varbinary(max)) OR (i.[SponsorName] IS NULL AND del.[SponsorName] IS NOT NULL) OR (i.[SponsorName] IS NOT NULL AND del.[SponsorName] IS NULL))
        UNION
        SELECT del.[DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[DocumentId] = del.[DocumentId]
        WHERE i.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[SponsorName] AS varbinary(max)) <> CAST(del.[SponsorName] AS varbinary(max)) OR (i.[SponsorName] IS NULL AND del.[SponsorName] IS NOT NULL) OR (i.[SponsorName] IS NOT NULL AND del.[SponsorName] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
    INNER JOIN affectedDocs a ON d.[DocumentId] = a.[DocumentId];
    IF EXISTS (SELECT 1 FROM deleted) AND (UPDATE([SponsorName]))
    BEGIN
        UPDATE d
        SET d.[IdentityVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[IdentityLastModifiedAt] = sysutcdatetime()
        FROM [dms].[Document] d
...
IF NOT EXISTS (SELECT 1 FROM [dms].[ResourceKey] WHERE [ResourceKeyId] = 2)
    INSERT INTO [dms].[ResourceKey] ([ResourceKeyId], [ProjectName], [ResourceName], [ResourceVersion])
    VALUES (2, N'Ed-Fi', N'Sponsor', N'1.0.0');
...
IF NOT EXISTS (SELECT 1 FROM [dms].[EffectiveSchema] WHERE [EffectiveSchemaSingletonId] = 1)
    ...
    VALUES (1, N'1.0.0', N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46', 2, 0x5B23660DAD5043287A28FF5C5D048A8BA519B836EB51BDB5A02AEAE548BBDDE6);
...
BEGIN
    ...
    END
    IF @es_stored_hash <> 0x5B23660DAD5043287A28FF5C5D048A8BA519B836EB51BDB5A02AEAE548BBDDE6
    BEGIN
        DECLARE @es_hash_msg nvarchar(200) = CONCAT(N'dms.EffectiveSchema ResourceKeySeedHash mismatch: stored ', CONVERT(nvarchar(66), @es_stored_hash, 1), N' but expected ', CONVERT(nvarchar(66), 0x5B23660DAD5043287A28FF5C5D048A8BA519B836EB51BDB5A02AEAE548BBDDE6, 1));
        THROW 50000, @es_hash_msg, 1;
    END
END

-- SchemaComponent seed inserts (insert-if-missing)
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46' AND [ProjectEndpointName] = N'aligned')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46', N'aligned', N'Aligned', N'1.0.0', 1);
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46' AND [ProjectEndpointName] = N'ed-fi')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46', N'ed-fi', N'Ed-Fi', N'1.0.0', 0);
...
SELECT @sc_actual_count = COUNT(*) FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46';
...
WHERE sc.[EffectiveSchemaHash] = N'628937b1d59bfba2ee05f07eb5a91568107f8d3da5374d8a6fc1e309f0629f46'
AND NOT EXISTS (
    SELECT 1 FROM (VALUES
