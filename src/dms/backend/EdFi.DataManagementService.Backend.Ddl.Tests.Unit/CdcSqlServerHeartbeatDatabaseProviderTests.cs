...
public class Given_MssqlCdcHeartbeatDatabase_ValidateOnly
{
    ...
    [Test]
    public async Task MssqlCdcCaptureInstances_should_fail_closed_when_extra_dms_owned_capture_instances_are_present()
    {
        var executor = RecordingSqlServerCdcExecutor.WithExistingHeartbeatDatabase(
            captureJobPresent: true,
            cleanupJobPresent: true,
            captureInstances: SqlServerCaptureInstanceTestData
                ...
                .Append(RecordingSqlServerCaptureInstance.UnrelatedThirdPartyTable())
                .ToArray()
        );
        var service = new CdcProviderSetupService([new CdcSqlServerHeartbeatDatabaseProvider()]);
        ...
        result
            .Diagnostics.Should()
            .NotContain(diagnostic => diagnostic.SafeName.Value == "external_school_cdc");
        result
            .ArtifactInventory.Should()
            .Contain(observation =>
                observation.ArtifactKind == CdcProviderArtifactKind.SqlServerCaptureInstance
            ...
            .And.NotContain(observation => observation.SafeArtifactName.Value == "external_school_cdc");
        result
            .ProviderHistoryObservations.Should()
            .Contain(observation =>
                observation.ArtifactKind == CdcProviderArtifactKind.SqlServerCaptureInstance
        ...
        result.ManifestPayload.Json.Should().Contain("edfi_school_cdc");
        result.ManifestPayload.Json.Should().Contain("edfi.School");
...
{
    [Test]
    public async Task It_should_create_membership_and_grant_only_required_privileges_to_clean_existing_gating_role()
    {
        var executor = ExistingArtifactsWithoutConnectorGrants(
            new RecordingSqlServerConnectorAccess { GatingRoleExists = true }
        );
        var service = new CdcProviderSetupService([new CdcSqlServerHeartbeatDatabaseProvider()]);
    ...
    [TestCase("svc-cdc", "svc_cdc")]
    [TestCase("svc@tenant.com", "svc_tenant.com")]
    public async Task It_should_accept_connector_principal_names_after_diagnostic_sanitization(
        string connectorPrincipalName,
        string expectedSafeObservedMember
    )
    {
        var executor = RecordingSqlServerCdcExecutor.WithExistingHeartbeatDatabase(
            captureJobPresent: true,
            ...
            connectorAccess: new RecordingSqlServerConnectorAccess
            {
                ConnectorPrincipalName = connectorPrincipalName,
                GatingRoleExists = true,
                GatingRoleDirectMembers = [connectorPrincipalName],
                GatingRoleDirectMemberPrincipalIds = ["101"],
            }
        );
        var service = new CdcProviderSetupService([new CdcSqlServerHeartbeatDatabaseProvider()]);
        ...
        var result = await service.SetupAsync(
            CdcProviderSetupContractTestData.BuildSqlServerRequest(
                connectorPrincipalName: new CdcSafeName(connectorPrincipalName),
                databaseExecutor: executor
            )
        );
        ...
        executor
            ...
            .HaveCount(3);
        executor
            .ExecutedSql.Single(sql => sql.Contains("cdc:sqlserver:grant-connector-access"))
            .Should()
            .Contain($"ADD MEMBER [{connectorPrincipalName}]");
        result
            .ArtifactInventory.Should()
            .Contain(observation =>
                observation.ArtifactKind == CdcProviderArtifactKind.SqlServerGatingRole
                && observation.SafeObservedValues["gating_role_direct_members"] == expectedSafeObservedMember
            );
        ...
        manifestDocument
            ...
            .Should()
            .Contain(artifact =>
                artifact.GetProperty("artifact_kind").GetString() == "sqlserver_gating_role"
                && artifact
                    .GetProperty("observed_values")
                    .GetProperty("gating_role_direct_members")
                    .GetString() == expectedSafeObservedMember
            );
    }

    [Test]
    public async Task It_should_accept_case_drifted_connector_principal_when_sql_server_resolves_the_database_principal()
    {
        var executor = ExistingArtifactsWithConnectorAccess(
            RecordingSqlServerConnectorAccess.Exact("SVC_CDC")
        );
        var service = new CdcProviderSetupService([new CdcSqlServerHeartbeatDatabaseProvider()]);

        var result = await service.SetupAsync(
            CdcProviderSetupContractTestData.BuildSqlServerRequest(
                connectorPrincipalName: new CdcSafeName("svc_cdc"),
                databaseExecutor: executor
            )
        );
        ...
        result
            ...
            .Contain(observation =>
                observation.ArtifactKind == CdcProviderArtifactKind.SqlServerGatingRole
                && observation.SafeObservedValues["gating_role_direct_members"] == "SVC_CDC"
            );
        executor.ExecutedSql.Should().BeEmpty();
        executor
            .QueriedSql.Single(sql => sql.Contains("cdc:sqlserver:connector-principal-access"))
            .Should()
            .Contain("connector_principal_id")
            .And.Contain("gating_role_direct_member_principal_ids")
            .And.Contain("role_member.member_principal_id = connector.principal_id");
    }

    [Test]
    public async Task It_should_fail_closed_when_initial_setup_gating_role_is_missing_after_expected_capture_instances_exist()
    {
        await AssertGatingRoleMissingAfterExpectedCaptureInstancesFailsClosedAsync(
            CdcProviderSetupMode.InitialCreateOrExactMatch
        );
    }

    [Test]
    public async Task It_should_fail_closed_when_validate_only_gating_role_is_missing_after_expected_capture_instances_exist()
    {
        await AssertGatingRoleMissingAfterExpectedCaptureInstancesFailsClosedAsync(
    ...
    private static async Task AssertGatingRoleMissingAfterExpectedCaptureInstancesFailsClosedAsync(
    ...
    )
    {
        ...
        result
            ...
            .Contain(diagnostic =>
                ...
                && diagnostic.ArtifactKind == CdcProviderArtifactKind.SqlServerGatingRole
                && diagnostic.SafeName.Value == "dms_binding_gate"
                && diagnostic.ObservedValue!.Contains("missing-role-after-expected-captures")
            );
        result
            .ArtifactInventory.Should()
            .ContainSingle(observation =>
    ...
    [Test]
    public async Task It_should_reject_gating_role_missing_select_on_expected_cdc_schema_object()
    {
        var executor = ExistingArtifactsWithConnectorAccess(
            new RecordingSqlServerConnectorAccess
            {
        ...
        result
            ...
            .ContainSingle(diagnostic =>
                ...
                && diagnostic.ObservedValue!.Contains(
                    "missing_cdc_selects:cdc.fn_cdc_get_all_changes_dms_binding_document.SELECT"
                )
            );
        result
            .ArtifactInventory.Should()
...
{
    public string ConnectorPrincipalName { get; init; } = "connector_principal";
    ...
    public IReadOnlyList<string>? GatingRoleDirectMemberPrincipalIds { get; set; }
    ...
    public static RecordingSqlServerConnectorAccess Exact(
        string connectorPrincipalName = "connector_principal"
    ) =>
        new()
        {
            ConnectorPrincipalName = connectorPrincipalName,
        ...
        ICdcProviderErrorIdentityMapper
{
    ...
    public Task ExecuteNonQueryAsync(string sql, CancellationToken cancellationToken)
    {
        ...
        if (sql.Contains("cdc:sqlserver:grant-connector-access"))
        {
            ...
            _connectorAccess.GatingRoleDirectMemberPrincipalIds = [_connectorAccess.ConnectorPrincipalId];
    ...
    {
        var expectedCdcObjectSelects = ExpectedGatingRoleCdcObjectSelects();
        var missingCdcObjectSelects = MissingGatingRoleCdcObjectSelects(expectedCdcObjectSelects);
        var observedCdcObjectSelects = expectedCdcObjectSelects
            .Except(missingCdcObjectSelects, StringComparer.Ordinal)
            .ToArray();
        ...
        return Row(
            ...
            ("connector_principal_id", ConnectorPrincipalId()),
            ...
            (
                "gating_role_member",
                GatingRoleDirectMemberPrincipalIds()
                    .Contains(_connectorAccess.ConnectorPrincipalId, StringComparer.Ordinal)
                    .ToString()
            ),
            ("gating_role_direct_members", Csv(_connectorAccess.GatingRoleDirectMembers)),
            ("gating_role_direct_member_principal_ids", Csv(GatingRoleDirectMemberPrincipalIds())),
            ...
            ("expected_cdc_object_count", expectedCdcObjectSelects.Count.ToString()),
            ("gating_role_cdc_object_select_count", observedCdcObjectSelects.Length.ToString()),
            ("gating_role_cdc_object_selects", Csv(observedCdcObjectSelects)),
            ("missing_gating_role_cdc_object_selects", Csv(missingCdcObjectSelects)),
    ...
    private string ConnectorPrincipalId() =>
        _connectorAccess.ConnectorExists && _connectorAccess.ConnectorIsDatabasePrincipal
            ? _connectorAccess.ConnectorPrincipalId
            : "";

    private IReadOnlyList<string> GatingRoleDirectMemberPrincipalIds()
    {
        if (_connectorAccess.GatingRoleDirectMemberPrincipalIds is { } configuredIds)
        {
            return configuredIds;
        }

        var nextPrincipalId = 200;
        return _connectorAccess
            .GatingRoleDirectMembers.Select(member =>
                string.Equals(member, _connectorAccess.ConnectorPrincipalName, StringComparison.Ordinal)
                    ? _connectorAccess.ConnectorPrincipalId
                    : (nextPrincipalId++).ToString()
            )
            .ToArray();
    }
    ...
    private IReadOnlyList<string> ExpectedGatingRoleCdcObjectSelects()
    {
        var expected = SqlServerCaptureInstanceTestData
            .Expected()
            .Select(capture => capture.CaptureInstanceName.Value)
    ...
    private IReadOnlyList<string> MissingGatingRoleCdcObjectSelects(
        IReadOnlyList<string> expectedCdcObjectSelects
    )
    {
        if (!_connectorAccess.GatingRoleExists)
        {
            return expectedCdcObjectSelects;
        }

        var missing = _connectorAccess.MissingGatingRoleCdcObjectSelects.ToHashSet(StringComparer.Ordinal);

        return expectedCdcObjectSelects.Where(missing.Contains).Order(StringComparer.Ordinal).ToArray();
    }
    ...
    private IReadOnlyList<IReadOnlyDictionary<string, string?>> CaptureInstanceRows()
    {
        ...
        foreach (
            var capture in _captureInstances
                .Values.Where(IsInSqlServerCdcProviderValidationScope)
    ...
    private static bool IsInSqlServerCdcProviderValidationScope(RecordingSqlServerCaptureInstance capture)
    {
        var expectedCaptureInstanceNames = SqlServerCaptureInstanceTestData
            .Expected()
            .Select(expected => expected.CaptureInstanceName.Value)
            .ToHashSet(StringComparer.Ordinal);
        var sourceSchema = capture.SourceTable.Schema.Value;

        return expectedCaptureInstanceNames.Contains(capture.CaptureInstanceName.Value)
            || string.Equals(capture.GatingRoleName.Value, "dms_binding_gate", StringComparison.Ordinal)
...
)
{
    ...
    public static RecordingSqlServerCaptureInstance UnrelatedThirdPartyTable() =>
        new(
            "unexpected",
            new DbTableName(new DbSchemaName("external"), "School"),
            new CdcSafeName("external_school_cdc"),
            new CdcSafeName("other_cdc_gate"),
            ["SchoolId", "NameOfInstitution"]
        );
