...
BEGIN
    ...
    BEGIN
        ...
        SELECT [dms].[uuidv5]('edf1edf1-3df1-3df1-3df1-3df1edf1edf1', CAST(N'Ed-FiSchool' AS nvarchar(max)) + N'$$.schoolId=' + N'$' + CAST(i.[SchoolId] AS nvarchar(max))), i.[DocumentId], 1
        FROM inserted i;
    END
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO [dms].[ReferentialIdentity] ([ReferentialId], [DocumentId], [ResourceKeyId])
        SELECT [dms].[uuidv5]('edf1edf1-3df1-3df1-3df1-3df1edf1edf1', CAST(N'Ed-FiStudentSchoolAssociation' AS nvarchar(max)) + N'$$.schoolReference.schoolId=' + N'$' + CAST(i.[SchoolId] AS nvarchar(max)) + N'#' + N'$$.studentReference.studentUniqueId=' + N'$' + i.[StudentUniqueId] + N'#' + N'$$.entryDate=' + N'$' + CONVERT(nvarchar(10), i.[EntryDate], 23) + N'#' + N'$$.entryTimestamp=' + N'$' + CONVERT(nvarchar(19), i.[EntryTimestamp], 126) + N'Z' + N'#' + N'$$.isActive=' + N'$' + CASE WHEN i.[IsActive] = 1 THEN N'true' ELSE N'false' END), i.[DocumentId], 2
        FROM inserted i;
    END
