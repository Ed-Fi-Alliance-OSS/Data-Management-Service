...
BEGIN
    ...
    LEFT JOIN deleted del ON del.[DocumentId] = i.[DocumentId]
    WHERE del.[DocumentId] IS NULL;
    IF EXISTS (SELECT 1 FROM deleted) AND (NOT EXISTS (SELECT 1 FROM inserted) OR UPDATE([DocumentId]) OR UPDATE([NamingStressItemId]) OR UPDATE([Order]) OR UPDATE([ShortName]) OR UPDATE([ThisIsAVeryLongFieldNameThatWillBeTruncatedByPostgreSQLAtSixtyThree]) OR UPDATE([ThisIsAVeryLongFieldNameThatWillBeTruncatedByPostgreSQLAtSixtyThreeX]) OR UPDATE([VeryLongIdentifierNameThatExceedsSixtyThreeCharactersInPostgreSQL]))
    BEGIN
        ;WITH affectedDocs AS (
            SELECT i.[DocumentId]
            FROM inserted i
    ...
    BEGIN
        ...
        SET r.[ContentVersion] = s.[ContentVersion],
            r.[ContentLastModifiedAt] = s.[ContentLastModifiedAt]
        FROM [edfi].[NamingStressItem] r WITH (FORCESEEK)
        INNER JOIN @stamped s ON s.[DocumentId] = r.[DocumentId];
    END
