...
BEGIN
    IF to_regclass('"dms"."EffectiveSchema"') IS NOT NULL THEN
        ...
        IF _stored_hash IS NOT NULL AND _stored_hash <> '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190' THEN
            RAISE EXCEPTION 'EffectiveSchemaHash mismatch: database has ''%'' but expected ''%''', _stored_hash, '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190';
        END IF;
    END IF;
END $$;
...
BEGIN
    IF to_regclass('"dms"."EffectiveSchema"') IS NOT NULL THEN
        SELECT "EffectiveSchemaHash" INTO _stored_hash FROM "dms"."EffectiveSchema"
        WHERE "EffectiveSchemaSingletonId" = 1;
        IF _stored_hash = '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190' THEN
...
(
    ...
    "ThisIsAVeryLongFieldNameThatWillBeTruncatedByPostgre_026b941dbf" varchar(100) NULL,
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'FK_NamingStressItem_Document'
        AND conrelid = to_regclass('"edfi"."NamingStressItem"')
    )
    THEN
...
DROP TRIGGER IF EXISTS "TR_NamingStressItem_Stamp" ON "edfi"."NamingStressItem";
CREATE TRIGGER "TR_NamingStressItem_Stamp"
BEFORE INSERT OR UPDATE OR DELETE ON "edfi"."NamingStressItem"
FOR EACH ROW
EXECUTE FUNCTION "edfi"."TF_TR_NamingStressItem_Stamp"();
...
VALUES (1, 'Ed-Fi', 'NamingStressItem', '5.0.0')
...
BEGIN
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM (VALUES
            (1::smallint, 'Ed-Fi', 'NamingStressItem', '5.0.0')
...
VALUES (1, '1.0.0', '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190', 1, '\xECD1439457B6F0560A20DF2668B314E42DCF6079592558C4695F2B299DFBC301'::bytea)
...
BEGIN
    ...
    IF _stored_count IS NOT NULL THEN
        ...
        END IF;
        IF _stored_hash <> '\xECD1439457B6F0560A20DF2668B314E42DCF6079592558C4695F2B299DFBC301'::bytea THEN
            RAISE EXCEPTION 'dms.EffectiveSchema ResourceKeySeedHash mismatch: stored % but expected %', encode(_stored_hash, 'hex'), encode('\xECD1439457B6F0560A20DF2668B314E42DCF6079592558C4695F2B299DFBC301'::bytea, 'hex');
        END IF;
    END IF;
END $$;
...
VALUES ('8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190', 'ed-fi', 'Ed-Fi', '5.0.0', false)
...
BEGIN
    SELECT COUNT(*) INTO _actual_count FROM "dms"."SchemaComponent" WHERE "EffectiveSchemaHash" = '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190';
    IF _actual_count <> 1 THEN
    ...
    WHERE sc."EffectiveSchemaHash" = '8cc3d6f59e878565cdccda12f77af46a37183ce5634cc317ed4e332aadc8a190'
    AND NOT EXISTS (
        SELECT 1 FROM (VALUES
