...
BEGIN
    IF to_regclass('"dms"."EffectiveSchema"') IS NOT NULL THEN
        ...
        IF _stored_hash IS NOT NULL AND _stored_hash <> 'b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810' THEN
            RAISE EXCEPTION 'EffectiveSchemaHash mismatch: database has ''%'' but expected ''%''', _stored_hash, 'b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810';
        END IF;
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS "edfi"."SchoolProgram"
(
    ...
    "ProgramReference_ProgramId" integer NULL,
    "ProgramReference_ProgramName" varchar(60) NULL,
    ...
    CONSTRAINT "UX_SchoolProgram_ProgramReference_ProgramId_ProgramN_727882f58e" UNIQUE ("School_DocumentId", "ProgramReference_ProgramId", "ProgramReference_ProgramName"),
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'FK_SchoolProgram_ProgramReference_RefKey'
        AND conrelid = to_regclass('"edfi"."SchoolProgram"')
    )
    THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'FK_SchoolProgram_School'
        AND conrelid = to_regclass('"edfi"."SchoolProgram"')
    )
    THEN
        ALTER TABLE "edfi"."SchoolProgram"
...
RETURNS TRIGGER AS $func$
BEGIN
    IF TG_OP = 'INSERT' OR (OLD."ProgramId" IS DISTINCT FROM NEW."ProgramId" OR OLD."ProgramName" IS DISTINCT FROM NEW."ProgramName") THEN
        DELETE FROM "dms"."ReferentialIdentity"
        WHERE "DocumentId" = NEW."DocumentId" AND "ResourceKeyId" = 1;
        INSERT INTO "dms"."ReferentialIdentity" ("ReferentialId", "DocumentId", "ResourceKeyId")
        VALUES ("dms"."uuidv5"('edf1edf1-3df1-3df1-3df1-3df1edf1edf1'::uuid, 'Ed-FiProgram' || '$.programId=' || NEW."ProgramId"::text || '#' || '$.programName=' || NEW."ProgramName"::text), NEW."DocumentId", 1);
    END IF;
    RETURN NEW;
END;
$func$ LANGUAGE plpgsql;
...
BEGIN
    ...
    END IF;
    IF TG_OP = 'UPDATE' AND (OLD."ProgramId" IS DISTINCT FROM NEW."ProgramId" OR OLD."ProgramName" IS DISTINCT FROM NEW."ProgramName") THEN
        UPDATE "dms"."Document"
...
VALUES (1, '1.0.0', 'b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810', 2, '\x3F4100845FD234C31689525E02A344E042EC3E11C81EF4405143942137997C00'::bytea)
...
VALUES ('b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810', 'ed-fi', 'Ed-Fi', '1.0.0', false)
...
BEGIN
    SELECT COUNT(*) INTO _actual_count FROM "dms"."SchemaComponent" WHERE "EffectiveSchemaHash" = 'b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810';
    ...
    WHERE sc."EffectiveSchemaHash" = 'b2f94938fc758230fc8cf8b75a483cfe39b39558365fa15c8b711ff16fdff810'
    AND NOT EXISTS (
        SELECT 1 FROM (VALUES
