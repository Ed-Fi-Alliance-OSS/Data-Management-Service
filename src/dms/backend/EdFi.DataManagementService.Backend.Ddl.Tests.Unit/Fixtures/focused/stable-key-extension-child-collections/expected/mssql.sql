...
BEGIN
    ...
    IF @preflight_stored_hash IS NOT NULL AND @preflight_stored_hash <> N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea'
    BEGIN
        DECLARE @preflight_msg nvarchar(500) = CONCAT(N'EffectiveSchemaHash mismatch: database has ''', @preflight_stored_hash, N''' but expected ''', N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea', N'''');
...
BEGIN
    ...
    ;WITH affectedDocs AS (
        ...
        WHERE i.[DocumentId] IS NULL OR (CAST(i.[Namespace] AS varbinary(max)) <> CAST(del.[Namespace] AS varbinary(max)) OR (i.[Namespace] IS NULL AND del.[Namespace] IS NOT NULL) OR (i.[Namespace] IS NOT NULL AND del.[Namespace] IS NULL)) OR (CAST(i.[CodeValue] AS varbinary(max)) <> CAST(del.[CodeValue] AS varbinary(max)) OR (i.[CodeValue] IS NULL AND del.[CodeValue] IS NOT NULL) OR (i.[CodeValue] IS NOT NULL AND del.[CodeValue] IS NULL)) OR (CAST(i.[ShortDescription] AS varbinary(max)) <> CAST(del.[ShortDescription] AS varbinary(max)) OR (i.[ShortDescription] IS NULL AND del.[ShortDescription] IS NOT NULL) OR (i.[ShortDescription] IS NOT NULL AND del.[ShortDescription] IS NULL)) OR (CAST(i.[Description] AS varbinary(max)) <> CAST(del.[Description] AS varbinary(max)) OR (i.[Description] IS NULL AND del.[Description] IS NOT NULL) OR (i.[Description] IS NOT NULL AND del.[Description] IS NULL)) OR (i.[EffectiveBeginDate] <> del.[EffectiveBeginDate] OR (i.[EffectiveBeginDate] IS NULL AND del.[EffectiveBeginDate] IS NOT NULL) OR (i.[EffectiveBeginDate] IS NOT NULL AND del.[EffectiveBeginDate] IS NULL)) OR (i.[EffectiveEndDate] <> del.[EffectiveEndDate] OR (i.[EffectiveEndDate] IS NULL AND del.[EffectiveEndDate] IS NOT NULL) OR (i.[EffectiveEndDate] IS NOT NULL AND del.[EffectiveEndDate] IS NULL)) OR (CAST(i.[Discriminator] AS varbinary(max)) <> CAST(del.[Discriminator] AS varbinary(max)) OR (i.[Discriminator] IS NULL AND del.[Discriminator] IS NOT NULL) OR (i.[Discriminator] IS NOT NULL AND del.[Discriminator] IS NULL)) OR (CAST(i.[Uri] AS varbinary(max)) <> CAST(del.[Uri] AS varbinary(max)) OR (i.[Uri] IS NULL AND del.[Uri] IS NOT NULL) OR (i.[Uri] IS NOT NULL AND del.[Uri] IS NULL))
    )
    UPDATE d
...
(
    ...
    CONSTRAINT [UX_SchoolExtensionAddress_BaseCollectionItemId_School_DocumentId] UNIQUE ([BaseCollectionItemId], [School_DocumentId])
);

IF OBJECT_ID(N'sample.SchoolExtensionIntervention', N'U') IS NULL
CREATE TABLE [sample].[SchoolExtensionIntervention]
(
    [CollectionItemId] bigint NOT NULL DEFAULT (NEXT VALUE FOR [dms].[CollectionItemIdSequence]),
    [Ordinal] int NOT NULL,
    [School_DocumentId] bigint NOT NULL,
    [InterventionCode] nvarchar(20) NULL,
    CONSTRAINT [PK_SchoolExtensionIntervention] PRIMARY KEY ([CollectionItemId]),
    CONSTRAINT [UX_SchoolExtensionIntervention_CollectionItemId_School_DocumentId] UNIQUE ([CollectionItemId], [School_DocumentId]),
    CONSTRAINT [UX_SchoolExtensionIntervention_InterventionCode_School_DocumentId] UNIQUE ([School_DocumentId], [InterventionCode]),
...
IF OBJECT_ID(N'sample.SchoolExtensionAddressSponsorReference', N'U') IS NULL
CREATE TABLE [sample].[SchoolExtensionAddressSponsorReference]
(
    [CollectionItemId] bigint NOT NULL DEFAULT (NEXT VALUE FOR [dms].[CollectionItemIdSequence]),
    [BaseCollectionItemId] bigint NOT NULL,
    [Ordinal] int NOT NULL,
...
IF OBJECT_ID(N'sample.SchoolExtensionInterventionVisit', N'U') IS NULL
CREATE TABLE [sample].[SchoolExtensionInterventionVisit]
(
    [CollectionItemId] bigint NOT NULL DEFAULT (NEXT VALUE FOR [dms].[CollectionItemIdSequence]),
    [Ordinal] int NOT NULL,
    [ParentCollectionItemId] bigint NOT NULL,
    [School_DocumentId] bigint NOT NULL,
    [VisitCode] nvarchar(20) NULL,
    CONSTRAINT [PK_SchoolExtensionInterventionVisit] PRIMARY KEY ([CollectionItemId]),
    CONSTRAINT [UX_SchoolExtensionInterventionVisit_Ordinal_ParentCollectionItemId] UNIQUE ([ParentCollectionItemId], [Ordinal]),
    CONSTRAINT [UX_SchoolExtensionInterventionVisit_ParentCollectionItemId_VisitCode] UNIQUE ([ParentCollectionItemId], [VisitCode])
);
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolExtensionAddress_SchoolAddress' AND parent_object_id = OBJECT_ID(N'sample.SchoolExtensionAddress')
)
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolExtensionIntervention_SchoolExtension' AND parent_object_id = OBJECT_ID(N'sample.SchoolExtensionIntervention')
)
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolExtensionAddressSponsorReference_Program_RefKey' AND parent_object_id = OBJECT_ID(N'sample.SchoolExtensionAddressSponsorReference')
)
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolExtensionAddressSponsorReference_SchoolExtensionAddress' AND parent_object_id = OBJECT_ID(N'sample.SchoolExtensionAddressSponsorReference')
)
ALTER TABLE [sample].[SchoolExtensionAddressSponsorReference]
ADD CONSTRAINT [FK_SchoolExtensionAddressSponsorReference_SchoolExtensionAddress]
FOREIGN KEY ([BaseCollectionItemId], [School_DocumentId])
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolExtensionInterventionVisit_SchoolExtensionIntervention' AND parent_object_id = OBJECT_ID(N'sample.SchoolExtensionInterventionVisit')
)
...
IF NOT EXISTS (
    SELECT 1 FROM sys.foreign_keys
    WHERE name = N'FK_SchoolAddressPeriod_SchoolAddress' AND parent_object_id = OBJECT_ID(N'edfi.SchoolAddressPeriod')
)
...
IF NOT EXISTS (
    ...
    JOIN sys.tables t ON i.object_id = t.object_id
    JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = N'edfi' AND t.name = N'SchoolAddressPeriod' AND i.name = N'IX_SchoolAddressPeriod_ParentCollectionItemId_School_DocumentId'
)
CREATE INDEX [IX_SchoolAddressPeriod_ParentCollectionItemId_School_DocumentId] ON [edfi].[SchoolAddressPeriod] ([ParentCollectionItemId], [School_DocumentId]);

IF NOT EXISTS (
    SELECT 1 FROM sys.indexes i
    JOIN sys.tables t ON i.object_id = t.object_id
    JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = N'sample' AND t.name = N'SchoolExtensionAddressSponsorReference' AND i.name = N'IX_SchoolExtensionAddressSponsorReference_BaseCollectionItemId_School_DocumentId'
)
CREATE INDEX [IX_SchoolExtensionAddressSponsorReference_BaseCollectionItemId_School_DocumentId] ON [sample].[SchoolExtensionAddressSponsorReference] ([BaseCollectionItemId], [School_DocumentId]);
...
IF NOT EXISTS (
    ...
    JOIN sys.tables t ON i.object_id = t.object_id
    JOIN sys.schemas s ON t.schema_id = s.schema_id
    WHERE s.name = N'sample' AND t.name = N'SchoolExtensionInterventionVisit' AND i.name = N'IX_SchoolExtensionInterventionVisit_ParentCollectionItemId_School_DocumentId'
)
CREATE INDEX [IX_SchoolExtensionInterventionVisit_ParentCollectionItemId_School_DocumentId] ON [sample].[SchoolExtensionInterventionVisit] ([ParentCollectionItemId], [School_DocumentId]);
...
BEGIN
    ...
    BEGIN
        ...
        SELECT [dms].[uuidv5]('edf1edf1-3df1-3df1-3df1-3df1edf1edf1', CAST(N'Ed-FiProgram' AS nvarchar(max)) + N'$.programName=' + i.[ProgramName]), i.[DocumentId], 1
        FROM inserted i;
    END
...
BEGIN
    ...
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[DocumentId] = i.[DocumentId]
        WHERE del.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[ProgramName] AS varbinary(max)) <> CAST(del.[ProgramName] AS varbinary(max)) OR (i.[ProgramName] IS NULL AND del.[ProgramName] IS NOT NULL) OR (i.[ProgramName] IS NOT NULL AND del.[ProgramName] IS NULL))
        UNION
        SELECT del.[DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[DocumentId] = del.[DocumentId]
        WHERE i.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[ProgramName] AS varbinary(max)) <> CAST(del.[ProgramName] AS varbinary(max)) OR (i.[ProgramName] IS NULL AND del.[ProgramName] IS NOT NULL) OR (i.[ProgramName] IS NOT NULL AND del.[ProgramName] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
    INNER JOIN affectedDocs a ON d.[DocumentId] = a.[DocumentId];
    IF EXISTS (SELECT 1 FROM deleted) AND (UPDATE([ProgramName]))
    BEGIN
        UPDATE d
...
BEGIN
    ...
    ;WITH affectedDocs AS (
        ...
        WHERE del.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[City] AS varbinary(max)) <> CAST(del.[City] AS varbinary(max)) OR (i.[City] IS NULL AND del.[City] IS NOT NULL) OR (i.[City] IS NOT NULL AND del.[City] IS NULL))
        UNION
        SELECT del.[School_DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[CollectionItemId] = del.[CollectionItemId]
        WHERE i.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[City] AS varbinary(max)) <> CAST(del.[City] AS varbinary(max)) OR (i.[City] IS NULL AND del.[City] IS NOT NULL) OR (i.[City] IS NOT NULL AND del.[City] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
CREATE OR ALTER TRIGGER [edfi].[TR_SchoolAddressPeriod_Stamp]
ON [edfi].[SchoolAddressPeriod]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[CollectionItemId] = i.[CollectionItemId]
        WHERE del.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[ParentCollectionItemId] <> del.[ParentCollectionItemId] OR (i.[ParentCollectionItemId] IS NULL AND del.[ParentCollectionItemId] IS NOT NULL) OR (i.[ParentCollectionItemId] IS NOT NULL AND del.[ParentCollectionItemId] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[PeriodName] AS varbinary(max)) <> CAST(del.[PeriodName] AS varbinary(max)) OR (i.[PeriodName] IS NULL AND del.[PeriodName] IS NOT NULL) OR (i.[PeriodName] IS NOT NULL AND del.[PeriodName] IS NULL))
        UNION
        SELECT del.[School_DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[CollectionItemId] = del.[CollectionItemId]
        WHERE i.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[ParentCollectionItemId] <> del.[ParentCollectionItemId] OR (i.[ParentCollectionItemId] IS NULL AND del.[ParentCollectionItemId] IS NOT NULL) OR (i.[ParentCollectionItemId] IS NOT NULL AND del.[ParentCollectionItemId] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[PeriodName] AS varbinary(max)) <> CAST(del.[PeriodName] AS varbinary(max)) OR (i.[PeriodName] IS NULL AND del.[PeriodName] IS NOT NULL) OR (i.[PeriodName] IS NOT NULL AND del.[PeriodName] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
BEGIN
    ...
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[DocumentId] = i.[DocumentId]
        WHERE del.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[CampusCode] AS varbinary(max)) <> CAST(del.[CampusCode] AS varbinary(max)) OR (i.[CampusCode] IS NULL AND del.[CampusCode] IS NOT NULL) OR (i.[CampusCode] IS NOT NULL AND del.[CampusCode] IS NULL))
        UNION
        SELECT del.[DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[DocumentId] = del.[DocumentId]
        WHERE i.[DocumentId] IS NULL OR (i.[DocumentId] <> del.[DocumentId] OR (i.[DocumentId] IS NULL AND del.[DocumentId] IS NOT NULL) OR (i.[DocumentId] IS NOT NULL AND del.[DocumentId] IS NULL)) OR (CAST(i.[CampusCode] AS varbinary(max)) <> CAST(del.[CampusCode] AS varbinary(max)) OR (i.[CampusCode] IS NULL AND del.[CampusCode] IS NOT NULL) OR (i.[CampusCode] IS NOT NULL AND del.[CampusCode] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
CREATE OR ALTER TRIGGER [sample].[TR_SchoolExtensionAddress_Stamp]
ON [sample].[SchoolExtensionAddress]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        WHERE del.[BaseCollectionItemId] IS NULL OR (i.[BaseCollectionItemId] <> del.[BaseCollectionItemId] OR (i.[BaseCollectionItemId] IS NULL AND del.[BaseCollectionItemId] IS NOT NULL) OR (i.[BaseCollectionItemId] IS NOT NULL AND del.[BaseCollectionItemId] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[Zone] AS varbinary(max)) <> CAST(del.[Zone] AS varbinary(max)) OR (i.[Zone] IS NULL AND del.[Zone] IS NOT NULL) OR (i.[Zone] IS NOT NULL AND del.[Zone] IS NULL))
        UNION
        SELECT del.[School_DocumentId]
        FROM deleted del
...
CREATE OR ALTER TRIGGER [sample].[TR_SchoolExtensionAddressSponsorReference_Stamp]
ON [sample].[SchoolExtensionAddressSponsorReference]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER TRIGGER [sample].[TR_SchoolExtensionIntervention_Stamp]
ON [sample].[SchoolExtensionIntervention]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[CollectionItemId] = i.[CollectionItemId]
        WHERE del.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[InterventionCode] AS varbinary(max)) <> CAST(del.[InterventionCode] AS varbinary(max)) OR (i.[InterventionCode] IS NULL AND del.[InterventionCode] IS NOT NULL) OR (i.[InterventionCode] IS NOT NULL AND del.[InterventionCode] IS NULL))
        UNION
        SELECT del.[School_DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[CollectionItemId] = del.[CollectionItemId]
        WHERE i.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[InterventionCode] AS varbinary(max)) <> CAST(del.[InterventionCode] AS varbinary(max)) OR (i.[InterventionCode] IS NULL AND del.[InterventionCode] IS NOT NULL) OR (i.[InterventionCode] IS NOT NULL AND del.[InterventionCode] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
CREATE OR ALTER TRIGGER [sample].[TR_SchoolExtensionInterventionVisit_Stamp]
ON [sample].[SchoolExtensionInterventionVisit]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ;WITH affectedDocs AS (
        ...
        FROM inserted i
        LEFT JOIN deleted del ON del.[CollectionItemId] = i.[CollectionItemId]
        WHERE del.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[ParentCollectionItemId] <> del.[ParentCollectionItemId] OR (i.[ParentCollectionItemId] IS NULL AND del.[ParentCollectionItemId] IS NOT NULL) OR (i.[ParentCollectionItemId] IS NOT NULL AND del.[ParentCollectionItemId] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[VisitCode] AS varbinary(max)) <> CAST(del.[VisitCode] AS varbinary(max)) OR (i.[VisitCode] IS NULL AND del.[VisitCode] IS NOT NULL) OR (i.[VisitCode] IS NOT NULL AND del.[VisitCode] IS NULL))
        UNION
        SELECT del.[School_DocumentId]
        FROM deleted del
        LEFT JOIN inserted i ON i.[CollectionItemId] = del.[CollectionItemId]
        WHERE i.[CollectionItemId] IS NULL OR (i.[CollectionItemId] <> del.[CollectionItemId] OR (i.[CollectionItemId] IS NULL AND del.[CollectionItemId] IS NOT NULL) OR (i.[CollectionItemId] IS NOT NULL AND del.[CollectionItemId] IS NULL)) OR (i.[Ordinal] <> del.[Ordinal] OR (i.[Ordinal] IS NULL AND del.[Ordinal] IS NOT NULL) OR (i.[Ordinal] IS NOT NULL AND del.[Ordinal] IS NULL)) OR (i.[ParentCollectionItemId] <> del.[ParentCollectionItemId] OR (i.[ParentCollectionItemId] IS NULL AND del.[ParentCollectionItemId] IS NOT NULL) OR (i.[ParentCollectionItemId] IS NOT NULL AND del.[ParentCollectionItemId] IS NULL)) OR (i.[School_DocumentId] <> del.[School_DocumentId] OR (i.[School_DocumentId] IS NULL AND del.[School_DocumentId] IS NOT NULL) OR (i.[School_DocumentId] IS NOT NULL AND del.[School_DocumentId] IS NULL)) OR (CAST(i.[VisitCode] AS varbinary(max)) <> CAST(del.[VisitCode] AS varbinary(max)) OR (i.[VisitCode] IS NULL AND del.[VisitCode] IS NOT NULL) OR (i.[VisitCode] IS NOT NULL AND del.[VisitCode] IS NULL))
    )
    UPDATE d
    SET d.[ContentVersion] = NEXT VALUE FOR [dms].[ChangeVersionSequence], d.[ContentLastModifiedAt] = sysutcdatetime()
    FROM [dms].[Document] d
...
IF NOT EXISTS (SELECT 1 FROM [dms].[EffectiveSchema] WHERE [EffectiveSchemaSingletonId] = 1)
    ...
    VALUES (1, N'1.0.0', N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea', 2, 0x3F4100845FD234C31689525E02A344E042EC3E11C81EF4405143942137997C00);
...
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea' AND [ProjectEndpointName] = N'ed-fi')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea', N'ed-fi', N'Ed-Fi', N'1.0.0', 0);
IF NOT EXISTS (SELECT 1 FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea' AND [ProjectEndpointName] = N'sample')
    INSERT INTO [dms].[SchemaComponent] ([EffectiveSchemaHash], [ProjectEndpointName], [ProjectName], [ProjectVersion], [IsExtensionProject])
    VALUES (N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea', N'sample', N'Sample', N'1.0.0', 1);
...
SELECT @sc_actual_count = COUNT(*) FROM [dms].[SchemaComponent] WHERE [EffectiveSchemaHash] = N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea';
...
WHERE sc.[EffectiveSchemaHash] = N'60037ca0d1357cab3f3c898dba14e939e3b12f2d01fcc7cb62364733958827ea'
AND NOT EXISTS (
    SELECT 1 FROM (VALUES
