...
file sealed class KeyUnifAllowAllResourceAuthorizationHandler : IResourceAuthorizationHandler
{
...
file sealed class KeyUnifNoOpUpdateCascadeHandler : IUpdateCascadeHandler
{
...
file sealed class KeyUnifHostApplicationLifetime : IHostApplicationLifetime
{
...
internal sealed record KeyUnifDocumentRow(
    long DocumentId,
    Guid DocumentUuid,
    short ResourceKeyId,
    long ContentVersion
...
internal sealed record KeyUnifWidgetRow(
    long DocumentId,
    int WidgetId,
    string? Name,
...
internal sealed record KeyUnifPersistedState(
    KeyUnifDocumentRow Document,
    KeyUnifWidgetRow Widget,
    long DocumentCount
);

file static class KeyUnifTestSupport
{
    ...
    public static ServiceProvider CreateServiceProvider()
    {
        ...
        services.AddSingleton<IHostApplicationLifetime, KeyUnifHostApplicationLifetime>();
    ...
    public static UpsertRequest CreateCreateRequest(
        ...
        new(
            ...
            UpdateCascadeHandler: new KeyUnifNoOpUpdateCascadeHandler(),
            ResourceAuthorizationHandler: new KeyUnifAllowAllResourceAuthorizationHandler(),
    ...
    public static async Task<KeyUnifPersistedState> ReadFullPersistedStateAsync(
    ...
    )
    {
        ...
        return new KeyUnifPersistedState(document, widget, documentCount);
    }
    ...
    private static async Task<KeyUnifDocumentRow> ReadDocumentAsync(
        PostgresqlGeneratedDdlTestDatabase database,
        Guid documentUuid
    )
    {
    ...
    private static async Task<KeyUnifWidgetRow> ReadWidgetAsync(
        PostgresqlGeneratedDdlTestDatabase database,
        long documentId
    )
    {
...
file sealed class KeyUnifFixedStoredStateProjectionInvoker(
...
file sealed class KeyUnifUnexpectedStoredStateProjectionInvoker : IStoredStateProjectionInvoker
{
    public ProfileAppliedWriteContext ProjectStoredState(
        JsonNode storedDocument,
        ProfileAppliedWriteRequest request,
...
file static class KeyUnifContextFactory
{
    ...
    )
    {
        ...
        return new BackendProfileWriteContext(
            ...
            StoredStateProjectionInvoker: new KeyUnifFixedStoredStateProjectionInvoker(
                storedScopeStates:
                [
...
[NonParallelizable]
public class Given_A_Postgresql_Key_Unified_Canonical_Preservation
{
    private static readonly DocumentUuid WidgetDocumentUuid = new(
        Guid.Parse("dddddddd-0000-0000-0000-000000000301")
    );
    ...
    private KeyUnifPersistedState _stateAfterUpdate = null!;
    ...
    public async Task Setup()
    {
        _fixture = PostgresqlGeneratedDdlFixtureLoader.LoadFromRepositoryRelativePath(
            KeyUnifTestSupport.FixtureRelativePath
        );
        _mappingSet = new MappingSetCompiler().Compile(_fixture.ModelSet);
        _database = await PostgresqlGeneratedDdlTestDatabase.CreateProvisionedAsync(_fixture.GeneratedDdl);
        _serviceProvider = KeyUnifTestSupport.CreateServiceProvider();

        await ExecuteInitialCreateAsync();

        _updateResult = await ExecuteProfiledUpdateAsync();
        _stateAfterUpdate = await KeyUnifTestSupport.ReadFullPersistedStateAsync(
            _database,
    ...
    private async Task ExecuteInitialCreateAsync()
    {
        ...
        scope
            ...
            .SetSelectedDmsInstance(
                new DmsInstance(
                    Id: 1,
                    InstanceType: "test",
                    InstanceName: "KeyUnifCanonicalPreservation",
        ...
        var result = await repository.UpsertDocument(
            KeyUnifTestSupport.CreateCreateRequest(
                _mappingSet,
                WidgetDocumentUuid,
                "pg-key-unif-canonical-create"
            )
        );
    ...
    private async Task<UpdateResult> ExecuteProfiledUpdateAsync()
    {
        ...
        return await repository.UpdateDocumentById(
            KeyUnifTestSupport.CreateUpdateRequest(
                _mappingSet,
                WidgetDocumentUuid,
                "pg-key-unif-canonical-update",
                UpdateBodyJson,
                KeyUnifContextFactory.CreateHiddenCanonicalPreservationContext(_mappingSet, UpdateBodyJson)
            )
        );
    }
...
[Category("PostgresqlIntegration")]
[NonParallelizable]
public class Given_A_Postgresql_Synthetic_Presence_Preservation
{
    private static readonly DocumentUuid WidgetDocumentUuid = new(
        Guid.Parse("dddddddd-0000-0000-0000-000000000302")
    );
        ...
        scope
            ...
            .SetSelectedDmsInstance(
                new DmsInstance(
                    Id: 1,
                    InstanceType: "test",
                    InstanceName: "KeyUnifPresencePreservation",
                    ConnectionString: _database.ConnectionString,
                    RouteContext: []
                )
            );
    ...
    private async Task<UpdateResult> ExecuteProfiledUpdateAsync()
    {
        ...
        return await repository.UpdateDocumentById(
            KeyUnifTestSupport.CreateUpdateRequest(
                _mappingSet,
                WidgetDocumentUuid,
                "pg-key-unif-presence-update",
                UpdateBodyJson,
                KeyUnifContextFactory.CreateHiddenPresencePreservationContext(_mappingSet, UpdateBodyJson)
            )
        );
    }
