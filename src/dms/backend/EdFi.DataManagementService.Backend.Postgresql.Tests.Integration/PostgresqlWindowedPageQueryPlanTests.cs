...
public class Given_A_Postgresql_Windowed_Page_Query_Plan
{
    ...
    /// </summary>
    /// <remarks>
    /// The measured plan is an ordered index scan under the row limit, with no sort node and no
    /// sequential scan - not the scan-and-sort a selectivity argument would predict. The reason is
    /// that page selection always orders by the anchor and always takes a bounded number of rows, so
    /// the index is worth reading for the ordering alone and the limit stops it early. That is
    /// requirement 1, depth insensitivity, holding for this shape: cost follows the page size, not
    /// how much of the collection the floor admits.
    /// <para>
    /// Asserting the absence of a sort is what makes this more than a duplicate of the selective
    /// case. A plan that materialized and sorted the admitted rows would still name the index and
    /// still avoid a sequential scan, and would still pass every assertion above.
    /// </para>
    /// </remarks>
    [Test]
    public async Task It_reads_the_content_version_index_in_order_for_an_unselective_min_only_floor()
    {
        ...
        explained
            ...
            .Contain(
                ...
                "the measured plan is an ordered index scan under a Limit; asserting it positively is "
