...
4. **04_partial_success.sql** – Mixes one valid and one missing alias. Expect `result = f`, no references persisted for the parent, and the missing referential ID staged with `aliasid` NULL (cleared once `dms.GetInvalidReferencesAndClear` is invoked).
