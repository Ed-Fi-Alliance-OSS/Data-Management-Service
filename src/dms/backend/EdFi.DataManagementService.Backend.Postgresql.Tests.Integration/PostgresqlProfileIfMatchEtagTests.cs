...
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Wildcard_IfMatch_Put_Against_An_Existing_Document
{
    private static readonly DocumentUuid DocumentUuid = new(
        Guid.Parse("b1c2d3e4-0001-4a1b-9c2d-1a2b3c4d5e01")
    );

    private const int NamingStressItemId = 7511;
    ...
    public async Task OneTimeSetUp()
    {
        ...
        (
            await PostgresqlProfileIfMatchEtagTestSupport.SeedAsync(
                ...
                seedBody,
                DocumentUuid,
                "pg-wildcard-put-existing-seed"
            )
        )
            .Should()
            .BeOfType<UpsertResult.InsertSuccess>();

        var getBeforeUpdate = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteGetByIdAsync(
            _serviceProvider,
            _database.ConnectionString,
            _mappingSet,
            DocumentUuid,
            "pg-wildcard-put-existing-get-before"
        );
        _etagBeforeUpdate = PostgresqlProfileIfMatchEtagTestSupport.GetEtag(
            PostgresqlProfileIfMatchEtagTestSupport.GetSuccessDocument(getBeforeUpdate)
        );
        ...
        _wildcardPutResult = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteUnprofiledPutAsync(
            _serviceProvider,
            _database.ConnectionString,
            _mappingSet,
            NamingStressItemId,
            DocumentUuid,
            putBody,
            "pg-wildcard-put-existing-put",
            "*"
        );

        _getAfterUpdate = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteGetByIdAsync(
            _serviceProvider,
            _database.ConnectionString,
            _mappingSet,
            DocumentUuid,
            "pg-wildcard-put-existing-get-after"
        );
    }
    ...
    {
        var afterDocument = PostgresqlProfileIfMatchEtagTestSupport.GetSuccessDocument(_getAfterUpdate);
        afterDocument["shortName"]!.GetValue<string>().Should().Be("Updated");
        PostgresqlProfileIfMatchEtagTestSupport.GetEtag(afterDocument).Should().NotBe(_etagBeforeUpdate);
    }
}
...
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Wildcard_IfMatch_Delete_Against_An_Existing_Document
{
    private static readonly DocumentUuid DocumentUuid = new(
        Guid.Parse("b1c2d3e4-0002-4a1b-9c2d-1a2b3c4d5e02")
    );

    private const int NamingStressItemId = 7522;
    ...
    public async Task OneTimeSetUp()
    {
        ...
        (
            await PostgresqlProfileIfMatchEtagTestSupport.SeedAsync(
                ...
                seedBody,
                DocumentUuid,
                "pg-wildcard-delete-existing-seed"
            )
        )
            .Should()
            .BeOfType<UpsertResult.InsertSuccess>();

        _wildcardDeleteResult = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteDeleteAsync(
            _serviceProvider,
            _database.ConnectionString,
            _mappingSet,
            DocumentUuid,
            "pg-wildcard-delete-existing-delete",
            "*"
        );
        ...
        _getAfterDelete = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteGetByIdAsync(
            ...
            _mappingSet,
            DocumentUuid,
            "pg-wildcard-delete-existing-get-after"
        );
    }
...
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Wildcard_IfMatch_Put_Against_A_Missing_Document
{
    private static readonly DocumentUuid DocumentUuid = new(
        Guid.Parse("b1c2d3e4-0003-4a1b-9c2d-1a2b3c4d5e03")
    );

    private const int NamingStressItemId = 7533;
...
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Wildcard_IfMatch_Delete_Against_A_Missing_Document
{
    private static readonly DocumentUuid DocumentUuid = new(
        Guid.Parse("b1c2d3e4-0004-4a1b-9c2d-1a2b3c4d5e04")
    );
    ...
    public async Task OneTimeSetUp()
    {
        ...
        _wildcardDeleteResult = await PostgresqlProfileIfMatchEtagTestSupport.ExecuteDeleteAsync(
            ...
            _mappingSet,
            DocumentUuid,
            "pg-wildcard-delete-missing-delete",
            "*"
        );
    }
...
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Wildcard_IfMatch_Post_Resolving_To_Insert
{
    private static readonly DocumentUuid DocumentUuid = new(
        Guid.Parse("b1c2d3e4-0005-4a1b-9c2d-1a2b3c4d5e05")
    );

    private const int NamingStressItemId = 7555;
    ...
    public async Task OneTimeSetUp()
    {
        ...
        _wildcardPostResult = await PostgresqlProfileIfMatchEtagTestSupport.ExecutePostAsync(
            _serviceProvider,
            _database.ConnectionString,
            _mappingSet,
            NamingStressItemId,
