...
file static class PostgresqlIfMatchCascadeEtagTestSupport
{
...
/// <summary>
/// ETag invalidation across an identity-propagation cascade: updating Student's identity cascades
/// the new value onto ResourceA's projected reference columns, which bumps ResourceA's stamp — so an
/// If-Match carrying ResourceA's pre-cascade ETag must be rejected with 412 even though the client
/// never touched ResourceA.
/// </summary>
/// <remarks>
/// The fixture directory this loads is named <c>small/referential-identity</c> for historical reasons;
/// nothing here reads <c>dms.ReferentialIdentity</c>. The only referential ids in this file are the
/// domain values <c>ReferentialIdCalculator</c> computes for the request's <c>DocumentInfo</c>.
/// </remarks>
[TestFixture]
[Category("DatabaseIntegration")]
[Category("PostgresqlIntegration")]
public class Given_A_Postgresql_Relational_Update_With_A_Stale_IfMatch_After_An_Identity_Cascade
{
    ...
    public async Task OneTimeSetUp()
    {
        var fixture = PostgresqlGeneratedDdlFixtureLoader.LoadFromRepositoryRelativePath(
            PostgresqlIfMatchCascadeEtagTestSupport.FixtureRelativePath
        );
    ...
    public async Task SetUp()
    {
        ...
        _serviceProvider = PostgresqlIfMatchCascadeEtagTestSupport.CreateServiceProvider();
    }
    ...
    public async Task It_returns_412_for_stale_IfMatch_after_a_student_identity_cascade_changes_resourcea_etag()
    {
        var createStudentResult = await ExecuteUpsertAsync(
            PostgresqlIfMatchCascadeEtagTestSupport.StudentResourceInfo,
        ...
        var createResourceAResult = await ExecuteUpsertAsync(
            PostgresqlIfMatchCascadeEtagTestSupport.ResourceAResourceInfo,
    ...
    )
    {
        ...
        return await repository.UpsertDocument(
            new UpsertRequest(
                ResourceInfo: resourceInfo,
                DocumentInfo: PostgresqlIfMatchCascadeEtagTestSupport.CreateDocumentInfo(
                    resourceInfo,
                    requestBody
                ),
        ...
        serviceProvider
            ...
            .SetSelectedDataStore(
                new DataStore(
                    Id: 1,
                    DataStoreType: "test",
                    Name: "PostgresqlIfMatchCascadeEtag",
