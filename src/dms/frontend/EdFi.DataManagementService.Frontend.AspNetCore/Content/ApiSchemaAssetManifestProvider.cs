...
/// <summary>
/// Provides access to the bootstrap ApiSchema asset manifest and validated path resolution.
/// </summary>
public interface IApiSchemaAssetManifestProvider
{
    ...
    /// Throws <see cref="InvalidOperationException"/> when the manifest is missing, malformed,
...
/// <summary>
/// Reads and validates the bootstrap-api-schema-manifest.json from the configured ApiSchemaPath,
...
) : IApiSchemaAssetManifestProvider
{
    ...
    private string GetOrResolveWorkspaceRoot()
    {
        ...
        var path = appSettings.Value.ApiSchemaPath;
    ...
    public ApiSchemaAssetManifest GetManifest()
    {
        ...
        {
            logger.LogError(
                "Bootstrap manifest file not found at workspace root {WorkspaceRoot}",
                SanitizeForLog(workspaceRoot)
            );
            throw new InvalidOperationException(
                $"Bootstrap manifest file '{ManifestFileName}' was not found under the configured "
                    + $"ApiSchemaPath. Expected path: {manifestPath}. "
                    + "Ensure the workspace has been staged by the bootstrap preparation script."
            );
        }
