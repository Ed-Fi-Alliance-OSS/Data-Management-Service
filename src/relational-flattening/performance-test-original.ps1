...
Write-Host "Starting performance test for original function..."
Write-Host "Running sp_imart_transform_dim_student_edfi_postgres() 10 times on northridge-original"
...
for ($i = 1; $i -le 10; $i++) {
    ...
    $output = docker exec postgres-northridge-original psql -U postgres -d "northridge-original" -c "SELECT COUNT(*) FROM sp_imart_transform_dim_student_edfi_postgres();" 2>&1
...
$results | ConvertTo-Json -Depth 3 | Out-File -FilePath "performance-original-results.json"
Write-Host "`nResults saved to performance-original-results.json"