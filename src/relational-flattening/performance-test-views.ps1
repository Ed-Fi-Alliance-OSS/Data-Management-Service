...
Write-Host "Starting performance test for views function..."
Write-Host "Running sp_imart_transform_dim_student_edfi_postgres_views() 10 times on northridge-flattened"
...
for ($i = 1; $i -le 10; $i++) {
    ...
    $output = docker exec postgres-northridge-flattened psql -U postgres -d "northridge-flattened" -c "SELECT COUNT(*) FROM sp_imart_transform_dim_student_edfi_postgres_views();" 2>&1
...
$results | ConvertTo-Json -Depth 3 | Out-File -FilePath "performance-views-results.json"
Write-Host "`nResults saved to performance-views-results.json"