...
$functionName = "sp_imart_transform_dim_student_edfi_postgres_original_optimized"
$functionCall = "SELECT COUNT(*) FROM $functionName();"

Write-Host "Performance Test: Original Optimized Function" -ForegroundColor Cyan
...
if (-not (Test-DatabaseFunction -Container $container -Database $database -FunctionName $functionName)) {
    ...
    Write-Host "   Please load the function from sp_iMart_Transform_DIM_STUDENT_edfi_Postgres_ORIGINAL_OPTIMIZED.sql" -ForegroundColor Yellow
    Write-Host "   Run: docker exec $container psql -U postgres -d '$database' -f /path/to/sp_iMart_Transform_DIM_STUDENT_edfi_Postgres_ORIGINAL_OPTIMIZED.sql" -ForegroundColor Cyan
    exit 1
}
...
$exportData = @{
    TestName = "Original Optimized Function"
...
$exportData | ConvertTo-Json -Depth 3 | Out-File -FilePath "performance-original-optimized-results.json"
Write-Host "`nResults saved to performance-original-optimized-results.json" -ForegroundColor Green
