...
[TestFixture]
public class InjectPropertiesToEdFiDocumentMiddlewareTests
{
    // Middleware to test
    internal static IPipelineStep Middleware()
    {
        return new InjectPropertiesToEdFiDocumentMiddleware(NullLogger.Instance);
    }
}

[TestFixture]
public class Given_Valid_Request_Body : InjectPropertiesToEdFiDocumentMiddlewareTests
{
    ...
    [Test]
    public void It_should_have_parsed_body_with_lastmodifieddate()
    {
        _context?.ParsedBody.Should().NotBeNull();
        _context?.ParsedBody.ToJsonString().Should().Contain(_propertyName);
    }
